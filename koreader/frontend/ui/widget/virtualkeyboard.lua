...
function VirtualKey:init()
  local label_font_size = G_reader_settings:read("keyboard_key_font_size") or DEFAULT_LABEL_SIZE
  ...
  if
    (
      (self.keyboard.shiftmode_keys[self.label] ~= nil or self.keyboard.shiftmode_keys[self.key])
      and self.keyboard.shiftmode
    )
    or (self.keyboard.umlautmode_keys[self.label] ~= nil and self.keyboard.umlautmode)
    or (self.keyboard.symbolmode_keys[self.label] ~= nil and self.keyboard.symbolmode)
  then
...
function VirtualKeyPopup:init()
  ...
  local v_key_padding = VerticalSpan:new({ height = parent_key.keyboard.key_padding })
  ...
  local keyboard_frame = FrameContainer:new({
    ...
    background = G_reader_settings:nilOrTrue("keyboard_key_border") and Blitbuffer.COLOR_LIGHT_GRAY
  ...
  elseif position_container:getSize().x + keyboard_frame:getSize().w > Screen:getWidth() then
  ...
  if position_container:getSize().y < 0 then
    ...
    parent_key.ignore_key_release = true
  elseif position_container:getSize().y + keyboard_frame:getSize().h > Screen:getHeight() then
...
function VirtualKeyboard:getKeyboardLayout()
  if G_reader_settings:isFalse("keyboard_remember_layout") and not keyboard_state.force_current_layout then
    local lang = G_reader_settings:read("keyboard_layout_default") or G_reader_settings:read("keyboard_layout") or "en"
    G_reader_settings:save("keyboard_layout", lang)
  end
  return G_reader_settings:read("keyboard_layout") or G_reader_settings:read("language") or "en"
end
...
function VirtualKeyboard:initLayer(layer)
  ...
  if layer then
    ...
    self.keyboard_layer =
      normalize(2 - boolnum(self.shiftmode) + 2 * boolnum(self.symbolmode) + 4 * boolnum(self.umlautmode))
  end
  self:addKeys()
end
...
function VirtualKeyboard:addKeys()
  ...
  local base_key_width =
    math.floor((self.width - (#self.KEYS[1] + 1) * self.key_padding - 2 * self.padding) / #self.KEYS[1])
  local base_key_height =
    math.floor((self.height - (#self.KEYS + 1) * self.key_padding - 2 * self.padding) / #self.KEYS)
