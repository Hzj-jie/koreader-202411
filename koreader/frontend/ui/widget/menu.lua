...
function MenuItem:init()
  ...
  if self.shortcut then
    local icon_width = self.entry.shortcut_icon_width or math.floor(self:getSize().h * 4 / 5)
  ...
  if self.single_line then
    ...
    if w > available_width then
      ...
      if not self.truncate_left then
        text_max_width_if_ellipsis = text_max_width_if_ellipsis
...
function Menu:_calculateLayout()
  local perpage = self.items_per_page or G_reader_settings:read("items_per_page") or self.items_per_page_default
  local font_size = self.items_font_size or G_reader_settings:read("items_font_size") or Menu.getItemFontSize(perpage)
...
function Menu:init()
  ...
  -- return button
  self.page_return_arrow = self.page_return_arrow
    or Button:new({
      icon = "back.top",
      callback = function()
  ...
  local header = self.no_title and VerticalSpan:new({ height = 0 }) or self.title_bar
...
function Menu:mergeTitleBarIntoLayout()
  ...
  logger.dbg(
    "Menu:mergeTitleBarIntoLayout: Adjusted focus position to account for added titlebar rows:",
    self.selected.x,
    ",",
    self.selected.y
  )
