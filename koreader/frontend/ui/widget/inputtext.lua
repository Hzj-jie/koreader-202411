...
local function initTouchEvents()
  if Device:isTouchDevice() then
    ...
    function InputText:onTapTextBox(arg, ges)
      ...
      if
        self._frame_textwidget.dimen ~= nil -- zh keyboard with candidates shown here has _frame_textwidget.dimen = nil
        and #self.charlist > 0
    ...
    function InputText:onSwipeTextBox(arg, ges)
      ...
      if
        ...
        or ges.direction == "northwest"
        or ges.direction == "southeast"
        or ges.direction == "southwest"
      then
...
dbg:guard(InputText, "onTextInput", function(self, text)
...
dbg:guard(InputText, "addChars", function(self, chars)
  assert(type(chars) == "string", "Wrong chars value type (expected string)!")
end)
