...
function ConfigOption:init()
        ...
        local option_name = Button:new({
          ...
          hold_callback = function()
            if self.options[c].name_text_hold_callback then
              self.options[c].name_text_hold_callback(
          ...
          hold_callback = function(arg)
            if arg == "-" or arg == "+" then
              self.config:onMakeFineTuneDefault(
                ...
                name_text,
                self.options[c].values,
                self.options[c].labels or self.options[c].args,
                arg
              )
            elseif arg ~= "⋮" then
              self.config:onMakeDefault(
  ...
  table.insert(vertical_group, VerticalSpan:new({ height = default_option_vpadding }))
...
function ConfigDialog:onConfigEvent(option_event, option_arg, when_applied_callback)
  UIManager:broadcastEvent(Event:new(option_event, option_arg, when_applied_callback))
  return true
end
    ...
    if values and event then
      ...
      if more_options_param.left_min then -- DoubleSpinWidget
        ...
        if more_options_param.names then -- allows managing 2 different settings
          ...
          left_default = G_reader_settings:read(self.config_options.prefix .. "_" .. more_options_param.names[1])
            or default_value_orig[1]
          right_default = G_reader_settings:read(self.config_options.prefix .. "_" .. more_options_param.names[2])
            or default_value_orig[2]
        else
          curr_values = self.configurable[name]
          local default_values = G_reader_settings:read(self.config_options.prefix .. "_" .. name) or default_value_orig
        ...
        local value_index
        local default_value = G_reader_settings:read(self.config_options.prefix .. "_" .. name) or default_value_orig
          ...
          extra_callback = function(spin)
            ...
            UIManager:show(ConfirmBox:new({
              ...
              ok_callback = function()
                ...
                G_reader_settings:save(self.config_options.prefix .. "_" .. name, spin_value)
                widget:update()
                self:update()
                UIManager:setDirty(self, function()
                  return "ui", self.dialog_frame.dimen
...
function ConfigDialog:onSwipeCloseMenu(arg, ges_ev)
  ...
  if
    ges_ev.direction == "south"
    and (
      ges_ev.pos:intersectWith(self.dialog_frame.dimen)
      or ges_ev.pos:intersectWith(range)
      or ges_ev.pos:intersectWith(range_ext)
    )
  then
    self:closeDialog()
    return true
