...
function DictQuickLookup:init()
  ...
  local word_to_definition_span = VerticalSpan:new({ height = content_padding_v })
  local definition_to_bottom_span = VerticalSpan:new({ height = content_padding_v })
      ...
      {
        {
          ...
          callback = function()
            ...
            dir = dir or G_reader_settings:read("wikipedia_save_dir") or DictQuickLookup.getWikiSaveEpubDefaultDir()
                  ...
                  Wikipedia:createEpubWithUI(epub_path, self.lookupword, lang, function(success)
                    if success then
                      ...
                      UIManager:show(InfoMessage:new({
                        text = gettext("Saving Wikipedia article failed or interrupted."),
                      }))
                    end
                  end)
                end)
    ...
    buttons = {
      {
        ...
        },
        {
          ...
          callback = function()
            ...
            this:setText(self.save_highlight and gettext("Unhighlight") or gettext("Highlight"), this.width)
            this:refresh()
          end,
        },
        {
          ...
          text = self.is_wiki
              and (
                #self.wiki_languages > 1
                  and BD.wrap(self.wiki_languages[1]) .. " > " .. BD.wrap(self.wiki_languages[2])
    ...
    if
      BD.flipIfMirroredUILayout(
        ges_ev.pos:intersectWith(
          self.definition_widget.dimen:copy():resize(require("apps/reader/modules/readerview").getForwardTapZone())
        )
      )
    then
...
function DictQuickLookup:lookupWikipedia(get_fullpage, word, is_sane, lang)
  ...
  UIManager:broadcastEvent(Event:new("LookupWikipedia", word, is_sane, self.word_boxes, get_fullpage, lang))
end
        ...
        table.insert(buttons2, {
          {
            ...
            hold_callback = function()
              -- Allow doing another lookup with this result word
              UIManager:broadcastEvent(Event:new("LookupWord", self.results[results[res]].word))
            end,
          },
        })
      end
