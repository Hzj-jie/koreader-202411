...
function TextBoxWidget:init()
  ...
  if
    ...
    and self.text:sub(1, #TextBoxWidget.PTF_HEADER) == TextBoxWidget.PTF_HEADER
  then
...
function TextBoxWidget:_measureWithXText()
  ...
  if type(self.charlist) == "table" then
    self._xtext = xtext.new(self.charlist, self.face, self.auto_para_direction, self.para_direction_rtl, self.lang)
  else
    if not self.text then
      self.text = ""
    elseif type(self.text) ~= "string" then
...
function TextBoxWidget:_splitToLines()
  ...
  while idx and idx <= size do
    ...
    if self.use_xtext then
      ...
      local line = self._xtext:makeLine(offset, targeted_width, false, self.tabstop_nb_space_width)
...
function TextBoxWidget:_shapeLine(line)
  ...
  local xshaping = self._xtext:shapeLine(line.offset, line.end_offset, line.idx_to_substitute_with_ellipsis)
  -- logger.dbg(xshaping)
...
function TextBoxWidget:onTapImage(arg, ges)
  if self.line_num_to_image and self.line_num_to_image[self.virtual_line_num] then
    ...
    if tap_x > self.width - image.width and tap_x < self.width and tap_y > 0 and tap_y < image.height then
...
function TextBoxWidget:moveCursorToCharPos(charpos)
  ...
  if self.virtual_line_num ~= self.prev_virtual_line_num then
    ...
    self.cursor_restore_bb =
      Blitbuffer.new(self.cursor_line:getSize().w, self.cursor_line:getSize().h, self._bb:getType())
...
function TextBoxWidget:moveCursorHome()
  self:moveCursorToCharPos(
    self.vertical_string_list[self.current_line_num] and self.vertical_string_list[self.current_line_num].offset or 1
  )
end
...
function TextBoxWidget:onHoldReleaseText(callback, ges)
  ...
  if self.use_xtext then
    ...
    callback(selected_text, hold_duration, sel_start_idx, sel_end_idx, function(idx)
