...
function PageBrowserWidget:updateLayout()
  ...
  self.nb_toc_spans = self.ui.doc_settings:read("page_browser_toc_depth") or self.max_toc_depth
  if self.ui.handmade:isHandmadeTocEnabled() then
    ...
    self.nb_toc_spans = self.ui.doc_settings:read("page_browser_toc_depth_handmade_toc") or self.max_toc_depth
  end
  ...
  self.nb_rows = self.ui.doc_settings:read("page_browser_nb_rows") or G_reader_settings:read("page_browser_nb_rows")
  self.nb_cols = self.ui.doc_settings:read("page_browser_nb_cols") or G_reader_settings:read("page_browser_nb_cols")
  ...
  self.thumbnails_pagenums = self.ui.doc_settings:read("page_browser_thumbnails_pagenums")
    or G_reader_settings:read("page_browser_thumbnails_pagenums")
    or 2
  ...
  self.grid_item_width = math.floor(
    (self.grid_width - 2 * grid_item_outer_h_margin - (self.nb_cols - 1) * grid_item_inner_h_margin) / self.nb_cols
  )
  ...
  grid_item_outer_h_margin = math.floor(
    (self.grid_width - self.nb_cols * self.grid_item_width - (self.nb_cols - 1) * grid_item_inner_h_margin) / 2
  )
...
function PageBrowserWidget:update()
  ...
  if blank_page_slots_before_start > 0 then
    left_spacing =
      BookMapRow:getLeftSpacingForNumberOfPageSlots(blank_page_slots_before_start, self.pages_per_row, self.row_width)
  end
...
function PageBrowserWidget:saveSettings(reset)
  ...
  if self.ui.handmade:isHandmadeTocEnabled() then
    self.ui.doc_settings:save("page_browser_toc_depth_handmade_toc", self.nb_toc_spans)
  else
  ...
  self.ui.doc_settings:save("page_browser_thumbnails_pagenums", self.thumbnails_pagenums)
  ...
  G_reader_settings:save("page_browser_thumbnails_pagenums", self.thumbnails_pagenums)
end
...
function PageBrowserWidget:onThumbnailHold(page, ges)
    ...
    table.insert(buttons, {
      {
        ...
        text = (has_toc_item and gettext("Edit or remove TOC chapter") or gettext("Start TOC chapter here"))
          .. " "
  ...
  if handmade_hidden_flows_edit_enabled then
    ...
    table.insert(buttons, {
      {
        text = is_in_hidden_flow and gettext("Restart regular flow here") or gettext("Start hidden flow here"),
        align = "left",
        callback = function()
          UIManager:close(button_dialog)
