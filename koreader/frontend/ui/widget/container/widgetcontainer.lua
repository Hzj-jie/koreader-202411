...
function WidgetContainer:getSize()
  ...
  if self.dimen == nil or self.dimen.w == nil or self.dimen.h == nil then
    logger.warn("FixMe: WidgetContainer:getSize() returns an empty Geom. ", debug.traceback())
    return Geom:new()
  end
  return self.dimen
end
...
function WidgetContainer:paintTo(bb, x, y)
  ...
  elseif self.align == "bottom" then
    local contentSize = self[1]:getSize()
    self[1]:paintTo(bb, x + math.floor((self:getSize().w - contentSize.w) / 2), y + (self:getSize().h - contentSize.h))
