...
function ScrollableContainer:init()
  ...
  if Device:isTouchDevice() then
    ...
    self.ges_events = {
      ScrollableTouch = not ignore.touch and {
        GestureRange:new({ ges = "touch", range = range }),
      } or nil,
      ScrollableSwipe = not ignore.swipe and {
        GestureRange:new({ ges = "swipe", range = range }),
      } or nil,
      ScrollableHold = not ignore.hold and {
        GestureRange:new({ ges = "hold", range = range }),
      } or nil,
      ScrollableHoldPan = not ignore.hold_pan and {
        GestureRange:new({ ges = "hold_pan", range = range }),
      } or nil,
      ScrollableHoldRelease = not ignore.hold_release and {
        GestureRange:new({ ges = "hold_release", range = range }),
      } or nil,
      ScrollablePan = not ignore.pan and {
        GestureRange:new({ ges = "pan", range = range }),
      } or nil,
      ScrollablePanRelease = not ignore.pan_release and {
...
function ScrollableContainer:_hideTruncatedGridItemsIfRequested()
  ...
  if self.hide_truncated_grid_items and self.step_scroll_grid then
    local new_bottom_row, new_bottom_row_fully_visible =
      self:_getStepScrollRowAtY(self._scroll_offset_y + self._crop_h - 1, false)
  ...
  if ensure_scroll_steps and self.step_scroll_grid then
    ...
    elseif orig_y >= self._max_scroll_offset_y and dy >= 0 then
      ...
      if dy >= 0 then -- Scrolling down
        ...
        if
          top_row
          and not top_row_content_visible
          and new_view_bottom_y < (top_row.content_bottom or top_row.bottom)
        then
...
function ScrollableContainer:onScrollableSwipe(_, ges)
  ...
  if self.swipe_full_view then
    ...
    elseif direction == "east" then
      self:_scrollBy(-self._crop_w, 0, true)
    elseif direction == "west" then
      self:_scrollBy(self._crop_w, 0, true)
    elseif direction == "northeast" then
      self:_scrollBy(-self._crop_w, self._crop_h, true)
    ...
    if direction == "north" then
      self:_scrollBy(0, distance, true)
    elseif direction == "south" then
      self:_scrollBy(0, -distance, true)
    elseif direction == "east" then
      self:_scrollBy(-distance, 0, true)
    elseif direction == "west" then
      self:_scrollBy(distance, 0, true)
    elseif direction == "northeast" then
      self:_scrollBy(-sq_distance, sq_distance, true)
    elseif direction == "northwest" then
      self:_scrollBy(sq_distance, sq_distance, true)
    elseif direction == "southeast" then
      self:_scrollBy(-sq_distance, -sq_distance, true)
