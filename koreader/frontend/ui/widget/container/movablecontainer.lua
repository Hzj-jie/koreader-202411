...
function MovableContainer:init()
  if Device:isTouchDevice() and not self.unmovable then
    ...
    self.ges_events = {
      MovableTouch = not ignore.touch and {
        GestureRange:new({ ges = "touch", range = range }),
      } or nil,
      MovableSwipe = not ignore.swipe and {
        GestureRange:new({ ges = "swipe", range = range }),
      } or nil,
      MovableHold = not ignore.hold and {
        GestureRange:new({ ges = "hold", range = range }),
      } or nil,
      MovableHoldPan = not ignore.hold_pan and {
        GestureRange:new({ ges = "hold_pan", range = range }),
      } or nil,
      MovableHoldRelease = not ignore.hold_release and {
        GestureRange:new({ ges = "hold_release", range = range }),
      } or nil,
      MovablePan = not ignore.pan and {
        GestureRange:new({ ges = "pan", range = range }),
      } or nil,
      MovablePanRelease = not ignore.pan_release and {
...
function MovableContainer:onMovableSwipe(_, ges)
  ...
  if direction == "north" then
    self:_moveBy(0, -distance, true)
  elseif direction == "south" then
    self:_moveBy(0, distance, true)
  elseif direction == "east" then
    self:_moveBy(distance, 0, true)
  elseif direction == "west" then
    self:_moveBy(-distance, 0, true)
  elseif direction == "northeast" then
    self:_moveBy(sq_distance, -sq_distance, true)
...
function MovableContainer:onMovableHoldRelease(_, ges)
  ...
  if self._moving or self._touch_pre_pan_was_inside then
    ...
    if
      math.abs(self._move_relative_x) < self.move_threshold
      and math.abs(self._move_relative_y) < self.move_threshold
    then
