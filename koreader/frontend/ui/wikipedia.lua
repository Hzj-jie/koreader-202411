...
function Wikipedia:createEpub(epub_path, page, lang, with_images)
  ...
  local include_images = G_reader_settings:read("wikipedia_epub_include_images")
  local use_img_2x = G_reader_settings:read("wikipedia_epub_highres_images")
  if with_images then
...
/* hiding .noprint may discard some interesting links */
  ...
  table.insert(
    toc_ncx_parts,
    string.format(
      [[<navPoint id="navpoint-%s" playOrder="%s"><navLabel><text>%s</text></navLabel><content src="content.html"/>]],
      num,
      num,
      page_cleaned
    )
  ...
  for isec, s in ipairs(sections) do
    ...
    elseif s_level > cur_level + 1 then
      ...
      while s_level > cur_level + 1 do
        ...
        table.insert(
          toc_ncx_parts,
          string.format(
            [[<navPoint id="navpoint-%s" playOrder="%s"><navLabel><text>-</text></navLabel><content src="content.html#%s"/>]],
            num,
            num,
            s_anchor
          )
    ...
    table.insert(
      toc_ncx_parts,
      string.format(
        [[<navPoint id="navpoint-%s" playOrder="%s"><navLabel><text>%s</text></navLabel><content src="content.html#%s"/>]],
        num,
        num,
        s_title,
        s_anchor
...
<ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1">
  ...
  local online_version_htmllink = string.format(
    ...
    wiki_base_url,
    page:gsub(" ", "_"),
    gettext("online version")
  )
  local see_online_version =
    T(gettext("See %1 for up-to-date content"), online_version_htmllink)
...
%s
  ...
  if include_images then
    ...
    for inum, img in ipairs(images) do
      ...
      local go_on = UI:info(
        T(gettext("Retrieving image %1 / %2 …"), inum, nb_images),
        inum >= 2
      )
      if not go_on then
        cancelled = true
