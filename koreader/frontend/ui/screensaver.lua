...
function Screensaver:expandSpecial(message, fallback)
  ...
  if ui and ui.document then
    ...
    if doc:hasHiddenFlows() then
      ...
      time_left_chapter =
        self:_calcAverageTimeForPages(ui.toc:getChapterPagesLeft(currentpageAll) or (totalpages - currentpage))
      ...
      time_left_chapter =
        self:_calcAverageTimeForPages(ui.toc:getChapterPagesLeft(currentpage) or doc:getTotalPagesLeft(currentpage))
  ...
  elseif DocSettings:hasSidecarFile(lastfile) then
    ...
    props = FileManagerBookInfo.extendProps(doc_settings:read("doc_props"), lastfile)
...
function Screensaver:modeIsImage()
  return self.screensaver_type == "cover" or self.screensaver_type == "random_image"
end
...
function Screensaver:setup(event, event_message)
  ...
  local screensaver_img_background = G_reader_settings:read("screensaver_img_background")
  local screensaver_msg_background = G_reader_settings:read("screensaver_msg_background")
  ...
  if G_reader_settings:has(self.prefix .. "screensaver_type") then
    self.screensaver_type = G_reader_settings:read(self.prefix .. "screensaver_type")
  else
  ...
  if self.screensaver_type == "random_image" then
    local screensaver_dir = G_reader_settings:read(self.prefix .. "screensaver_dir")
...
function Screensaver:show()
  ...
  local with_gesture_lock = Device:isTouchDevice() and G_reader_settings:read("screensaver_delay") == "gesture"
  ...
  if self.screensaver_type == "cover" or self.screensaver_type == "random_image" then
    local widget_settings = {
      ...
      stretch_limit_percentage = G_reader_settings:read("screensaver_stretch_limit_percentage"),
    }
    if self.image then
  ...
  if self.show_message then
    ...
    if G_reader_settings:has(self.prefix .. "screensaver_message") then
      screensaver_message = G_reader_settings:read(self.prefix .. "screensaver_message")
    ...
    if screensaver_message:find("%%") then
      screensaver_message =
        self:expandSpecial(screensaver_message, self.event_message or self.default_screensaver_message)
    end

    local message_pos
    if G_reader_settings:has(self.prefix .. "screensaver_message_position") then
      message_pos = G_reader_settings:read(self.prefix .. "screensaver_message_position")
    else
