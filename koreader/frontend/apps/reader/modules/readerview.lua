...
local ReaderView = OverlapGroup:extend({
  ...
  -- single page state
  state = nil, -- table
  outer_page_color = Blitbuffer.gray(G_defaults:read("DOUTER_PAGE_COLOR") * (1 / 15)),
  ...
  page_scroll = nil,
  page_bgcolor = Blitbuffer.gray(G_defaults:read("DBACKGROUND_COLOR") * (1 / 15)),
...
function ReaderView:init()
  ...
  self.page_gap = {
    ...
    color = Blitbuffer.gray((G_reader_settings:read("page_gap_color") or 8) * (1 / 15)),
  }
...
function ReaderView:drawHighlightIndicator(bb, x, y)
  ...
  bb:paintRect(rect.x, rect.y + rect.h / 2 - Size.border.thick / 2, rect.w, Size.border.thick)
  bb:paintRect(rect.x + rect.w / 2 - Size.border.thick / 2, rect.y, Size.border.thick, rect.h)
end
...
function ReaderView:recalculate()
  ...
  if self.ui.paging and self.state.page then
    ...
    -- clear dim area
    self.dim_area:clear()
    UIManager:broadcastEvent(Event:new("ViewRecalculate", self.visible_area, self.page_area))
  else
    self.visible_area:setSizeTo(self.dimen)
  end
  ...
  if self:getSize().h > self.visible_area.h then
    if self.footer_visible and not self.footer.settings.reclaim_height then
      self.state.offset.y = (self:getSize().h - (self.visible_area.h + self.footer:getHeight())) / 2
    else
...
function ReaderView:rotate(mode, old_mode)
  ...
  Notification:notify(T(gettext("Rotation mode set to: %1"), optionsutil:getOptionText("SetRotationMode", mode)))
end
...
function ReaderView:onReadSettings(config)
  if self.ui.paging then
    ...
    self.render_mode = config:read("render_mode") or G_defaults:read("DRENDER_MODE")
  ...
  local full_screen = config:read("kopt_full_screen") or self.document.configurable.full_screen
  if full_screen == 0 then
    self.footer_visible = false
  end
  self:resetLayout()
  local page_scroll = config:read("kopt_page_scroll") or self.document.configurable.page_scroll
  ...
  self.page_overlap_style = config:read("page_overlap_style") or G_reader_settings:read("page_overlap_style") or "dim"
  self.page_gap.height =
    Screen:scaleBySize(config:read("kopt_page_gap_height") or G_reader_settings:read("kopt_page_gap_height") or 8)
end
...
function ReaderView:onSetViewMode(new_mode)
  if new_mode ~= self.view_mode then
    ...
    Notification:notify(T(gettext("View mode set to: %1"), optionsutil:getOptionText("SetViewMode", new_mode)))
  end
end

function ReaderView:onPageGapUpdate(page_gap)
  self.page_gap.height = Screen:scaleBySize(page_gap)
  Notification:notify(T(gettext("Page gap set to %1."), optionsutil.formatFlexSize(page_gap)))
  return true
end
...
function ReaderView:onSaveSettings()
  if self.ui.paging then
    ...
    self.ui.doc_settings:save("tile_cache_validity_ts", self.document:getTileCacheValidity())
...
function ReaderView:onToggleReadingOrder()
  ...
  Notification:notify(is_rtl and gettext("RTL page turning.") or gettext("LTR page turning."))
  return true
end
...
function ReaderView:getForwardTapZone()
  ...
  local tap_zone_forward_w = G_reader_settings:read("page_turns_tap_zone_forward_size_ratio") or 0.6
  local forward_zone
