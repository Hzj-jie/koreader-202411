...
function ReaderBookmark:init()
  ...
  if G_reader_settings:hasNot("bookmarks_items_per_page") then
    ...
    local items_per_page = G_reader_settings:read("items_per_page") or Menu.items_per_page_default
...
function ReaderBookmark:genSortByMenuItems(value, separator)
  ...
  return {
    ...
    radio = true,
    callback = function()
      G_reader_settings:save("bookmarks_items_sorting", value ~= "page" and value or nil)
    end,
    separator = separator,
  }
end
...
function ReaderBookmark:removeItemByIndex(index)
  ...
  elseif item_type == "note" then
    UIManager:broadcastEvent(Event:new("AnnotationsModified", { item, nb_notes_added = -1 }))
  end
...
function ReaderBookmark:deleteItemNote(item)
  ...
  UIManager:broadcastEvent(Event:new("AnnotationsModified", { item, nb_highlights_added = 1, nb_notes_added = -1 }))
end
...
function ReaderBookmark:onShowBookmark()
  self.sorting_mode = G_reader_settings:read("bookmarks_items_sorting") or "page"
  ...
  if self.sorting_mode == "date" and #item_table > 0 then
    ...
    local sort_func = self.is_reverse_sorting and function(a, b)
  ...
  local items_font_size = G_reader_settings:read("bookmarks_items_font_size") or Menu.getItemFontSize(items_per_page)
            ...
            if type_before ~= type_after then
              if type_before == "highlight" then
                UIManager:broadcastEvent(
                  Event:new("AnnotationsModified", { annotation, nb_highlights_added = -1, nb_notes_added = 1 })
                )
              else
                UIManager:broadcastEvent(
                  Event:new("AnnotationsModified", { annotation, nb_highlights_added = 1, nb_notes_added = -1 })
