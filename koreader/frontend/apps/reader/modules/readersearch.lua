...
SRELL_ERROR_CODES[111] =
  _("Expression too complex, some hits will not be shown.")
SRELL_ERROR_CODES[666] =
  _("Expression may lead to an extremely long search time.")
...
function ReaderSearch:registerKeyEvents()
  if Device:hasKeyboard() then
    ...
    self.key_events.ShowFulltextSearchInputRecent =
      { { "Alt", "S" }, { "Ctrl", "S" }, event = "ShowFulltextSearchInput" }
  end
end
  ...
  menu_items.fulltext_search_settings = {
    ...
    sub_item_table = {
      ...
      },
      {
        text_func = function()
          return T(
            _("Max lines per result: %1"),
            self.findall_results_max_lines or _("disabled")
          )
        end,
        keep_menu_open = true,
        callback = function(touchmenu_instance)
      ...
      {
        text_func = function()
          local curr_perpage = self.findall_results_max_lines and _("flexible")
            ...
            for _, b in ipairs(bounds) do
              ...
              if page then -- it should resolve to a page
                if not current_page then -- initial search
                  ...
                  if
                    (self._expect_back_results and page <= current_page)
                    or (not self._expect_back_results and page >= current_page)
                  then
                    valid_link = xpointer
                  end
