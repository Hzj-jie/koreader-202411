...
function ReaderScrolling:getDefaultScrollActivationDelay_ms()
  if (self.ui.gestures and self.ui.gestures.multiswipes_enabled) or G_named_settings.activate_menu() ~= "tap" then
...
Default value: %1 ms]]),
              scroll_activation_delay_default_ms
            ),
...
function ReaderScrolling:onReaderReady()
  ...
  self.scroll_activation_delay_ms = G_reader_settings:read("scroll_activation_delay")
...
function ReaderScrolling:applyScrollSettings()
  ...
  if self.scroll_activation_delay_ms == self:getDefaultScrollActivationDelay_ms() then
    ...
    G_reader_settings:save("scroll_activation_delay", self.scroll_activation_delay_ms)
  end
  if self.scroll_method == self.SCROLL_METHOD_CLASSIC then
...
function ReaderScrolling:_setupAction()
  self._inertial_scroll_action = function(action)
    ...
    if action ~= nil then
      ...
      if time.since(self._last_manual_scroll_timev) >= self.pause_before_release_cancel_duration then
