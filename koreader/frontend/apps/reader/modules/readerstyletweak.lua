...
function TweakInfoWidget:init()
  ...
  local buttons = {
    {
      {
        text = self.is_tweak_in_dispatcher and gettext("Don't show in action list") or gettext("Show in action list"),
        callback = function()
    ...
    },
    {
      ...
      {
        text = self.is_global_default and gettext("Don't use on all books") or gettext("Use on all books"),
        callback = function()
...
You can enable individual tweaks on this book with a tap, or view more details about a tweak and enable it on all books with hold.]]
  ...
  addTweakMenuItem = function(menu, item)
    ...
    elseif item.id then -- tweak menu item
      ...
      table.insert(menu, {
        ...
        enabled_func = is_enabled,
        checked_func = function()
          return self:isTweakEnabled(item.id)
        end,
        text_func = function()
  ...
  process_tweaks_dir = function(dir, item_table, if_empty_menu_title)
    ...
    if #item_table == 0 then
      table.insert(item_table, {
        if_empty_menu_title = if_empty_menu_title or gettext("No CSS tweak found in this directory"),
      })
    end
  end
...

  {
    gettext("Common classic properties"),
    {
      ...
      },
      {
        ...
        gettext("Start a new page with this element. Use 'avoid' to avoid a new page."),
      },
...

  {
    gettext("Private CSS properties"),
    {
      ...
      },
      {
        ...
        gettext("Can be set on contiguous footnote blocks to ignore them in the linear pages flow."),
      },
      {
        "-cr-hint: toc-level1;",
        gettext(
      ...
      {
        "-cr-hint: toc-ignore;",
        gettext("When set on an element, it will be ignored when building the alternative table of contents."),
      },
      {
        "-cr-hint: footnote;",
        gettext(
...
If used as-is, they will act on ALL elements!]]),
      ...
      {
        "::before {content: '\\A0 '}",
        gettext("Insert a visible non-breakable space before an element, so it sticks to what's before."),
      },
      {
        "::before {content: '\\2060'}",
        gettext(
      ...
      },
      {
        ...
        gettext("Insert the value of the attribute 'title' at start of an element content."),
      },
          ...
          callback = function()
            ...
            for _, suggestion in ipairs(CSS_SUGGESTIONS) do
              ...
              if is_submenu then -- add the same arrow we use for top menu submenus
                text = require("ui/widget/menu").getMenuText({
                  text = title,
                  sub_item_table = true,
                })
