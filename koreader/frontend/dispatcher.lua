...
function Dispatcher:init()
  if Dispatcher.initialized then
    return
  end
  ...
  if location[settings] and location[settings].settings then
    if location[settings].settings.order then
      if item then
        ...
        if k then
          table.remove(location[settings].settings.order, k)
        end
      else
        location[settings].settings.order = {}
      end
        ...
        table.insert(menu, {
          ...
          callback = function(touchmenu_instance)
            ...
            local items = SpinWidget:new({
              value = location[settings] ~= nil and location[settings][k]
                or settingsList[k].default
                or settingsList[k].min,
        ...
        table.insert(menu, {
          ...
          callback = function(touchmenu_instance)
            ...
            local items = SpinWidget:new({
              ...
              option_text = caller.profiles == nil and gettext("Use gesture distance"), -- Gesture manager only
    ...
    table.insert(buttons, {
      {
        ...
        hold_callback = function()
          if v.key:sub(1, 13) == "profile_exec_" then
            UIManager:close(quickmenu)
            UIManager:userInput(Event:new(settingsList[v.key].event, settingsList[v.key].arg, { qm_show = true }))
          end
        end,
      },
    })
  ...
  for k, v in iter_func(settings) do
    ...
    if Dispatcher:isActionEnabled(settingsList[k]) then
      if settings.settings and settings.settings.notify then
        Notification:notify(T(gettext("Executing profile: %1"), settings.settings.name))
      end
      if settingsList[k].configurable then
        ...
        if type(v) ~= "number" then
          for i, r in ipairs(settingsList[k].args) do
            if v == r then
              value = settingsList[k].configurable.values[i]
              break
            end
          end
        end
        UIManager:userInput(Event:new("ConfigChange", settingsList[k].configurable.name, value))
      end
