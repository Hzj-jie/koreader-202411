...
local metadata = {
  ...
  desc = {
    ...
    highlights = {
      ...
      gettext("Special multi-page highlight mode with many local and online export options."),
      gettext("Can synchronize your reading progress across all your KOReader running devices."),
      gettext("Integrated with Calibre, Wallabag, Wikipedia, Google Translate and other content providers."),
    },
  },
  notes = {
...
local function tag(element, lang, str, pad)
  ...
  if lang == "en" then
    return string.format("%s<%s>%s</%s>", offset, element, str, element)
  else
    return string.format('%s<%s xml:lang="%s">%s</%s>', offset, element, lang, str, element)
  end
end
