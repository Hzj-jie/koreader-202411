...
local booleanCapture = lpeg.P("true") * lpeg.Cc(true)
...
local function generateLexer(options)
  ...
  local valueCapture = (
    booleanCapture + nullCapture * lpeg.Cc(options.null or nil)
  )
  if options.allowUndefined then
    valueCapture = valueCapture
      + undefinedCapture * lpeg.Cc(options.undefined or nil)
  else
    valueCapture = valueCapture
      + #undefinedCapture * util.denied("undefined", "others.allowUndefined")
  end
  return valueCapture
end
