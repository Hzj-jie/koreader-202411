...
local function CHECK_TABLE_SEC(tbl, key, sz)
  ...
  for i = #tbl, 1, -1 do
    if (tbl[i][1] == SECS or tbl[i][1] == SECI) and tbl[i][2] == name then
      return true
    end
  end
end
    ...
    while 1 do
      ...
      elseif instr == TKEE then
        if type(obj[y]) == "table" and obj[y][arg] then
          ...
          elseif type(obj[y][arg]) == "function" then
            -- May also be a function.
            buf:append_luastr_right(escape.html_escape(obj[y][arg](arg) or ""))
          end
        else
          local in_parent = Mustache._find_in_obj_parents(arg, obj_parents)
          if in_parent then
            ...
            elseif type(in_parent) == "function" then
              -- May also be a function.
              buf:append_luastr_right(escape.html_escape(in_parent(arg)) or "")
            end
          elseif safe == true then
            error(
              string.format(
...
function Mustache._template_dump(tbl, mark)
  ...
  dmp = dmp .. "\
          ====================================\n"
  for i = 1, #tbl do
    if tbl[i][1] == SECS or tbl[i][1] == SECI or tbl[i][1] == SECE then
      dmp = dmp
        .. string.format(
