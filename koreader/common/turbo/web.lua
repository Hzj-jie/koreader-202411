...
function web.RequestHandler:head(...)
  error(web.HTTPError:new(405))
end
function web.RequestHandler:get(...)
  error(web.HTTPError:new(405))
end
function web.RequestHandler:post(...)
  error(web.HTTPError:new(405))
end
function web.RequestHandler:delete(...)
  error(web.HTTPError:new(405))
end
function web.RequestHandler:put(...)
  error(web.HTTPError:new(405))
end
function web.RequestHandler:options(...)
  error(web.HTTPError:new(405))
end
...
function web.RequestHandler:set_secure_cookie(name, value, domain, expire_hours)
  ...
  assert(
    type(self.application.kwargs.cookie_secret) == "string",
...
function web.RequestHandler:_gen_headers()
  if self:get_status() ~= 204 then
    ...
    if not self:get_header("Content-Length") and not self.chunked then
      ...
      self:add_header("Content-Length", tonumber(self._write_buffer:len()))
    end
  end
...
local function get_cookie_table(text_cookie)
  ...
  while j <= len do
    ...
    elseif state == EXPECT_VALUE then
      if
        string.byte(text_cookie, j) == SEMICOLON
...
function web.StaticFileHandler:_headers_flushed_cb()
  self.request:write_zero_copy(self._static_buffer, self.finish, self)
end
...
function web.StaticFileHandler:_send_next_chunk()
  ...
  local sz = math.min(
    1024 * 32, -- 32KB chunks seems like a good value?
  ...
  if not data then
    log.error(string.format("[web.lua] Could not read file; %s.", err))
    self:finish()
    return
  end
...
function web.RedirectHandler:prepare()
  if not self.options or type(self.options) ~= "string" then
    error(web.HTTPError(500, "RedirectHandler executed without URL argument."))
  end
...
function web.Application:get_server_name()
  ...
  if handlers then
    ...
    if err then
      if instanceOf(web.HTTPError, err) then
        web.ErrorHandler(self, request, err.code, err.message)
      elseif type(err) == "string" then
        ...
        web.ErrorHandler(
          ...
          500,
          string.format(
            '<pre style="font-size:12px; font-family:monospace; color:#8B0000;">[web.lua] Unknown error in RequestHandler, %s is dead.\r\n%s\r\n%s\r\n%s</pre>',
            thread,
            _str_borders_down,
            escape.html_escape(trace),
            _str_borders_up
