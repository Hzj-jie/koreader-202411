...
local SOCK_STREAM, AF_UNSPEC, EWOULDBLOCK, EINPROGRESS, ECONNRESET, EPIPE, EAGAIN, EAI_AGAIN
...
function iostream.IOStream:_handle_events(fd, events)
  if not self.socket then
    ...
    log.warning(string.format("Got events for closed stream on fd %d.", fd))
    return
  end
...
local function _run_callback_protected(call)
  ...
  if arg then
    ...
    success = xpcall(callback, _run_callback_error_handler, arg, res)
  else
    success = xpcall(callback, _run_callback_error_handler, res)
  end
  ...
  function iostream.IOStream:_read_from_socket()
    ...
    while true do
      ...
      elseif err then
        local fd = self.socket
        self:close()
        error(string.format("Error when reading from socket %s: %s", fd, err))
      end
    end
  ...
  elseif self._read_delimiter ~= nil then
    if self._read_buffer_size ~= 0 then
      ...
      if loc then
        ...
        if arg then
          self:_run_callback(callback, arg, self:_consume(delimiter_end))
        else
...
if platform.__LINUX__ and not _G.__TURBO_USE_LUASOCKET__ then
  ...
  function iostream.IOStream:_handle_write_const()
    ...
    local num_bytes = C.send(self.socket, ptr, _sz, 0)
    if num_bytes == -1 then
      errno = ffi.errno()
      if errno == EWOULDBLOCK or errno == EAGAIN then
        return
  ...
  function iostream.SSLIOStream:_read_from_socket()
    ...
    if sz == -1 then
      ...
      elseif err == crypto.SSL_ERROR_WANT_READ then
        ...
        self:close()
        error(string.format("SSL error. %s", ssl_str_err))
      end
    end
    if sz == 0 then
      self:close()
...
if platform.__LINUX__ and not _G.__TURBO_USE_LUASOCKET__ then
  ...
  function iostream.DNSResolv:resolv(address, port, family)
    ...
    self._dns_timeout = self.io_loop:add_timeout(
      util.gettimemonotonic() + ((self.args.dns_timeout or 30) * 1000),
      function()
        if _self.pid then
