...
function crypto.ssl_do_handshake(SSLIOStream)
  ...
  if rc <= 0 then
      ...
      if verify_err ~= 0 then
        error(
          string.format(
            ...
            ffi.string(crypto.lib.X509_verify_cert_error_string(verify_err))
          )
        )
      end
    end
    ...
    if
      err == crypto.SSL_ERROR_WANT_READ or err == crypto.SSL_ERROR_WANT_WRITE
    then
      return false
    ...
    if client and SSLIOStream._ssl_verify then
      ...
      if rc ~= crypto.validate.MatchFound then
        error("SSL certficate hostname validation failed, rc " .. tonumber(rc))
      end
    end
  end
  return true
