...
if platform.__LINUX__ and not _G.__TURBO_USE_LUASOCKET__ then
  function util.gettimeofday()
    ...
    return (tonumber((g_timeval.tv_sec * 1000) + (g_timeval.tv_usec / 1000)))
  end
else
  function util.gettimeofday()
...
if platform.__LINUX__ and not _G.__TURBO_USE_LUASOCKET__ then
  function util.time_format_cookie(epoch)
    ...
    local sz = C.strftime(g_time_str_buf, 1024, "%a, %d-%b-%Y %H:%M:%S GMT", tm)
...
if platform.__LINUX__ and not _G.__TURBO_USE_LUASOCKET__ then
  ...
  function util.time_format_http_header(time)
    return os.date("%a, %d %b %Y %H:%M:%S GMT", time / 1000)
  end
end
