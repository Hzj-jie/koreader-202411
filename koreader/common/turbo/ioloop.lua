...
function ioloop.IOLoop:_run_handler(fd, events)
  ...
  if arg then
    ok = xpcall(func, _run_handler_error_handler, arg, fd, events)
  else
    ok = xpcall(func, _run_handler_error_handler, fd, events)
  end
  if ok == false then
...
if platform.__LINUX__ and not _G.__TURBO_USE_LUASOCKET__ then
  ...
  function _EPoll_FFI:unregister(fd)
    return epoll_ffi.epoll_ctl(self._epoll_fd, epoll_ffi.EPOLL_CTL_DEL, fd, 0)
  end
  ...
  function _LuaSocketPoll:register(fd, events)
    if bit.band(events, ioloop.WRITE) ~= 0 then
      if #self.sendt >= 64 then
        return -1, "More than 64 sockets in select() table. Can not complete."
      end
