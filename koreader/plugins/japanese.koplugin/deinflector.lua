...
for i, c in ipairs(HALFWIDTH_KATAKANA) do
  HALFWIDTH_KATAKANA[i] = Utf8Proc.normalize_NFC(c)
end
for i, c in ipairs(FULLWIDTH_KATAKANA) do
  FULLWIDTH_KATAKANA[i] = Utf8Proc.normalize_NFC(c)
end
for i, c in ipairs(FULLWIDTH_HIRAGANA) do
  FULLWIDTH_HIRAGANA[i] = Utf8Proc.normalize_NFC(c)
end
...
function Deinflector:deinflect(text)
  ...
  for name, enabled in pairs(self.enabled_text_conversions) do
    if enabled then
      table.insert(enabled_text_conversions, name)
    end
  end
    ...
    },
    {
      ...
      callback = function()
        ...
        UIManager:show(InfoMessage:new({
          ...
          text = T(
            gettext("Deinflector has %1 and %2 loaded."),
            nrules_str,
            nvariants_str
          ),
        }))
