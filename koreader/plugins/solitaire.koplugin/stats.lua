...
function Stats:getStatsText()
  ...
  table.insert(
    lines,
    string.format("Win Rate:        %d%%", self:getWinPercentage())
  )
  table.insert(lines, "")
  ...
  table.insert(
    lines,
    string.format("Average Score:   %d", self:getAverageScore())
  )
  table.insert(lines, "")
  ...
  table.insert(
    lines,
    string.format("Best Time:       %s", self:formatTime(self.best_time))
  )
  table.insert(
    lines,
    string.format("Average Time:    %s", self:formatTime(self:getAverageTime()))
  )
  table.insert(lines, "")
  table.insert(lines, "── Streaks ──")
  table.insert(
    lines,
    string.format("Current Streak:  %d wins", self.current_win_streak)
  )
  table.insert(
    lines,
    string.format("Longest Streak:  %d wins", self.longest_win_streak)
  )
  ...
  if self.draw1_games_played > 0 or self.draw3_games_played > 0 then
    ...
    if self.draw1_games_played > 0 then
      local d1_pct =
        math.floor((self.draw1_games_won / self.draw1_games_played) * 100)
      table.insert(
        lines,
        string.format(
          "Draw-1:  %d/%d (%d%%)",
          self.draw1_games_won,
          self.draw1_games_played,
          d1_pct
        )
      )
    end
    if self.draw3_games_played > 0 then
      local d3_pct =
        math.floor((self.draw3_games_won / self.draw3_games_played) * 100)
      table.insert(
        lines,
        string.format(
          "Draw-3:  %d/%d (%d%%)",
          self.draw3_games_won,
          self.draw3_games_played,
          d3_pct
        )
      )
    end
