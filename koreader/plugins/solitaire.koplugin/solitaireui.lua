...
function SolitaireUI:buildUI()
  ...
  local status_bar = FrameContainer:new({
    ...
    CenterContainer:new({
      dimen = Geom:new({
        w = self.screen_width - 10,
        h = self.status_bar_height - 10,
      }),
      TextWidget:new({
        text = status_text,
...
function SolitaireUI:drawGame(bb, offset_x, offset_y)
  ...
  self:drawWaste(
    bb,
    self.margin + self.card_width + self.card_spacing,
    y_offset
  )

  local foundation_start_x = self.screen_width
    - (4 * self.card_width)
    - (3 * self.card_spacing)
    - self.margin
  for f = 1, 4 do
    local x = foundation_start_x
      + (f - 1) * (self.card_width + self.card_spacing)
  ...
  for col = 1, 7 do
    local x = tableau_start_x
      + (col - 1) * (self.card_width + self.card_spacing)
    self:drawTableauColumn(bb, x, tableau_y, col)
  end
end

function SolitaireUI:drawCard(
  bb,
  x,
  y,
  card,
  ...
  if card and card.face_up then
    ...
    if is_top_card then -- Draw Centered suit symbol on top card of each pile
      ...
      if is_stock then
        suit_tr_widget:paintTo(
          bb,
          x + padding,
          y + self.card_height - stw - padding * 2
        )
      else
...
function SolitaireUI:drawWaste(bb, x, y)
  local is_selected = self.selected_source
    and self.selected_source.type == "waste"
  local is_hint = self.hint_highlight
    and (
      self.hint_highlight.type == "waste_to_foundation"
      or self.hint_highlight.type == "waste_to_tableau"
    )
...
function SolitaireUI:drawFoundation(bb, x, y, foundation_idx)
  table.insert(self.touch_zones, {
    ...
    h = self.card_height,
    type = "foundation",
    index = foundation_idx,
  })

  local foundation = self.game.foundations[foundation_idx]
  local is_hint = self.hint_highlight
    and self.hint_highlight.foundation == foundation_idx
...
function SolitaireUI:drawTableauColumn(bb, x, y, col_idx)
  ...
  local is_selected_col = self.selected_source
    and self.selected_source.type == "tableau"
    and self.selected_source.index == col_idx

  local is_hint_from = self.hint_highlight
    and self.hint_highlight.from == col_idx
  local is_hint_to = self.hint_highlight
    and (
      self.hint_highlight.to == col_idx
      or self.hint_highlight.tableau == col_idx
    )
  ...
  if #column == 0 then
    ...
    for i, card in ipairs(column) do
      ...
      local card_height = (i == #column) and self.card_height
        or self.stack_offset

      local is_card_selected = is_selected_col
        and i >= self.selected_source.card_pos
      local is_card_hint = (
        is_hint_from
        and self.hint_highlight.card_idx
        and i >= self.hint_highlight.card_idx
      ) or (is_hint_to and i == #column)
      ...
      self:drawCard(
        ...
        card_y,
        card,
        is_card_selected or is_card_hint,
        is_top
      )
    end
  end
    ...
    if self.selected_source then
      ...
      elseif self.selected_source.type == "tableau" then
        ...
        if self.selected_source.card_pos == #col then
          success = self.game:moveToFoundation(
            "tableau",
    ...
    if self.selected_source then
      ...
      elseif self.selected_source.type == "tableau" then
        if self.selected_source.index ~= zone.index then
          success = self.game:moveToTableau(
            "tableau",
            self.selected_source.index,
...
function SolitaireUI:showHint()
  ...
  if hint then
    ...
    UIManager:show(InfoMessage:new({
      text = gettext("No moves available. Try drawing from stock."),
      timeout = 2,
    }))
  end
end
...
function SolitaireUI:autoMove()
  ...
  elseif total_moves == 0 then
    UIManager:show(InfoMessage:new({
      text = gettext("No cards can be automatically moved to foundations."),
      timeout = 2,
    }))
  end
end
...
function SolitaireUI:toggleDrawMode()
  ...
  UIManager:show(InfoMessage:new({
    text = string.format(gettext("Switched to %s mode"), mode_name),
    timeout = 1,
  }))
...
function SolitaireUI:confirmResetStats()
  ...
  local dialog
  dialog = ButtonDialog:new({
    title = gettext("Reset Statistics"),
    text = gettext(
      "Are you sure you want to reset all statistics and leaderboard data? This cannot be undone."
    ),
    buttons = {
      {
        {
        ...
        },
        {
          ...
          callback = function()
            ...
            self.stats:reset()
            UIManager:show(InfoMessage:new({
              text = gettext("Statistics have been reset."),
              timeout = 2,
            }))
          end,
        },
