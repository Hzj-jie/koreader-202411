...
function Aliases:load()
  ...
  if file then
    for line in file:lines() do
      ...
      local __, separator = line:find("^[%a%d][%a%d-_]*%=") -- find separator
      if line ~= "" and line:sub(1, 1) ~= "#" and separator then
        ...
        table.insert(self.kv_pairs, {
          ...
          alias_command,
          callback = function()
            self:editAlias(alias_name, alias_command)
          end,
        })
      end
    end
...
function Aliases:save()
  ...
  if not file then
    UIManager:show(InfoMessage:new({
      text = T(gettext("Terminal emulator: error saving: %1"), self.filename),
    }))
    return
  end
...
function Aliases:editAlias(alias_name, alias_command)
  ...
  alias_input = MultiInputDialog:new({
    title = gettext("Edit alias"),
    fields = {
      {
        description = gettext("Alias name:"),
        text = alias_name,
      },
      {
        description = gettext("Alias command:"),
        text = alias_command,
      },
    },
    buttons = {
