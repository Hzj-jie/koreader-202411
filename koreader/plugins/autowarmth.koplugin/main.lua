...
function AutoWarmth:init()
  ...
  self.warmth = G_reader_settings:readTableRef("autowarmth_warmth", { 90, 90, 80, 60, 20, 20, 20, 60, 80, 90, 90 })

  self.fl_off_during_day = G_reader_settings:read("autowarmth_fl_off_during_day")
  self.fl_off_during_day_offset_s = G_reader_settings:read("autowarmth_fl_off_during_day_offset_s") or 0
  ...
  elseif not self.control_warmth and not self.control_nightmode then
    logger.dbg(
      "AutoWarmth: autowarmth_control_warmth and autowarmth_control_nightmode are both false, set them to true"
    )
  ...
  while i < midnight_index do
    ...
    if j <= midnight_index and self.scheduler_times[j] and self.scheduler_times[i] > self.scheduler_times[j] then
...
function AutoWarmth:onDispatcherRegisterActions()
  Dispatcher:registerAction("show_ephemeris", {
    category = "none",
    event = "ShowEphemeris",
    title = gettext("Show ephemeris"),
    general = true,
  })
  Dispatcher:registerAction("auto_warmth_off", {
    category = "none",
    event = "AutoWarmthOff",
    title = gettext("Auto warmth off"),
    screen = true,
  })
  Dispatcher:registerAction("auto_warmth_activate_sun", {
    ...
    arg = activate_sun,
    title = gettext("Auto warmth use sun position"),
    screen = true,
  })
  Dispatcher:registerAction("auto_warmth_activate_schedule", {
    ...
    title = gettext("Auto warmth use schedule"),
    screen = true,
  })
  Dispatcher:registerAction("auto_warmth_activate_closer_midnight", {
    ...
    title = gettext("Auto warmth use closer midnight"),
    screen = true,
  })
  Dispatcher:registerAction("auto_warmth_activate_closer_noon", {
    ...
    title = gettext("Auto warmth use closer noon"),
    screen = true,
  })
  ...
  Dispatcher:registerAction("auto_warmth_cycle_trough", {
    category = "none",
    event = "AutoWarmthMode",
    title = gettext("Auto warmth cycle through modes"),
    screen = true,
  })
end

function AutoWarmth:onShowEphemeris()
  self:showTimesInfo(gettext("Information about the sun in"), true, activate_sun, false)
end
...
function AutoWarmth:onAutoWarmthMode(forced_method)
  ...
  local notify_text
  if self.activate == 0 then
    notify_text = gettext("Auto warmth turned off")
  elseif self.activate == activate_sun then
    notify_text = gettext("Auto warmth use sun position")
  elseif self.activate == activate_schedule then
    notify_text = gettext("Auto warmth use schedule")
  elseif self.activate == activate_closer_midnight then
    notify_text = gettext("Auto warmth use whatever is closer to midnight")
  elseif self.activate == activate_closer_noon then
    notify_text = gettext("Auto warmth use whatever is closer to noon")
  end
...
function AutoWarmth:_onResume()
  ...
  if
    resume_date.day == SunTime.date.day
    and resume_date.month == SunTime.date.month
    and resume_date.year == SunTime.date.year
  then
...
function AutoWarmth:_onToggleNightMode()
  ...
  if not self.hide_nightmode_warning then
    ...
    local radio_buttons = {
      {
        {
          text = gettext("Show this warning again"),
          provider = function() end,
        },
      },
      {
        {
          text = gettext("Hide the warning until the next book is opened"),
          provider = function()
      ...
      {
        {
          text = gettext("Disable AutoWarmth's nightmode control"),
          provider = function()
    ...
    UIManager:show(RadioButtonWidget:new({
      title_text = gettext("Night mode changed"),
      info_text = gettext("The AutoWarmth plugin might change it again."),
      width_factor = 0.9,
  ...
  elseif self.activate == activate_schedule then
    ...
    if self.activate == activate_closer_noon then
      for i = 1, midnight_index do
        ...
        elseif
          self.scheduler_times[i]
          and math.abs(self.current_times_h[i] % 24 - 12) > math.abs(self.scheduler_times[i] % 24 - 12)
        then
          self.current_times_h[i] = self.scheduler_times[i]
        end
      end
...
function AutoWarmth:addToMainMenu(menu_items)
  menu_items.autowarmth = {
    text = Device:hasNaturalLight() and gettext("Auto warmth and night mode") or gettext("Auto night mode"),
...
function AutoWarmth:getSubMenuItems()
  return {
    ...
    {
      text = gettext("Expert mode"),
      checked_func = function()
        return not self.easy_mode
      end,
      help_text = gettext("In the expert mode, different types of twilight can be used in addition to civil twilight."),
    ...
    {
      text = gettext("Location settings"),
      sub_item_table = self:getLocationMenu(),
    },
    {
      text = gettext("Fixed schedule settings"),
    ...
    },
    {
      ...
      text = Device:hasNaturalLight() and gettext("Warmth and night mode settings") or gettext("Night mode settings"),
    ...
    self:getTimesMenu(gettext("Fixed schedule information"), false, activate_schedule),
  }
end
  ...
  return {
    ...
    text_func = function()
      if self.fl_off_during_day and self.fl_off_during_day_offset_s ~= 0 and not self.easy_mode then
        return T(gettext("Frontlight off during day: %1 min offset"), math.floor(self.fl_off_during_day_offset_s / 60))
      else
    ...
    callback = function(touchmenu_instance)
      if self.easy_mode then
        ...
        G_reader_settings:save("autowarmth_fl_off_during_day", self.fl_off_during_day)
        self:scheduleMidnightUpdate()
        self:toggleFrontlight()
      else
        -- hard limit offset to 15 min after sunset and 30 mins before sunset; sunrise vice versa
        UIManager:show(SpinWidget:new({
          title_text = gettext("Offset for frontlight off"),
          info_text = gettext([[At this time the frontlight will be turned
  • off after sunrise and
  • on before sunset.]]),
          ok_always_enabled = true,
          -- read the saved setting, as this gets overwritten by toggling easy_mode
          value = (G_reader_settings:read("autowarmth_fl_off_during_day_offset_s") or 0) * (1 / 60),
          value_min = -15,
          value_max = 30,
          wrap = false,
          value_step = 5,
          ...
          callback = function(spin)
            ...
            G_reader_settings:save("autowarmth_fl_off_during_day_offset_s", self.fl_off_during_day_offset_s)
...
function AutoWarmth:getActivateMenu()
  local function getActivateMenuEntry(text, help_text, activator)
    return {
      ...
      help_text = help_text,
      checked_func = function()
        return self.activate == activator
      end,
      callback = function()
  ...
  return {
    getActivateMenuEntry(
      gettext("According to the sun's position"),
      gettext("Only use the times calculated from the position of the sun."),
      activate_sun
    ),
    getActivateMenuEntry(
      gettext("According to the fixed schedule"),
      gettext("Only use the times from the fixed schedule."),
      activate_schedule
    ),
    getActivateMenuEntry(
      gettext("Whatever is closer to noon"),
      gettext("Use the times from the sun position or schedule that are closer to noon."),
      activate_closer_noon
    ),
    getActivateMenuEntry(
      gettext("Whatever is closer to midnight"),
      gettext("Use the times from the sun position or schedule that are closer to midnight."),
  ...
  return {
    {
      text_func = function()
        if self.location ~= "" then
          return T(gettext("Location: %1"), self.location)
        else
    ...
    },
    {
      ...
      callback = function(touchmenu_instance)
        local location_widget = DoubleSpinWidget:new({
          ...
          info_text = gettext("Enter decimal degrees, northern hemisphere and eastern length are '+'."),
          left_text = gettext("Latitude"),
...
function AutoWarmth:getScheduleMenu()
  ...
  local function getScheduleMenuEntry(text, num, mode)
    return {
      mode = mode,
      text_func = function()
        return T(gettext("%1: %2"), text, self:hoursToClock(self.scheduler_times[num]))
      end,
      checked_func = function()
      ...
      callback = function(touchmenu_instance)
        ...
        UIManager:show(DateTimeWidget:new({
          ...
          info_text = gettext("Enter time in hours and minutes."),
          hour = hh,
          hour_min = -1,
          hour_max = 24,
          min = mm,
          ...
          callback = function(time)
            ...
            if num > 1 and new_time < get_valid_time(num, -1) then
              UIManager:show(ConfirmBox:new({
                text = gettext("This time is before the previous time.\nAdjust the previous time?"),
            ...
            elseif num < midnight_index and new_time > get_valid_time(num, 1) then
              UIManager:show(ConfirmBox:new({
                text = gettext("This time is after the subsequent time.\nAdjust the subsequent time?"),
                ok_callback = function()
  ...
  local retval = {
    getScheduleMenuEntry(gettext("Solar midnight (previous day)"), 1, false),
    getScheduleMenuEntry(gettext("Astronomical dawn"), 2, false),
    getScheduleMenuEntry(gettext("Nautical dawn"), 3, false),
    getScheduleMenuEntry(gettext("Civil dawn"), 4),
    getScheduleMenuEntry(gettext("Sunrise"), 5),
    getScheduleMenuEntry(gettext("Solar noon"), 6, false),
    getScheduleMenuEntry(gettext("Sunset"), 7),
    getScheduleMenuEntry(gettext("Civil dusk"), 8),
    getScheduleMenuEntry(gettext("Nautical dusk"), 9, false),
    getScheduleMenuEntry(gettext("Astronomical dusk"), 10, false),
    getScheduleMenuEntry(gettext("Solar midnight"), 11, false),
  }
        ...
        if Device:hasNaturalLight() and self.control_warmth then
          if self.control_nightmode then
            if self.warmth[num] <= 100 then
              return T(gettext("%1: %2 %"), text, self.warmth[num])
            else
              return T(gettext("%1: 100 % + ☾"), text)
            end
          else
            if self.warmth[num] <= 100 then
              return T(gettext("%1: %2 %"), text, self.warmth[num])
            else
              return T(gettext("%1: %2 %"), text, math.max(self.warmth[num] - 1000, 0))
            end
          end
        else
          if self.warmth[num] <= 100 then
            return T(gettext("%1: ☼"), text)
          else
            return T(gettext("%1: ☾"), text)
          end
        end
      ...
      callback = function(touchmenu_instance)
        if Device:hasNaturalLight() and self.control_warmth then
          local warmth_spinner = SpinWidget:new({
            title_text = text,
            info_text = gettext("Enter percentage of warmth."),
  ...
  local retval = {
    {
      text_func = function()
        if Device:hasNaturalLight() then
          return T(
            gettext("Control: %1%2%3"),
            self.control_warmth and gettext("warmth") or "",
            self.control_warmth and self.control_nightmode and T(" %1 ", gettext("and")) or "",
            self.control_nightmode and gettext("night mode") or ""
          )
        else
          return gettext("Control: night mode")
        end
      end,
      checked_func = function()
        if Device:hasNaturalLight() then
      ...
      hold_callback = function()
        if Device:hasNaturalLight() then
          UIManager:show(InfoMessage:new({
            text = gettext(
              "Tapping here chooses between the different AutoWarmth modes: 'warmth only', 'warmth and night mode', 'night mode only'."
            ),
          }))
        else
          UIManager:show(InfoMessage:new({
            text = gettext("Your device supports 'night mode' control only."),
          }))
        end
      end,
      callback = function(touchmenu_instance)
    ...
    {
      text = Device:hasNaturalLight() and gettext("Set warmth and night mode for:") or gettext("Set night mode for:"),
      enabled = false,
    },
...
function AutoWarmth:showTimesInfo(title, location, activator, request_easy)
  ...
  local function info_line(indent, text, time, num, face, easy)
    ...
    local retval = string.rep(" ", indent) .. text .. string.rep(" ", tab_width - str_len) .. self:hoursToClock(time)
    if easy then
  ...
  UIManager:show(InfoMessage:new({
    ...
    text = title
      ...
      .. info_line(
        request_easy and 0 or 4,
        request_easy and gettext("Twilight:") or gettext("Civil:"),
        times[4],
        4,
        face
      )
      .. add_line(2, gettext("Dawn"), request_easy)
      .. info_line(0, gettext("Sunrise:"), times[5], 5, face)
      .. "\n"
      .. info_line(0, gettext("Solar noon:"), times[6], 6, face, request_easy)
      .. add_line(0, "", request_easy)
      .. info_line(0, gettext("Sunset:"), times[7], 7, face)
      .. add_line(2, gettext("Dusk"), request_easy)
      .. info_line(
        request_easy and 0 or 4,
        request_easy and gettext("Twilight:") or gettext("Civil:"),
        times[8],
      ...
      .. info_line(4, gettext("Nautical:"), times[9], 9, face, request_easy)
      .. info_line(4, gettext("Astronomic:"), times[10], 10, face, request_easy)
      .. add_line(2, gettext("Dusk"), request_easy)
      .. info_line(0, gettext("Solar midnight:"), times[midnight_index], midnight_index, face, request_easy)
      -- add fl toggle
      .. add_line(0, "", not self.fl_off_during_day)
      .. add_line(0, gettext("Toggle frontlight off between"), not self.fl_off_during_day)
      .. add_line(
        4,
        T(
          gettext("%1 and %2"),
...
function AutoWarmth:getTimesMenu(title, location, activator)
  return {
    enabled_func = function()
      ...
      return self.activate ~= 0 and (self.activate ~= activate_sun or activator == nil) or activator == activate_sun
    end,
    text_func = function()
      if location then
