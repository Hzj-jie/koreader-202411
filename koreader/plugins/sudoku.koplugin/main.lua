...
function SudokuBoard:recalcConflicts()
  ...
  for r = 1, 9 do
    ...
    for c = 1, 9 do
      cells[#cells + 1] =
        { row = r, col = c, value = self:getWorkingValue(r, c) }
    end
    markConflicts(cells)
  end
  for c = 1, 9 do
    ...
    for box_col = 0, 2 do
      ...
      for r = 1, 3 do
        for c = 1, 3 do
          ...
          cells[#cells + 1] =
            { row = row, col = col, value = self:getWorkingValue(row, col) }
        end
      end
      markConflicts(cells)
    end
...
function SudokuBoard:setSelection(row, col)
  self.selected =
    { row = math.max(1, math.min(9, row)), col = math.max(1, math.min(9, col)) }
end
...
function SudokuBoard:setValue(value)
  if self.reveal_solution then
    return false, gettext("Hide result to keep playing.")
  end
  local row, col = self:getSelection()
  if self:isGiven(row, col) then
    return false, gettext("This cell is fixed.")
  end
  ...
  if prev_value == new_value and not prev_notes then
    if not value then
      return false, gettext("Cell already empty.")
    end
    return true
  end
...
function SudokuBoard:toggleNoteDigit(value)
  ...
  if self.user[row][col] ~= 0 then
    return false, gettext("Clear the cell before adding notes.")
  end
...
function SudokuBoard:undo()
  ...
  if not entry then
    return false, gettext("Nothing to undo.")
  end
...
function SudokuBoard:isSolved()
  ...
  for r = 1, 9 do
    for c = 1, 9 do
      if
        self:getWorkingValue(r, c) ~= self.solution[r][c]
        or self.conflicts[r][c]
      then
        return false
      end
    end
...
function SudokuBoardWidget:init()
  ...
  self.number_face =
    Font:getFace("cfont", math.max(28, math.floor(self.size / 14)))
  self.note_face =
    Font:getFace("smallinfofont", math.max(16, math.floor(self.size / 28)))
...
function SudokuBoardWidget:paintTo(bb, x, y)
  ...
  self.paint_rect =
    Geom:new({ x = x, y = y, w = self.dimen.w, h = self.dimen.h })
  ...
  for row = 1, 9 do
    for col = 1, 9 do
      ...
      if value then
        ...
        local baseline = cell_y
          + cell_padding
          + math.floor((cell_inner + metrics.y_top - metrics.y_bottom) / 2)
        local text_x = cell_x
          + cell_padding
          + math.floor((cell_inner - text_w) / 2)
        ...
        if notes then
          ...
          for digit = 1, 9 do
            if notes[digit] then
              ...
              local note_metrics = RenderText:sizeUtf8Text(
                0,
                mini_inner,
                self.note_face,
                note_text,
              ...
              local note_baseline = mini_y
                + mini_padding
                + math.floor(
                  (mini_inner + note_metrics.y_top - note_metrics.y_bottom) / 2
                )
              local note_x = mini_x
                + mini_padding
                + math.floor((mini_inner - note_metrics.x) / 2)
              RenderText:renderUtf8Text(
                bb,
                note_x,
                note_baseline,
...
function SudokuScreen:init()
  ...
  self.status_text = TextWidget:new({
    text = gettext("Tap a cell, then pick a number."),
...
function SudokuScreen:getNoteButtonText()
  return self.note_mode and gettext("Note: On") or gettext("Note: Off")
end
...
function SudokuScreen:toggleNoteMode()
  ...
  self:updateNoteButton()
  self:updateStatus(
    self.note_mode and gettext("Note mode enabled.")
      or gettext("Note mode disabled.")
  )
end

function SudokuScreen:getDifficultyButtonText()
  local label = DIFFICULTY_LABELS[self.board.difficulty]
    or self.board.difficulty
  return T(gettext("Difficulty: %1"), label)
end
...
function SudokuScreen:openDifficultyMenu()
  ...
  local function selectDifficulty(level)
    if level ~= self.board.difficulty then
      ...
      self:ensureShowButtonState()
      self:updateStatus(
        T(gettext("Started a %1 game."), DIFFICULTY_LABELS[level] or level)
      )
    else
      self:updateStatus()
    end
  ...
  menu = Menu:new({
    title = gettext("Select difficulty"),
    item_table = items,
...
function SudokuScreen:updateStatus(message)
  ...
  if message then
    ...
    status =
      T(gettext("Selected: %1,%2  ·  Empty cells: %3"), row, col, remaining)
    if self.board:isShowingSolution() then
      status = status
        .. "\n"
        .. gettext("Result is being shown; editing is disabled.")
    elseif self.board:isSolved() then
      status = gettext("Congratulations! Puzzle solved.")
    elseif self.note_mode then
      status = status .. "\n" .. gettext("Note mode is ON.")
    end
  end
...
function SudokuScreen:inputDigit(value)
  ...
  if self.board:isSolved() then
    UIManager:show(
      InfoMessage:new({ text = gettext("Puzzle complete!"), timeout = 4 })
    )
  end
end
...
function SudokuScreen:startNewGame()
  ...
  self:ensureShowButtonState()
  self:updateUndoButton()
  self:updateStatus(gettext("Started a new game."))
end
...
function SudokuScreen:toggleSolution()
  ...
  self:ensureShowButtonState()
  self:updateStatus(
    self.board:isShowingSolution() and gettext("Showing the solution.") or nil
  )
end
...
function SudokuScreen:ensureShowButtonState()
  ...
  local text = self.board:isShowingSolution() and gettext("Hide result")
    or gettext("Show result")
...
function SudokuScreen:checkProgress()
  ...
  self.plugin:saveState()
  if self.board:isSolved() then
    self:updateStatus(gettext("Everything looks good!"))
  elseif self.board:getRemainingCells() == 0 then
    self:updateStatus(gettext("There are mistakes highlighted in red."))
  else
    self:updateStatus(gettext("Keep going!"))
  end
end
...
function SudokuScreen:undoMove()
  ...
  self.board_widget:refresh()
  self:updateStatus(gettext("Last move undone."))
  self.plugin:saveState()
  self:updateUndoButton()
end
