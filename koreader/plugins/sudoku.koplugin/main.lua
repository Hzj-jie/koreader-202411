...
function SudokuScreen:toggleNoteMode()
    ...
    self:updateNoteButton()
    self:updateStatus(self.note_mode and gettext("Note mode enabled.") or gettext("Note mode disabled."))
end
...
function SudokuScreen:openDifficultyMenu()
    ...
    local function selectDifficulty(level)
        if level ~= self.board.difficulty then
            ...
            self:ensureShowButtonState()
            self:updateStatus(T(gettext("Started a %1 game."), DIFFICULTY_LABELS[level] or level))
        else
            self:updateStatus()
        end
        self:updateDifficultyButton()
...
function SudokuScreen:updateStatus(message)
    ...
    if message then
        ...
        status = T(gettext("Selected: %1,%2  ·  Empty cells: %3"), row, col, remaining)
...
function SudokuScreen:onDigit(value)
    ...
    if self.board:isSolved() then
        UIManager:show(InfoMessage:new{ text = gettext("Puzzle complete!"), timeout = 4 })
    end
end
...
function SudokuScreen:toggleSolution()
    ...
    self.board_widget:refresh()
    self:ensureShowButtonState()
    self:updateStatus(self.board:isShowingSolution() and gettext("Showing the solution.") or nil)
end
...
function SudokuScreen:ensureShowButtonState()
    ...
    local text = self.board:isShowingSolution() and gettext("Hide result") or gettext("Show result")
