...
function PerceptionExpander:createUI(readSettings)
  if readSettings then
    ...
    self.line_color_intensity = tonumber(self.settings:read("line_color_intensity"))
...
function PerceptionExpander:onPageUpdate(pageno)
  ...
  if
    self.shift_each_pages ~= 0
    and self.page_counter >= self.shift_each_pages
    and self.margin < self.ALMOST_CENTER_OF_THE_SCREEN
  then
    self.page_counter = 0
    self.margin = self.margin + self.margin_shift
