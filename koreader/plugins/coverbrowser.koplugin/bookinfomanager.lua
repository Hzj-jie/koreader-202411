...
local BOOKINFO_INSERT_SQL = "INSERT OR REPLACE INTO bookinfo "
  ...
  .. ") "
  .. "VALUES ("
  .. table.concat(bookinfo_values_sql, ",")
  .. ");"
local BOOKINFO_SELECT_SQL = "SELECT "
  .. table.concat(BOOKINFO_COLS_SET, ",")
  .. " FROM bookinfo "
  ...
  for num, col in ipairs(BOOKINFO_COLS_SET) do
    ...
    if col == "cover_w" then
      ...
      if bookinfo["has_cover"] then
        ...
        assert(
          cover_size == expected_cover_size,
          "Uncompressed a "
            .. tonumber(cover_size)
            .. "b BB instead of the expected "
            .. tonumber(expected_cover_size)
            .. "b"
        )
...
function BookInfoManager:collectSubprocesses()
  ...
  if #self.subprocesses_pids > 0 then
    ...
    if #self.subprocesses_pids > 0 then
      ...
      if time.monotonic() > self.subprocesses_last_added_time + self.subprocesses_killall_timeout_time then
...
function BookInfoManager:extractInBackground(files)
  ...
  self.subprocesses_last_added_time = time.monotonic()
