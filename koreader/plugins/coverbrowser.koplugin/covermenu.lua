...
function CoverMenu:updateCache(file, status, do_create, pages)
  if do_create then -- create new cache entry if absent
    ...
    local has_highlight = (next(doc_settings:readTableSetting("annotations")) and true) or
      (next(doc_settings:readTableSetting("highlight")) and true)
...
function CoverMenu:updateItems(select_number, no_recalculate_dimen)
  ...
  if
    self.path -- FileManager only
    and (
      not self.showFileDialog_ours -- never replaced
      or self.showFileDialog ~= self.showFileDialog_ours
    )
  then -- it is no more ours
...
function CoverMenu:tapPlus()
  ...
  if self.file_dialog.select_mode then
    return
  end -- do not change select menu
