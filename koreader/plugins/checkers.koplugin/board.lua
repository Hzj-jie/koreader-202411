...
local function sq_id(rank, file)
  return rank * BOARD_SIZE + file + 1
end
...
local function rebuild_overlay(og)
  ...
  for _, purpose in ipairs(OVERLAY_ORDER) do
    ...
    if name then
      og[#og + 1] = new_overlay_icon(name, og._w, og._h)
    end
  end
end
...
local function clear_overlay(button, purpose)
  ...
  local og = lc[1]
  if not (og and og._is_overlay) then
    return
  end
  if purpose then
...
function Board:init()
  if not self.game then
    error("CheckersBoard: must be initialised with a game")
  end
...
function Board:_apply_square_colors()
  for rank = 0, BOARD_SIZE - 1 do
    for file = 0, BOARD_SIZE - 1 do
      ...
      local color = (rank + file) % 2 == 1 and Blitbuffer.COLOR_DARK_GRAY
...
function Board:handleClick(rank, file)
  ...
  if game:is_mid_jump() then
    ...
    for _, m in ipairs(valid) do
      if m[2] == pos then
        self:_execute_move(from_pos, pos)
        return
      end
    end
    return
...
function Board:_execute_move(from_pos, to_pos)
  ...
  if self.moveCallback then
    self.moveCallback(from_pos, to_pos)
  end
end
...
function Board:updateBoard()
  for pos = 1, 32 do
    self:updateSquare(pos)
  end
  UIManager:setDirty(self, "ui")
end

function Board:updateSquare(pos)
  local v = pos_to_visual[pos]
  if not v then
    return
  end
  ...
  local color = (v.rank + v.file) % 2 == 1 and Blitbuffer.COLOR_DARK_GRAY
    or Blitbuffer.COLOR_LIGHT_GRAY
  btn.frame.background = color
  btn.frame.border_color = color
end
...
function Board:clearValidMoves()
  if not self.hint_positions then
    return
  end
