...
function Checkers:buildLayout()
  ...
  self.board = CheckersBoard:new({
    ...
    moveCallback = function(from, to)
      self:onMoveExecuted(from, to)
    end,
  })

  local btn_w = math.floor(self.full_width / 2)
  local toolbar = HorizontalGroup:new({
    self:_toolbar_btn("chevron.left", btn_w, toolbar_h, function()
      self:doUndo()
    end),
    self:_toolbar_btn("plus", btn_w, toolbar_h, function()
      self:askNewGame()
    end),
  })
...
function Checkers:_turn_text()
  if self.game and self.game:is_over() then
    ...
    if winner == 1 then
      return gettext("Black wins!")
    elseif winner == 2 then
      return gettext("White wins!")
    else
      return gettext("Draw!")
    end
  end
  if self.game then
    ...
    if turn == 1 then
      if mid then
        return gettext("Black — must continue jump")
      end
      return is_ai and gettext("AI thinking…") or gettext("Black's turn")
    else
      if mid then
        return gettext("White — must continue jump")
      end
...
function Checkers:doAIMove()
  ...
  if self.human[self.game:whose_turn()] then
    return
  end
...
function Checkers:doUndo()
  if not self.game:can_undo() then
    return
  end
...
function Checkers:showGameOver()
  ...
  local text
  if winner == 1 then
    text = gettext("Black wins!")
  elseif winner == 2 then
    text = gettext("White wins!")
  else
    text = gettext("Draw — move limit reached.")
  end
  UIManager:show(ConfirmBox:new({
    text = text,
    ok_text = gettext("New Game"),
