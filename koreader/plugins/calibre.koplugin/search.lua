...
function CalibreSearch:onMenuHold(item)
  if not item.info or item.info:len() <= 0 then
    return
  end
...
function CalibreSearch:find(option)
  ...
  logger.info(
    string.format(
      "search done in %.3f milliseconds (%s, %s, %s, %s, %s)",
  ...
  UIManager:show(ConfirmBox:new({
    ...
    ok_callback = function()
      ...
      if count == 0 then
        ...
        info_text = T(
          gettext("Found %1 calibre libraries with %2 books:\n%3"),
          count,
          #self.books,
          paths
        )
...
function CalibreSearch:getMetadata()
  ...
  if self.cache_metadata then
    local function cacheIsNewer(timestamp)
      ...
      logger.dbg(
        "CalibreSearch:getMetadata: Cache timestamp   :",
        cache_timestamp,
        os.date("!%FT%T.000000+00:00", cache_timestamp),
        os.date("(%F %T %z)", cache_timestamp)
      )
      logger.dbg(
        "CalibreSearch:getMetadata: Metadata timestamp:",
        meta_timestamp,
        timestamp,
        os.date("(%F %T %z)", meta_timestamp)
      )
