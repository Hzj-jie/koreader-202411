...
function Board:init()
  ...
  self.offset_y =
    math.floor((self.height - self.cell_size * self.game.rows) / 2)
  ...
  for r = 1, self.game.rows do
    for c = 1, self.game.cols do
      ...
      for _, icon_name in ipairs(layers) do
        local sprite = (icon_name == "player") and self.player_sprite
          or icon_name
...
function Board:onSwipe(ges)
  ...
  if dir == "east" then
    dc = 1
    self.player_sprite = "player_right"
  elseif dir == "west" then
    dc = -1
    self.player_sprite = "player_left"
  elseif dir == "south" then
    dr = 1
    self.player_sprite = "player"
  elseif dir == "north" then
    dr = -1
    self.player_sprite = "player_up"
  else
    return false
  end
