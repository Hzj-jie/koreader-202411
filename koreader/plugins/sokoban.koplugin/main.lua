...
function Sokoban:_buildWidget()
  ...
  local flex_w =
    math.max(0, sw - left_toolbar:getSize().w - right_toolbar:getSize().w)
...
function Sokoban:_onSolved()
  ...
  self.furthest_reached[self.current_set] =
    math.max(fr, math.min(self.current_level + 1, #ls.levels))

  -- full refresh so e-ink ghosts clear
  UIManager:setDirty(self.widget, "full")

  local msg = gettext("Solved!")
    .. "\n"
    .. gettext("Moves: ")
    .. self.game.moves
    .. "  "
  ...
  UIManager:show(ConfirmBox:new({
    text = msg,
    ok_text = has_next and gettext("Next Level") or gettext("OK"),
    cancel_text = gettext("Menu"),
...
function Sokoban:_onSkip(set_idx, level_num)
  ...
  self.furthest_reached[ls.name] =
    math.max(fr, math.min(level_num + 1, #ls.levels))
  self:_saveSettings()
