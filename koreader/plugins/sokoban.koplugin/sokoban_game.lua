...
function Game:move(dr, dc)
  ...
  if dest == BOX or dest == BOX_ON then
    local br = nr + dr
    local bc = nc + dc
    if br < 1 or br > self.rows or bc < 1 or bc > self.cols then
      return false
    end
    local behind = self.grid[br][bc]
    if behind == WALL or behind == BOX or behind == BOX_ON then
      return false
    end
    -- move box
