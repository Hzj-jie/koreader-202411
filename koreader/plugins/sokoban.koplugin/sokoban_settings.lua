...
function SettingsWidget:init()
  ...
  for _, ls in ipairs(self.level_sets) do
    if ls.name == self.current_set then
      total = ls.count
      break
    end
  end
  ...
  local at_frontier = (
    self.current_set == (self.playing_set or self.current_set)
  )
    and (playing >= fr)
    and not play_solved
    and (playing < total)
    ...
    buttons = {
      {
        ...
        {
          text = mark .. tostring(self.current_level) .. " / " .. tostring(
            total
          ),
  ...
  local play_btn = ButtonTable:new({
    ...
    buttons = {
      {
        {
          text = at_frontier and gettext("Skip Level") or gettext("Play"),
          callback = function()
            local set_idx = 1
            for i, ls in ipairs(self.level_sets) do
              if ls.name == self.current_set then
