...
function BookDailyItem:init()
  ...
  local title_max_width = self:getSize().w - 2 * Size.padding.default - checked_widget:getSize().w - self.value_width
...
function CalendarDayView:setupView()
  ...
  self.footer_container[1] = self.pages > 1 and self.page_info or VerticalSpan:new({ height = 0 })
...
function CalendarDayView:generateSpan(start, finish, bgcolor, fgcolor, title)
  ...
  return FrameContainer:new({
    ...
    CenterContainer:new({
      dimen = Geom:new({
        h = self.hour_height - 2 * self.inner_padding,
        w = width,
      }),
...
function CalendarView:init()
  ...
  -- 7 days in a week
  self.day_width = math.floor((self:getSize().w - 2 * self.outer_padding - 6 * self.inner_padding) * (1 / 7))
  -- Put back the possible 7px lost in rounding into outer_padding
  self.outer_padding = math.floor((self:getSize().w - 7 * self.day_width - 6 * self.inner_padding) * (1 / 2))
...
function CalendarView:_populateItems()
  ...
  table.insert(self.main_content, VerticalSpan:new({ height = self.inner_padding }))
  self.weeks = {}
