...
function ReaderProgress:genDoubleHeader(title_left, title_right)
  ...
  local line_width = (
    self.screen_width
    - header_title_left:getSize().w
    - header_title_right:getSize().w
    - self.padding * 7
  ) / 4
  local line_container = LeftContainer:new({
    dimen = Geom:new({ w = line_width, h = self.screen_height * (1 / 25) }),
...
function ReaderProgress:genWeekStats(stats_day)
  ...
  for i = 1, stats_day do
    ...
    if day_time > max_week_time then
...
function ReaderProgress:genSummaryDay(width)
  ...
  return CenterContainer:new({
    dimen = Geom:new({
      w = self.screen_width,
      h = math.floor(self.screen_height * 0.13),
    }),
    statistics_container,
  })
end
...
function ReaderProgress:genSummaryWeek(width)
  ...
  return CenterContainer:new({
    dimen = Geom:new({
      w = self.screen_width,
      h = math.floor(self.screen_height * 0.10),
    }),
    statistics_container,
  })
end
