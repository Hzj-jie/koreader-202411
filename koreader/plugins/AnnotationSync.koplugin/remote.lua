...
function M._sync_progress_callback(local_file, _, income_file)
  ...
  for device_id, data in pairs(income_data) do
    if
      not local_data[device_id]
      or (data.timestamp or "") > (local_data[device_id].timestamp or "")
    then
...
local function run_silent(func, on_timeout)
  ...
  new_show = function(self, widget_item)
    if widget_item.text == gettext("Successfully synchronized.") then
      return
    end
...
function M.push_progress(widget, json_path, on_complete)
  ...
  if server then
    ...
    run_silent(function(restore)
      local success = widget.ui.cloudstorage:sync(
        server,
        json_path,
        function(local_file, cached_file, income_file)
          cb_called = true
          local success =
            M._sync_progress_callback(local_file, cached_file, income_file)
  ...
  if server then
    ...
    Trapper:wrap(function()
      local completed, success = Trapper:dismissableRunInSubprocess(function()
        ...
        run_silent(function(restore)
          local res = widget.ui.cloudstorage:sync(
            server,
            json_path,
            function(local_file, cached_file, income_file)
              sync_success =
      ...
      if completed and not success then
        logger.info(
          "AnnotationSync: background progress sync failed/unsupported, falling back to in-process sync"
        )
