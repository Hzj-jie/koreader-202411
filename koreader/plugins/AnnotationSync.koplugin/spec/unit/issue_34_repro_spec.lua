describe("Issue #34 Reproduction & Fix Verification", function()
  ...
  it(
    "verifies that Sync All no longer reports false success on push failure",
    function()
      ...
      assert.is_true(
        found,
        "Sync All should have reported failure when sync failed"
      )
      ...
      assert.is_equal(
        1,
        count,
        "Document should still be pending if sync failed"
      )
    end
  )
  ...
  it(
    ...
    function()
      ...
      assert.is_equal(
        0,
        count,
        "Sync should have succeeded by treating 404 body as empty state"
      )
    end
  )
end)
