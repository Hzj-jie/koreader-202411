describe("AnnotationSync Mixed Documents & Offline Sync All", function()
  ...
  local test_data_dir = require("datastorage"):getDataDir() .. "/test_sync_mixed_offline_tmp"
  ...
  it(
    "Sync All processes both dirty EPUB and PDF and they stay dirty if offline",
    function()
      ...
      assert.is_equal(
        2,
        (select(1, sync_instance.manager:getPendingChangedDocuments()))
      )
  ...
  it(
    "Sync All performs full bidirectional merge for both mixed document types",
    function()
      local doc_epub = sample_epub
      local doc_pdf = sample_pdf_dest
      ...
      ds_pdf:save("annotations", { ann_pdf_l })
      ds_pdf:flush()
      sync_instance.manager:addToChangedDocumentsFile(doc_pdf)
