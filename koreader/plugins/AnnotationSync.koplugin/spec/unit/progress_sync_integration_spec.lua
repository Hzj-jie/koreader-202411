describe("Reading Progress Sync Integration", function()
  ...
  before_each(function()
    ...
    readerui.getCurrentPage = function(this)
      return readerui.document.page or 1
    end
    if not readerui.paging then
      readerui.paging = {
        number_of_pages = 100,
        getLastPercent = function(this)
          return 0
        end,
        getLastProgress = function(this)
          return "mock-pos-123"
        end,
      }
    else
      readerui.paging.getLastPercent = function(this)
        return 0
      end
      readerui.paging.getLastProgress = function(this)
        return "mock-pos-123"
      end
    end
    ...
    readerui.document.gotoPage = function(this, page)
      this.page = page
    end
    readerui.document.getPageIndex = function(this)
      return readerui.document.page or 1
    end
    readerui.document.getPercentage = function(this)
      return 0.1
    end
  ...
  it("displays remote entries in jump menu and allows jumping", function()
    ...
    UIManager.broadcastEvent = function(this, event)
      if
        event.handler == "onJumpToPage"
        and (
          event.args == 10
          or (type(event.args) == "table" and event.args[1] == 10)
        )
      then
  ...
  it(
    ...
    function()
      ...
      assert.is_nil(
        require("libs/libkoreader-lfs").attributes(test_sdr_dir, "mode")
      )
  ...
  it(
    ...
    function()
      ...
      local old_getPrevVisibleWordStart =
        readerui.document.getPrevVisibleWordStart
      ...
      local hash = util.partialMD5(readerui.document.file)
      local sdr_dir =
        require("docsettings"):getSidecarDir(readerui.document.file)
      local json_path = sdr_dir .. "/" .. hash .. ".progress.json"
    ...
    function()
      ...
      UIManager.show = function(this, widget)
        if widget.title == "Jump to device progress" then
          for _, item in ipairs(widget.item_table) do
            if
              item.text:find("MyCustomDevice")
              and item.text:find("%(this device%)")
            then
