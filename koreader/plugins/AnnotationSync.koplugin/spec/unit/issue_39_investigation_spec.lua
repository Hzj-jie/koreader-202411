describe("Issue #39 Investigation: Unintended Deletion", function()
  ...
  it(
    "reproduces re-deletion if timestamps are identical (Hypothesis: latest-wins preference)",
    function()
      ...
      local ok, merged_list = annotations_mod.sync_callback(
        readerui.document,
        local_path,
        sync_path,
        income_path,
      ...
      assert.is_equal(
        1,
        #merged_list,
        "Annotation should stay restored even if timestamps are identical"
      )
    end
  )
  ...
  it("reproduces 'Delayed JSON Flush' (Metadata Inconsistency)", function()
    ...
    assert.is_false(
      disk_map[key].deleted,
      "Disk JSON should have updated 'deleted' status after restoration"
    )
  end)
  ...
  it("reproduces 'Menu Ghosting' (Metadata Inconsistency)", function()
    ...
    assert.is_equal(
      0,
      #still_deleted,
      "Annotation should not be listed as deleted after restoration"
    )
  end)
  ...
  it("reproduces 'Sync Collision' (Metadata Inconsistency)", function()
    ...
    SyncService.sync = function(server, local_path, callback)
      ...
      assert.is_false(
        local_content[key].deleted,
        "Sync should use updated local state (not stale disk JSON)"
      )
