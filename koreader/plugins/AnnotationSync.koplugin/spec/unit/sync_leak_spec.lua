...
describe("Sync Document Leak Verification", function()
  ...
  it(
    "closes document handles even if sync fails (pcall verification)",
    function()
      ...
      manager.syncDocument = function()
        error("Simulated sync crash")
      end
      ...
      assert.is_equal(
        1,
        open_calls[doc_fail],
        "fail.epub should have been opened"
      )
      assert.is_equal(
        1,
        close_calls[doc_fail],
        "fail.epub should have been closed despite crash"
      )
    end
  )
end)
