...
function Screen:init()
  ...
  self.board_widget = BoardWidget:new({
    ...
    onTileTap = function(row, col)
      self:performTap(row, col)
    end,
    onSwipeDir = function(direction)
      self:performSwipe(direction)
    end,
  })
  self:buildLayout()
  self:updateHeader()
...
function Screen:updateHeader()
  ...
  self.header_text:setText(
    T(
      I18n.t("%1x%1 Puzzle    Moves: %2    Time: %3"),
      n,
      self.game:getMoves(),
      formatTime(live)
    )
...
function Screen:updateBestLabel()
  ...
  if stats and (stats.best_time or stats.best_moves) then
    ...
    self.best_text:setText(
      T(I18n.t("Best: %1    Fewest moves: %2"), time_s, moves_s)
    )
  else
...
function Screen:_scheduleTick()
  UIManager:scheduleIn(
    TICK_INTERVAL,
    self.tick_handler or self:_makeTickHandler()
  )
end
...
function Screen:afterMove()
  ...
  if self.game:isWon() then
    self:updateMessage(
      T(
        I18n.t("Solved in %1 moves and %2."),
        self.game:getMoves(),
  ...
  if self.game:isWon() then
    ...
    UIManager:show(InfoMessage:new({
      text = T(
        I18n.t("You solved the %1x%1 puzzle!\nMoves: %2\nTime: %3"),
        self.game:getSize(),
        self.game:getMoves(),
...
function Screen:switchSize(size)
  if size == self.game:getSize() then
    return
  end
...
function Screen:showStats()
  ...
  for size = Game.getMinSize(), Game.getMaxSize() do
    ...
    local time_s = (stats and stats.best_time) and formatTime(stats.best_time)
      or "--:--"
    local moves_s = (stats and stats.best_moves) and tostring(stats.best_moves)
      or "--"
