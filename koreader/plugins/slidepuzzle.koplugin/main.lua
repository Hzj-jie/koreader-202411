...
function SlidePuzzle:init()
  ...
  self.active_size = tonumber(self.settings:read("active_size"))
    or Game.getMinSize()
  if
    self.active_size < Game.getMinSize()
    or self.active_size > Game.getMaxSize()
  then
    self.active_size = Game.getMinSize()
  end
...
function SlidePuzzle:getCurrentGame()
  if
    not self._cached_game or self._cached_game:getSize() ~= self.active_size
  then
...
function SlidePuzzle:setActiveSize(size)
  ...
  if size < Game.getMinSize() then
    size = Game.getMinSize()
  end
  if size > Game.getMaxSize() then
    size = Game.getMaxSize()
  end
