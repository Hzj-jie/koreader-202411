...
function BoardWidget:_computeMetrics()
  ...
  local available = self.max_size
    or math.min(Device.screen:getWidth(), Device.screen:getHeight())
...
function BoardWidget:_cellFromPoint(x, y)
  if not self.paint_rect or self.cell <= 0 then
    return nil
  end
...
function BoardWidget:onTap(_, ges)
  if not (ges and ges.pos and self.onTileTap) then
    return false
  end
...
function BoardWidget:onSwipe(_, ges)
  if not (ges and ges.direction and self.onSwipeDir) then
    return false
  end
...
function BoardWidget:paintTo(bb, x, y)
  self.paint_rect =
    Geom:new({ x = x, y = y, w = self.board_size, h = self.board_size })
  local n = self.game:getSize()
  ...
  for r = 1, n do
    for c = 1, n do
      ...
      if value == 0 then
        ...
        local metrics =
          RenderText:sizeUtf8Text(0, cell, self.number_face, text, true, false)
        local text_w = metrics.x
        local baseline = tile_y
          + math.floor((cell + metrics.y_top - metrics.y_bottom) / 2)
