...
Settings.FONTS = {
  { id = "default", i18n_key = "Default", face_name = nil },
  {
    id = "sans",
    label = "Sans (Noto)",
    face_name = "NotoSans-Regular.ttf",
  },
  {
    id = "sans_bold",
    label = "Sans Bold (Noto)",
    face_name = "NotoSans-Bold.ttf",
  },
  {
    id = "serif",
    label = "Serif (FreeSerif)",
    face_name = "freefont/FreeSerif.ttf",
  },
  {
    id = "free_sans",
    label = "FreeSans",
    face_name = "freefont/FreeSans.ttf",
  },
  {
    id = "mono",
    label = "Monospace (Droid)",
    face_name = "DroidSansMono.ttf",
  },
}
...
function Settings.computeAutoFontSize(plugin)
  ...
  if px < Settings.FONT_SIZE_MIN then
    px = Settings.FONT_SIZE_MIN
  end
  if px > Settings.FONT_SIZE_MAX then
    px = Settings.FONT_SIZE_MAX
  end
  return px
end
...
function Settings.getFontSize(plugin)
  local size = tonumber(plugin.settings:read("font_size"))
    or Settings.AUTO_FONT_SIZE
  if size < 0 then
    size = Settings.AUTO_FONT_SIZE
  end
  return size
end
...
function Settings.buildSubMenu(plugin)
  ...
  items[#items + 1] = {
    ...
    callback = function()
      plugin.settings:save(
        "always_new_on_open",
        not plugin.settings:isTrue("always_new_on_open")
      )
  ...
  local function fontRadioItem(font_def)
    return {
      ...
      callback = function()
        ...
        plugin.settings:flush()
        if plugin.onSettingsChanged then
  ...
  items[#items + 1] = {
    text_func = function()
      return I18n.t("Reset best results")
    end,
    keep_menu_open = true,
    callback = function()
