...
function NewsDownloader:lazyInitialization()
  if not self.initialized then
    ...
    if not lfs.attributes(self.feed_config_path, "mode") then
      ...
      FFIUtil.copyFile(FFIUtil.joinPath(self.path, self.feed_config_file), self.feed_config_path)
    end
...
function NewsDownloader:loadConfigAndProcessFeeds(touchmenu_instance)
  ...
  if not ok or not feed_config then
    UI:info(T(gettext("Invalid configuration file. Detailed error message:\n%1"), feed_config))
    return
  end
  ...
  if #feed_config <= 0 then
    ...
    local should_edit_feed_list = UI:confirm(
      T(gettext("Feed list is empty. If you want to download news, you'll have to add a feed first.")),
      gettext("Close"),
    ...
    if should_edit_feed_list then
      ...
      local feed_item_vc = FeedView:getItem(1, self.empty_feed, function(id, edit_key, value)
        self:editFeedAttribute(id, edit_key, value)
      end)
      self:viewFeedItem(feed_item_vc)
    end
    return
  end
  ...
  for idx, feed in ipairs(feed_config) do
    ...
    if url and limit then
      feed_message = T(gettext("Processing %1/%2:\n%3"), idx, total_feed_entries, BD.url(url))
...
review your feed configuration file.]]),
  ...
  if touchmenu_instance then
    ...
    local should_go_to_downloads = UI:confirm(feed_message, gettext("Close"), gettext("Go to downloads"))
...
function NewsDownloader:processFeedSource(
  url,
  credentials,
  limit,
  unsupported_feeds_urls,
...
function NewsDownloader:processFeed(
  feed_type,
  feeds,
  cookies,
  limit,
  ...
  if feed_type == FEED_TYPE_RSS then
    ...
    total_items = (limit == 0) and #feeds.rss.channel.item or limit
  else
    feed_title = getFeedTitle(feeds.feed.title)
    feed_item = feeds.feed.entry
    total_items = (limit == 0) and #feeds.feed.entry or limit
  end
  -- Get the path to the output directory.
  local feed_output_dir = ("%s%s/"):format(self.download_dir, util.getSafeFilename(util.htmlEntitiesToUtf8(feed_title)))
  ...
  for index, feed in pairs(feed_item) do
    ...
    if download_full_article then
      self:downloadFeed(feed, cookies, feed_output_dir, include_images, article_message, enable_filter, filter_element)
    else
      self:createFromDescription(feed, feed_description, feed_output_dir, include_images, article_message)
    end
  end
end
...
function NewsDownloader:downloadFeed(
  feed,
  cookies,
  feed_output_dir,
  include_images,
  ...
  local news_file_path = ("%s%s%s"):format(feed_output_dir, title_with_date, self.file_extension)
  ...
  require("ui/downloadmgr")
    :new({
      onConfirm = function(path)
        ...
        logger.dbg(
          "NewsDownloader: Coping to new download folder previous self.feed_config_file from: ",
...
function NewsDownloader:viewFeedList()
  ...
  local view_content = FeedView:getList(feed_config, function(feed_item_vc)
    self:viewFeedItem(feed_item_vc)
  end, function(id, edit_key, value)
    self:editFeedAttribute(id, edit_key, value)
  end, function(id)
    self:deleteFeed(id)
  end)
  ...
  table.insert(view_content, {
    ...
    callback = function()
      ...
      local feed_item_vc = FeedView:getItem(#feed_config + 1, self.empty_feed, function(id, edit_key, value)
        self:editFeedAttribute(id, edit_key, value)
      end)
      self:viewFeedItem(feed_item_vc)
    end,
...
function NewsDownloader:editFeedAttribute(id, key, value)
  ...
  if key == FeedView.URL or key == FeedView.LIMIT or key == FeedView.FILTER_ELEMENT then
    local title
    local input_type
    local description
    ...
    elseif key == FeedView.FILTER_ELEMENT then
      ...
      description = gettext("Filter based on the given CSS selector. E.g.: name_of_css.element.class")
      input_type = "string"
    else
      return false
    end
...
function NewsDownloader:updateFeedConfig(id, key, value)
  ...
  if id > #feed_config then
    table.insert(feed_config, self.empty_feed)
  end
  ...
  for idx, feed in ipairs(feed_config) do
    ...
    table.insert(new_config, feed)
  end
  -- Save the config
  logger.dbg("NewsDownloader: config to save", new_config)
  self:saveConfig(new_config)
  -- Refresh the view
  local feed_item_vc = FeedView:getItem(id, new_config[id], function(cb_id, cb_edit_key, cb_value)
...
function NewsDownloader:onCloseDocument()
  ...
  if
    ...
    and self.download_dir == string.sub(document_full_path, 1, string.len(self.download_dir))
  then
