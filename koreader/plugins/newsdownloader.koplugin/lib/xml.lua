...
local xmlParser = function(handler)
  ...
  obj.parse = function(self, str, parseAttributes)
    ...
    local match, pos = 0, 1
    local text, endt1, endt2, tagstr, tagname, attrs, endtext, endmatch, errend, extend, __
    while match do
      ...
      elseif string.sub(tagstr, 1, 8) == "![CDATA[" then
        ...
        while 1 do
          __, errend = string.find(tagstr, self._ATTRERR1)
          if errend == nil then
            __, errend = string.find(tagstr, self._ATTRERR2)
            if errend == nil then
              break
            end
          end
          __, extend, endt2 = string.find(str, self._TAGEXT, endmatch + 1)
