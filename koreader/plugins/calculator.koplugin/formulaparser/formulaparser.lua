...
function Parser.storeVal(l, r, comment)
  if l == nil or type(l) ~= "string" then
    return nil, "No variable"
  end
  local rval, err
  rval, err = Parser:_eval(r)
  if not rval or err then
    return nil, err or "Value expected"
  end
...
function Parser.incVal(l, r)
  ...
  if not val or err then
    return val, err or "Value expected"
  end
  rval, err = Parser:_eval(r)
  if not rval or err then
    return nil, err or "Value expected"
...
function Parser.basicOperator(str, operator, right)
  if not str:find("%" .. operator) then
    return
  end -- quick check, lua find is faster than lua code
  ...
  if right == 1 then
    ...
    until (opPos > str_len or str:sub(opPos, opPos + opLen - 1) == operator)
      and (
        charRight ~= "=" -- do this because "=" has higher priority than e.g. ">="
        and charLeft ~= "<"
        and charLeft ~= ">"
        and charLeft ~= "="
        and charLeft ~= "!"
