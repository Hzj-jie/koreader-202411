...
function Game2048Storage:_init()
  self._settings =
    LuaSettings:open(DataStorage:getSettingsDir() .. "/" .. self.name .. ".lua")
end
...
function Game2048Storage:readGameState(profile, state)
  ...
  if state_dump then
    ...
    if
      state.history:read(state_dump.history)
      and state.info:readUntracked(state_dump.info)
    then
...
function Game2048Storage:switchGameState(state)
  ...
  self:saveGameState(state.profile, state)
  state.profile = state.settings.profile
  if not self:readGameState(state.profile, state) then
    -- If no game state is present in given profile, start a new game
    state:newGame()
  end
  return true
end
...
function Game2048State:_applyState(state)
  if
    not state
    or not self.board:setFieldCopy(state.board)
    or not self.info:read(state.info)
  then
    return false
  end
  return true
