  ...
  self.options = {
    ...
    },
    {
      ...
      options = {
        {
          name = "new_tile_delay",
          name_text = gettext("New Tile Delay"),
        ...
        {
          name = "theme",
          name_text = gettext("Color Theme"),
          item_text = { gettext("Select") .. "…" },
          event = "SelectTheme",
        },
        {
          name = "profile",
          name_text = gettext("Profile"),
          toggle = { "1", "2", "3", "4", "5", "6" },
          values = {
            "default",
            "player2",
...
function Game2048Config:onConfigChange(option_name, option_value)
  if not self._did_show_size_notification and "size" == option_name then
    ...
    UIManager:show(InfoMessage:new({
      timeout = 10,
      text = gettext("Start a new game to change the size of the board"),
    }))
  end
