...
function Game2048Widget:init()
  ...
  self._palette_is_rgb = self.palette[1]
      and not Blitbuffer.isColor8(self.palette[1])
    or false
...
function Game2048Widget:paintTo(bb, x, y)
  ...
  local number_pos = 1
  local paintRoundedRect = self._palette_is_rgb and bb.paintRoundedRectRGB32
    or bb.paintRoundedRect
  for row = 1, size do
    ...
    for col = 1, size do
      ...
      local value = number_pos ~= self._hidden_tile_index
          and self.numbers[number_pos]
        or 0
      if value > 0 then
        local value_str, prop =
          VALUE_STR[value] or TILE_FALLBACK_TEXT,
          self._value_str_props[value]
            or self._value_str_props[MAX_VALUE + 1] -- ... or fallback
        local bg_color, fg_color = self:_getTileColors(value)
        paintRoundedRect(
          bb,
          tile_x + INNER_TILE_MARGIN,
          tile_y + INNER_TILE_MARGIN,
          tile_side - (2 * INNER_TILE_MARGIN),
          tile_side - (2 * INNER_TILE_MARGIN),
          bg_color,
          TILE_RADIUS - INNER_TILE_MARGIN
        )
...
function Game2048Widget:setPalette(palette)
  ...
  self._palette_is_rgb = palette[1] and not Blitbuffer.isColor8(palette[1])
    or false
  UIManager:setDirty(self.show_parent or self, "ui", self.dimen)
end
...
function Game2048Widget:_update()
  ...
  local max_text_width = self._tile_side
    - (2 * (INNER_TILE_MARGIN + INNER_TILE_PADDING))
  ...
  for n = 1, (MAX_VALUE + 1) do -- One additional props for the fallback text
    ...
    value_str_props[n] =
      { face = face, size = { w = tsize.x, h = tsize.y_top } }
  end
...
function Game2048Widget:_getTileColors(value)
  ...
  local bg_color = self.palette[math.min(#self.palette, value)]
    or INNER_TILE_DEFAULT_COLOR
...
function Game2048Widget:_registerKeyEvents()
  if Device:hasDPad() then
    ...
    self.key_events.DownMove =
      { { "Down" }, event = "Game2048Move", args = "down" }
    self.key_events.LeftMove =
      { { "Left" }, event = "Game2048Move", args = "left" }
    self.key_events.RightMove =
      { { "Right" }, event = "Game2048Move", args = "right" }
