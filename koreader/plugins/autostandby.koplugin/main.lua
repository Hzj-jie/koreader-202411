...
if
  not Device:isPocketBook() --[[and not Device:isKobo()]]
then
...
function AutoStandby:addToMainMenu(menu_items)
  menu_items.autostandby = {
    ...
    sub_item_table = {
      ...
      self:genSpinMenuItem(gettext("Always standby if battery below"), "bat", function()
        return 0
      end, function()
        return 100
      end),
...
function AutoStandby:onInputEvent()
  ...
  elseif t > AutoStandby.lastInput + config.max then
    ...
    logger.dbg(
      "AutoStandby: input too far in future, resetting adaptive standby delay from",
      AutoStandby.delay,
      "to",
      config.min
    )
