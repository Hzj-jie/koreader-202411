...
function NonogramBoard:generateRandomPuzzle(rows, cols, density)
  ...
  density =
    math.max(0.2, math.min(0.8, density or (0.35 + math.random() * 0.3)))
  local grid = {}
  for r = 1, rows do
    grid[r] = {}
    for c = 1, cols do
  ...
  local puzzle =
    buildPuzzleFromBooleanGrid(grid, T(gettext("Random %1x%2"), rows, cols))
...
function NonogramBoardWidget:init()
  self.number_face =
    Font:getFace("cfont", math.max(22, math.floor(Screen:getWidth() / 32)))
  self.hint_face = Font:getFace(
    "smallinfofont",
    math.max(18, math.floor(Screen:getWidth() / 40))
  )
...
function NonogramBoardWidget:getCellFromPoint(x, y)
  ...
  if
    row < 1
    or row > self.board:getRowCount()
    or col < 1
    or col > self.board:getColCount()
  then
    return nil
  end
  return row, col
...
function NonogramScreen:init()
  ...
  self.status_text = TextWidget:new({
    text = gettext(
      "Tap a cell, then use the buttons below to fill or mark it."
    ),
...
function NonogramScreen:buildLayout()
  ...
  local board_inner_width = board_frame_width
    - 2 * (board_frame_padding + frame_border + frame_margin)
  ...
  local board_inner_height = available_height
    - 2 * (frame_border + frame_margin + board_frame_padding)
  ...
  board_frame.height = math.max(
    0,
    board_inner_height + 2 * (board_frame_padding + frame_border + frame_margin)
  )
  ...
  local scrolling_height = Screen:getHeight()
    - 2 * layout_vertical_margin
    - action_buttons:getSize().h
    - self.status_text:getSize().h
...
function NonogramScreen:onAction(action)
  ...
  if self.board:isSolved() then
    UIManager:show(
      InfoMessage:new({ text = gettext("Great job!"), timeout = 3 })
    )
  end
  self:updateStatus()
end
...
function NonogramScreen:updateActionButtons()
  local function applyBackground(button, is_active)
    ...
    button.frame.background = is_active and Blitbuffer.COLOR_GRAY_E
...
function NonogramScreen:updateSolutionButton()
  ...
  local text = self.board:isShowingSolution() and gettext("Hide solution")
    or gettext("Show solution")
