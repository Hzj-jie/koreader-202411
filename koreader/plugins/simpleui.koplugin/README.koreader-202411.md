...
- **Top-Left Drawing Bug**: Because SimpleUI's dynamic panels add rendering/loading overhead, the startup time gap allows the minute-tick clock update to fire *before* the first layout pass has positioned the widget. This triggers direct widget repainting while the coordinates are still uninitialized, locking the widget's position to `(0, 0)` (top-left of the screen).
