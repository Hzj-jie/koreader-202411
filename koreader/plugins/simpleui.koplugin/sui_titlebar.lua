...
function M.isEnabled()
  return SUISettings:nilOrTrue(SETTING_KEY)
end
function M.setEnabled(v)
  SUISettings:saveSetting(SETTING_KEY, v)
end

local function _visKey(id)
  return "simpleui_tb_item_" .. id
end

function M.isItemVisible(id)
  local v = SUISettings:readSetting(_visKey(id))
  if v == nil then
    return _VIS_DEFAULTS[id] ~= false
  end
  return v == true
end
function M.setItemVisible(id, v)
  SUISettings:saveSetting(_visKey(id), v)
end

function M.getSizeKey()
  return SUISettings:readSetting(SIZE_KEY) or "default"
end
function M.setSizeKey(v)
  SUISettings:saveSetting(SIZE_KEY, v)
end
function M.getSizeScale()
  return _SIZE_SCALE[M.getSizeKey()] or 1.0
end
...
local function _loadCfg(key, defaults)
  ...
  if type(raw.side) == "table" then
    for k, v in pairs(raw.side) do
      side[k] = v
    end
  end
  local order_left = (type(raw.order_left) == "table") and raw.order_left
    or defaults.order_left
  local order_right = (type(raw.order_right) == "table") and raw.order_right
    or defaults.order_right
  ...
  for _, id in ipairs(defaults.order_right) do
    if not in_saved[id] then
      ...
      if not side[id] then
        side[id] = defaults.side[id] or "right"
      end
    end
  end
  for _, id in ipairs(defaults.order_left) do
    if not in_saved[id] then
      ...
      if not side[id] then
        side[id] = defaults.side[id] or "left"
      end
    end
  end
...
function M.getFMConfig()
  return _loadCfg(FM_CFG_KEY, _FM_DEFAULTS)
end
function M.getSubConfig()
  return _loadCfg(SUB_CFG_KEY, _SUB_DEFAULTS)
end
function M.saveFMConfig(cfg)
  SUISettings:saveSetting(FM_CFG_KEY, cfg)
end
function M.saveSubConfig(cfg)
  SUISettings:saveSetting(SUB_CFG_KEY, cfg)
end
...
local function _isSubFolder(path)
  ...
  if G_reader_settings:isTrue("lock_home_folder") then
    ...
    if not h then
      return false
    end -- no home set → hide
    if p == h then
      return false
    end -- at home root → hide
...
local function _isLockedAtHome(path)
  return _isAtFSRoot(path)
    and G_reader_settings:isTrue("lock_home_folder")
    and _normPath(path) == _normHome()
end
...
local function _resizeAndStrip(btn, new_w, keep_top_pad)
  ...
  if
    btn.label_widget
    and type(btn.label_widget.file) == "string"
    and btn.label_widget.file:match("^nerd:")
  then
    btn.label_widget.file = nil
  end
  if
    btn.label_widget
    and type(btn.label_widget.icon) == "string"
    and btn.label_widget.icon:match("^nerd:")
  then
...
local function _snapBtn(btn, opts)
  ...
  if opts then
    ...
    if opts.save_dimen then
      snap.dimen = btn.dimen
    end
  end
  return snap
end
...
local function _restoreBtn(btn, snap)
  ...
  if snap.callback ~= nil then
    btn.callback = snap.callback
  end
  if snap.hold_cb ~= nil then
    btn.hold_callback = snap.hold_cb
  end
  if snap.dimen ~= nil then
    btn.dimen = snap.dimen
  end
end
...
local function _layoutParams(tb)
  ...
  pcall(function()
    local sz = (
      tb.right_button
      and tb.right_button.image
      and tb.right_button.image:getSize()
    )
      or (
        tb.left_button
        and tb.left_button.image
        and tb.left_button.image:getSize()
      )
    if sz and sz.w and sz.w > 0 then
      base_iw = sz.w
    end
  end)
  return {
...
function M.apply(fm_self)
  ...
  if fm_self._titlebar_patched then
    return
  end
  ...
  local show_browse = M.isItemVisible("fm_browse")
    and (function()
  ...
  local visible = {}
  if show_menu then
    visible["fm_menu"] = true
  end
  if show_up then
    visible["fm_back"] = true
  end
  if show_search then
    visible["fm_search"] = true
  end
  if show_browse then
    visible["fm_browse"] = true
  end
  ...
  if tb.right_button then
    ...
    fm_self._titlebar_rb =
      _snapBtn(rb, { save_icon = true, save_callback = true })
    ...
    tb.setRightIcon = function(tb_self, icon, ...)
      ...
      if icon == "plus" and _icon_enabled then
        if tb_self.right_button then
          local _ss = SUIStyle()
          if
            not (_ss and _ss.applyIconToBtn("sui_menu", tb_self.right_button))
          then
            if _ss and _ss.restoreDefaultIcon then
              _ss.restoreDefaultIcon(
                tb_self.right_button,
                nil,
                Config.ICON.ko_menu
              )
  ...
  if tb.left_button then
    ...
    fm_self._titlebar_lb =
      _snapBtn(lb, { save_icon = true, save_callback = true })
    ...
    if ok_ib and IconButton then
      ...
      if s then
        ...
        do
          local _ss = SUIStyle()
          if _ss then
            _ss.applyIconToBtn("sui_back", up_btn)
          end
        end

        up_btn.overlap_align = nil
        up_btn.overlap_offset =
          { _buttonX(s.side, s.slot, iw, pad, gap, sw), 0 }
          ...
          local function _applyBackButtonState(fc_self, is_sub, page)
            ...
            if not is_sub and page <= 1 then
              ...
              for _, entry in ipairs(neighbors) do
                local dslot = entry.slot > up_slot and entry.slot - 1
                  or entry.slot
                entry.btn.overlap_offset = {
                  _buttonX("left", dslot, iw, pad, gap, sw),
                  0,
                }
              end
            else
              ...
              btn.overlap_offset = {
                _buttonX("left", up_slot, iw, pad, gap, sw),
                0,
              }
              for _, entry in ipairs(neighbors) do
                entry.btn.overlap_offset = {
                  _buttonX("left", entry.slot, iw, pad, gap, sw),
                  0,
                }
              end
              if page > 1 then
    ...
    if ok_ib and IconButton then
      ...
      if s then
        ...
        do
          local _ss = SUIStyle()
          if _ss then
            _ss.applyIconToBtn("sui_search", search_btn)
          end
        end
  ...
  if show_browse then
    ...
    if ok_ib and IconButton then
      ...
      if s then
        ...
        browse_btn = IconButton:new({
          ...
          callback = function()
            ...
            local function _navigate(dlg, mode)
              ...
              local _ss = SUIStyle()
              if
                not (_ss and _ss.applyIconToBtn(_BM_SLOT[mode], browse_btn))
              then
                if _ss and _ss.restoreDefaultIcon then
                  _ss.restoreDefaultIcon(
                    browse_btn,
                    nil,
                    _BROWSE_ICONS_DEFAULT[mode] or _BROWSE_ICONS_DEFAULT.normal
                  )
                elseif browse_btn.image then
                  browse_btn.image.file = _BROWSE_ICONS_DEFAULT[mode]
                    or _BROWSE_ICONS_DEFAULT.normal
                  _reloadImage(browse_btn.image)
                elseif browse_btn.label_widget then
                  browse_btn.label_widget.file = _BROWSE_ICONS_DEFAULT[mode]
                    or _BROWSE_ICONS_DEFAULT.normal
        ...
        if not (_ss and _ss.applyIconToBtn(_BM_SLOT[mode0], browse_btn)) then
          if _ss and _ss.restoreDefaultIcon then
            _ss.restoreDefaultIcon(
              browse_btn,
              nil,
              _BROWSE_ICONS_DEFAULT[mode0] or _BROWSE_ICONS_DEFAULT.normal
            )
          elseif browse_btn.image then
            browse_btn.image.file = _BROWSE_ICONS_DEFAULT[mode0]
              or _BROWSE_ICONS_DEFAULT.normal
            _reloadImage(browse_btn.image)
          end
        end
        browse_btn.overlap_align = nil
        browse_btn.overlap_offset =
          { _buttonX(s.side, s.slot, iw, pad, gap, sw), 0 }
        ...
        if s.side == "left" then
          local up_slot_b = slot_map["fm_back"] and slot_map["fm_back"].slot
            or 0
        ...
        if fc_b and fc_b._simpleui_gen_listeners then
          fc_b._simpleui_gen_listeners[#fc_b._simpleui_gen_listeners + 1] = function(
            fc_self
          )
            -- Improvement #4: use cached _BrowseMeta().
            ...
            if BM2 and browse_btn then
              ...
              local _ss2 = SUIStyle()
              if
                not (_ss2 and _ss2.applyIconToBtn(_BM_SLOT[mode2], browse_btn))
              then
                if _ss2 and _ss2.restoreDefaultIcon then
                  _ss2.restoreDefaultIcon(
                    browse_btn,
                    nil,
                    _BROWSE_ICONS_DEFAULT[mode2] or _BROWSE_ICONS_DEFAULT.normal
                  )
                elseif browse_btn.image then
                  browse_btn.image.file = _BROWSE_ICONS_DEFAULT[mode2]
                    or _BROWSE_ICONS_DEFAULT.normal
                  _reloadImage(browse_btn.image)
                elseif browse_btn.label_widget then
                  browse_btn.label_widget.file = _BROWSE_ICONS_DEFAULT[mode2]
                    or _BROWSE_ICONS_DEFAULT.normal
                  _reloadImage(browse_btn.label_widget)
                end
              end
  ...
  if fm_self._simpleui_force_refresh_layout and fm_self.file_chooser then
    ...
    fm_self._simpleui_force_refresh_layout(
      fm_self.file_chooser,
      current_is_sub,
      current_page
    )
...
function M.restore(fm_self)
  ...
  if not fm_self._titlebar_patched then
    return
  end
  ...
  if tb.right_button then
    _restoreBtn(tb.right_button, fm_self._titlebar_rb)
  end
  fm_self._titlebar_rb = nil
  if tb.left_button then
    _restoreBtn(tb.left_button, fm_self._titlebar_lb)
  end
  ...
  for _, key in ipairs({
    "_titlebar_up_btn",
    "_titlebar_search_btn",
    "_titlebar_browse_btn",
  }) do
...
function M.applyToSub(widget)
  ...
  if widget._titlebar_sub_patched then
    return
  end
  ...
  local visible = {}
  if show_menu then
    visible["sub_menu"] = true
  end
  if show_close then
    visible["sub_close"] = true
  end
  if show_back then
    visible["sub_back"] = true
  end
  local slot_map = _buildSlotMap(cfg.order_left, cfg.order_right, visible)
      ...
      tb.setLeftIcon = function(tb_self, icon, ...)
        ...
        if icon == "appbar.menu" then
          ...
          if tb_self.left_button then
            ...
            if
              not (
                _ss2 and _ss2.applyIconToBtn("sui_menu", tb_self.left_button)
              )
            then
              if _ss2 and _ss2.restoreDefaultIcon then
                _ss2.restoreDefaultIcon(
  ...
  if tb.right_button then
    local rb = tb.right_button
    widget._titlebar_sub_rb =
      _snapBtn(rb, { save_callback = true, save_dimen = true })
    if show_close then
    ...
    if ok_ib and IconButton then
      ...
      if s then
        ...
        do
          local _ss = SUIStyle()
          if _ss then
            _ss.applyIconToBtn("sui_back", sub_back_btn)
          end
        end
        sub_back_btn.overlap_align = nil
        sub_back_btn.overlap_offset =
          { _buttonX(s.side, s.slot, iw, pad, gap, sw), 0 }
        ...
        local function _applySubBackButtonState(w_self, page)
          ...
          if not visible then
            ...
            if sl then
              btn.overlap_offset =
                { _buttonX(sl.side, sl.slot, iw, pad, gap, sw), 0 }
            end

            if page > 1 then
              -- Paginated: tap goes back one page, hold goes to page 1.
              btn.callback = function()
                w_self:onGotoPage(page - 1)
              end
              btn.hold_callback = function()
                w_self:onGotoPage(1)
              end
          ...
          if w_self.title_bar then
            require("ui/uimanager"):setDirty(
              w_self.title_bar.show_parent or w_self,
              "ui",
              w_self.title_bar.dimen
            )
          end
        end
        ...
        widget.updatePageInfo = function(w_self, select_number)
          if inherited then
            inherited(w_self, select_number)
          end
        ...
        if widget.return_button and widget.return_button[1] then
          ...
          widget.return_button[1] =
            require("ui/widget/verticalspan"):new({ width = 0 })
        end
        ...
        if pra then
          ...
          widget._titlebar_sub_orig_pra_showHide = rawget(pra, "showHide")
            or false
...
function M.restoreSub(widget)
  ...
  if not widget._titlebar_sub_patched then
    return
  end
  if tb.left_button then
    _restoreBtn(tb.left_button, widget._titlebar_sub_lb)
  end
  if tb.right_button then
    _restoreBtn(tb.right_button, widget._titlebar_sub_rb)
  end
  ...
  if type(window_stack) == "table" then
    for _, entry in ipairs(window_stack) do
      ...
      if w and w._titlebar_sub_patched then
        ...
        if not ok then
          logger.warn(
            "simpleui: titlebar.reapplyAll widget failed:",
            tostring(err)
          )
        end
      end
...
function M.refreshBrowseIcons(fm)
  if not (fm and fm._titlebar_browse_btn) then
    return
  end
  local browse_btn = fm._titlebar_browse_btn
  if not browse_btn then
    return
  end
