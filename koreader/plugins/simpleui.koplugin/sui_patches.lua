...
local function isStartWithHS()
  if _start_with_hs == nil then
    _start_with_hs = G_reader_settings:readSetting("start_with", "filemanager")
      == "homescreen_simpleui"
  end
    ...
    FileManager.setupLayout = function(fm_self)
      ...
      if not FileChooser._navbar_patched then
        ...
        FileChooser.changeToPath = function(fc_self, path, focused_path)
          ...
          if
            fm_ref
            and fc_self.ui == fm_ref
            and not fm_ref._navbar_suppress_path_change
          then
      ...
      if not FileManager._simpleui_reinit_patched then
        ...
        FileManager.reinit = function(fm_self, path, focused_file)
          ...
          if fm_self.file_chooser and resolved then
            ...
            pcall(function()
              fm_self.file_chooser:changeToPath(resolved)
            end)
          ...
          if fm_self.updateTitleBarPath then
            pcall(function()
              fm_self:updateTitleBarPath(resolved, false)
            end)
          end
          ...
          if sui then
            sui.active_action = "home"
          end
          local tabs = Config.loadTabConfig()
          if fm_self._navbar_container then
            Bottombar.replaceBar(
              fm_self,
              Bottombar.buildBarWidget("home", tabs),
              tabs
            )
      ...
      Bottombar.resizePaginationButtons(
        fm_self.file_chooser or fm_self,
        Bottombar.getPaginationIconSize()
      )
      ...
      fm_self.onShow = function(this)
        ...
        if this._navbar_already_shown then
          return
        end
        ...
        if this._navbar_container then
          ...
          local active = return_to_folder
              and M._resolveTabForPath(
                this.file_chooser and this.file_chooser.path,
                t
              )
            or "home"
      ...
      fm_self.onCloseAllMenus = function(this)
        if orig_onCloseAllMenus then
          orig_onCloseAllMenus(this)
        end
        ...
        if not this._navbar_container then
          return
        end
        ...
        if this._navbar_tab_nav_in_progress then
          return
        end
        local t = Config.loadTabConfig()
        ...
        Bottombar.replaceBar(
          this,
          Bottombar.buildBarWidget(plugin.active_action, t),
          t
        )
        UIManager:setDirty(this, "ui")
      end
      ...
      fm_self.onPathChanged = function(this, new_path)
        if this._navbar_suppress_path_change then
          return
        end
        ...
        if this.updateTitleBarPath then
          local is_home = new_path
            and (new_path:gsub("/$", "") == home_dir_norm)
          this:updateTitleBarPath(new_path, is_home or nil)
        end

        local t = Config.loadTabConfig()
        local new_active = M._resolveTabForPath(new_path, t, home_dir_norm)
          or "home"
      ...
      local function _enterNavbarKbFocus(return_fn)
        ...
        for i, t in ipairs(tabs) do
          if t == plugin.active_action then
            _navbar_kb_idx = i
            break
          end
        end
        ...
        local target0 = M._getNavbarTarget and M._getNavbarTarget(liveFM())
          or liveFM()
        if target0 then
          Bottombar.replaceBar(
            target0,
            Bottombar.buildBarWidgetWithKeyFocus(
              plugin.active_action,
              tabs,
              _navbar_kb_idx
            ),
            tabs
        ...
        local function _moveNavbar(delta)
          ...
          local target2 = M._getNavbarTarget and M._getNavbarTarget(liveFM())
            or liveFM()
          if target2 then
            Bottombar.replaceBar(
              target2,
        ...
        local function _exitNavbarKb()
          ...
          if target2 then
            ...
            Bottombar.replaceBar(
              target2,
              Bottombar.buildBarWidget(plugin.active_action, t2),
              t2
            )
            UIManager:setDirty(target2, "ui")
          end
        ...
        function capture:onNavbarKbLeft()
          _moveNavbar(-1)
          return true
        end
        function capture:onNavbarKbRight()
          _moveNavbar(1)
          return true
        end
        function capture:onNavbarKbUp()
          _exitNavbarKb()
          return true
        end
        function capture:onNavbarKbBack()
          _exitNavbarKb()
          return true
        end
...
function M._resolveTabForPath(path, tabs, home_dir_norm)
  ...
  for _, tab_id in ipairs(tabs) do
    if tab_id == "home" then
      if home_dir_norm and path == home_dir_norm then
        return "home"
      end
...
function M.patchStartWithMenu()
  ...
  if FileManagerMenu._simpleui_startwith_patched then
    return
  end
  ...
  FileManagerMenu.getStartWithMenuTable = function(fmm_self)
    ...
    local found = false
    for _, item in ipairs(sub) do
      if item.text == hs_text and item.radio then
        found = true
        break
      end
    end
    ...
    result.text_func = function()
      if
        G_reader_settings:readSetting("start_with") == "homescreen_simpleui"
      then
...
function M.patchBookList(plugin)
  local ok, target = pcall(require, "ui/widget/booklist")
  if not ok or not target then
    ok, target = pcall(require, "ui/widget/filechooser")
  end
  if not ok or not target then
    return
  end

  local orig_new = target.new
  plugin._orig_booklist_new = orig_new
  target.new = function(class, attrs, ...)
    attrs = attrs or {}
...
function M.patchCollections(plugin)
  ...
  Menu.new = function(class, attrs, ...)
    ...
    if
      patch_depth > 0
      and attrs.covers_fullscreen
      and attrs.is_borderless
      and attrs.is_popout == false
      and not attrs.height
      and not attrs._navbar_height_reduced
    then
      attrs.height = UI.getContentHeight()
      attrs.y = UI.getContentTop()
      attrs._navbar_height_reduced = true
  ...
  if type(RC.removeCollection) == "function" then
    ...
    RC.removeCollection = function(rc_self, coll_name, ...)
      ...
      if not ok2 then
        logger.warn("simpleui: removeCollection hook:", tostring(err))
      end
      return result
    end
  end
  ...
  if type(RC.renameCollection) == "function" then
    ...
    RC.renameCollection = function(rc_self, old_name, new_name, ...)
      ...
      if not ok2 then
        logger.warn("simpleui: renameCollection hook:", tostring(err))
      end
      return result
    end
  end
  ...
  if type(RC.addItem) == "function" then
    ...
    RC.addItem = function(rc_self, file, coll_name, attr, ...)
      ...
      if TBR and coll_name == TBR.TBR_COLL_NAME then
        ...
        if not ok2 then
          logger.warn("simpleui: RC.addItem TBR hook:", tostring(err))
        end
      end
    end
  end
  ...
  if type(RC.removeItem) == "function" then
    ...
    RC.removeItem = function(rc_self, file, coll_name, no_write, ...)
      ...
      if TBR and (coll_name == TBR.TBR_COLL_NAME or coll_name == nil) then
        ...
        if not ok2 then
          logger.warn("simpleui: RC.removeItem TBR hook:", tostring(err))
        end
      end
    end
  end
  ...
  if type(FMColl.updateCollListItemTable) == "function" then
    ...
    FMColl.updateCollListItemTable = function(fmc_self, do_init, item_number)
      ...
      if not (coll_list and coll_list.item_table) then
        return
      end
...
function M.patchFullscreenWidgets(plugin)
  ...
  if ok_sw and SortWidget then
    ...
    SortWidget.new = function(class, attrs, ...)
      ...
      local sw = sw_or_err
      if not attrs.covers_fullscreen then
        return sw
      end
...
function M.patchUIManagerShow(plugin)
  ...
  local function _fmMenu()
    ...
    if
      ...
      and fm.menu.name:find("filemanager")
    then
      return fm.menu
    end
    local inst = liveFM()
  ...
  UIManager.show = function(um_self, widget, ...)
    ...
    local ok, result = pcall(function()
      ...
      local action_before = plugin._navbar_prev_action_pending
        or plugin.active_action
      plugin._navbar_prev_action_pending = nil
      local effective_action = nil

      -- Activate the tab that matches this widget.
      if
        widget.name == "collections"
        and Config.isFavoritesWidget(widget)
        and tabs_set["favorites"]
      then
        effective_action =
          Bottombar.setActiveAndRefreshFM(plugin, "favorites", tabs)
      elseif widget.name == "history" and tabs_set["history"] then
        effective_action =
          Bottombar.setActiveAndRefreshFM(plugin, "history", tabs)
      ...
      elseif
        widget.name == "coll_list"
        or (
          widget.name == "collections" and not Config.isFavoritesWidget(widget)
        )
      then
        if tabs_set["collections"] then
          effective_action =
            Bottombar.setActiveAndRefreshFM(plugin, "collections", tabs)
        end
      end

      local display_action = effective_action or action_before
      if not widget._navbar_inner then
        widget._navbar_inner = widget[1]
      end
      ...
      if widget.registerTouchZones then
        ...
        if DTAP_ZONE_MENU and DTAP_ZONE_MENU_EXT then
          ...
          local function _tapOnSubBtn(ges)
            ...
            local btns = {}
            if tb and tb.left_button then
              btns[#btns + 1] = tb.left_button
            end
            if tb and tb.right_button then
              btns[#btns + 1] = tb.right_button
            end
            local logger = require("logger")
            logger.dbg(
              "simpleui _tapOnSubBtn: pos=",
              pos.x,
              pos.y,
              "n_btns=",
              #btns
            ...
            for i, btn in ipairs(btns) do
              ...
              logger.dbg(
                ...
                tostring(bx_dm),
                "dimen.w=",
                tostring(btn.dimen and btn.dimen.w)
              )
              if bw <= 0 then
                goto continue
              end
          ...
          widget:registerTouchZones({
            {
              ...
              handler = function(ges)
                local logger = require("logger")
                logger.dbg(
                  "simpleui_menu_tap FIRED pos=",
                  ges.pos and ges.pos.x,
                  ges.pos and ges.pos.y
                )
      ...
      if rb and rb[1] then
        rb[1].width = UI.SIDE_M()
      end

      -- On D-pad devices, pressing Down at the last item enters navbar focus.
      if Device:hasDPad() then
        if
          rawget(widget, "_wrapAroundY") == nil
          and type(widget._wrapAroundY) == "function"
        then
      ...
      if
        SUISettings:isTrue("simpleui_bar_navpager_enabled")
        and not _navpager_rebuild_pending
      then
        local has_prev_snap, has_next_snap = Config.getNavpagerState()
        _navpager_rebuild_pending = true
        UIManager:scheduleIn(0, function()
          _navpager_rebuild_pending = false
          if not SUISettings:isTrue("simpleui_bar_navpager_enabled") then
            return
          end
          local fm2 = plugin.ui
          if not (fm2 and fm2._navbar_container) then
            return
          end
          local target2 = (widget._navbar_container and widget) or fm2
          if
            not Bottombar.updateNavpagerArrows(
              target2,
              has_prev_snap,
              has_next_snap
            )
    ...
    if
      ...
      and widget ~= plugin.ui
      and not widget._sui_keep_homescreen
    then
      local stack = UI.getWindowStack()
      for _, entry in ipairs(stack) do
        local w = entry.widget
...
function M.patchUIManagerClose(plugin)
  ...
  local function _doShowHS(fm, plugin_ref)
    local HS = liveHS()
    if not HS or HS._instance then
      return
    end
    ...
    if widget._navbar_injected and not widget._navbar_closing_intentionally then
      ...
      if fm_r then
        ...
        if fc_r and fc_r.path then
          pcall(function()
            fm_r:updateTitleBarPath(fc_r.path, false)
          end)
        end
      end
    end
      ...
      if not other_open then
        if widget.name == "ReaderUI" then
          ...
          if not widget.tearing_down then
            local return_to_folder =
              SUISettings:isTrue("simpleui_hs_return_to_book_folder")
            if not return_to_folder then
              ...
              if
                active_plugin.active_action == nil
                or not _ao2[active_plugin.active_action]
              then
              ...
              UIManager:nextTick(function()
                ...
                if RUI2 and RUI2.instance then
                  return
                end
                if _raiseHSFromStack(active_plugin, prev_action) then
                  return
                end
                local HS2 = liveHS()
                if not (HS2 and not HS2._instance) then
                  return
                end
...
function M.patchMenuInitForPagination(plugin)
  ...
  Menu.init = function(menu_self, ...)
    ...
    pcall(function()
      ...
      if
        SS.applyCollBackIcon
        and (menu_self.name == "collections" or menu_self.name == "coll_list")
      then
    ...
    if SUISettings:nilOrTrue("simpleui_bar_pagination_visible") then
      return
    end
...
function M.patchMenuForNavpager(plugin)
  ...
  local Menu = require("ui/widget/menu")
  if Menu._simpleui_navpager_patched then
    return
  end
  ...
  local function _setSubtitleUnified(tb, path_base, page, page_num)
    if not tb or not tb.subtitle_widget then
      return
    end
    local parts = {}
  ...
  local function _setPageSubtitle(tb, page, page_num, menu_self)
    ...
    local path_base = (menu_self and menu_self._navbar_injected) and ""
      or _fm_path_base
  ...
  Menu.updatePageInfo = function(menu_self, select_number)
    ...
    if menu_self.page_info and menu_self._navbar_injected then
      UIManager:setDirty(
        menu_self.show_parent or menu_self,
        "ui",
        menu_self.page_info.dimen
      )
    end

    if not _subtitleEnabled() then
      return
    end
    ...
    if _navpager_rebuild_pending then
      return
    end
    _navpager_rebuild_pending = true
  ...
  FileManager.updateTitleBarPath = function(fm_self, path, force_home)
    ...
    if at_home then
      ...
      _fm_path_base = (tb2 and tb2.subtitle_widget and tb2.subtitle_widget.text)
        or ""
    end
...
function M.patchBookInfoNavigation(plugin)
  ...
  if not ok_util or not fmutil then
    return
  end
  if fmutil._simpleui_bookinfo_nav_patched then
    return
  end
  ...
  fmutil.genBookInformationButton = function(
    ...
    btn.callback = function()
      ...
      if not (fm and fm.bookinfo and fm.bookinfo.kvp_widget) then
        return
      end
...
local function _onStatusChanged(file)
  ...
  pcall(function()
    ...
    if not (DB and DB.isEnabled()) then
      return
    end
...
function M.patchStatusButtons(plugin)
  ...
  if fmutil._simpleui_status_buttons_patched then
    return
  end
  ...
  fmutil.genMultipleStatusButtonsRow = function(
    files,
    caller_callback,
    button_disabled
  )
...
function M.unpatchStatusButtons(plugin)
  ...
  if not fmutil or not fmutil._simpleui_status_buttons_patched then
    return
  end
...
function M.installButtonBoundsDebug(plugin)
  ...
  if not Button then
    ...
    _G.require = function(modname, ...)
      ...
      if
        modname == "ui/widget/button" and not result._simpleui_bounds_patched
      then
...
function M._wrapButtonPaintTo(plugin, Button)
  ...
  Button.paintTo = function(btn_self, bb, x, y)
    ...
    if not SUISettings:isTrue("simpleui_debug_button_bounds") then
      return
    end
...
-- ---------------------------------------------------------------------------
local function _closeReaderToHomescreenSync(
  plugin,
  readerui,
  file,
  return_to_folder,
  ...
  if _raiseHSFromStack(plugin, prev_action) then
    return
  end
...
function M.closeReaderToHomescreen(plugin, via_gesture)
  ...
  UIManager:nextTick(function()
    ...
    local RUI2 = package.loaded["apps/reader/readerui"]
    if RUI2 and RUI2.instance and RUI2.instance ~= readerui then
      return
    end
    _closeReaderToHomescreenSync(
      plugin,
...
function M.patchReloadDocument(plugin, readerui)
  ...
  if readerui._simpleui_reload_patched then
    return
  end
...
function M.wireReaderMenuFMTab(plugin, readerui)
  if not (readerui and readerui.menu) then
    return
  end
  local menu_ref = readerui.menu
  if menu_ref._simpleui_fm_tab_wrapped then
    return
  end
  local items = menu_ref.menu_items
  if not (items and items.filemanager) then
    return
  end
  ...
  items.filemanager.callback = function()
    ...
    if menu_ref.onTapCloseMenu then
      menu_ref:onTapCloseMenu()
    end
  ...
  UIManager:scheduleIn(0, function()
    ...
    if home and fm_ref.file_chooser then
      ...
      if fm_ref.updateTitleBarPath then
        pcall(function()
          fm_ref:updateTitleBarPath(home, true)
        end)
      end
...
local function _wallpaperBg()
  ...
  if
    not (
      ok
      and HS
      and HS.styleGetWallpaperShowInFM
      and HS.styleGetWallpaperShowInFM()
    )
  then
    return nil
  end
  if not HS.styleGetBgWidget then
    return nil
  end
...
local function _clearWhiteBackgrounds(w, depth)
  if not w or type(w) ~= "table" or depth > 12 then
    return
  end
  local Blitbuffer = require("ffi/blitbuffer")
  ...
  local ok, is_white = pcall(function()
    return w.background == Blitbuffer.COLOR_WHITE
  end)
...
local function _injectWallpaperIntoWidget(widget)
  ...
  if not outer or outer._sui_wallpaper_patched then
    return
  end
...
function M.patchWallpaperFM(plugin)
  ...
  if FileManager._simpleui_wallpaper_fm_patched then
    return
  end
  ...
  FileManager.paintTo = function(fm_self, bb, x, y)
    ...
    if _live_bg_fm then
      _paintWallpaper(_live_bg_fm, bb, x, y)
    end
  ...
  FileManager.setupLayout = function(fm_self)
    ...
    if not _wallpaperEnabledFM() then
      return
    end
  ...
  if ok_btn and Button and not plugin._orig_wp_button_paintTo then
    ...
    Button.paintTo = function(btn_self, bb, x, y)
      ...
      if
        _wallpaperEnabledFM()
        and not btn_self.background
        and btn_self[1]
        and btn_self[1].background
      then
  ...
  -- -----------------------------------------------------------------------
  local ok_uc, UnderlineContainer =
    pcall(require, "ui/widget/container/underlinecontainer")
  ...
  if ok_tbw and TextBoxWidget and not plugin._orig_wp_tbw_paintTo then
    ...
    TextBoxWidget.paintTo = function(tbw_self, bb, x, y)
      if
        not (
          _wallpaperEnabledFM()
          and tbw_self.bgcolor == Blitbuffer.COLOR_WHITE
        )
      then
      ...
      if
        not tbw_self._sui_tmp_bb
        or tbw_self._sui_tmp_bb:getWidth() ~= w
        or tbw_self._sui_tmp_bb:getHeight() ~= h
      then
        if tbw_self._sui_tmp_bb then
          tbw_self._sui_tmp_bb:free()
        end
        tbw_self._sui_tmp_bb = Blitbuffer.new(w, h, Blitbuffer.TYPE_BB8)
      end

      local fgcolor = tbw_self.fgcolor or Blitbuffer.COLOR_BLACK
      UI.paintWithAlphaMask(
        tbw_self,
        bb,
        x,
        y,
    ...
    TextBoxWidget.free = function(tbw_self, full)
      if tbw_self._sui_tmp_bb and full ~= false then
        tbw_self._sui_tmp_bb:free()
        tbw_self._sui_tmp_bb = nil
      end
      if orig_tbw_free then
        orig_tbw_free(tbw_self, full)
      end
    end
  end
  ...
  if ok_pw and ProgressWidget and not plugin._orig_wp_pw_paintTo then
    ...
    ProgressWidget.paintTo = function(pw_self, bb, x, y)
      if
        _wallpaperEnabledFM()
        and pw_self.bgcolor == Blitbuffer.COLOR_WHITE
      then
...
function M.patchPurgeDir(plugin)
  ...
  if ffiUtil._simpleui_purgeDir_patched then
    return
  end
  ...
  if plugin then
    plugin._orig_ffi_purgeDir = orig_purgeDir
  end
...
function M.patchDeleteFile(FileManager, plugin)
  if FileManager._simpleui_deleteFile_patched then
    return
  end
  ...
  if plugin then
    plugin._orig_fm_deleteFile = orig_deleteFile
  end
  ...
  FileManager.deleteFile = function(fm_self, file, is_file)
    if not is_file then
      if not file then
        logger.warn(
          "simpleui: deleteFile called with nil folder path, aborting"
        )
        return false
      end
      ...
      if not lfs2.attributes(file, "mode") then
        logger.warn(
          "simpleui: deleteFile: folder gone before purgeDir:",
          tostring(file)
        )
    ...
    if is_file then
      pcall(function()
        local DB = SUISettings.DeletedBooks
        if not DB or not DB.isEnabled() then
          return
        end
        local ok_DS, DocSettings = pcall(require, "docsettings")
        if not ok_DS or not DocSettings then
        ...
        logger.dbg(
          "simpleui: preserved deleted finished book in stats:",
          title,
          "(md5:",
          md5,
          "year:",
...
function M.patchHistoryMenuHold()
  ...
  if not (ok and FMH) then
    return
  end
  if FMH._sui_onMenuHold_patched then
    return
  end
...
function M.injectWallpaperIntoFullscreenWidget(widget)
  ...
  if not (M._WALLPAPER_NAMES and M._WALLPAPER_NAMES[widget.name]) then
    return
  end
  if not (_wallpaperEnabledFM() and _getWallpaperBg()) then
    return
  end
...
function M.installAll(plugin)
  ...
  if ok_fm and FileManager then
    M.patchDeleteFile(FileManager, plugin)
  end
...
function M.teardownAll(plugin)
  ...
  if
    ffiUtil
    and ffiUtil._simpleui_purgeDir_patched
    and plugin._orig_ffi_purgeDir
  then
  ...
  local BookList = package.loaded["ui/widget/booklist"]
    or package.loaded["ui/widget/filechooser"]
  ...
  if FileManagerMenu and FileManagerMenu._simpleui_startwith_patched then
    FileManagerMenu.getStartWithMenuTable =
      FileManagerMenu._simpleui_startwith_orig
  ...
  if plugin._sysicon_fmmenu_patched then
    local ok_ss, SUIStyle = pcall(require, "sui_style")
    if ok_ss and SUIStyle then
      pcall(SUIStyle.removeTabIconPatch)
    end
  ...
  local fm_td = liveFM()
  if fm_td then
    fm_td._sui_lazy_refresh_path = nil
  end
