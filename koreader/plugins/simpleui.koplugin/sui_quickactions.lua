...
function QA.getIconsDir()
  if not _icons_dir_cache then
    ...
    if ok_ds and DataStorage then
      ...
      local _qa_plugin_dir = debug.getinfo(1, "S").source:match("^@(.+/)[^/]+$")
        or "./"
...
local function _goHome(target_fm)
  ...
  local in_virtual = ok_fc_mod
    and FC_mod.isInSeriesView
    and FC_mod.isInSeriesView(fc)
  if in_virtual then
...
local function _showBookmarkBrowserSourceDialog(bb_ui)
  ...
  local BB = _Bottombar()
  if plugin then
    BB.setTempTabActive(plugin, "bookmark_browser", true, prev_action)
  end
...
local function _registerBuiltins()
  local function _simpleui_plugin()
    ...
    local fm = _liveFM()
    if fm and fm._simpleui_plugin then
      return fm._simpleui_plugin
    end
    ...
    },
    {
      ...
      execute = function(ctx)
        ...
        if fm and fm.collections then
          ...
          su(_("Collections not available."))
        end
      end,
    },
    {
    ...
    -- ── Overlay / dialog actions ────────────────────────────────────────
    {
      ...
      execute = function(ctx)
        ...
        if _bb_ui and not _bb_ui.bookinfo then
          local ok_bi, BookInfo =
            pcall(require, "apps/filemanager/filemanagerbookinfo")
        ...
        if ok and SW and SW.showReadingInsightsWindow then
          ...
          local ok2, err = pcall(function()
            UIManager:broadcastEvent(
              require("ui/event"):new("ShowCalendarView")
            )
          end)
          if not ok2 then
            su(_("Statistics plugin not available."))
          end
        end
      end,
    },
    ...
    -- ── Browse meta actions ─────────────────────────────────────────────
    {
      ...
      execute = function(ctx)
        ...
        if not (ok_bm and BM) then
          su(_("Browse by Authors/Series/Tags not available."))
          return
        end
        if not BM.isEnabled() then
          su(
            _(
              "Enable 'Browse by Author / Series / Tags' in the library menu first."
            )
          )
          return
        end
        ...
        if ctx.already_active then
          BM.navigateToRoot(fc, fm, "author")
        else
          BM.navigateTo(fm, "author")
        end
      end,
    },
    {
      ...
      execute = function(ctx)
        ...
        if ctx.already_active then
          BM.navigateToRoot(fc, fm, "series")
        else
          BM.navigateTo(fm, "series")
        end
      end,
    },
    {
      ...
      execute = function(ctx)
        ...
        if ctx.already_active then
          BM.navigateToRoot(fc, fm, "tags")
        else
          BM.navigateTo(fm, "tags")
        end
      end,
    },
  }
...
function QA.register(descriptor)
  ...
  if not descriptor.execute then
    logger.warn(
      "simpleui: QA.register: descriptor missing 'execute' for id="
...
function QA.unregister(id)
  ...
  for _, desc in ipairs(_builtin_descriptors) do
    if desc.id == id then
      logger.warn(
        "simpleui: QA.unregister: cannot unregister built-in action",
        id
      )
      return
    end
...
function QA.saveCustomQAConfig(
  qa_id,
  label,
  path,
  collection,
...
function QA.deleteCustomQA(qa_id)
  ...
  for _i, id in ipairs(list) do
    if id ~= qa_id then
      new_list[#new_list + 1] = id
    end
  end
  QA.saveCustomQAList(new_list)
  local mqa = package.loaded["desktop_modules/module_quick_actions"]
  if mqa and mqa.invalidateCustomQACache then
    mqa.invalidateCustomQACache()
  end
...
function QA.sanitizeQASlots()
  ...
  if type(tabs) == "table" then
    ...
    for _i, id in ipairs(tabs) do
      if
        Config.ACTION_BY_ID[id]
        or (id:match("^custom_qa_%d+$") and valid_custom[id])
      then
...
function QA.nextCustomQAId()
  ...
  local n = max_n + 1
  while SUISettings:get("simpleui_qa_custom_qa_" .. n) do
    n = n + 1
  end
  return "custom_qa_" .. n
end
...
function QA.clearQAKeyCache()
  for k in pairs(_qa_key_cache) do
    _qa_key_cache[k] = nil
  end
end
...
local function _defaultLabelKey(id)
  return "simpleui_action_" .. id .. "_label"
end
local function _defaultIconKey(id)
  return "simpleui_action_" .. id .. "_icon"
end
...
function QA.performResetAllQAIcons(plugin)
  ...
  if ok and HS and HS._instance then
...
function QA.sui_show_qa_list(plugin, ctx_menu, ctx)
      ...
      rows[#rows + 1] = {
        ...
        on_delete = function()
          ...
          ctx_menu.UIManager:show(ConfirmBox:new({
            text = string.format(
              ctx_menu._('Delete quick action "%s"?'),
              c.label
            ),
  ...
  ctx_menu.show_row_page({
    ...
    footer_enabled = function()
      return #Config.getCustomQAList() < MAX_CUSTOM_QA
    end,
    footer_action = function(ctx2)
      if #Config.getCustomQAList() >= MAX_CUSTOM_QA then
        ...
        ctx_menu.UIManager:show(InfoMessage:new({
          text = string.format(
            ctx_menu.N_(
              "The maximum of %d quick action has been reached. Delete one first.",
      ...
      QA.showQuickActionDialog(plugin, nil, function()
        ...
        if ctx2.jump_to_last_page then
          ctx2.jump_to_last_page()
        end
        ctx2.repaint()
      end)
    end,
...
function QA.sui_build_qa_icons(plugin, ctx_menu, ctx)
  ...
  local function makeIconPreview(icon_path, is_nerd, fallback_label)
    ...
    elseif icon_path then
      local safe_path = ok_ss
        and SUIStyle
        and SUIStyle.safeIconPath(icon_path, nil)
      if safe_path then
    ...
    return FrameContainer:new({
      ...
      [1] = CenterContainer:new({
        dimen = Geom:new({
          w = btn_size - border_sz * 2,
          h = btn_size - border_sz * 2,
        }),
        [1] = icon_widget,
      }),
    })
  ...
  for _k, a in ipairs(Config.ALL_ACTIONS) do
    ...
    if a.id == "wifi_toggle" then
      pool[#pool + 1] =
        { id = a.id, is_default = true, title = lbl .. "  (" .. _("On") .. ")" }
      pool[#pool + 1] = {
        id = "wifi_toggle_off",
        is_default = true,
        title = lbl .. "  (" .. _("Off") .. ")",
  ...
  for _i, qa_id in ipairs(Config.getCustomQAList()) do
    ...
    pool[#pool + 1] =
      { id = qa_id, is_default = false, title = c.label or qa_id }
  end
  ...
  local function get_rows()
    ...
    for _, entry in ipairs(pool) do
      if entry.is_default then
        table.insert(defaults, entry)
      else
        table.insert(customs, entry)
      end
    end
    ...
    local function add_entry(entry)
      ...
      local has_custom = _is_default and QA.getDefaultActionIcon(_id) ~= nil
        or (
          not _is_default
          and (function()
            local c = Config.getCustomQAConfig(_id)
        ...
        on_tap = function()
          ...
          QA.showIconPicker(current_icon, function(new_icon)
            local function _guardedSetIcon(path, on_valid)
              ...
              local safe = ok_ss
                and SUIStyle
                and SUIStyle.safeIconPath(path, nil)
              if safe then
                on_valid(safe)
              else
...
function QA.getEntry(id)
  ...
  if id and id:match("^custom_qa_%d+$") then
    ...
    if cfg.dispatcher_action and cfg.dispatcher_action ~= "" then
      default_icon = (ok_ss and SUIStyle and SUIStyle.getIcon("sui_qa_system"))
        or Config.CUSTOM_DISPATCHER_ICON
    elseif cfg.plugin_key and cfg.plugin_key ~= "" then
      default_icon = (ok_ss and SUIStyle and SUIStyle.getIcon("sui_qa_plugin"))
        or Config.CUSTOM_PLUGIN_ICON
    else
      default_icon = (ok_ss and SUIStyle and SUIStyle.getIcon("sui_qa_folder"))
        or Config.CUSTOM_ICON
    end

    local icon = cfg.icon or default_icon
    if
      cfg.icon == Config.CUSTOM_ICON
      or cfg.icon == Config.CUSTOM_PLUGIN_ICON
      or cfg.icon == Config.CUSTOM_DISPATCHER_ICON
    then
      icon = default_icon
    end
  ...
  if id == "wifi_toggle" then
    ...
    _wifi_entry.label = label_ov
      or (desc.get_label and desc.get_label(id))
      or desc.label
    return _wifi_entry
  end
            ...
            callback = function()
              ...
              if hex and #hex >= 1 and #hex <= 6 then
                ...
                if Config.nerdIconChar(sentinel) then
                  UIManager:close(dlg)
                  _showNerdIconPreview(sentinel, on_select, function()
                    UIManager:nextTick(_openInputDlg)
                  end)
                else
                  UIManager:show(InfoMessage:new({
                    text = _(
                      "Codepoint out of valid Unicode range (0–10FFFF)."
                    ),
                    timeout = 3,
                  }))
                end
                ...
                UIManager:show(InfoMessage:new({
                  text = _(
                    "Invalid input. Please enter 1–6 hexadecimal digits (0–9, A–F)."
                  ),
                  timeout = 3,
                }))
              end
  ...
  if #icons == 0 then
    ...
    for _i, icon in ipairs(icons) do
      ...
      buttons[#buttons + 1] = {
        {
          text = p.label
            .. ((is_svg and current_icon == p.path) and "  ✓" or ""),
          callback = function()
            UIManager:close(_picker_handle[picker_key])
            on_select(p.path)
          end,
...
local function _scanFMPlugins()
  ...
  local known = {
    ...
    {
      key = "collections",
      method = "onShowCollList",
      title = _("Collections"),
    },
    {
      key = "filesearcher",
      method = "onShowFileSearch",
      title = _("File Search"),
    },
    {
      key = "folder_shortcuts",
      method = "onShowFolderShortcutsDialog",
      title = _("Folder Shortcuts"),
    },
    {
      key = "dictionary",
      method = "onShowDictionaryLookup",
      title = _("Dictionary Lookup"),
    },
    {
      key = "wikipedia",
      method = "onShowWikipediaLookup",
  ...
  for _, entry in ipairs(known) do
    ...
    if mod and type(mod[entry.method]) == "function" then
      results[#results + 1] =
        { fm_key = entry.key, fm_method = entry.method, title = entry.title }
    end
  end
  local native_keys = {
    screenshot = true,
  ...
  for i = 1, #fm do
    local val = fm[i]
    if type(val) ~= "table" or type(val.name) ~= "string" then
      goto cont
    end
    local fm_key = fm_val_to_key[val]
    if
      not fm_key
      or native_keys[fm_key]
      or seen_keys[fm_key]
      or fm_key == our_name
    then
      goto cont
    ...
    if method then
      ...
      results[#results + 1] =
        { fm_key = fm_key, fm_method = method, title = display }
    end
    ::cont::
  end
...
local function _scanDispatcherActions()
  ...
  if not ok_d or not Dispatcher then
    return {}
  end
  pcall(function()
    Dispatcher:init()
  ...
  local order = (
    type(dispatcher_menu_order) == "table" and dispatcher_menu_order
  )
    or (function()
      local t = {}
      for k in pairs(settingsList) do
        t[#t + 1] = k
      end
      table.sort(t)
      return t
  ...
  for _i, action_id in ipairs(order) do
    ...
    if
      type(def) == "table"
      and def.title
      and def.category == "none"
      and (def.condition == nil or def.condition == true)
    then
...
function QA.showQuickActionDialog(plugin, qa_id, on_done)
  ...
  local function iconButtonLabel(default_lbl)
    if not chosen_icon then
      return default_lbl or _("Icon: Default")
    end
  ...
  local function _buildSaveDialog(update_name_with_title)
    ...
    if current_action_type == "path" or current_action_type == "collection" then
      default_lbl = _("Default (Folder)")
    elseif current_action_type == "plugin" then
      default_lbl = _("Default (Plugin)")
    elseif current_action_type == "dispatcher" then
      default_lbl = _("Default (System)")
    end
    ...
    local fields = {
      {
        description = _("Name"),
        text = cfg.label or current_action_title or "",
        hint = _("Action name…"),
      },
    }
        ...
        },
        {
          ...
          },
          {
            ...
            callback = function()
              ...
              local inputs = active_dialog:getFields()
              local final_label = Config.sanitizeLabel(inputs[1])
                or current_action_title
                or _("Action")
              ...
              if current_action_type == "path" then
                p_path = current_action_val1
              elseif current_action_type == "collection" then
                p_coll = current_action_val1
              elseif current_action_type == "plugin" then
                p_pk = current_action_val1
                p_pm = current_action_val2
              elseif current_action_type == "dispatcher" then
                p_da = current_action_val1
              end
  ...
  local function openFolderPicker()
    ...
    if not ok_pc or not PathChooser then
      UIManager:show(
        InfoMessage:new({ text = _("Path chooser not available."), timeout = 3 })
      )
      if active_dialog then
        UIManager:show(active_dialog)
      else
  ...
  local function openCollectionPicker()
    ...
    plugin._qa_coll_picker = ButtonDialog:new({
      title = _("Collection"),
      title_align = "center",
      buttons = buttons,
    })
  ...
  local function openPluginPicker()
    ...
    if #plugin_actions == 0 then
      UIManager:show(
        InfoMessage:new({ text = _("No plugins found."), timeout = 3 })
      )
      cancelActionPicker()
      return
    end
  ...
  local function openDispatcherPicker()
    ...
    if #actions == 0 then
      UIManager:show(
        InfoMessage:new({ text = _("No system actions found."), timeout = 3 })
      )
      cancelActionPicker()
      return
    end
    ...
    plugin._qa_dispatcher_picker = ButtonDialog:new({
      ...
      if slot.group == "sui_qa_defaults" then
        items[#items + 1] = {
          text_func = function()
            ...
            local label = type(slot.label) == "function" and slot.label()
              or slot.label
            if path then
              return label .. "  \u{270E}"
            end
...
function QA.executeCustomQA(action_id, fm, show_unavailable_fn)
  ...
  if cfg.dispatcher_action and cfg.dispatcher_action ~= "" then
    ...
    if ok_disp and Dispatcher then
      ...
      if not ok then
        logger.warn(
          "simpleui: dispatcher_action failed:",
          cfg.dispatcher_action,
          tostring(err)
        )
  ...
  elseif cfg.plugin_key and cfg.plugin_method and cfg.plugin_key ~= "" then
    ...
    if plugin_inst and not plugin_inst[method] and method == "_sui_launch" then
      local probe = {}
      local ok = pcall(function()
        plugin_inst:addToMainMenu(probe)
      end)
      if ok then
        local entry = probe[cfg.plugin_key] or probe[plugin_inst.name]
        if entry and type(entry.callback) == "function" then
          local cb = entry.callback
          plugin_inst._sui_launch = function(_self)
            cb()
          end
        end
      end
    ...
    if plugin_inst and type(plugin_inst[method]) == "function" then
      ...
      if not ok then
        _unavail(string.format(_("Plugin error: %s"), tostring(err)))
      end
    else
  ...
  elseif cfg.collection and cfg.collection ~= "" then
    if fm and fm.collections then
      local ok, err = pcall(function()
        fm.collections:onShowColl(cfg.collection)
      end)
      if not ok then
        _unavail(
          string.format(_("Collection not available: %s"), cfg.collection)
        )
      end
    end
  elseif cfg.path and cfg.path ~= "" then
    if fm and fm.file_chooser then
      fm.file_chooser:changeToPath(cfg.path)
    end
  else
    _unavail(
      _(
...
function QA.isInPlaceCustomQA(action_id)
  ...
  if cfg.plugin_key and cfg.plugin_method and cfg.plugin_key ~= "" then
    return true
  end
  return false
end
