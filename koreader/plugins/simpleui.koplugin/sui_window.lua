...
function SUIWindow:new(opts)
  assert(
    type(opts.screens) == "table" and type(opts.screens.__root__) == "function",
  ...
  if self._navpager_mode then
    ...
    if Device:isTouchDevice() then
      ...
      if ok_bb and Bottombar then
        ...
        local n_center = (ok_cfg and Config and Config.NAVPAGER_CENTER_TABS)
          or 4
...
function SUIWindow:close()
  ...
  if self._navpager_mode then
    UIManager:nextTick(function()
      local ok_bb, Bottombar = pcall(require, "sui_bottombar")
      if not ok_bb or not Bottombar then
        return
      end
      ...
      if patches and patches._getNavbarTarget then
        ...
        if stack then
          for i = #stack, 1, -1 do
            ...
            if w and w._navbar_container then
              target = w
              break
            end
          end
      ...
      if not Bottombar.updateNavpagerArrows(target, has_prev, has_next) then
        local tabs = target._navbar_tabs
          or (ok_cfg and Config2 and Config2.loadTabConfig and Config2.loadTabConfig())
          or {}
        local mode = (
          ok_cfg
          and Config2
          and Config2.getNavbarMode
          and Config2.getNavbarMode()
        ) or "icons"
...
function SUIWindow:_navPush(screen_id, params)
  ...
  if top then
    top._saved_page = self._current_page
  end
...
function SUIWindow:_rebuildFrame(ctx, items)
  ...
  if not footer_widget and params.footer_action then
    ...
    local c = type(params.item_count) == "function" and params.item_count()
      or params.item_count
    local m = type(params.max_items) == "function" and params.max_items()
      or params.max_items
  ...
  if req_dot_space then
    ...
    if footer_widget then
      ...
      final_widget =
        VerticalGroup:new({ align = "left", content_block, dot_bar })
    end
  else
    if footer_widget then
      final_widget =
        VerticalGroup:new({ align = "left", content_block, footer_widget })
    else
...
function SUIWindow:_repaint()
  ...
  local ctx = {
    ...
    pad = self._pad_h,
    current = function()
      return win:_navCurrent()
    end,
    push = function(id, params)
      win:_navPush(id, params)
    end,
    pop = function(opts)
    ...
    jump_to_last_page = function()
      win._force_last_page = true
    end,
    withOverlay = function(fn)
      win:withOverlay(fn)
    end,
    lockOverlay = function()
      win._overlay_count = (win._overlay_count or 0) + 1
    end,
    unlockOverlay = function()
      win._overlay_count = math.max(0, (win._overlay_count or 0) - 1)
    end,
  ...
  if build_fn then
    ...
    logger.warn(
      "SUIWindow: no builder for screen '" .. tostring(frame.id) .. "'"
    )
  end
  ...
  if self._navpager_mode and self._wrapper then
    ...
    if UIManager._window_stack then
      ...
      for i = #UIManager._window_stack, 1, -1 do
        if
          UIManager._window_stack[i]
          and UIManager._window_stack[i].widget == self._wrapper
        then
          on_stack = true
          break
        end
...
function SUIWindow:_updateNavbarArrows()
  if not self._navpager_mode then
    return
  end
  local ok_bb, Bottombar = pcall(require, "sui_bottombar")
  if not ok_bb or not Bottombar then
  ...
  if not Bottombar.updateNavpagerArrows(target, has_prev, has_next) then
    ...
    local new_bar =
      Bottombar.buildBarWidgetWithArrows(nil, tabs, mode, has_prev, has_next)
    Bottombar.replaceBar(target, new_bar, tabs)
  end
  UIManager:setDirty(target, "ui")
end
...
local function _getPageDotClass()
  ...
  function _PageDotClass:getSize()
    return Geom:new({
      w = self.total_pages * self.dot_touch_w,
      h = self.dot_bar_h,
    })
  end
  function _PageDotClass:paintTo(bb, x, y)
    ...
    local clr_off = SUIStyle.getThemeColor("text_secondary")
      or Blitbuffer.gray(0.55)
    for i = 1, self.total_pages do
      local cx = x
        + (i - 1) * self.dot_touch_w
        + math.floor(self.dot_touch_w / 2)
...
function SUIWindow:_buildDotBar(total_pages)
  ...
  function bar:onTapDot(_, ges)
    ...
    if ges.pos.y >= self.dimen.y + DOT_H then
      return false
    end
    ...
    local bar_x = self.dimen.x
      + math.floor((self._inner_w - self._dot_sz.w) / 2)
...
local function _clrPrimary()
  return Blitbuffer.COLOR_BLACK
end
local function _clrSecondary()
  return SUIStyle.getThemeColor("text_secondary") or Blitbuffer.gray(0.55)
end
local function _clrSeparator()
  return Blitbuffer.gray(0.85)
end
...
function SUIWindow.ListRow(opts)
  ...
  local c = type(opts.item_count) == "function" and opts.item_count()
    or opts.item_count
  local m = type(opts.max_items) == "function" and opts.max_items()
    or opts.max_items
  if c and m then
  ...
  local has_more = opts.on_more ~= nil
    or (type(opts.more_items) == "table" and #opts.more_items > 0)
  local has_value = opts.right_value and opts.right_value ~= ""
  local has_right_widget = opts.right_widget ~= nil
  local has_right = has_delete
    or has_edit
    or has_update
    or has_more
    or opts.show_chevron
    or opts.checked ~= nil
    or opts.right_icon
    or has_value
    or has_right_widget
  ...
  if has_right_widget then
    local sz = type(opts.right_widget.getSize) == "function"
        and opts.right_widget:getSize()
      or opts.right_widget.dimen
    widget_w = sz and sz.w or 0
  end
  local extra_w = (opts.show_chevron or opts.checked ~= nil or opts.right_icon)
      and _CHEVRON_W
    or 0
  local left_w = math.max(
    1,
    inner_w - del_w - edit_w - upd_w - more_w - val_w - widget_w - extra_w
  )
      ...
      more_ic = InputContainer:new({
        ...
        [1] = CenterContainer:new({
          ...
          TextWidget:new({
            ...
            face = Font:getFace(
              SUIStyle.FACE_ICONS,
              Screen:scaleBySize(SUIStyle.FS_DETAIL)
            ),
            fgcolor = _clrPrimary(),
          }),
        }),
  ...
  if not on_hold and opts.show_hold_menu then
    ...
    if opts.on_tap then
      table.insert(
        hold_items,
        { text = _("Select"), icon = "check", on_tap = opts.on_tap }
      )
    end
    ...
    if opts.on_update then
      table.insert(
        hold_items,
        { text = _("Update"), icon = "update", on_tap = opts.on_update }
      )
    end
    if opts.on_edit then
      table.insert(
        hold_items,
        { text = _("Rename"), icon = "edit", on_tap = opts.on_edit }
      )
    end
    if opts.on_delete then
      table.insert(
        hold_items,
        { text = _("Delete"), icon = "delete", on_tap = opts.on_delete }
      )
    end
...
function SUIWindow.MenuTable(opts)
  ...
  for _, item in ipairs(opts.items or {}) do
    ...
    local title = type(item.text_func) == "function" and item.text_func()
      or item.text
      ...
      if type(sub) == "table" then
        for _, child in ipairs(sub) do
          if
            ...
            and child.sub_item_table == nil
            and child.sub_item_table_func == nil
            and type(child.checked_func) == "function"
          then
            local ok, chk = pcall(child.checked_func)
            if ok and chk then
              local ct = type(child.text_func) == "function"
                  and child.text_func()
                or child.text
              if ct and ct ~= "" then
                right_value = ct
...
local function _CardBase(opts)
  ...
  local has_more = opts.on_more ~= nil
    or (type(opts.more_items) == "table" and #opts.more_items > 0)
  local has_move = opts.on_move_up ~= nil
    or opts.on_move_down ~= nil
    or opts.arrange_mode
  ...
  if not on_hold and opts.show_hold_menu then
    ...
    if opts.on_move_up then
      table.insert(
        hold_items,
        { text = _("Move up"), icon = "arrow_up", on_tap = opts.on_move_up }
      )
    end
    if opts.on_move_down then
      table.insert(hold_items, {
...
function SUIWindow.ArrangeList(opts)
  ...
  local repaint = type(opts.on_repaint) == "function" and opts.on_repaint
    or function() end
  local result = {}
  ...
  if opts.ctx then
    ...
    if cur then
      ...
      if last_count and #items > last_count then
        if opts.ctx.jump_to_last_page then
          opts.ctx.jump_to_last_page()
        end
      end
  ...
  for i, item in ipairs(items) do
    ...
    if item.dim or item._is_break or item.is_divider then
      ...
      if
        can_move_up
        and _i == 2
        and (items[1].dim or items[1]._is_break or items[1].is_divider)
      then
        can_move_up = false
      end
...
function SUIWindow.CenteredButtonFooter(ctx, opts)
  ...
  local text_w = TextWidget:new({
    text = opts.icon
        and (SUIStyle.icon(opts.icon) .. "  " .. (opts.text or ""))
      or (opts.text or ""),
    face = face,
    fgcolor = is_disabled
        and (SUIStyle.getThemeColor("text_secondary") or Blitbuffer.gray(0.55))
    ...
    return SUIWindow.ArrangeList({
      ...
      on_delete = params.on_delete and function(idx, item)
        ...
        if params.on_change then
          params.on_change(items)
        end
        ctx.repaint()
      end or nil,
      on_change = function(new_items)
        if type(params.on_change) == "function" then
          params.on_change(new_items)
        end
        ctx.repaint()
      end,
    })
...
function SUIWindow.makeCtxMenu(ctx)
  return {
    ...
    show_row_page = function(params)
      ctx.push("row_page", params)
    end,
