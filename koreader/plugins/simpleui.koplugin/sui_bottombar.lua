...
local _Geom, _Font
local function FrameContainer()
  _FrameContainer = _FrameContainer
    or require("ui/widget/container/framecontainer")
  return _FrameContainer
end
local function CenterContainer()
  _CenterContainer = _CenterContainer
    or require("ui/widget/container/centercontainer")
  return _CenterContainer
end
local function HorizontalGroup()
  _HorizontalGroup = _HorizontalGroup or require("ui/widget/horizontalgroup")
  return _HorizontalGroup
end
local function VerticalGroup()
  _VerticalGroup = _VerticalGroup or require("ui/widget/verticalgroup")
  return _VerticalGroup
end
local function VerticalSpan()
  _VerticalSpan = _VerticalSpan or require("ui/widget/verticalspan")
  return _VerticalSpan
end
local function LineWidget()
  _LineWidget = _LineWidget or require("ui/widget/linewidget")
  return _LineWidget
end
local function OverlapGroup()
  _OverlapGroup = _OverlapGroup or require("ui/widget/overlapgroup")
  return _OverlapGroup
end
local function TextWidget()
  _TextWidget = _TextWidget or require("ui/widget/textwidget")
  return _TextWidget
end
local function ImageWidget()
  _ImageWidget = _ImageWidget or require("ui/widget/imagewidget")
  return _ImageWidget
end
local function Geom()
  _Geom = _Geom or require("ui/geometry")
  return _Geom
end
local function Font()
  _Font = _Font or require("ui/font")
  return _Font
end
...
local function _getBarBg()
  if
    M.getBarStyle() == "bare"
    or SUISettings:isTrue("simpleui_navbar_transparent")
  then
    return nil
  end
  local ok, style = pcall(_SUIStyle)
...
function M.ICON_SZ()
  return _cached("icon_sz", function()
    return math.floor(
      Screen:scaleBySize(44) * _getNavbarScale() * (_getIconScalePct() / 100)
    )
  end)
end
...
function M.BOT_SP()
  return _cached("bot_sp", function()
    return math.floor(Screen:scaleBySize(12) * _getBottomMarginPct() / 100)
  end)
end
function M.SIDE_M()
...
function M.getPaginationFontSize()
  ...
  local key = _getPaginationKey()
  if key == "xs" then
    return (ok and ss and ss.FS_CAPTION) or 12 -- FS_CAPTION (12)
  elseif key == "s" then
    return (ok and ss and ss.FS_DETAIL) or 15 -- FS_DETAIL (15)
  else
    return (ok and ss and ss.FS_SUBTITLE) or 20
  end -- FS_SUBTITLE (20)
end
...
function M.patchDimmedIcon(btn)
  ...
  if not lw or lw._sui_dim_patched then
    return
  end
  ...
  lw.paintTo = function(self_lw, bb, x, y)
    if not btn.enabled then
      ...
      if w > 0 and h > 0 then
        if
          not self_lw._sui_tmp_bb
          or self_lw._sui_tmp_bb:getWidth() ~= w
          or self_lw._sui_tmp_bb:getHeight() ~= h
        then
          if self_lw._sui_tmp_bb then
            self_lw._sui_tmp_bb:free()
          end
  ...
  lw.free = function(self_lw)
    if self_lw._sui_tmp_bb then
      self_lw._sui_tmp_bb:free()
      self_lw._sui_tmp_bb = nil
    end
    if orig_lw_free then
      orig_lw_free(self_lw)
    end
  end
end
...
function M.getTabWidths(num_tabs, usable_w)
  ...
  for i = 1, num_tabs do
    _tab_widths_cache[i] = (i == num_tabs)
        and (usable_w - base_w * (num_tabs - 1))
      or base_w
  end
...
local function _makeColoredIcon(file, size, fgcolor)
  ...
  if Config.isNerdIcon(file) then
    ...
    local tw = TextWidget():new({
      text = nerd_char,
      face = Font():getFace(
        SUIStyle and SUIStyle.FACE_ICONS or "symbols",
    ...
    function widget:onToggleNightMode()
      require("ui/uimanager"):setDirty(self)
    end
    function widget:onSetNightMode()
      require("ui/uimanager"):setDirty(self)
    end
    function widget:onApplyTheme()
      require("ui/uimanager"):setDirty(self)
    end
    return widget
  end
  ...
  if not safe_file then
    logger.warn(
      "simpleui/bottombar: _makeColoredIcon skipped, invalid file: "
        .. tostring(file)
    )
  ...
  function widget:paintTo(bb, x, y)
    ...
    if
      not self._tmp_bb
      or self._tmp_bb:getWidth() ~= w
      or self._tmp_bb:getHeight() ~= h
    then
      ...
      if safe_file then
        if bar_style == "framed" then
          ...
          if ok_render then
            ...
            iw:free()
            logger.warn(
              "simpleui/bottombar: buildTabCell ignorou imagem corrompida:",
              safe_file
            )
          end
        end
...
function M.buildNavpagerArrowCell(is_prev, enabled, tab_w, mode)
  ...
  if SUIStyle then
    local override =
      SUIStyle.getIcon(is_prev and "sui_navpager_prev" or "sui_navpager_next")
    if override then
      icon_file = override
    end
  end
  ...
  if
    bar_style == "default"
    and not SUISettings:isTrue("simpleui_navbar_transparent")
  then
    og[#og + 1] = LineWidget():new({
      dimen = Geom():new({ w = tab_w, h = M.INDIC_H() }),
      background = _getBarBg() or Blitbuffer.COLOR_WHITE,
...
function M.updateNavpagerArrows(widget, has_prev, has_next)
  ...
  if
    not (
      prev_cc
      and prev_cc._arrow_enabled ~= nil
      and next_cc
      and next_cc._arrow_enabled ~= nil
    )
  then
    return false
  end
  ...
  if
    prev_cc._arrow_enabled == has_prev and next_cc._arrow_enabled == has_next
  then
    return true
  end
...
local function _buildBarContainer(hg_args, is_navpager)
  ...
  if style == "framed" then
    ...
    local border_color = (
      ok
      and SUIStyle
      and SUIStyle.getThemeColor("separator")
    ) or Blitbuffer.gray(0.72)
  ...
  if style == "default" and sep_h > 0 then
    local sep_bg = SUISettings:isTrue("simpleui_navbar_transparent") and nil
      or M.sepColor()
...
function M.buildBarWidget(active_action_id, tab_config, num_tabs, mode)
  ...
  for i = 1, num_tabs do
    ...
    hg_args[#hg_args + 1] =
      M.buildTabCell(action_id, action_id == active_action_id, widths[i], mode)
  end
...
function M.buildBarWidgetWithArrows(
  active_action_id,
  tab_config,
  mode,
  has_prev,
  ...
  -- Prev arrow
  hg_args[#hg_args + 1] =
    M.buildNavpagerArrowCell(true, has_prev, widths[1], mode)
  ...
  for i = 1, center_n do
    ...
    if action_id then
      hg_args[#hg_args + 1] =
        M.buildTabCell(action_id, action_id == active_action_id, w, mode)
    else
  ...
  -- Next arrow
  hg_args[#hg_args + 1] =
    M.buildNavpagerArrowCell(false, has_next, widths[total_n], mode)
...
function M.buildBarWidgetWithKeyFocus(
  active_action_id,
  tab_config,
  kbfocus_idx,
  num_tabs,
  ...
  for i = 1, num_tabs do
    ...
    if i == kbfocus_idx then
      ...
      cell = OverlapGroup():new({
        ...
        cell,
        LineWidget():new({
          dimen = Geom():new({ w = tw, h = bw }),
          background = kbfg,
        }),
        LineWidget():new({
          dimen = Geom():new({ w = tw, h = bw }),
          background = kbfg,
          overlap_offset = { 0, tab_h - bw },
        }),
        LineWidget():new({
          dimen = Geom():new({ w = bw, h = tab_h }),
          background = kbfg,
        }),
        LineWidget():new({
          dimen = Geom():new({ w = bw, h = tab_h }),
          background = kbfg,
          overlap_offset = { tw - bw, 0 },
        }),
      })
    end
...
function M.replaceBar(widget, new_bar, tabs)
  ...
  if not idx then
    logger.err(
      "simpleui: replaceBar called without _navbar_bar_idx — widget not initialised."
    )
    return
  end
  local topbar_on = SUISettings:nilOrTrue("simpleui_topbar_enabled")
  if
    widget._navbar_bar_idx_topbar_on ~= nil
    and widget._navbar_bar_idx_topbar_on ~= topbar_on
  then
...
function M.registerTouchZones(plugin, fm_self)
  ...
  local function _callPageFn(fn_name)
    ...
    for i = #stack, 1, -1 do
      ...
      if w then
        ...
        if
          fc
          and type(fc[fn_name]) == "function"
          and type(fc.page_num) == "number"
        then
          pcall(function()
            fc[fn_name](fc)
          end)
  ...
  local function _callGotoPage(page)
    ...
    for i = #stack, 1, -1 do
      ...
      if w then
        local target, fn
        if
          type(w.onGotoPage) == "function" and type(w.page_num) == "number"
        then
          ...
          if
            fc
            and type(fc.onGotoPage) == "function"
            and type(fc.page_num) == "number"
          then
    ...
    if fm_inst and fm_inst.file_chooser then
      ...
      if
        type(fc.onGotoPage) == "function" and type(fc.page_num) == "number"
      then
  ...
  local next_x = arrows_active and (side_m + usable_w - widths[total_slots])
    or screen_w
  ...
  if arrows_active then
    ...
    zones[#zones + 1] = {
      ...
      handler = function(_ges)
        ...
        logger.dbg(
          "simpleui tz: navbar_pos_prev fired has_prev=",
          tostring(has_prev)
        )
        if has_prev then
...
function M.onTabTap(plugin, action_id, fm_self)
  ...
  if
    fm_self._navbar_container
    and action_id ~= "homescreen"
    and not hs_open
    and not already_active
    and not injected_open
  then
...
local function _executeInPlace(action_id, plugin, fm)
  ...
  _QA().execute(
    action_id,
    { plugin = plugin, fm = fm, show_unavailable = showUnavailable }
  )
...
function M.navigate(plugin, action_id, fm_self, tabs, force)
  ...
  if fm_self ~= fm and fm._navbar_container then
    M.replaceBar(
      fm,
      M.buildBarWidget(_resolveActiveTab(action_id, tabs), tabs),
      tabs
    )
    UIManager:setDirty(fm, "ui")
  end
...
function M.rebuildAllNavbars(plugin)
  ...
  local function rebuildWidget(w)
    ...
    M.replaceBar(
      w,
      M.buildBarWidget(plugin.active_action, tabs, num_tabs, mode),
      tabs
    )
    if new_topbar then
...
function M.setTempTabActive(plugin, action_id, active, prev_action)
  ...
  for _i, entry in ipairs(stack) do
    ...
    if not ok then
      logger.warn(
        "simpleui: setPowerTabActive updateWidget failed:",
        tostring(err)
      )
    end
  end
...
function M.restoreTabInFM(plugin, tabs, prev_action)
  ...
  pcall(function()
    for _i, entry in ipairs(UI.getWindowStack()) do
      if
        entry.widget
        and entry.widget._navbar_injected
        and entry.widget ~= fm
      then
        should_skip = true
        return
  ...
  local restored = (
    fm.file_chooser and Patches._resolveTabForPath(fm.file_chooser.path, t)
  )
    or prev_action
    or t[1]
