...
local function _hsMatchesKey(key)
  if HS_EXACT[key] then
    return true
  end
  for _i, pfx in ipairs(HS_PREFIXES) do
    if key:sub(1, #pfx) == pfx then
      if key == HS_PRESET_KEY then
        return false
      end
      if key == HS_PRESET_KEY or key == "simpleui_hs_active_preset" then
        return false
      end
      return true
    end
...
local function _getDirs()
  ...
  if not ok_ds or not DataStorage then
    return nil, nil
  end
  ...
  if ok_lfs and lfs then
    ...
    if lfs.attributes(exp_dir, "mode") ~= "directory" then
      lfs.mkdir(exp_dir)
    end
    if lfs.attributes(imp_dir, "mode") ~= "directory" then
      lfs.mkdir(imp_dir)
    end
  end
  ...
  if lfs.attributes(dir, "mode") == "directory" then
    for fname in lfs.dir(dir) do
      if
        fname ~= "."
        and fname ~= ".."
        and fname:lower():match(pattern:lower())
      then
        files[#files + 1] = {
          name = fname,
          path = dir .. "/" .. fname,
...
local BUILTIN_PRESETS = {
  {
    ...
    name = _("At a Glance"),
    desc = _("Clock") .. ", " .. _("Currently Reading") .. ", " .. _(
      "Recent Books"
    ),
    layout = {
  ...
  },
  {
    ...
    desc = _("Quote of the Day")
      .. ", "
      .. _("Currently Reading")
      .. ", "
      .. _("Recent Books"),
    layout = {
      pages = { { id = 1, modules = { "quote", "currently", "recent" } } },
    },
    settings = {
  ...
  },
  {
    id = "builtin_momentum",
    name = _("Momentum"),
    desc = _("Cover Deck") .. ", " .. _("Reading Goals") .. ", " .. _(
      "Reading Stats"
    ),
    layout = {
      pages = {
        { id = 1, modules = { "coverdeck", "reading_goals", "reading_stats" } },
      },
    },
    settings = {
...
function SUIPresets.apply(name)
  ...
  for _i, k in ipairs(to_delete) do
    SUISettings:del(k)
  end
...
function SUIPresets.applyBuiltin(id)
  ...
  for _, mod in ipairs(Registry.list()) do
    ...
    elseif mod.enabled_key then
      SUISettings:set("simpleui_hs_" .. mod.enabled_key, is_active)
    end
  end
...
function SUIPresets.listNames()
  local names = {}
  for name in pairs(SUIPresets.getAll()) do
    names[#names + 1] = name
  end
...
function SUIPresets.export(name)
  ...
  if not dir then
    return nil, _("No export directory")
  end
  ...
  if not ok or not LuaSettings then
...
function SUIIconPresets.save(name)
  ...
  for k, v in SUISettings:iterateKeys() do
    if _isScalarIconKey(k) then
      snapshot._scalar[k] = v
    end
  end
...
function SUIIconPresets.apply(name, QA)
  ...
  for k, v in pairs(snapshot._scalar or {}) do
    SUISettings:set(k, v)
  end
  ...
  if QA and QA.invalidateCustomQACache then
...
function SUIIconPresets.listNames()
  local names = {}
  for n in pairs(SUIIconPresets.getAll()) do
    names[#names + 1] = n
  end
  table.sort(names, function(a, b)
    return a:lower() < b:lower()
...
SUIPresets.listImportFiles = function()
  return _listImportFiles("_sui_hs.lua")
end
  ...
  for _, bp in ipairs(BUILTIN_PRESETS) do
    ...
    select_items[#select_items + 1] = {
      ...
      checked_func = function()
        return SUISettings:get("simpleui_hs_active_preset") == _bp.id
      end,
      callback = function()
                      ...
                      sui_build = function(ctx2)
                        ...
                        for _, bp in ipairs(BUILTIN_PRESETS) do
                          rows[#rows + 1] = SUIWindow2.ListRow({
                            ...
                            radio = true,
                            checked = (
                              SUISettings:get("simpleui_hs_active_preset")
                              == bp.id
                            ),
                            on_tap = function()
            ...
            sub_item_table = {
              {
                ...
                callback = function()
                  showDialog(ConfirmBox():new({
                    text = string.format(
                      _(
                        'Overwrite preset "%s" with the current homescreen settings?'
                      ),
                      _name
                    ),
                    ok_text = _("Overwrite"),
...
SUIIconPresets.listImportFiles = function()
  return _listImportFiles("_sui_icn.lua")
end
