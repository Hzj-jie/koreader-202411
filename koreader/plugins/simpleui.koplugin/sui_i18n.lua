...
local function parsePluralExpression(expr)
  local function translateTernary(s)
    ...
    return "("
      .. translateTernary(cond)
      .. " and ("
      .. translateTernary(truthy)
      .. ") or ("
      .. translateTernary(falsy)
      .. "))"
  end
  ...
  if not ok or type(pluralFn) ~= "function" then
    return nil
  end
  return pluralFn
end
...
local function detectLang()
  local lang = G_reader_settings and G_reader_settings:read("language")
  if type(lang) == "string" and lang ~= "" then
    return lang
  end
  local lc = os.getenv("LANG")
    or os.getenv("LC_ALL")
...
local function loadTranslations()
  ...
  local function try(name)
    ...
    if entries and next(entries) then
      ...
      logger.info(
        "simpleui i18n: loaded " .. path .. " — " .. n .. " strings"
      )
