...
local function getSH()
  if not _SH then
    ...
    if ok and m then
      ...
      logger.warn(
        "simpleui: module_new_books: cannot load module_books_shared: "
          .. tostring(m)
      )
    end
  end
...
local function collectBooks(dir, files, depth, state)
  if depth > 5 or state.count > 5000 then
    return
  end
...
function M.build(w, ctx)
    ...
    for _, fp in ipairs(new_fps) do
      if fp ~= ctx.current_fp then
        ...
        if pre and pre ~= false then
          ...
          is_complete = type(summary) == "table"
  ...
  local label_fs =
    math.max(8, math.floor(_BASE_NB_LABEL_FS * scale * lbl_scale))
  ...
  for i = 1, cols do
    ...
    if (bd.percent or 0) < 0.01 then
      ...
      label_text =
        string.format(_("%d%% Read"), math.floor((bd.percent or 0) * 100 + 0.5))
    end
...
function M.getHeight(_ctx)
  local SH = getSH()
  local D = SH.getDims(
    Config.getModuleScale("new_books", _ctx and _ctx.pfx),
    Config.getThumbScale("new_books", _ctx and _ctx.pfx)
  )
  local h = D.RECENT_CELL_H
  if
    SUISettings:isTrue(_ctx and _ctx.pfx .. "new_books_show_frame")
    or SUISettings:isTrue(_ctx and _ctx.pfx .. "new_books_solid_bg")
  then
    h = h + PAD * 2
  end
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return require("sui_config").getModuleScalePct("new_books", pfx)
    end,
    set = function(v)
      require("sui_config").setModuleScale(v, "new_books", pfx)
    end,
...
local function _makeThumbScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return require("sui_config").getThumbScalePct("new_books", pfx)
    end,
    set = function(v)
      require("sui_config").setThumbScale(v, "new_books", pfx)
    end,
    refresh = ctx_menu.refresh,
  })
end
...
function M.getMenuItems(ctx_menu)
  ...
  local label_item = Config.makeScaleItem({
    ...
    get = function()
      return Config.getItemLabelScalePct("new_books", ctx_menu.pfx)
    end,
    set = function(v)
      Config.setItemLabelScale(v, "new_books", ctx_menu.pfx)
    end,
    refresh = ctx_menu.refresh,
  })
  local frame_item = {
    ...
    checked_func = function()
      return SUISettings:isTrue(ctx_menu.pfx .. "new_books_show_frame")
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        ctx_menu.pfx .. "new_books_show_frame",
        not SUISettings:isTrue(ctx_menu.pfx .. "new_books_show_frame")
      )
      ctx_menu.refresh()
    end,
  }
  local solid_bg_item = {
    ...
    checked_func = function()
      return SUISettings:isTrue(ctx_menu.pfx .. "new_books_solid_bg")
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        ctx_menu.pfx .. "new_books_solid_bg",
        not SUISettings:isTrue(ctx_menu.pfx .. "new_books_solid_bg")
      )
      ctx_menu.refresh()
    end,
  }
