...
local function isCompact()
  return SUISettings:readSetting(LAYOUT_KEY) == "compact"
end
local function showAnnual()
  return SUISettings:readSetting(SHOW_ANNUAL) ~= false
end
local function showMonthly()
  return SUISettings:readSetting(SHOW_MONTHLY) ~= false
end
local function showDaily()
  return SUISettings:readSetting(SHOW_DAILY) ~= false
end

local function getAnnualGoal()
  return tonumber(SUISettings:readSetting("simpleui_reading_goal")) or 0
end
local function getAnnualPhysical()
  return tonumber(SUISettings:readSetting("simpleui_reading_goal_physical"))
    or 0
end
local function getMonthlyGoalSecs()
  return tonumber(SUISettings:readSetting("simpleui_monthly_reading_goal_secs"))
    or 0
end
local function getDailyGoalSecs()
  return tonumber(SUISettings:readSetting("simpleui_daily_reading_goal_secs"))
    or 0
end

local function _getElemOrder(pfx)
  local saved = SUISettings:readSetting(
    (pfx or "simpleui_hs_") .. "reading_goals_elem_order"
  )
...
local function _buildInnerCompact(
  inner_w,
  lbl_w,
  pct_w,
  label_str,
  ...
  local right_w =
    math.max(math.floor(inner_w * 0.28), pct_w + PCT_DETAIL_GAP + MIN_DETAIL_W)
  ...
  if available < Screen:scaleBySize(40) then
    ...
    right_w =
      math.max(0, inner_w - lbl_w - LBL_BAR_GAP - BAR_PCT_GAP - available)
  end
...
local function buildCompactGoalRow(
  inner_w,
  lbl_w,
  pct_w,
  label_str,
...
local function _buildInnerDefault(
  inner_w,
  label_str,
  pct,
  pct_str,
  ...
  if available < d.min_bar_w then
    ...
    PCT_W =
      math.max(0, inner_w - d.lbl_w - LBL_BAR_GAP - BAR_PCT_GAP - available)
  end
  local bar_w = available
...
local function buildGoalRow(
  ...
  local block = _buildInnerDefault(
    inner_w,
    label_str,
    pct,
    pct_str,
...
local function showMonthlySettingsDialog(on_confirm)
  ...
  UIManager:show(SpinWidget:new({
    ...
    callback = function(spin)
      SUISettings:saveSetting(
        "simpleui_monthly_reading_goal_secs",
...
local function _annualData(books_read)
  ...
  logger.dbg(
    "simpleui reading_goals: annual bar — goal=",
    goal,
    "books_read=",
    books_read,
    "physical=",
  ...
  if ok_fmt then
    ...
    logger.warn(
      "simpleui reading_goals annual format error: " .. tostring(fmt_res)
    )
    detail_text = (goal > 0) and (read .. "/" .. goal .. " books")
      or (read .. " books")
  end
  ...
  local ok_fmt, fmt_res = pcall(function()
    if m_goal_secs <= 0 then
      ...
      return string.format(
        "%s/%s",
        formatDuration(month_secs),
        formatDuration(m_goal_secs)
      )
    end
  end)
  if ok_fmt then
    ...
    logger.warn(
      "simpleui reading_goals monthly format error: " .. tostring(fmt_res)
    )
    detail_text = (m_goal_secs <= 0) and (formatDuration(month_secs) .. " read")
...
local function _dailyData(today_secs)
  ...
  local ok_fmt, fmt_res = pcall(function()
    if goal_secs <= 0 then
      ...
      return string.format(
        "%s/%s",
        formatDuration(today_secs),
        formatDuration(goal_secs)
      )
    end
  end)
  if ok_fmt then
    ...
    logger.warn(
      "simpleui reading_goals daily format error: " .. tostring(fmt_res)
    )
    detail_text = (goal_secs <= 0) and (formatDuration(today_secs) .. " read")
...
function M.build(w, ctx)
  ...
  if not show_ann and not show_mon and not show_day then
    return nil
  end
  ...
  local ok, res = pcall(function()
    ...
    if sp.db_conn_fatal and ctx then
    ...
    local _theme_secondary = ok_ss
      and SUIStyle
      and SUIStyle.getThemeColor("text_secondary")
    ...
    if compact then
      ...
      if show_ann then
        ann_pct, ann_pct_str, ann_detail = _annualData(books_read)
      end
      if show_mon then
        mon_pct, mon_pct_str, mon_detail = _monthlyData(month_secs)
      end
      if show_day then
        day_pct, day_pct_str, day_detail = _dailyData(today_secs)
      end
      -- Measure pct column width across all active rows.
      local pct_strs = {}
      if show_ann and ann_pct_str ~= "" then
        pct_strs[#pct_strs + 1] = ann_pct_str
      end
      if show_mon and mon_pct_str ~= "" then
        pct_strs[#pct_strs + 1] = mon_pct_str
      end
      if show_day and day_pct_str ~= "" then
        pct_strs[#pct_strs + 1] = day_pct_str
      end
      ...
      for _i, k in ipairs(_getElemOrder(ctx.pfx)) do
        if k == "annual" and show_ann then
          ...
          table.insert(rg_update_funcs, {
            cat = "books",
            fn = function(books_r, _today_s, _month_s)
        ...
        elseif k == "monthly" and show_mon then
          ...
          table.insert(rg_update_funcs, {
            cat = "timeseries",
            fn = function(_books_r, _today_s, month_s)
        ...
        elseif k == "daily" and show_day then
          ...
          table.insert(rg_update_funcs, {
            cat = "timeseries",
            fn = function(_books_r, today_s, _month_s)
...
function M.updateStats(widget, ctx)
  if not widget or not widget._rg_update_funcs then
    return false
  end
  local sp = ctx.stats or {}
...
function M.getHeight(_ctx)
  local n = (showAnnual() and 1 or 0)
    + (showMonthly() and 1 or 0)
    + (showDaily() and 1 or 0)
  if n == 0 then
    return 0
  end
  ...
  local h = 0
  if isCompact() then
    h = _compactRowsHeight(
      n,
      _compactDims(Config.getModuleScale("reading_goals", pfx))
    )
  else
  ...
  if
    SUISettings:isTrue(pfx .. "reading_goals_show_frame")
    or SUISettings:isTrue(pfx .. "reading_goals_solid_bg")
  then
    h = h + PAD * 2
  end
  return label_h + h
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    enabled_func = function()
      return not Config.isScaleLinked()
    end,
    title = _lc("Scale"),
    info = _lc("Scale for this module.\n100% is the default size."),
    get = function()
      return Config.getModuleScalePct("reading_goals", pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, "reading_goals", pfx)
    end,
...
function M.getMenuItems(ctx_menu)
  ...
  return {
      ...
      sub_item_table = {
        ...
        },
        {
          text_func = function()
            ...
            return g > 0
                and string.format(
                  N_lc(
                    "  Set Goal  (%d book in %s)",
                    "  Set Goal  (%d books in %s)",
                    g
                  ),
                  g,
                  _getYearStr()
        ...
        },
        {
          text_func = function()
            ...
            local h = math.floor(secs / 3600)
            if secs <= 0 then
              return _lc("  Set Goal  (disabled)")
            else
              return string.format(_lc("  Set Goal  (%d hr/month)"), h)
            end
          end,
          keep_menu_open = true,
          callback = function()
            showMonthlySettingsDialog(refresh)
          end,
        },
        ...
        },
        {
          text_func = function()
            ...
            if secs <= 0 then
              ...
              return string.format(_lc("  Set Goal  (%d min/day)"), m)
            end
          end,
          keep_menu_open = true,
          callback = function()
            showDailySettingsDialog(refresh)
          end,
        },
      },
              ...
              subtitle = function()
                ...
                for _, k in ipairs(_getElemOrder(ctx_menu.pfx)) do
                  if k == "annual" and showAnnual() then
                    names[#names + 1] = _lc("Annual Goal")
                  end
                  if k == "monthly" and showMonthly() then
                    names[#names + 1] = _lc("Monthly Goal")
                  end
                  if k == "daily" and showDaily() then
                    names[#names + 1] = _lc("Daily Goal")
                  end
                end
                return #names > 0 and table.concat(names, "  ·  ")
                  or _lc("No items selected.")
              end,
              inner_w = ctx.inner_w,
              show_chevron = true,
              on_tap = function()
                ctx.push("arrange", {
                  ...
                  footer_action = function(ctx2)
                    ...
                    local function _enable(key, flag_key, label)
                      picker_items[#picker_items + 1] = {
                        ...
                        on_tap = function(picker_ctx)
                          ...
                          SUISettings:saveSetting(
                            ctx_menu.pfx .. "reading_goals_elem_order",
                            new_order
                          )
                          refresh()
                          picker_ctx.pop()
                    ...
                    if not showAnnual() then
                      _enable("annual", SHOW_ANNUAL, _lc("Annual Goal"))
                    end
                    if not showMonthly() then
                      _enable("monthly", SHOW_MONTHLY, _lc("Monthly Goal"))
                    end
                    if not showDaily() then
                      _enable("daily", SHOW_DAILY, _lc("Daily Goal"))
                    end
                  ...
                  items_func = function()
                    ...
                    for _, k in ipairs(_getElemOrder(ctx_menu.pfx)) do
                      if k == "annual" and showAnnual() then
                        ...
                        local subtitle = g > 0
                            and string.format(
                              N_lc("%d book in %s", "%d books in %s", g),
                              g,
                              _getYearStr()
                            )
                          or _lc("Not set")
                          ...
                          more_items = {
                            {
                              ...
                              icon = "edit",
                              on_tap = function()
                                showMonthlySettingsDialog(function()
                                  refresh()
                                end)
                              end,
                            },
                          ...
                          more_items = {
                            {
                              ...
                              icon = "edit",
                              on_tap = function()
                                showDailySettingsDialog(function()
                                  refresh()
                                end)
                              end,
                            },
    ...
    scale_item,
    {
      ...
      keep_menu_open = true,
      callback = function()
        showAnnualPhysicalDialog(refresh)
      end,
      sui_build = ctx_menu.is_sui
          and function(ctx, _item)
            local SUIWindow = require("sui_window")
    ...
    ),
    {
      text = _lc("Frame"),
      checked_func = function()
        return SUISettings:isTrue(ctx_menu.pfx .. "reading_goals_show_frame")
      end,
      keep_menu_open = true,
      callback = function()
        SUISettings:saveSetting(
          ctx_menu.pfx .. "reading_goals_show_frame",
          not SUISettings:isTrue(ctx_menu.pfx .. "reading_goals_show_frame")
        )
        refresh()
      end,
    },
    {
      ...
      checked_func = function()
        return SUISettings:isTrue(ctx_menu.pfx .. "reading_goals_solid_bg")
      end,
      keep_menu_open = true,
      callback = function()
        SUISettings:saveSetting(
          ctx_menu.pfx .. "reading_goals_solid_bg",
          not SUISettings:isTrue(ctx_menu.pfx .. "reading_goals_solid_bg")
        )
        refresh()
      end,
    },
    {
      ...
      callback = function()
        ...
        if MCD and MCD.invalidateCache then
          MCD.invalidateCache()
        end
