...
local function getAlignment(pfx)
  ...
  for _, a in ipairs(_ALIGN_VALUES) do
    if a == v then
      return v
    end
  end
...
local function loadCustomQuotes(filename)
  ...
  _custom_quotes_cache = nil
  _custom_quotes_cache_file = nil
  ...
  if ok and type(data) == "table" and #data > 0 then
    _custom_quotes_cache = data
    _custom_quotes_cache_file = filename
    logger.info(
      "simpleui: quote: loaded " .. #data .. " custom quotes from " .. filename
    )
    return _custom_quotes_cache
  end
  logger.warn(
    "simpleui: quote: failed to load "
      .. tostring(path)
      .. ": "
      .. tostring(data)
  )
...
local function _extractStr(line, key)
  ...
  if not line:find(key, 1, true) then
    return nil
  end
...
local function _buildPool()
  ...
  if not ReadHistory or not ReadHistory.hist then
    return {}
  end
  ...
  for _, cand in ipairs(candidates) do
    if books_read >= MAX_POOL_BOOKS or #pool >= MAX_POOL_HIGHLIGHTS then
      break
    end
    ...
    if f then
      ...
      for line in f:lines() do
        ...
        elseif
          line:find('["page"]', 1, true)
          and not line:find('["pagemap"]', 1, true)
        then
  ...
  local function makeTBW(text, face, fgcolor, bold)
    ...
    if has_wallpaper then
      ...
      if ok_tbx then
        ...
        logger.warn(
          "simpleui: module_quote: makeAlphaTextBox failed, falling back: "
            .. tostring(tbx)
        )
...
local function buildFromCustomQuote(
  inner_w,
  face_quote,
  face_attr,
  vspan_gap,
  ...
  if not q then
    return TextBoxWidget:new({
      ...
      text = _(
        "No custom quotes found. Add a .lua file to the plugin's sui_quotes/ folder and select it in Settings."
      ),
...
local function buildFromHighlight(
  inner_w,
  face_quote,
  face_attr,
  vspan_gap,
  ...
  logger.warn(
    "simpleui: quote: showing highlight from '"
      .. tostring(h.title)
      .. "': "
      .. tostring(h.text):sub(1, 60)
  )

  local attr = "— " .. h.title

  if h.authors and h.authors ~= "" then
    attr = attr .. ",  " .. h.authors
  end
  ...
  local LEADING_QUOTES =
    "^[\"'\u{201C}\u{2018}\u{201E}\u{201A}\u{00AB}\u{2039}%s]+"
  local TRAILING_QUOTES =
    "[\"'\u{201D}\u{2019}\u{201E}\u{201A}\u{00BB}\u{203A}%s]+$"
...
function M.build(w, ctx)
  ...
  logger.warn(
    "simpleui: quote: build source=" .. source .. " align=" .. alignment
  )
  ...
  if source == "highlights" then
    content, hl_filepath, hl_title, hl_pos0, hl_page = buildFromHighlight(
      inner_w,
      face_quote,
      face_attr,
      vspan_gap,
  ...
  elseif source == "mixed" then
    content, hl_filepath, hl_title, hl_pos0, hl_page = buildFromMixed(
      inner_w,
      face_quote,
      face_attr,
      vspan_gap,
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct("quote", pfx)
    end,

    set = function(v)
      Config.setModuleScale(v, "quote", pfx)
    end,
...
function M.getMenuItems(ctx_menu)
  ...
  return {

    {
      ...
      sub_item_table = {
...

        {
          ...
          checked_func = function()
            return getSource(pfx) == "highlights"
          end,
...

    {
      ...
      value_func = function()
        return alignLabel(getAlignment(pfx))
      end,
      sub_item_table = {
        {
          text = _lc("Left"),
          checked_func = function()
            return getAlignment(pfx) == "left"
          end,
          keep_menu_open = true,
          callback = function()
            setAlignment(pfx, "left")
            refresh()
          end,
        },
        {
          text = _lc("Center"),
          checked_func = function()
            return getAlignment(pfx) == "center"
          end,
          keep_menu_open = true,
          callback = function()
            setAlignment(pfx, "center")
            refresh()
          end,
        },
        {
          text = _lc("Right"),
          checked_func = function()
            return getAlignment(pfx) == "right"
          end,
          keep_menu_open = true,
          callback = function()
            setAlignment(pfx, "right")
            refresh()
          end,
        },
      },
