...
local _ELEM_DEFAULT_ORDER =
  { "percent", "book_days", "book_time", "book_remaining" }
local _ELEM_LABELS = {
...
local function _toggleElem(pfx, key)
  SUISettings:saveSetting(
    pfx .. "coverdeck_show_" .. key,
    not _showElem(pfx, key)
  )
end
...
local function fetchBookStats(md5, shared_conn, ctx, force)
  ...
  if result then
    if _bstats_cache_count >= BSTATS_CACHE_MAX then
      _bstats_evict()
    end
  ...
  if ctx.recent_fps then
    ...
    for _i, fp in ipairs(ctx.recent_fps) do
      if not seen[fp] then
        ...
        local is_done = (pct >= 1.0)
          or (
            type(pd) == "table"
            and type(pd.summary) == "table"
            and pd.summary.status == "complete"
          )
        if show_fin or not is_done then
          fps[#fps + 1] = fp
          seen[fp] = true
          if #fps >= MAX_RECENT_FPS then
            break
          end
        end
      end
...
function M.build(w, ctx)
  ...
  logger.dbg(
    "coverdeck: build source="
      .. tostring(source)
      .. " current_fp="
      .. tostring(ctx.current_fp)
      .. " recent_fps="
      .. tostring(ctx.recent_fps and #ctx.recent_fps or "nil")
  )

  local fps = getFps(source, ctx)
  if not fps or #fps == 0 then
    logger.warn(
      string.format("coverdeck: no books found (source=%s)", tostring(source))
    )
    return nil
  end
  ...
  local thumb_scale = c and c.thumb_scale
    or Config.getThumbScale("coverdeck", pfx)
  local lbl_scale = c and c.lbl_scale
    or Config.getItemLabelScale("coverdeck", pfx)
  ...
  if c and c.show then
    ...
    local stats_order = (
      type(stats_order_saved) == "table" and #stats_order_saved > 0
    )
        and (function()
          local seen, result = {}, {}
  ...
  local function buildCover(fp, cw, ch, align)
    ...
    local cover = SH.getBookCover(fp, cw, ch, align, 0.20)
  ...
  if count >= 5 then
    items[#items + 1] =
      buildCover(fps[carouselIdx(curIdx, -2, count)], far_w, far_h, "left")
    items[#items].overlap_offset = {
      math.floor(centerX - half_cw - offset_far),
      yTopOf(centerY, far_h),
    }
    cover_slots[#cover_slots + 1] = {
      fp = fps[carouselIdx(curIdx, -2, count)],
      w = far_w,
      h = far_h,
      align = "left",
    ...
    items[#items + 1] =
      buildCover(fps[carouselIdx(curIdx, 2, count)], far_w, far_h, "right")
    items[#items].overlap_offset = {
      math.floor(centerX + half_cw + offset_far - far_w),
      yTopOf(centerY, far_h),
    }
    cover_slots[#cover_slots + 1] = {
      fp = fps[carouselIdx(curIdx, 2, count)],
  ...
  if count >= 2 then
    items[#items + 1] =
      buildCover(fps[carouselIdx(curIdx, -1, count)], side_w, side_h, "left")
    items[#items].overlap_offset = {
      math.floor(centerX - half_cw - offset_near),
      yTopOf(centerY, side_h),
    }
    cover_slots[#cover_slots + 1] = {
      fp = fps[carouselIdx(curIdx, -1, count)],
      w = side_w,
      h = side_h,
      align = "left",
      overlap_offset = items[#items].overlap_offset,
  ...
  if count >= 3 then
    items[#items + 1] =
      buildCover(fps[carouselIdx(curIdx, 1, count)], side_w, side_h, "right")
    items[#items].overlap_offset = {
      math.floor(centerX + half_cw + offset_near - side_w),
      yTopOf(centerY, side_h),
    }
    cover_slots[#cover_slots + 1] = {
      fp = fps[carouselIdx(curIdx, 1, count)],
  ...
  items[#items].overlap_offset =
    { math.floor(centerX - half_cw), yTopOf(centerY, center_h) }
  cover_slots[#cover_slots + 1] = {
    fp = fps[curIdx],
    w = center_w,
    h = center_h,
  ...
  local bd =
    SH.getBookData(fps[curIdx], ctx.prefetched and ctx.prefetched[fps[curIdx]])
  ...
  for _i, key in ipairs(stats_order) do
    ...
    if show_this == nil then
      show_this = _showElem(pfx, key)
    end
    if show_this then
      has_any_stats = true
      break
  ...
  if has_any_stats then
    if #meta > 0 then
      meta[#meta + 1] = SH.vspan(PAD2, ctx.vspan_pool)
    end
      ...
      for _i, key in ipairs(stats_order) do
        ...
        if show_this then
          ...
          if key == "percent" then
            text = string.format(
              _("%d%% Read"),
              math.floor((nd.percent or 0) * 100 + 0.5)
            )
          elseif nb then
            ...
            elseif key == "book_remaining" then
              local avg_t = (nb.avg_time and nb.avg_time > 0) and nb.avg_time
                or nd.avg_time
              if avg_t and avg_t > 0 and nd.pages and nd.pages > 0 then
                local secs_left =
                  math.floor(avg_t * nd.pages * (1 - (nd.percent or 0)))
                if secs_left > 0 then
          ...
          if text then
            stats_parts[#stats_parts + 1] = text
          end
        end
      end
      local final_text = #stats_parts > 0 and table.concat(stats_parts, " · ")
        or ""
...
function M.updateStats(widget, ctx)
  ...
  if not actual_widget or not actual_widget._cd_update_funcs then
    return false
  end
...
function M.getMenuItems(ctx_menu)
  ...
  local scale_items = {
    Config.makeScaleItem({
      ...
      get = function()
        return Config.getModuleScalePct("coverdeck", pfx)
      end,
      set = function(v)
        Config.setModuleScale(v, "coverdeck", pfx)
      end,
      refresh = refresh,
    }),
    Config.makeScaleItem({
      ...
      get = function()
        return Config.getThumbScalePct("coverdeck", pfx)
      end,
      set = function(v)
        Config.setThumbScale(v, "coverdeck", pfx)
      end,
      refresh = refresh,
    }),
    Config.makeScaleItem({
      ...
      get = function()
        return Config.getItemLabelScalePct("coverdeck", pfx)
      end,
      set = function(v)
        Config.setItemLabelScale(v, "coverdeck", pfx)
      end,
      refresh = refresh,
    }),
  }
  ...
  local source_item = {
    ...
    sub_item_table = {
      {
        text = _lc("Recent Books"),
        radio = true,
        checked_func = function()
          return getSource(pfx) == "recent"
        end,
        keep_menu_open = true,
        callback = function()
      ...
      toggle_item("Progress bar", "progress"),
      {
        ...
        callback = function()
          ...
          local function on_save()
            ...
            local active_set = {}
            for _i, k in ipairs(new_order) do
              active_set[k] = true
            end
      ...
      return SUIWindow.ListRow({
        ...
        on_tap = function()
          ...
          for _, key in ipairs(_getElemOrder(pfx)) do
            if _showElem(pfx, key) then
              sort_items[#sort_items + 1] =
                { text = _lc(_ELEM_LABELS[key]), orig_item = key }
            end
          end
  ...
  for _i, item in ipairs(scale_items) do
    menu[#menu + 1] = item
  end
  ...
  menu[#menu + 1] = {
    ...
    checked_func = function()
      return showFinished(pfx)
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        pfx .. SETTING_SHOW_FINISHED,
        not showFinished(pfx)
      )
      refresh()
    end,
  }
