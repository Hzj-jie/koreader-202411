...
local function _pruneOrphanInstances(mod, placed)
  ...
  for _, iid in ipairs(orphans) do
    ...
    logger.dbg(
      "simpleui: moduleregistry: pruned orphaned instance '"
        .. iid
        .. "' (key="
        .. inst_key
        .. ")"
...
local function _load()
  ...
  for _, def in ipairs(MODULES) do
    ...
    if not ok or not mod then
      logger.warn(
        "simpleui: moduleregistry: failed to load '"
          .. def.require_mod
          .. "': "
          .. tostring(mod)
      )
  ...
  for _, entry in ipairs(_external) do
    ...
    if type(entry) == "string" then
      ...
      if not ok or not mod then
        logger.warn(
          "simpleui: moduleregistry: failed to load external '"
            .. entry
            .. "': "
            .. tostring(mod)
        )
...
function Registry.countEnabled(pfx)
  ...
  for _, mod in ipairs(Registry.list()) do
    if Registry.isEnabled(mod, pfx) then
      n = n + 1
    end
  end
  return n
...
function Registry.createInstance(base_id)
  ...
  local inst_id = (base_id or "quick_actions_row")
    .. "_"
    .. string.format("%06x", math.floor(os.time() * 1000) % 0xFFFFFF)
  -- Avoid collisions (unlikely but safe).
  if _by_id[inst_id] then
    inst_id = inst_id .. "x"
  end
