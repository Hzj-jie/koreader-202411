...
local function getAlignment(pfx)
  ...
  for _, a in ipairs(ALIGN_VALUES) do
    if a == v then
      return v
    end
  end
...
local function getClockStyle(pfx)
  ...
  for _, s in ipairs(STYLE_VALUES) do
    if s == v then
      return v
    end
  end
...
local function timeToWords(hour, min, is_12h)
  ...
  elseif _wc_units_cache[h] then
    ...
    if units == 0 then
      ...
      hour_str = (_wc_tens_cache[tens] or "")
        .. _wc_sep_cache
        .. (_wc_units_cache[units] or tostring(units))
    end
  end
...
local function _buildWordClockWidget(text, face, inner_w, align, theme_fg)
  ...
  local function makeLine(txt)
    ...
    if not wgt.dimen then
      wgt.dimen = wgt:getSize()
    end
...
local function _battInfo()
  ...
  if pwr.isCharging then
    local ok, v = pcall(pwr.isCharging, pwr)
    if ok then
      charging = v
    end
  end
...
local function build(w, pfx, vspan_pool)
  ...
  local date_gap =
    math.max(0, math.floor(_BASE_DATE_GAP * scale * getDateGapPct(pfx) / 100))
  local batt_gap =
    math.max(0, math.floor(_BASE_BATT_GAP * scale * getBattGapPct(pfx) / 100))
  ...
  if show_clock then
    if clock_style == "word" then
      ...
      local wc_widget =
        _buildWordClockWidget(wc_text, face, inner_w, align, theme_fg)
      vg[#vg + 1] = wc_widget
    else
...
local function _tick()
  ...
  if not HS or HS._instance ~= hs then
    _hs_widget = nil
    return
  end
  ...
  if
    hs._suspended
    or (plugin and plugin._simpleui_suspended)
    or Device.screen_saver_mode
  then
    return
  end
  ...
  if body and idx and body[idx] and hs._navbar_container then
    ...
    local ok_w, new_widget =
      pcall(build, inner_w, hs._clock_pfx, hs._vspan_pool)
...
function M.getHeight(ctx)
  ...
  local clock_w = math.floor(_BASE_CLOCK_W * scale)
  local date_h = math.max(8, math.floor(_BASE_DATE_H * scale))
  local date_gap = math.max(
    0,
    math.floor(_BASE_DATE_GAP * scale * getDateGapPct(ctx.pfx) / 100)
  )
  local batt_gap = math.max(
    0,
    math.floor(_BASE_BATT_GAP * scale * getBattGapPct(ctx.pfx) / 100)
  )
  local batt_h = math.max(7, math.floor(_BASE_BATT_H * scale))
  ...
  if show_batt then
    h = h + batt_h
    if show_clock or show_date then
      h = h + batt_gap
    end
  end
  return h
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct("clock", pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, "clock", pfx)
    end,
  ...
  return {
    {
      ...
      checked_func = function()
        return isClockEnabled(pfx)
      end,
      keep_menu_open = true,
      callback = function()
        toggle(SETTING_ON, isClockEnabled(pfx))
      end,
    },
    {
      text = _lc("Show Date"),
      checked_func = function()
        return isDateEnabled(pfx)
      end,
      keep_menu_open = true,
      callback = function()
        toggle(SETTING_DATE, isDateEnabled(pfx))
      end,
    },
    {
      text = _lc("Show Battery"),
      checked_func = function()
        return isBattEnabled(pfx)
      end,
      keep_menu_open = true,
      callback = function()
        toggle(SETTING_BATTERY, isBattEnabled(pfx))
      end,
    },
      ...
      sub_item_table = {
        {
          ...
          radio = true,
          checked_func = function()
            return getClockStyle(pfx) == "digital"
          end,
          keep_menu_open = true,
          callback = function()
            setClockStyle(pfx, "digital")
            refresh()
          end,
        },
        {
          ...
          radio = true,
          checked_func = function()
            return getClockStyle(pfx) == "word"
          end,
          keep_menu_open = true,
          callback = function()
            setClockStyle(pfx, "word")
            refresh()
          end,
        },
      },
    },
    {
      ...
      value_func = function()
        return alignLabel(getAlignment(pfx), _lc)
      end,
      separator = true,
      sub_item_table = {
        {
    ...
    {
      text_func = function()
        return _lc("Date Spacing")
      end,
      value_func = function()
        return getDateGapPct(pfx) .. "%"
      end,
      enabled_func = function()
        return isDateEnabled(pfx)
      end,
        ...
        UIManager_:show(SpinWidget:new({
          ...
          callback = function(spin)
            SUISettings:saveSetting(
              pfx .. SETTING_DATE_GAP,
              _clampElemGap(spin.value)
            )
            refresh()
          end,
        }))
    ...
    {
      text_func = function()
        return _lc("Battery Spacing")
      end,
      value_func = function()
        return getBattGapPct(pfx) .. "%"
      end,
      enabled_func = function()
        return isBattEnabled(pfx)
      end,
