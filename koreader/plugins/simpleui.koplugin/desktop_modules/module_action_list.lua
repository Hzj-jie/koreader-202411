...
-- Core widget builder
-- ---------------------------------------------------------------------------
local function buildListWidget(
  w,
  action_ids,
  show_icons,
  align,
...
function M.getMenuItems(ctx_menu)
  ...
  local pool = {}
  for _, a in ipairs(getALPool()) do
    pool[#pool + 1] = a
  end
  ...
  items[#items + 1] = Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct(MOD_ID, pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, MOD_ID, pfx)
    end,
    refresh = refresh,
  })
  items[#items + 1] = Config.makeScaleItem({
    ...
    get = function()
      return Config.getItemLabelScalePct(MOD_ID, pfx)
    end,
    set = function(v)
      Config.setItemLabelScale(v, MOD_ID, pfx)
    end,
    refresh = refresh,
  })
  ...
  items[#items + 1] = {
    text = _lc("Show Icon"),
    checked_func = function()
      return not isIconHidden(pfx, MOD_SUFFIX)
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        pfx .. HIDE_ICON_KEY,
        not isIconHidden(pfx, MOD_SUFFIX)
      )
      refresh()
    end,
  }
  items[#items + 1] = {
    ...
    value_func = function()
      return alignLabel(getAlignment(pfx, MOD_SUFFIX))
    end,
    sub_item_table = {
      {
        text = _lc("Left"),
        checked_func = function()
          return getAlignment(pfx, MOD_SUFFIX) == "left"
        end,
        keep_menu_open = true,
        callback = function()
          setAlignment(pfx, MOD_SUFFIX, "left")
          refresh()
        end,
      },
      {
        text = _lc("Center"),
        checked_func = function()
          return getAlignment(pfx, MOD_SUFFIX) == "center"
        end,
        keep_menu_open = true,
        callback = function()
          setAlignment(pfx, MOD_SUFFIX, "center")
          refresh()
        end,
      },
      {
        text = _lc("Right"),
        checked_func = function()
          return getAlignment(pfx, MOD_SUFFIX) == "right"
        end,
        keep_menu_open = true,
        callback = function()
          setAlignment(pfx, MOD_SUFFIX, "right")
          refresh()
        end,
      },
    },
