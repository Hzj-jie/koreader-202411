...
local function getSH()
  if not _SH then
    ...
    if ok and m then
      ...
      logger.warn(
        "simpleui: module_recent: cannot load module_books_shared: "
          .. tostring(m)
      )
    end
  end
...
function M.build(w, ctx)
  ...
  for _, fp in ipairs(ctx.recent_fps) do
    ...
    if show_fin or not is_done then
      ...
      if #recent_fps >= 5 then
        break
      end
    end
  end
  if #recent_fps == 0 then
    return nil
  end
...
function M.getHeight(ctx)
  ...
  local D = SH.getDims(
    Config.getModuleScale("recent", pfx),
  ...
  if
    SUISettings:isTrue(pfx .. "recent_show_frame")
    or SUISettings:isTrue(pfx .. "recent_solid_bg")
  then
    h = h + PAD * 2
  end
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return require("sui_config").getModuleScalePct("recent", pfx)
    end,
    set = function(v)
      require("sui_config").setModuleScale(v, "recent", pfx)
    end,
...
local function _makeThumbScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return require("sui_config").getThumbScalePct("recent", pfx)
    end,
    set = function(v)
      require("sui_config").setThumbScale(v, "recent", pfx)
    end,
    refresh = ctx_menu.refresh,
  })
end
...
function M.getMenuItems(ctx_menu)
  ...
  local label_item = Config.makeScaleItem({
    ...
    get = function()
      return Config.getItemLabelScalePct("recent", pfx)
    end,
    set = function(v)
      Config.setItemLabelScale(v, "recent", pfx)
    end,
    refresh = refresh,
  })
  return {
    ...
    Config.makeLabelToggleItem("recent", _("Recent Books"), refresh, _lc),
    {
      ...
      checked_func = function()
        return SUISettings:isTrue(pfx .. "recent_show_frame")
      end,
      keep_menu_open = true,
      callback = function()
        SUISettings:saveSetting(
          pfx .. "recent_show_frame",
          not SUISettings:isTrue(pfx .. "recent_show_frame")
        )
        refresh()
      end,
    },
    {
      ...
      checked_func = function()
        return SUISettings:isTrue(pfx .. "recent_solid_bg")
      end,
      keep_menu_open = true,
      callback = function()
        SUISettings:saveSetting(
          pfx .. "recent_solid_bg",
          not SUISettings:isTrue(pfx .. "recent_solid_bg")
        )
        refresh()
      end,
    },
