...
local function applyCustomProps(fp, title, authors)
  ...
  if not ok or not custom_file then
    return title, authors
  end
  ...
  return custom_props.title or title, custom_props.authors or authors
end
...
SH.RECENT_CELL_H = _BASE_RECENT_H
  + _BASE_RB_GAP1
  + _BASE_RB_BAR_H
  + _BASE_RB_GAP2
  + _BASE_RB_LABEL_H
  ...
  while true do
    ...
    if authors then
      authors_wg = TextBoxWidget:new({
        ...
        face = Font:getFace(
          ...
          math_max(authors_font_max - sizedec, authors_font_min)
        ),
        width = text_width,
        alignment = "center",
        bgcolor = nil,
    ...
    if title then
      title_wg = TextBoxWidget:new({
        ...
        face = Font:getFace(
          SUIStyle.FACE_REGULAR,
          math_max(title_font_max - sizedec, title_font_min)
        ),
        width = text_width,
        alignment = "center",
        bgcolor = nil,
    ...
    if authors then
      free_height = free_height - top_pad
    end
    ...
    if textboxes_ok and free_height > 0.2 * height then
      break
    end
...
function SH.formatTimeLeft(pct, pages, avg_time)
  if not avg_time or avg_time <= 0 or not pct or pct < 0 or not pages then
    return nil
  end
    ...
    if DS and lfs.attributes(filepath, "mode") == "file" then
      ...
      if ok2 and ds then
        ...
        meta.title, meta.authors =
          applyCustomProps(filepath, rp.title, rp.authors)
        stat_pages = rs.pages
...
function SH.prefetchBooks(show_currently, show_recent, max_recent)
  ...
  if not show_currently and not show_recent then
    return state
  end
  ...
  for i = 1, #(ReadHistory.hist or {}) do
    ...
    if not show_recent and state.current_fp then
      break
    end
    if state.current_fp and #state.recent_fps >= max_recent then
      break
    end
  end
  return state
