...
local function _getItems(pfx)
  local saved =
    SUISettings:readSetting((pfx or "simpleui_hs_") .. "reading_stats_items")
  if type(saved) ~= "table" or #saved == 0 then
    return { "total_books", "today_time", "streak" }
  end
  return saved
end
...
local STAT_MAP = {
  today_time = {
    ...
    value = function(s)
      return fmtTime(s.today_secs)
    end,
    label = _("of reading today"),
  },
  today_pages = {
    display_label = _("Today — Pages"),
    value = function(s)
      return tostring(s.today_pages)
    end,
    label = _("pages read today"),
  },
  week_time = {
    display_label = _("This week — Time"),
    value = function(s)
      return fmtTime(s.week_secs)
    end,
    label = _("of reading this week"),
  },
  week_pages = {
    display_label = _("This week — Pages"),
    value = function(s)
      return tostring(s.week_pages)
    end,
    label = _("pages read this week"),
  },
  avg_time = {
    display_label = _("Daily avg — Time"),
    value = function(s)
      return fmtTime(s.avg_secs)
    end,
    label = _("daily avg (7 days)"),
  },
  avg_pages = {
    display_label = _("Daily avg — Pages"),
    value = function(s)
      return tostring(s.avg_pages)
    end,
    label = _("pages/day (7 days)"),
  },
  month_time = {
    display_label = _("This month — Time"),
    value = function(s)
      return fmtTime(s.month_secs)
    end,
    label = _("of reading this month"),
  },
  month_pages = {
    display_label = _("This month — Pages"),
    value = function(s)
      return tostring(s.month_pages)
    end,
    label = _("pages read this month"),
  },
  total_time = {
    display_label = _("All time — Time"),
    value = function(s)
      return fmtTime(s.total_secs)
    end,
    label = _("of reading, all time"),
  },
  total_books = {
    display_label = _("All time — Books"),
    value = function(s)
      return tostring(s.total_books)
    end,
    label = _("books finished"),
  },
  streak = {
    display_label = _("Streak"),
    value = function(s)
      return s.streak > 0 and tostring(s.streak) or "—"
    end,
    label_fn = function(s)
      return s.streak == 1 and _("day streak")
        or (s.streak == 0 and _("no streak") or _("days streak"))
    end,
  },
}
...
for _, sid in ipairs(STAT_POOL) do
  _sorted_pool[#_sorted_pool + 1] =
    { id = sid, label = STAT_MAP[sid].display_label }
end
table.sort(_sorted_pool, function(a, b)
...
local function _buildCardInner(
  stat_id,
  stats,
  d,
  align,
  ...
  local actual_max_w = max_w and math.max(1, max_w - Screen:scaleBySize(8))
    or nil
  return VerticalGroup:new({
    align = align,
    stat_id == "streak"
...
local function buildStatCardWidget(
  card_w,
  stat_id,
  stats,
  d,
  ...
  local update_fn = function(new_stats)
    cc[1] =
      _buildCardInner(stat_id, new_stats, d, align, clr_blk, clr_sub, card_w)
  end
  return fc, update_fn
end
...
local function buildStatListCell(
  cell_w,
  stat_id,
  stats,
  show_sep,
  ...
  local update_fn = function(new_stats)
    cc[1] =
      _buildCardInner(stat_id, new_stats, d, align, clr_blk, clr_sub, cell_w)
  end
  return og, update_fn
end
...
function M.getCountLabel(pfx)
  ...
  if rem <= 0 then
    return string.format(_("(%d/%d — at limit)"), n, max_rs)
  end
  return string.format(
    N_("(%d/%d — %d left)", "(%d/%d — %d left)", rem),
    n,
    max_rs,
    rem
  )
...
function M.build(w, ctx)
  if not M.isEnabled(ctx.pfx) then
    return nil
  end
  ...
  if #stat_ids == 0 then
    ...
    local ph_text = hold_on
        and _("No stats selected  —  long press to configure")
  ...
  if mode == "list" then
    ...
    for i = 1, n do
      local cell, update_fn =
        buildStatListCell(cell_w, stat_ids[i], stats, i < n, d, align, colors)
      if not cell then
        cell =
          OverlapGroup:new({ dimen = Geom:new({ w = cell_w, h = d.card_h }) })
      end
      if update_fn then
        table.insert(update_funcs, { fn = update_fn, id = stat_ids[i] })
      end
      row[#row + 1] = cell
    end
    ...
    for i = 1, n do
      ...
      if mode == "flat" then
        card, update_fn =
          buildStatFlatWidget(card_w, stat_ids[i], stats, d, align, colors)
      elseif mode == "cards_transparent" then
        card, update_fn = buildStatCardWidget(
          card_w,
          stat_ids[i],
          stats,
          d,
...
function M.updateStats(widget, ctx)
  if not widget or not widget._rs_update_funcs then
    return false
  end
  local sp = ctx.stats or {}
...
function M.getHeight(_ctx)
  local card_h = math.floor(
    _BASE_RS_CARD_H * Config.getModuleScale("reading_stats", _ctx and _ctx.pfx)
  )
  return card_h
end
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct("reading_stats", pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, "reading_stats", pfx)
    end,
...
function M.getMenuItems(ctx_menu)
    ...
    if not found then
      if #cur >= MAX_RS then
        _UIManager:show(InfoMessage:new({
          text = string.format(
            N_lc(
              "The maximum of %d stat per row has been reached. Remove one first.",
  ...
  local items = {
      ...
      callback = function()
        ...
        for _, id in ipairs(rs_ids) do
          sort_items[#sort_items + 1] =
            { text = M.getStatLabel(id), orig_item = id }
        end
                        ...
                        picker_items[#picker_items + 1] = {
                          ...
                          on_tap = function(picker_ctx)
                            ...
                            if #cur >= MAX_RS then
                              ...
                              local uim = ctx_menu.UIManager
                                or require("ui/uimanager")
                              local N_ = ctx_menu.N_
    ...
    Config.makeScaleItem({
      text_func = function()
        ...
        return pct == Config.RS_TEXT_SCALE_DEF and _lc("Text Size")
      ...
      get = function()
        return Config.getRSTextScalePct()
      end,
      set = function(pct)
        Config.setRSTextScalePct(pct)
      end,
      refresh = refresh,
    ...
    }),
    {
      ...
      sub_item_table = {
        ...
        },
        {
          ...
          keep_menu_open = true,
          checked_func = function()
            return getType(pfx) == "cards_transparent"
          end,
          callback = function()
    ...
    {
      text_func = function()
        return _lc("Alignment")
      end,
      value_func = function()
        return alignLabel(getAlign(pfx))
      end,
      sub_item_table = {
        {
          text = _lc("Left"),
  ...
  for _, entry in ipairs(_sorted_pool) do
    ...
    items[#items + 1] = {
      text_func = function()
        if isSelected(_sid) then
          return _lbl
        end
        local rem = MAX_RS - #getItems()
        if rem <= 2 then
          return _lbl
            .. string.format(N_lc("  (%d left)", "  (%d left)", rem), rem)
        end
        return _lbl
      end,
      checked_func = function()
        return isSelected(_sid)
      end,
      keep_menu_open = true,
      callback = function()
        toggleItem(_sid)
