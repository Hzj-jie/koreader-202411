...
local function getSH()
  if not _SH then
    ...
    if ok and m then
      ...
      logger.warn(
        "simpleui: module_tbr: cannot load module_books_shared: " .. tostring(m)
      )
    end
  end
  return _SH
...
local function _migrate()
  ...
  if RC.coll and RC.coll[TBR_COLL_NAME] and next(RC.coll[TBR_COLL_NAME]) then
    return
  end
  ...
  if added > 0 then
    ...
    logger.dbg(
      "simpleui: module_tbr: migrated",
      added,
      "entries to ReadCollection"
    )
  end
end
...
local function addTBR(filepath)
  if isTBR(filepath) then
    return true
  end
  local list = getTBRList()
  if #list >= TBR_MAX then
    return false
  end
  ...
  if RC then
    ...
    if real and lfs2.attributes(real, "mode") == "file" then
      ...
      for _, item in pairs(RC.coll[TBR_COLL_NAME]) do
        if (item.order or 0) > max_order then
  ...
  for i = 1, cols do
    ...
    if use_overlay then
      ...
      local badge = FrameContainer:new({
        ...
        CenterContainer:new({
          dimen = Geom:new({
            w = badge_d - 2 * border_sz,
            h = badge_d - 2 * border_sz,
          }),
          UI.makeColoredText({
    ...
    if draw_text then
      cell[#cell + 1] =
        SH.vspan(draw_progress and D.RB_GAP2 or D.RB_GAP1, ctx.vspan_pool)
    ...
    if ctx.kb_recent_focus_idx == i then
      ...
      cell_widget = OverlapGroup:new({
        ...
        tappable,
        LineWidget:new({
          dimen = Geom:new({ w = cw, h = bw }),
          background = Blitbuffer.COLOR_BLACK,
        }),
        LineWidget:new({
          dimen = Geom:new({ w = cw, h = bw }),
          background = Blitbuffer.COLOR_BLACK,
          overlap_offset = { 0, cell_h - bw },
        }),
        LineWidget:new({
          dimen = Geom:new({ w = bw, h = cell_h }),
          background = Blitbuffer.COLOR_BLACK,
        }),
        LineWidget:new({
          dimen = Geom:new({ w = bw, h = cell_h }),
          background = Blitbuffer.COLOR_BLACK,
          overlap_offset = { cw - bw, 0 },
        }),
      })
    end
...
function M.getHeight(_ctx)
  ...
  if
    SUISettings:isTrue(pfx .. "tbr_show_frame")
    or SUISettings:isTrue(pfx .. "tbr_solid_bg")
  then
    h = h + PAD * 2
  end
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct("tbr", pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, "tbr", pfx)
    end,
...
function M.getMenuItems(ctx_menu)
  ...
  items[#items + 1] = {
    ...
    sub_item_table_func = function()
      ...
      sub_items[#sub_items + 1] =
        { text = _lc("To Be Read Books"), enabled = false, separator = true }

      local list = getTBRList()
      if #list == 0 then
        sub_items[#sub_items + 1] =
          { text = _lc("No books in To Be Read list."), enabled = false }
      else
        for _, fp in ipairs(list) do
          local _fp = fp
        ...
        and function(ctx, _item)
          ...
          return SUIWindow.ListRow({
            ...
            subtitle = function()
              local list = getTBRList()
              if #list == 0 then
                return _lc("No books in To Be Read list.")
              end
              local names = {}
              for _, fp in ipairs(list) do
  ...
  items[#items + 1] = Config.makeScaleItem({
    ...
    get = function()
      return Config.getItemLabelScalePct("tbr", pfx)
    end,
    set = function(v)
      Config.setItemLabelScale(v, "tbr", pfx)
    end,
    refresh = refresh,
  })
  items[#items + 1] = Config.makeScaleItem({
    ...
    get = function()
      return Config.getThumbScalePct("tbr", pfx)
    end,
    set = function(v)
      Config.setThumbScale(v, "tbr", pfx)
    end,
    refresh = refresh,
  })

  items[#items + 1] =
    Config.makeLabelToggleItem("tbr", _("To Be Read"), refresh, _lc)
  items[#items + 1] = {
    text = _lc("Frame"),
    checked_func = function()
      return SUISettings:isTrue(pfx .. "tbr_show_frame")
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        pfx .. "tbr_show_frame",
        not SUISettings:isTrue(pfx .. "tbr_show_frame")
      )
      refresh()
    end,
  }
  items[#items + 1] = {
    ...
    checked_func = function()
      return SUISettings:isTrue(pfx .. "tbr_solid_bg")
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        pfx .. "tbr_solid_bg",
        not SUISettings:isTrue(pfx .. "tbr_solid_bg")
      )
      refresh()
    end,
  }
  items[#items + 1] = {
    ...
    checked_func = function()
      return showProgress(pfx)
    end,
    enabled_func = function()
      return not showOverlay(pfx)
    end,
    keep_menu_open = true,
    callback = function()
