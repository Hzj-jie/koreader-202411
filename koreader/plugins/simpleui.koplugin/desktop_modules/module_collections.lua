...
local function getCollectionFilesFromRC(rc, coll_name)
  ...
  for fp, info in pairs(coll) do
    entries[i] =
      { filepath = fp, order = (type(info) == "table" and info.order) or 9999 }
    i = i + 1
  end
...
local function buildCoverCell(
  ...
  if not front_fp and #files > 0 then
    front_fp = files[1]
  end
...
local function openCollection(coll_name)
  ...
  if not ok_fm or not FM or not FM.instance then
    return
  end
  ...
  elseif
    fm.collections and type(fm.collections.onShowCollList) == "function"
  then
    pcall(function()
...
function M.getCountLabel(_pfx)
  ...
  if rem <= 0 then
    return string.format(_("(%d/%d — at limit)"), n, MAX_COLL)
  end
...
function M.build(w, ctx)
  ...
  if _rc_sync and (_rc_sync.coll or _rc_sync.coll_folders) then
    local coll_set = {}
    if _rc_sync.coll then
      for n in pairs(_rc_sync.coll) do
        coll_set[n] = true
      end
    end
    if _rc_sync.coll_folders then
      for n in pairs(_rc_sync.coll_folders) do
        coll_set[n] = true
      end
    end
  ...
  if #selected == 0 then
    ...
    local ph_text = hold_on
        and _("No collections selected  —  long press to configure")
  ...
  for i = 1, cols do
    ...
    local count = #files
    local thumb, stack, front_fp = buildCoverCell(
      files,
      overrides[coll_name],
      coll_name,
      count,
    ...
    if ctx.has_wallpaper then
      ...
      if ok_tbx then
        ...
        logger.warn(
          "simpleui: module_collections: makeAlphaTextBox failed, falling back: "
            .. tostring(tbx)
        )
  ...
  local result = FrameContainer:new({
    ...
    padding_bottom = has_box and (PAD + PAD2) or 0,
    row,
  })
...
function M.updateCovers(widget, _ctx)
  if not widget or not widget._cover_slots then
    return true
  end
  local all_done = true
...
function M.getHeight(ctx)
  local d = getDims(
    Config.getModuleScale("collections", _ctx and _ctx.pfx),
  ...
  if
    SUISettings:isTrue(_ctx and _ctx.pfx .. "collections_show_frame")
    or SUISettings:isTrue(_ctx and _ctx.pfx .. "collections_solid_bg")
  then
    h = h + PAD * 2 + PAD2
  end
...
function M.saveSelected(list)
  saveSelectedCollections(list)
end
function M.getCoverOverrides()
  return getCoverOverrides()
end
function M.saveCoverOverrides(t)
...
function M.saveBadgePosition(v)
  saveBadgePosition(v)
end
function M.getBadgeColor()
  return getBadgeColor()
end
function M.saveBadgeColor(v)
  saveBadgeColor(v)
end
function M.getBadgeHidden()
  return getBadgeHidden()
end
function M.saveBadgeHidden(v)
  saveBadgeHidden(v)
end
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct("collections", pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, "collections", pfx)
    end,
...
local function _makeItemLabelScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getItemLabelScalePct("collections", pfx)
    end,
    set = function(v)
      Config.setItemLabelScale(v, "collections", pfx)
    end,
    refresh = ctx_menu.refresh,
  })
end
function M.getMenuItems(ctx_menu)
  ...
  if ok_rc and rc then
    ...
    local coll_set = {}
    if rc.coll then
      for n in pairs(rc.coll) do
        coll_set[n] = true
      end
    end
    if rc.coll_folders then
      for n in pairs(rc.coll_folders) do
        coll_set[n] = true
      end
    end
  ...
  local function openCoverPicker(coll_name)
    ...
    if not coll then
      _UIManager:show(
        InfoMessage:new({ text = _lc("Collection is empty."), timeout = 2 })
      )
      return
    end
    local fps = {}
    for fp in pairs(coll) do
      fps[#fps + 1] = fp
    end
    table.sort(fps)
    if #fps == 0 then
    ...
    cover_buttons[#cover_buttons + 1] = {
      {
        text = (not overrides[_n] and "✓ " or "  ")
          .. _lc("Auto (first book)"),
        callback = function()
          _UIManager:close(ctx_menu._cover_picker)
          M.clearCoverOverride(_n)
          refresh()
        end,
      },
    }
    ...
    callback = function()
      ...
      if #cur_sel < 2 then
        _UIManager:show(InfoMessage:new({
          text = _lc("Select at least 2 collections to arrange."),
          timeout = 2,
        }))
        return
      end
          ...
          return SUIWindow.ListRow({
            ...
            subtitle = function()
              ...
              for _, n in ipairs(cur_sel) do
                names[#names + 1] = (TBR and n == TBR.TBR_COLL_NAME)
                    and TBR.getDisplayName()
                  or n
              end
            ...
            on_tap = function()
              ...
              for _, n in ipairs(cur_sel) do
                ...
                local TBR = package.loaded["desktop_modules/module_tbr"]
                if TBR and n == TBR.TBR_COLL_NAME then
                  _display_n = TBR.getDisplayName()
                end
                sort_items[#sort_items + 1] =
                  { text = _display_n, orig_item = n }
              end
                        ...
                        on_tap = function(picker_ctx)
                          ...
                          if #new_sel >= MAX_COLL then
                            ...
                            uim:show(InfoMessage:new({
                              text = _lc(
                                "Maximum 5 collections. Remove one first."
                              ),
                              timeout = 2,
                            }))
                            return
  ...
  items[#items + 1] = Config.makeScaleItem({
    ...
    get = function()
      return Config.getThumbScalePct("collections", ctx_menu.pfx)
    end,
    set = function(v)
      Config.setThumbScale(v, "collections", ctx_menu.pfx)
    end,
    refresh = ctx_menu.refresh,
  })

  items[#items + 1] =
    Config.makeLabelToggleItem("collections", _("Collections"), refresh, _lc)
  items[#items + 1] = {
    text = _lc("Frame"),
    checked_func = function()
      return SUISettings:isTrue(ctx_menu.pfx .. "collections_show_frame")
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        ctx_menu.pfx .. "collections_show_frame",
        not SUISettings:isTrue(ctx_menu.pfx .. "collections_show_frame")
      )
      refresh()
    end,
  }
  items[#items + 1] = {
    ...
    checked_func = function()
      return SUISettings:isTrue(ctx_menu.pfx .. "collections_solid_bg")
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:saveSetting(
        ctx_menu.pfx .. "collections_solid_bg",
        not SUISettings:isTrue(ctx_menu.pfx .. "collections_solid_bg")
      )
      refresh()
    end,
  }
  items[#items + 1] = {
    ...
    sub_item_table = {
      ...
      },
      {
        ...
        radio = true,
        checked_func = function()
          return not getBadgeHidden() and getBadgePosition() == "top"
        end,
        enabled_func = function()
          return not getBadgeHidden()
        end,
        keep_menu_open = true,
        callback = function()
          saveBadgePosition("top")
          refresh()
        end,
      },
      {
        ...
        radio = true,
        checked_func = function()
          return not getBadgeHidden() and getBadgePosition() == "bottom"
        end,
        enabled_func = function()
          return not getBadgeHidden()
        end,
      ...
      },
      {
        ...
        radio = true,
        checked_func = function()
          return getBadgeColor() == "light"
        end,
        keep_menu_open = true,
        callback = function()
          saveBadgeColor("light")
          refresh()
        end,
      },
    },
      ...
      items[#items + 1] = {
        ...
        callback = function()
          ...
          if not found then
            if #cur >= 5 then
              _UIManager:show(InfoMessage:new({
                text = _lc("Maximum 5 collections. Remove one first."),
                timeout = 2,
              }))
              return
            end
