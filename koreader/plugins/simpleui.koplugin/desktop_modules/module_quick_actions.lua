...
local function buildQAWidget(
  ...
  local ph_fs = math.max(
    8,
    math.floor(
      _BASE_PH_FS * (d.frame_sz / (_BASE_ICON_SZ + _BASE_FRAME_PAD * 2))
    )
  )
  local function _placeholder()
    local hold_on = SUISettings:nilOrTrue("simpleui_hs_settings_on_hold")
    local ph_text = hold_on
        and _("No actions configured  —  long press to configure")
  ...
  for i = 1, n do
    ...
    local corner_r = is_bare and 0
      or ((shape == "round") and math.floor(d.frame_sz / 2) or d.corner_r)
    local current_border = (
      not is_bare and (bg == "solid" or bg == "transparent")
    )
        and SUIStyle.BORDER_SZ
      or 0
    local bg_color = nil
    if not is_bare then
      if bg == "flat" then
        bg_color = _CLR_FLAT_BG
    ...
    function tappable:onTapQA()
      if self._on_tap_fn then
        self._on_tap_fn(self._action_id)
      end
      return true
    end
...
local function makeInstance(inst_id)
  ...
  local function getQAPool()
    ...
    for desc in QA.iterBuiltin() do
      if actionAvailable(desc.id) then
        available[#available + 1] = {
          id = desc.id,
          label = desc.id == "home" and Config.homeLabel()
            or QA.getEntry(desc.id).label,
        }
      end
    end
    for _, qa_id in ipairs(Config.getCustomQAList()) do
      local _qid = qa_id
      available[#available + 1] =
        { id = _qid, label = Config.getCustomQAConfig(_qid).label }
    end
    return available
  end
    ...
    items_sub[#items_sub + 1] = {
      ...
      callback = function()
        ...
        local SortWidget = ctx_menu.SortWidget
          or require("ui/widget/sortwidget")
        local uim = ctx_menu.UIManager or UIManager
  ...
  function S.getCountLabel(pfx)
    ...
    if rem <= 0 then
      return string.format(_("(%d/%d — at limit)"), n, MAX_QA)
    end
  ...
  function S.build(w, ctx)
    if not S.isEnabled(ctx.pfx) then
      return nil
    end
  ...
  function S.getMenuItems(ctx_menu)
    ...
    local items = {}
    local fn = (type(ctx_menu.makeQAMenu) == "function") and ctx_menu.makeQAMenu
      or makeQAMenuFallback
    local qa = fn(ctx_menu, inst_id) or {}

    local items_node = nil
    local hide_text_node = nil
    for _, v in ipairs(qa) do
      if v.text == _lc("Quick Actions") then
        items_node = v
      end
      if v.text == _lc("Hide Label") then
        hide_text_node = v
      end
    end
    if items_node then
      items[#items + 1] = items_node
    end
    ...
    items[#items + 1] = Config.makeScaleItem({
      ...
      get = function()
        return Config.getModuleScalePct(S.id, pfx)
      end,
      set = function(v)
        Config.setModuleScale(v, S.id, pfx)
      end,
      refresh = refresh,
    })
    ...
    items[#items + 1] = {
      ...
      sub_item_table = {
        Config.makeScaleItem({
          ...
          get = function()
            return Config.getItemLabelScalePct(S.id, pfx)
          end,
          set = function(v)
            Config.setItemLabelScale(v, S.id, pfx)
          end,
          refresh = refresh,
        }),
        hide_text_node,
    ...
    items[#items + 1] = {
      ...
      sub_item_table = {
        ...
        },
        {
          ...
          radio = true,
          checked_func = function()
            return getShape(pfx) == "rounded_square"
          end,
          keep_menu_open = true,
          callback = function()
