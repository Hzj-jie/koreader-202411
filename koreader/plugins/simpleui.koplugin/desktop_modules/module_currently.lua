...
local function getSH()
  if not _SH then
    ...
    if ok and m then
      ...
      logger.warn(
        "simpleui: module_currently: cannot load module_books_shared: "
          .. tostring(m)
      )
    end
  end
...
local function buildProgressBarWithPct(
  w,
  pct,
  bar_h,
  scale,
...
local function fetchBookStats(md5, shared_conn, ctx, force)
  ...
  if not ok then
    logger.warn(
      "simpleui: module_currently: fetchBookStats failed: " .. tostring(err)
    )
...
local function _computeContentH(params)
  ...
  local title_line_h =
    math.max(8, math.floor(_BASE_TITLE_FS * scale * lbl_scale))
  local author_line_h =
    math.max(8, math.floor(_BASE_AUTHOR_FS * scale * lbl_scale))
  local pct_line_h = math.max(8, math.floor(_BASE_PCT_FS * scale * lbl_scale))
  local stats_line_h =
    math.max(7, math.floor(_BASE_STATS_FS * scale * lbl_scale))
  ...
  local function statsHasData(key)
    ...
    if key == "days" then
      return show.days and bstats.days and bstats.days > 0
    end
    if key == "time" then
      return show.time and bstats.total_secs and bstats.total_secs > 0
    end
    if key == "remain" then
      local avg_t = (bstats.avg_time and bstats.avg_time > 0)
          and bstats.avg_time
        or bd.avg_time
      return show.remain and avg_t and avg_t > 0 and bd.pages and bd.pages > 0
    end
    return false
  end
  ...
  if stats_style == "compact" then
    if
      statsHasData("days")
      or statsHasData("time")
      or statsHasData("remain")
    then
      elems[#elems + 1] = { pct_gap, stats_line_h }
    end
  else
    if statsHasData("days") then
      elems[#elems + 1] = { pct_gap, stats_line_h }
    end
    if statsHasData("time") then
      elems[#elems + 1] = { pct_gap, stats_line_h }
    end
    if statsHasData("remain") then
      elems[#elems + 1] = { pct_gap, stats_line_h }
    end
  end
  ...
  for i, e in ipairs(elems) do
    if i > 1 then
      h = h + e[1]
    end -- gap (skipped for first element)
    h = h + e[2] -- line height
  end

  local content_h = math.max(D.COVER_H, h)
  if
    SUISettings:isTrue(params.pfx .. "currently_show_frame")
    or SUISettings:isTrue(params.pfx .. "currently_solid_bg")
  then
...
function M.build(w, ctx)
  ...
  local thumb_scale = c and c.thumb_scale
    or Config.getThumbScale("currently", pfx)
  local lbl_scale = c and c.lbl_scale
    or Config.getItemLabelScale("currently", pfx)
  local bar_style = c and c.bar_style or getBarStyle(pfx)
  local stats_style = c and c.stats_style or getStatsStyle(pfx)
  local show = c and c.show
    or {
  ...
  local elem_order = _resolveElemOrder(
    c and c.elem_order or SUISettings:readSetting(pfx .. ELEM_ORDER_KEY)
  )

  local D = SH.getDims(scale, thumb_scale)

  -- Scale gaps (layout scale only).
  local cover_gap = math.max(
    0,
    math.floor(_BASE_COVER_GAP * scale * (getCoverGapPct(pfx) / 100))
  )
  local title_gap = math.max(1, math.floor(_BASE_TITLE_GAP * scale))
  local author_gap = math.max(1, math.floor(_BASE_AUTHOR_GAP * scale))
  ...
  if show.days or show.time or show.remain then
    ...
    if not book_md5 then
      logger.dbg(
        "simpleui: module_currently: no md5 for "
  ...
  local face_inlinepct = Font:getFace(
    SUIStyle.FACE_REGULAR,
    math.max(7, math.floor(_BASE_INLINEPCT_FS * scale * lbl_scale))
  )
  ...
  for _i, elem in ipairs(elem_order) do
    if elem == "title" and show.title then
      ...
      if ctx.has_wallpaper then
        ...
        if ok_tbx then
          ...
          logger.warn(
            "simpleui: module_currently: makeAlphaTextBox failed, falling back to TextBoxWidget: "
              .. tostring(tbx)
          )
    ...
    elseif elem == "progress" and show.progress then
      ...
      if bar_style == "with_pct" then
        ...
        local _init_bar = buildProgressBarWithPct(
          _bar_w,
          bd.percent,
          _bar_h,
          _bar_sc,
        ...
        local function _update_bar(nb, nd)
          bar_container[1] =
            UI.progressBar(_bar_w, (nd and nd.percent or 0), _bar_h)
        end
        table.insert(_cr_bd_only_funcs, _update_bar)
        meta[#meta + 1] = bar_container
      end
    ...
    elseif elem == "percent" and show.percent and bar_style ~= "with_pct" then
      ...
      local pct_w = UI.makeColoredText({
        text = string.format(
          _("%d%% Read"),
          math.floor((bd.percent or 0) * 100 + 0.5)
        ),
        face = face_pct,
        bold = true,
        fgcolor = _CLR_DARK_EFF,
      ...
      local function _update_pct(nb, nd)
        _updateColoredText(
          ...
          string.format(
            _("%d%% Read"),
            math.floor((nd and nd.percent or 0) * 100 + 0.5)
          ),
          _pct_fg
        )
      end
    ...
    elseif elem == "book_days" and show.days and stats_style == "default" then
      ...
      gap_before(pct_gap)
      local days_w = UI.makeColoredText({
        text = "",
        face = face_s,
        fgcolor = CLR_PH_EFF,
        width = tw,
      ...
      local function _update(nb, nd)
        ...
        local days_lbl = has_d
            and string.format(
              N_("%d day of reading", "%d days of reading", nb.days),
              nb.days
            )
        ...
        _updateColoredText(
          days_w,
          days_lbl,
          has_d and CLR_TEXT_SUB_EFF or CLR_PH_EFF
        )
      end
      _update(bstats, bd)
    ...
    elseif elem == "book_time" and show.time and stats_style == "default" then
      ...
      gap_before(pct_gap)
      local time_w = UI.makeColoredText({
        text = "",
        face = face_s,
        fgcolor = CLR_PH_EFF,
        width = tw,
    ...
    then
      ...
      if pct_done < 1.0 then
        gap_before(pct_gap)
        local remain_w = UI.makeColoredText({
          text = "",
          face = face_s,
          fgcolor = CLR_PH_EFF,
          width = tw,
        ...
        local function _update(nb, nd)
          local avg_t
          if nb and nb.avg_time and nb.avg_time > 0 then
            avg_t = nb.avg_time
          elseif nd.avg_time and nd.avg_time > 0 then
            avg_t = nd.avg_time
          end
          ...
          if not avg_t or not nd.pages or nd.pages <= 0 then
            ...
            if secs_left > 0 then
              _updateColoredText(
                remain_w,
                string.format(_("%s remaining"), fmtTime(secs_left)),
                CLR_TEXT_SUB_EFF
              )
            else
              _updateColoredText(remain_w, "", CLR_PH_EFF)
            end
          end
        end
        _update(bstats, bd)
    ...
    elseif
      (elem == "book_days" or elem == "book_time" or elem == "book_remaining")
      and stats_style == "compact"
    then
      ...
      if not _compact_stats_rendered then
        ...
        local function _update(nb, nd)
          local secs_left
          local avg_t = (nb and nb.avg_time and nb.avg_time > 0) and nb.avg_time
            or nd.avg_time
          ...
          for _i, e in ipairs(elem_order) do
            ...
            elseif e == "book_remaining" and show.remain and secs_left then
              parts[#parts + 1] = {
                text = string.format(_("%s left"), fmtTime(secs_left)),
                placeholder = false,
              }
          ...
          if #parts == 0 then
            ...
            if any_active then
              parts[#parts + 1] = {
                text = string.format(_("%s read"), "—"),
                placeholder = true,
              }
            end
          end

          for i = #stats_row, 1, -1 do
            stats_row[i] = nil
          end
  ...
  if ctx.kb_currently_focused then
    ...
    return OverlapGroup:new({
      ...
      LineWidget:new({
        ...
        background = _CLR_DARK_EFF,
      }),
      LineWidget:new({
        dimen = Geom:new({ w = tw, h = bw }),
        background = _CLR_DARK_EFF,
        overlap_offset = { 0, th - bw },
      }),
      LineWidget:new({
...
function M.updateCovers(widget, _ctx)
  ...
  if not tappable or not tappable._cover_slots then
    return true
  end
  ...
  for _, slot in ipairs(tappable._cover_slots) do
    local new_cover =
      SH.getBookCover(slot.fp, slot.w, slot.h, slot.align, slot.stretch)
    if new_cover then
...
function M.getHeight(_ctx)
  ...
  local n_stats = (show.days and 1 or 0)
...
local function _makeScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getModuleScalePct("currently", pfx)
    end,
    set = function(v)
      Config.setModuleScale(v, "currently", pfx)
    end,
...
local function _makeThumbScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getThumbScalePct("currently", pfx)
    end,
    set = function(v)
      Config.setThumbScale(v, "currently", pfx)
    end,
    refresh = ctx_menu.refresh,
  })
end
...
local function _makeTextScaleItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return Config.getItemLabelScalePct("currently", pfx)
    end,
    set = function(v)
      Config.setItemLabelScale(v, "currently", pfx)
    end,
    refresh = ctx_menu.refresh,
  })
end
...
local function _makeCoverGapItem(ctx_menu)
  ...
  return Config.makeScaleItem({
    ...
    get = function()
      return getCoverGapPct(pfx)
    end,
    set = function(v)
      SUISettings:saveSetting(pfx .. COVER_GAP_KEY, v)
    end,
    refresh = ctx_menu.refresh,
    value_min = 0,
    value_max = 300,
...
function M.getMenuItems(ctx_menu)
  ...
  local items_submenu = {
    -- Arrange Items: drag-to-reorder the visible elements. Disabled when fewer than 2 are active.
    {
      ...
      callback = function()
        ...
        local function on_save()
          ...
          local active_set = {}
          for _, k in ipairs(new_order) do
            active_set[k] = true
          end
    ...
    {
      text_func = function()
        return _lc("Percentage read")
      end,
      -- Greyed out when with_pct bar style is active (percentage is already in the bar).
      enabled_func = function()
        return getBarStyle(pfx) == "simple"
      end,
      checked_func = function()
        return _showElem(pfx, "percent")
      end,
      sui_hidden = function()
        return getBarStyle(pfx) == "with_pct"
      end,
      keep_menu_open = true,
      callback = function()
  ...
  return {
                  ...
                  elseif _showElem(pfx, key) then
                    if
                      ...
                      and (
                        key == "book_days"
                        or key == "book_time"
                        or key == "book_remaining"
                      )
                    then
                      if not stats_added then
                        names[#names + 1] = _lc("Stats")
                        stats_added = true
                      end
                    else
                    ...
                    return {
                      {
                        ...
                        sui_build = function(ctx2)
                          ...
                          local function make_sort_items()
                            ...
                            for _, key in ipairs(_getElemOrder(pfx)) do
                              if
                                key == "percent" and bar_style == "with_pct"
                              then
                                -- skip
                              elseif _showElem(pfx, key) then
                                if
                          ...
                          for i, item in ipairs(sort_items) do
                            ...
                            cards[#cards + 1] = SUIWindow.ArrangeCard({
                                        ...
                                        return {
                                          {
                                            ...
                                            sui_build = function(ctx3)
                                              ...
                                              local function save_sub_order(
                                                new_sub
                                              )
                                                local n_ord = {}
                                                local s_idx = 1
                                                local active_stats = {}
                                                for _, it in ipairs(new_sub) do
                                                  active_stats[it.orig_item] =
                                                    true
                                                end
                              ...
                              on_move_up = (_i > 1)
                                  and function()
                                    sort_items[_i], sort_items[_i - 1] =
                                      sort_items[_i - 1], sort_items[_i]
                                    save_order(sort_items)
                                    refresh()
                                    ctx2.repaint()
                                  end
                                or nil,
                              on_move_down = (_i < #sort_items)
                                  and function()
                                    sort_items[_i], sort_items[_i + 1] =
                                      sort_items[_i + 1], sort_items[_i]
                                    save_order(sort_items)
    ...
    ),
    {
      ...
      callback = function()
        SUISettings:saveSetting(
          pfx .. "currently_show_frame",
          not SUISettings:isTrue(pfx .. "currently_show_frame")
        )
        refresh()
      end,
    },
    {
      ...
      callback = function()
        SUISettings:saveSetting(
          pfx .. "currently_solid_bg",
          not SUISettings:isTrue(pfx .. "currently_solid_bg")
        )
        refresh()
      end,
    },
      ...
      sub_item_table = {
        {
          ...
          keep_menu_open = true,
          checked_func = function()
            return getStatsStyle(pfx) == "default"
          end,
          callback = function()
        ...
        },
        {
          ...
          keep_menu_open = true,
          checked_func = function()
            return getStatsStyle(pfx) == "compact"
          end,
          callback = function()
...
function M.updateStats(widget, ctx)
  ...
  if not actual_widget or not actual_widget._cr_update_funcs then
    return false
  end
