...
local function _wpStretch()
  return SUISettings:isTrue("simpleui_style_wallpaper_stretch")
end
local function _wpAutoRotate()
  return SUISettings:nilOrTrue("simpleui_style_wallpaper_autorotate")
end
local function _wpInvertNight()
  return SUISettings:isTrue("simpleui_style_wallpaper_invert_night")
end
local function _wpOpacity()
  return SUISettings:readSetting("simpleui_style_wallpaper_opacity", 0)
end
...
local function _inZone(z, x, y, sw, sh)
  ...
  return x >= z.ratio_x * sw
    and x < (z.ratio_x + z.ratio_w) * sw
    and y >= z.ratio_y * sh
    and y < (z.ratio_y + z.ratio_h) * sh
end
...
local function _styleGetBgWidget()
  if not SUISettings:isTrue("simpleui_style_wallpaper_enabled") then
    return nil
  end
  ...
  if _style_bg_cache then
    _style_bg_cache:free()
  end
  if _style_bg_cache_bb then
    _style_bg_cache_bb:free()
  end
  _style_bg_cache = nil
  ...
  if not img_w or not img_h then
    ...
    if ok_ri and RenderImage then
      local ok_bb, bb =
        pcall(RenderImage.renderImageFile, RenderImage, path, false, nil, nil)
  ...
  if _wpAutoRotate() and img_w and img_h and img_w > 0 and img_h > 0 then
    ...
    if img_landscape ~= screen_landscape then
      rotation_angle = G_reader_settings:isTrue(
  ...
  then
    ...
    if ok_ri and RenderImage then
      ...
      if ok_bb and raw_bb then
        local ok_sc, scaled = pcall(function()
          return raw_bb:scale(eff_w, eff_h)
        end)
        raw_bb:free()
        raw_bb = nil
...
local _face_empty_sub =
  Font:getFace(SUIStyle.FACE_REGULAR, SUIStyle.FS_SUBTITLE) -- FS_SUBTITLE (20)
...
local function buildChevronFooter(goto_fn)
  ...
  pcall(function()
    ...
    if not (ok_ss and SS and SS.applyPaginationIcons) then
      return
    end
...
local function _updateNavpagerForHS(current_page, total_pages)
  if not Config.isNavpagerEnabled() then
    return
  end
...
local function _normalizeKoboPath(filepath)
  ...
  if not kobo or not kobo.virtual_library then
    return filepath
  end
  ...
  if vl:isVirtualPath(filepath) then
    return filepath
  end
  ...
  local doOpen = function()
    ...
    if pos0 or page then
      UIManager:scheduleIn(0.5, function()
        ...
        elseif page then
          rui.instance:handleEvent(require("ui/event"):new("GotoPage", page))
        end
      end)
    end
  end
...
function HomescreenWidget:init()
    ...
    if gt == "swipe" then
      ...
      if is_diag then
        ...
        if ges_event.distance and ges_event.distance <= short_thresh then
          n = n + 1
          _candidates[n] = "short_diagonal_swipe"
        end
      elseif _inZone(_gz_left_edge, x, y, sw, sh) then
        if dir == "south" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_left_edge_down"
        elseif dir == "north" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_left_edge_up"
        end
      elseif _inZone(_gz_right_edge, x, y, sw, sh) then
        if dir == "south" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_right_edge_down"
        elseif dir == "north" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_right_edge_up"
        end
      elseif _inZone(_gz_top_edge, x, y, sw, sh) then
        if dir == "east" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_top_edge_right"
        elseif dir == "west" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_top_edge_left"
        end
      elseif _inZone(_gz_bot_edge, x, y, sw, sh) then
        if dir == "east" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_bottom_edge_right"
        elseif dir == "west" then
          n = n + 1
          _candidates[n] = "one_finger_swipe_bottom_edge_left"
        end
      end
    elseif gt == "tap" then
      if _inZone(_gz_top_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "tap_top_left_corner"
      elseif _inZone(_gz_top_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "tap_top_right_corner"
      elseif _inZone(_gz_bot_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "tap_left_bottom_corner"
      elseif _inZone(_gz_bot_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "tap_right_bottom_corner"
      end
    elseif gt == "hold" then
      if _inZone(_gz_top_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "hold_top_left_corner"
      elseif _inZone(_gz_top_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "hold_top_right_corner"
      elseif _inZone(_gz_bot_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "hold_bottom_left_corner"
      elseif _inZone(_gz_bot_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "hold_bottom_right_corner"
      end
    elseif gt == "double_tap" then
      if _inZone(_gz_left_side, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "double_tap_left_side"
      elseif _inZone(_gz_right_side, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "double_tap_right_side"
      elseif _inZone(_gz_top_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "double_tap_top_left_corner"
      elseif _inZone(_gz_top_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "double_tap_top_right_corner"
      elseif _inZone(_gz_bot_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "double_tap_bottom_left_corner"
      elseif _inZone(_gz_bot_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "double_tap_bottom_right_corner"
      end
    elseif gt == "two_finger_tap" then
      if _inZone(_gz_top_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "two_finger_tap_top_left_corner"
      elseif _inZone(_gz_top_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "two_finger_tap_top_right_corner"
      elseif _inZone(_gz_bot_left, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "two_finger_tap_bottom_left_corner"
      elseif _inZone(_gz_bot_right, x, y, sw, sh) then
        n = n + 1
        _candidates[n] = "two_finger_tap_bottom_right_corner"
      end
    elseif gt == "two_finger_swipe" then
      local map = {
        east = "two_finger_swipe_east",
        west = "two_finger_swipe_west",
        north = "two_finger_swipe_north",
        south = "two_finger_swipe_south",
        northeast = "two_finger_swipe_northeast",
        northwest = "two_finger_swipe_northwest",
        southeast = "two_finger_swipe_southeast",
        southwest = "two_finger_swipe_southwest",
      }
      if map[dir] then
        n = n + 1
        _candidates[n] = map[dir]
      end
    elseif gt == "multiswipe" then
      local orig_sendEvent = UIManager.sendEvent
      UIManager.sendEvent = function(um, ev)
        return UIManager:broadcastEvent(ev)
      end
      local ok, err =
        pcall(g.multiswipeAction, g, ges_event.multiswipe_directions, ges_event)
      UIManager.sendEvent = orig_sendEvent
      if not ok then
        logger.warn("simpleui hs gesture multiswipe:", err)
      end
      return true
    elseif gt == "spread" then
      n = n + 1
      _candidates[n] = "spread_gesture"
    elseif gt == "pinch" then
      n = n + 1
      _candidates[n] = "pinch_gesture"
    elseif gt == "rotate" then
      if dir == "cw" then
        n = n + 1
        _candidates[n] = "rotate_cw"
      elseif dir == "ccw" then
        n = n + 1
        _candidates[n] = "rotate_ccw"
      end
    end
    ...
    if ges then
      ...
      if (dir == "west" or dir == "east") and not _isSideEdge(ges) then
        ...
        if ges.pos then
          ...
          do
            local pom = self._enabled_mods_cache
              and self._enabled_mods_cache.pages_of_mods
            ...
            for _, cur_mods in ipairs(pages_to_check) do
              if cur_mods then
                for _, m in ipairs(cur_mods) do
                  if m.id == "coverdeck" then
                    cd_on_current_page = true
                    break
                  end
                end
              end
              if cd_on_current_page then
                break
              end
            end
          end
          local cd_wrapper = cd_on_current_page
            and self._wrapper_pool
            and self._wrapper_pool["coverdeck"]
          if
            cd_wrapper
            and cd_wrapper.dimen
            and ges.pos:intersectWith(cd_wrapper.dimen)
          then
  ...
  function self:onHSTwoFingerSwipe(_args, ges)
    return _fmGestureAction(ges)
  end
  function self:onHSDoubleTap(_args, ges)
    return _fmGestureAction(ges)
  end
  function self:onHSTwoFingerTap(_args, ges)
    return _fmGestureAction(ges)
  end
  function self:onHSMultiswipe(_args, ges)
    return _fmGestureAction(ges)
  end
  function self:onHSSpread(_args, ges)
    return _fmGestureAction(ges)
  end
  function self:onHSPinch(_args, ges)
    return _fmGestureAction(ges)
  end
  function self:onHSRotate(_args, ges)
    return _fmGestureAction(ges)
  end
  ...
  function self:onHSFocusLeft()
    local frec = self._kb_first_rec_idx
    if not frec or not self._kb_focus_idx then
      return
    end
  ...
  function self:onHSFocusRight()
    local frec = self._kb_first_rec_idx
    local books = self._kb_book_items_fp
    if not frec or not self._kb_focus_idx or not books then
      return
    end
    if self._kb_focus_idx < frec then
      return
  ...
  function self:onBlockNavbarTap(_args, ges)
    if _hasModalOnTop(self) then
      return false
    end
    if _fmGestureAction(ges) then
      return true
    end
    ...
    self:registerTouchZones({
      ...
      },
      {
        ...
        handler = function(ges)
          ...
          local m = _hsMenu()
          if m and m:onSwipeShowMenu(ges) then
            return true
          end
          return _fmGestureAction(ges)
        end,
  ...
  self:registerTouchZones({
    {
      ...
      screen_zone = {
        ratio_x = 0,
        ratio_y = footer_ratio_y,
        ratio_w = 1,
        ratio_h = footer_ratio_h,
      },
      overrides = { "BlockNavbarTap" },
      handler = function(ges)
        if _hasModalOnTop(self_ref_fc) then
          return false
        end
        if _fmGestureAction(ges) then
          return true
        ...
        if not footer_bc or footer_bc.dimen.h == 0 then
          return false
        end
        ...
        if fc then
          local buttons =
            { fc.btn_first, fc.btn_prev, fc.btn_next, fc.btn_last }
  ...
  local _gesture_types = {
    {
      ges = "double_tap",
      id_suffix = "double_tap",
      override = "HSDoubleTap",
    },
    {
      ges = "two_finger_tap",
      id_suffix = "two_finger_tap",
      override = "HSTwoFingerTap",
    },
    {
      ges = "two_finger_swipe",
      id_suffix = "two_finger_swipe",
      override = "HSTwoFingerSwipe",
    },
    {
      ges = "multiswipe",
      id_suffix = "multiswipe",
      override = "HSMultiswipe",
    },
    {
      ges = "spread",
      id_suffix = "spread",
  ...
  for _, gt in ipairs(_gesture_types) do
    priority_zones[#priority_zones + 1] = {
      ...
      handler = function(ges)
        return _hasModalOnTop(self) and false or _fmGestureAction(ges)
      end,
    }
    priority_zones[#priority_zones + 1] = {
...
function HomescreenWidget:_buildCtx()
  ...
  if not cfg then
    cfg = {
      currently = {
        ...
        bar_style = SUISettings:readSetting(PFX .. "currently_bar_style")
          or "with_pct",
        ...
        show = {
          ...
          remain = SUISettings:nilOrTrue(
            PFX .. "currently_show_book_remaining"
          ),
        },
      },
      coverdeck = {
        ...
        title_pos = SUISettings:readSetting(PFX .. "coverdeck_title_pos")
          or "below",
        show_finished = SUISettings:readSetting(
          PFX .. "coverdeck_show_finished"
        ) == true,
        show = {
          ...
          book_remaining = SUISettings:nilOrTrue(
  ...
  if not self._cached_books_state then
    ...
    if SH then
      if show_c or show_r then
        ...
        self._cached_books_state =
          { current_fp = nil, recent_fps = {}, prefetched_data = {} }
      end
    else
  ...
  for _, mod in ipairs(Registry.list()) do
    if mod.needs and Registry.isEnabled(mod, PFX) then
      if mod.needs.db then
        ext_needs_db = true
      end
      if mod.needs.stats then
        ext_needs_stats = true
      end
      if mod.needs.books then
        ext_needs_books = true
      end
    end
  end
  if ext_needs_stats then
    wants_stats = true
  end
  ...
  local coverdeck_needs_db = mod_cd
    and Registry.isEnabled(mod_cd, PFX)
    and (
      (
        cd_cfg
        and cd_cfg.show
        and (
          cd_cfg.show.book_days
          or cd_cfg.show.book_time
          or cd_cfg.show.book_remaining
        )
      )
      or (
        not (cd_cfg and cd_cfg.show)
        and (
          SUISettings:nilOrTrue(PFX .. "coverdeck_show_book_days")
          or SUISettings:nilOrTrue(PFX .. "coverdeck_show_book_time")
          or SUISettings:nilOrTrue(PFX .. "coverdeck_show_book_remaining")
        )
      )
    )
  ...
  if wants_stats then
    ...
    if SP then
      ...
      if stats_data and stats_data.db_conn_fatal then
        logger.warn(
          "simpleui: homescreen: StatsProvider reported fatal DB error — dropping connection"
        )
  ...
  if coverdeck_needs_db and self._db_conn then
    ...
    local pe = center_fp
      and bs.prefetched_data
      and bs.prefetched_data[center_fp]
    ...
    if center_md5 then
      ...
      if cd_mod and cd_mod.fetchBookStatsForCtx then
        coverdeck_center_stats = {
          fp = center_fp,
          stats = cd_mod.fetchBookStatsForCtx(
            center_md5,
            self._db_conn,
            not self._defer_stats
          ),
        }
      end
    end
  ...
  if
    mod_c
    and Registry.isEnabled(mod_c, PFX)
    and self._db_conn
    and bs.current_fp
  then
    local c_cfg = cfg and cfg.currently
    local needs_bstats = (
      c_cfg and (c_cfg.show.days or c_cfg.show.time or c_cfg.show.remain)
    )
      or (
        not c_cfg
        and (
          SUISettings:nilOrTrue(PFX .. "currently_show_book_days")
          or SUISettings:nilOrTrue(PFX .. "currently_show_book_time")
          or SUISettings:nilOrTrue(PFX .. "currently_show_book_remaining")
        )
      )
      ...
      if c_md5 then
        ...
        if mc_ok and mc_mod and mc_mod.fetchBookStatsForCtx then
          currently_book_stats = {
            fp = bs.current_fp,
            stats = mc_mod.fetchBookStatsForCtx(
              c_md5,
              self._db_conn,
              not self._defer_stats
            ),
  ...
  return {
    ...
    open_fn = function(fp, pos0, page)
      openBook(fp, pos0, page)
    end,
    on_qa_tap = function(aid)
      if self_ref._on_qa_tap then
        self_ref._on_qa_tap(aid)
      end
    end,
    on_goal_tap = function()
      if self_ref._on_goal_tap then
        self_ref._on_goal_tap()
      end
    end,
...
function HomescreenWidget:_updateFooter(current_page, total_pages, topbar_on)
  ...
  if not footer_bc then
    return
  end

  local sw = self._layout_sw or Screen:getWidth()
  local content_h = self._layout_content_h
    or (self._navbar_content_h or Screen:getHeight())
  ...
  local show_bar = not hs_pag_hidden
    and total_pages > 1
    and (navpager_on or pag_visible or dot_pager_on)
...
function HomescreenWidget:_getHsCtxMenu()
  if self._hs_ctx_menu then
    return self._hs_ctx_menu
  end
...
function HomescreenWidget:_onHoldModRelease(wrapper)
  ...
  local function buildRoot(ctx)
    ...
    local local_ctx_menu = setmetatable({
      ...
      show_arrange = function(params)
        ctx.push("arrange", params)
      end,
      show_item_picker = function(params)
        ctx.push("item_picker", params)
      end,
    }, { __index = ctx_menu })

    local items = type(mod.getMenuItems) == "function"
        and mod.getMenuItems(local_ctx_menu)
      or {}
    if not mod.no_top_margin then
      local gap_item = Config.makeGapItem({
        ...
        get = function()
          return Config.getModuleGapPct(mod.id, PFX)
        end,
        set = function(v)
  ...
  local function titleFn(ctx)
    ...
    if id == "nested_menu" then
      return cur.params.title or ""
    end
    if id == "arrange" then
      return cur.params.title or _("Arrange Items")
    end
    if id == "item_picker" then
      return cur.params and cur.params.title or _("Add Item")
    end
...
function HomescreenWidget:_updatePage(keep_cache, books_only, stats_only)
  ...
  if
    not self._enabled_mods_cache
    or self._enabled_mods_cache.layout_fingerprint ~= layout_fingerprint
  then
      ...
      if cd and Registry.isEnabled(cd, PFX) then
        ...
        if not found then
          ...
          for i, m in ipairs(pages_of_mods[1]) do
            ...
            if m.id == "currently" then
              insert_at = i + 1
            end
          end
          table.insert(pages_of_mods[1], insert_at, cd)
          mod_gaps["coverdeck"] =
            Config.getModuleGapPx("coverdeck", PFX, MOD_GAP)
          if cd.is_book_mod then
            has_book_mod = true
          end
        end
      end
  ...
  local empty_widget
  if
    (ctx._show_c or ctx._show_r)
    and not ctx._has_content
    and not has_book_mod
  then
  ...
  if ctx.recent_fps and #ctx.recent_fps > 0 then
    ...
    if
      self._kb_focus_idx
      and self._kb_focus_idx >= first_rec_idx
      and self._kb_focus_idx <= #_kb_books
    then
  ...
  if is_landscape then
    ...
    if is_spread then
      for _, mod in ipairs(cur_page_mods) do
        if mod.has_covers then
          page_has_covers = true
        end
        local ok_w, widget = pcall(mod.build, col_w, ctx)
        if not ok_w or not widget then
          logger.warn(
            "simpleui homescreen: build failed for "
              .. tostring(mod.id)
              .. ": "
              .. tostring(widget)
          )
    ...
    local function _build_col_group(entries)
      ...
      for _, entry in ipairs(entries) do
        ...
        if mod.label then
          col_body[#col_body + 1] = sectionLabel(mod.label, col_w)
        end
    ...
    for _, mod in ipairs(cur_page_mods) do
      ...
      elseif widget then
        ...
        if mod.label then
          body[#body + 1] = sectionLabel(mod.label, inner_w)
        end
        local has_menu = type(mod.getMenuItems) == "function"
  ...
  if ctx.db_conn_fatal and self._db_conn then
    logger.warn(
      "simpleui: homescreen: fatal DB error detected — dropping shared connection"
    )
    pcall(function()
      self._db_conn:close()
    end)
  ...
  then
    ...
    for i = 1, #body do
      local ok, sz = pcall(function()
        return body[i]:getSize()
      end)
      if ok and sz and sz.h then
        total_body_h = total_body_h + sz.h
      end
    end
    ...
    if total_body_h > avail_h then
      ...
      local warn_key = tostring(self._current_page)
        .. ":"
        .. tostring(total_body_h)
      if self._overflow_warn_key ~= warn_key then
        ...
        UIManager:scheduleIn(0.5, function()
          ...
          if (self_ref._layout_sw or Screen:getWidth()) ~= snap_sw then
            return
          end
          if
            (self_ref._layout_content_h or UI.getContentHeight())
            ~= snap_avail_h
          then
            return
          end
...
function HomescreenWidget:_refresh(keep_cache, books_only, stats_only)
  ...
  if keep_cache and self._body then
    ...
    if defer_async then
      ...
      UIManager:scheduleIn(0.05, function()
        if self._pending_refresh_token ~= token then
          return
        end
        ...
        if self._ctx_cache then
          ...
          if SP then
            local new_stats =
              SP.get(self._db_conn, os.date("%Y"), self._ctx_cache._needs_books)
            if new_stats then
              self._ctx_cache.stats = new_stats
            end
          end
          ...
          if MC and MC.fetchBookStatsForCtx and self._ctx_cache.current_fp then
            local pe = self._ctx_cache.prefetched
              and self._ctx_cache.prefetched[self._ctx_cache.current_fp]
          ...
          if
            MCD
            and MCD.fetchBookStatsForCtx
            and self._ctx_cache.recent_fps
          then
            local saved_center_fp =
              SUISettings:readSetting(PFX .. "flow_recent_fp")
            local center_fp = saved_center_fp or self._ctx_cache.recent_fps[1]
            local pe = center_fp
              and self._ctx_cache.prefetched
              and self._ctx_cache.prefetched[center_fp]
            local md5 = pe and pe.partial_md5_checksum
            if md5 then
          ...
          if not stats_only then
            for id, slot in pairs(self._book_mod_slots or {}) do
              ...
              if type(slot.mod.updateStats) == "function" then
                local ok, result =
                  pcall(slot.mod.updateStats, slot.widget, self._ctx_cache)
                ...
                if new_widget then
                  if slot.has_menu then
                    ...
                    if wrapper then
                      ...
                      UIManager:setDirty(self, function()
                        return "ui", wrapper.dimen, true
                      end)
                    end
                  else
                    slot.parent[slot.index] = new_widget
                    UIManager:setDirty(self, function()
                      return "ui", new_widget.dimen, true
                    end)
                  end
                end
              end
  ...
  UIManager:scheduleIn(0, function()
    ...
    self._refresh_scheduled = false
    if not self._navbar_container then
      return
    end
...
function HomescreenWidget:_scheduleCoverPoll()
  ...
  timer = function()
    ...
    logger.dbg(
      "simpleui cover poll: BIM done, one final retry for missing covers"
    )
    self_ref._cover_poll_timer = timer
    UIManager:scheduleIn(1, timer)
  end
...
function HomescreenWidget:onShow()
  ...
  if self._navbar_container then
    ...
    if
      ClockMod
      and Registry.isEnabled(ClockMod, PFX)
      and ClockMod.scheduleRefresh
    then
      ClockMod.scheduleRefresh(self)
    end
...
function HomescreenWidget:onSuspend()
  ...
  local ClockMod = Registry.get("clock")
  if ClockMod and ClockMod.cancelRefresh then
    ClockMod.cancelRefresh()
  end
end
...
local function _rebuildHomescreenLayout()
  ...
  if not hs_inst or not hs_inst._navbar_container then
    return
  end
...
function Homescreen.styleStatusbarTransparent()
  if
    not Homescreen.styleGetWallpaperEnabled()
    or not Homescreen.styleGetWallpaper()
  then
    return false
  end
...
function Homescreen.styleSetStatusbarTransparent(on)
  SUISettings:saveSetting(
    "simpleui_statusbar_transparent",
...
function Homescreen.styleSetWallpaperStretch(on)
  SUISettings:saveSetting(
    "simpleui_style_wallpaper_stretch",
    on ~= false and true or false
  )
  _styleFreeBgCache()
  _rebuildHomescreenLayout()
end
...
function Homescreen.styleSetWallpaperAutoRotate(on)
  SUISettings:saveSetting(
    "simpleui_style_wallpaper_autorotate",
    on ~= false and true or false
  )
  _styleFreeBgCache()
  _rebuildHomescreenLayout()
...
function Homescreen.styleSetWallpaperInvertNight(on)
  SUISettings:saveSetting(
    "simpleui_style_wallpaper_invert_night",
    on and true or false
  )
  _styleFreeBgCache()
  _rebuildHomescreenLayout()
...
function Homescreen.styleSetWallpaperOpacity(val)
  SUISettings:saveSetting(
    "simpleui_style_wallpaper_opacity",
    math.max(0, math.min(99, val or 0))
  )
