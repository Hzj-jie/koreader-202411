...
local function _getFlag(key)
  return SUISettings:readSetting(key) ~= false
end
local function _setFlag(key, v)
  SUISettings:saveSetting(key, v)
end

function M.isEnabled()
  return SUISettings:isTrue(SK.enabled)
end
function M.setEnabled(v)
  SUISettings:saveSetting(SK.enabled, v)
end

function M.getShowName()
  return _getFlag(SK.show_name)
end
function M.setShowName(v)
  _setFlag(SK.show_name, v)
end
function M.getHideUnderline()
  return _getFlag(SK.hide_underline)
end
function M.setHideUnderline(v)
  _setFlag(SK.hide_underline, v)
end

-- "alpha" = semitransparent white overlay; "frame" = solid grey border.
function M.getLabelStyle()
  return SUISettings:readSetting(SK.label_style) or "alpha"
end
function M.setLabelStyle(v)
  SUISettings:saveSetting(SK.label_style, v)
end

-- "bottom" (default) | "center" | "top"
function M.getLabelPosition()
  return SUISettings:readSetting(SK.label_position) or "bottom"
end
function M.setLabelPosition(v)
  SUISettings:saveSetting(SK.label_position, v)
end

-- "top" (default) | "bottom"
function M.getBadgePosition()
  return SUISettings:readSetting(SK.badge_position) or "top"
end
function M.setBadgePosition(v)
  SUISettings:saveSetting(SK.badge_position, v)
end

function M.getBadgeHidden()
  return SUISettings:isTrue(SK.badge_hidden)
end
function M.setBadgeHidden(v)
  SUISettings:saveSetting(SK.badge_hidden, v)
end

-- "default" = scale-to-fit; "2_3" = force 2:3 aspect ratio.
function M.getCoverMode()
  return SUISettings:readSetting(SK.cover_mode) or "default"
end
function M.setCoverMode(v)
  SUISettings:saveSetting(SK.cover_mode, v)
end

-- "overlay" (default) = folder name on cover; "hidden" = no label.
function M.getLabelMode()
  return SUISettings:readSetting(SK.label_mode) or "overlay"
end
function M.setLabelMode(v)
  SUISettings:saveSetting(SK.label_mode, v)
end

-- Pages badge: default on; hidden for completed books.
function M.getOverlayPages()
  return SUISettings:readSetting(SK.overlay_pages) ~= false
end
function M.setOverlayPages(v)
  _setFlag(SK.overlay_pages, v)
end

-- Series index badge: default off.
function M.getOverlaySeries()
  return SUISettings:isTrue(SK.overlay_series)
end
function M.setOverlaySeries(v)
  _setFlag(SK.overlay_series, v)
end
...
function M.getOverlayProgress()
  return SUISettings:readSetting(SK.overlay_progress) ~= false
end
function M.setOverlayProgress(v)
  _setFlag(SK.overlay_progress, v)
end
...
function M.getNewMode()
  ...
  if stored == "ribbon" or stored == "badge" or stored == "none" then
    return stored
  end
  -- Migrate from legacy bool: if the old key was explicitly false → "none".
  if SUISettings:readSetting(SK.overlay_new) == false then
    return "none"
  end
  return "ribbon"
end
...
function M.getOverlayNew()
  return M.getNewMode() ~= "none"
end
function M.setOverlayNew(v)
  M.setNewMode(v and "badge" or "none")
end

-- Virtual series folders in the mosaic.
function M.getSeriesGrouping()
  return SUISettings:isTrue(SK.series_grouping)
end
function M.setSeriesGrouping(v)
  _setFlag(SK.series_grouping, v)
end

-- Placeholder cover for folders with no direct ebooks.
function M.getSubfolderCover()
  return SUISettings:isTrue(SK.subfolder_cover)
end
function M.setSubfolderCover(v)
  _setFlag(SK.subfolder_cover, v)
end

-- Scan up to 3 subfolder levels for a cached cover.
function M.getRecursiveCover()
  return SUISettings:isTrue(SK.recursive_cover)
end
function M.setRecursiveCover(v)
  _setFlag(SK.recursive_cover, v)
end

-- "single" (default) | "quad" (2×2 grid) | "auto" (single <4 books, quad ≥4).
function M.getFolderStyle()
  return SUISettings:readSetting(SK.folder_style) or "single"
end
function M.setFolderStyle(v)
  SUISettings:saveSetting(SK.folder_style, v)
end

-- Hide the book spine decoration on folder covers.
function M.getHideSpine()
  return SUISettings:isTrue(SK.hide_spine)
end
function M.setHideSpine(v)
  SUISettings:saveSetting(SK.hide_spine, v)
end

-- Title/author strip below mosaic covers. Requires restart to take effect.
function M.getShowTitleStrip()
  return SUISettings:isTrue(SK.show_title_strip)
end
function M.setShowTitleStrip(v)
  SUISettings:saveSetting(SK.show_title_strip, v)
end
function M.getShowAuthorStrip()
  return SUISettings:isTrue(SK.show_author_strip)
end
function M.setShowAuthorStrip(v)
  SUISettings:saveSetting(SK.show_author_strip, v)
end

-- Per-badge color: "dark" = black bg / white text; "light" = white bg / black text.
function M.getBadgeColorPages()
  return SUISettings:readSetting(SK.badge_color_pages) or "light"
end
function M.setBadgeColorPages(v)
  SUISettings:saveSetting(SK.badge_color_pages, v)
end
function M.getBadgeColorSeries()
  return SUISettings:readSetting(SK.badge_color_series) or "light"
end
function M.setBadgeColorSeries(v)
  SUISettings:saveSetting(SK.badge_color_series, v)
end
function M.getBadgeColorProgress()
  return SUISettings:readSetting(SK.badge_color_progress) or "dark"
end
function M.setBadgeColorProgress(v)
  SUISettings:saveSetting(SK.badge_color_progress, v)
end
function M.getBadgeColorNew()
  return SUISettings:readSetting(SK.badge_color_new) or "dark"
end
function M.setBadgeColorNew(v)
  SUISettings:saveSetting(SK.badge_color_new, v)
end
function M.getBadgeColorFolder()
  return SUISettings:readSetting(SK.badge_color_folder) or "dark"
end
function M.setBadgeColorFolder(v)
  SUISettings:saveSetting(SK.badge_color_folder, v)
end
...
function M.getFadeFinished()
  return SUISettings:readSetting(SK.fade_finished) ~= false
end
function M.setFadeFinished(v)
  SUISettings:saveSetting(SK.fade_finished, v)
end
...
function M.setFadeAmountPct(v)
  v = math.max(
    _FADE_AMOUNT_MIN,
    math.min(_FADE_AMOUNT_MAX, math.floor(tonumber(v) or _FADE_AMOUNT_DEF))
  )
  SUISettings:saveSetting(SK.fade_amount, v)
end
-- lightenRect expects a 0.0–1.0 float.
function M.getFadeAmount()
  return M.getFadeAmountPct() / 100
end
...
function M.setLabelScale(pct)
  SUISettings:saveSetting(SK.label_scale, _clampFCScale(pct))
end
...
local function _clampFCBadgeScale(n)
  return math.max(
    _FC_BADGE_SCALE_MIN,
    math.min(_FC_BADGE_SCALE_MAX, math.floor(n))
  )
end
...
function M.getBadgeScalePct()
  ...
  if not n then
    return _FC_BADGE_SCALE_DEF
  end
  return _clampFCBadgeScale(n)
end
function M.getBadgeScale()
  return M.getBadgeScalePct() / 100
end
function M.setBadgeScale(pct)
  SUISettings:saveSetting(SK.badge_scale, _clampFCBadgeScale(pct))
end
  ...
  return {
    {
      ...
      checked_func = function()
        return M.getProgressMode() == "banner"
      end,
      callback = function()
        _set("banner")
      end,
    },
    {
      ...
      checked_func = function()
        return M.getProgressMode() == "native"
      end,
      callback = function()
        _set("native")
      end,
...
local function _cfcGet(key)
  return _cover_file_cache[key] or _cfc_b[key]
end
...
local function _lmcGet(key)
  return _lm_dir_cover_cache[key] or _lmc_b[key]
end
...
local function _installItemCache()
  if FileChooser._simpleui_fc_cache_patched then
    return
  end
...
local function _uninstallItemCache()
  if not FileChooser._simpleui_fc_cache_patched then
    return
  end
...
local function _collectCoversRecursive(
  menu,
  dir_path,
  depth,
  max_depth,
  ...
  if depth > max_depth or needed <= 0 then
    return {}
  end
...
local function _findCoverRecursive(
  menu,
  dir_path,
  depth,
  max_depth,
  ...
  local found = _collectCoversRecursive(
    menu,
    dir_path,
    depth,
    max_depth,
  ...
  if menu.layout then
    for _, row in ipairs(menu.layout) do
      for _, item in ipairs(row) do
        if
          item._foldercover_processed
          and item.entry
          and item.entry.path == dir_path
        then
...
_resolveStyle = function(menu, dir_path, entry)
  ...
  if not menu or not dir_path then
    return "single"
  end
  ...
  local is_sg = (entry and entry.is_series_group)
    or (_sg_items_cache[dir_path] ~= nil)
  if is_sg then
    ...
    buttons = {
      {
        ...
        },
        {
          ...
          callback = function()
            ...
            UIManager:show(InfoMessage:new({
              text = T(
                _('Collection "%1" created with %2 books.'),
                name,
                count
              ),
              timeout = 3,
...
local function _openSeriesGroupCoverPicker(vpath, menu, BookInfoManager)
  ...
  for _, item in ipairs(series_items) do
    ...
    if fp then
      ...
      local title = (bi and bi.title and bi.title ~= "") and bi.title
        or (fp:match("([^/]+)%.[^%.]+$") or fp)
      local _fp = fp
      buttons[#buttons + 1] = {
        {
...
local function _installFileDialogButton(BookInfoManager)
  ...
  FileManager:addFileDialogButtons(
    ...
    function(file, is_file, _book_props)
      ...
      if effective_style == "quad" and not in_list_view then
        return nil
      end
...
local function _sgProcessItemTable(item_table, file_chooser)
  ...
  if not file_chooser or not item_table then
    return
  end
  if item_table._sg_is_series_view then
    return
  end
  if file_chooser.show_current_dir_for_hold then
    return
  end
  ...
  for _, item in ipairs(item_table) do
    if item.is_go_up then
      ...
      if not item.sort_percent then
        item.sort_percent = item.percent_finished or 0
      end
  ...
  local mixed = G_reader_settings:isTrue("collate_mixed")
    and collate_obj
    and collate_obj.can_collate_mixed
  ...
  if mixed then
    ...
    if sort_func then
      pcall(table.sort, files, sort_func)
    end
    if up_item then
      final[#final + 1] = up_item
    end
...
local function _installSeriesGrouping()
  if FileChooser._simpleui_sg_patched then
    return
  end
  ...
  FileChooser.switchItemTable = function(
    fc,
    new_title,
    new_item_table,
    itemnumber,
  ...
  FileChooser.refreshPath = function(fc)
    ...
    if HS and HS._library_was_visited then
      for k in pairs(_cover_file_cache) do
        _cover_file_cache[k] = nil
      end
      for k in pairs(_cfc_b) do
        _cfc_b[k] = nil
      end
      _cfc_cnt = 0
      for k in pairs(_lm_dir_cover_cache) do
        _lm_dir_cover_cache[k] = nil
      end
      for k in pairs(_lmc_b) do
        _lmc_b[k] = nil
      end
      _lmc_cnt = 0
    end
  ...
  FileChooser.updateItems = function(fc, ...)
    ...
    if
      _sg_current
      and _sg_current.should_restore
      and fc.item_table
      and #fc.item_table > 0
    then
      local sname = _sg_current.series_name
...
local function _uninstallSeriesGrouping()
  if not FileChooser._simpleui_sg_patched then
    return
  end
...
function M.exitSeriesView(fc)
  if not M.isInSeriesView(fc) then
    return
  end
  local parent = fc.item_table._sg_parent_path
  _sg_current = nil
...
local function _buildProgressBadgeDesc(
  eff_size,
  status,
  percent_finished,
  border,
...
local function _drawProgressBadge(bb, ox, oy, desc)
  ...
  local fill_color = desc.dark and Blitbuffer.COLOR_BLACK
    or Blitbuffer.COLOR_WHITE
  local text_color = desc.dark and Blitbuffer.COLOR_WHITE
...
local function _paintCornerRibbon(
  bb,
  cover_left,
  cover_right,
  cover_top,
  ...
  local cache_key = string.format(
    ...
    font_sz,
    dark and 1 or 0,
    tostring(SUIStyle.BADGE_BORDER_SZ),
    tostring(brd)
  )
  ...
  if not tmp then
    ...
    repeat
      ...
      lsz = lbl:getSize()
      if lsz.w <= max_w and lsz.h <= inner_h then
        break
      end
      fs = fs - 1
    until fs < 6
...
local function _buildRectBadgeWidget(
  text,
  bold,
  cell_min,
  dark,
  ...
  local eff_size =
    math.max(8, math.floor((cell_min or 40) * 0.15 * badge_scale))
...
local function _buildLabel(
  ...
  if display.label_mode ~= "overlay" then
    return nil
  end
  if not display.show_name then
    return nil
  end
  ...
  local img_dimen =
    Geom:new({ w = size.w + border * 2, h = size.h + border * 2 })
...
local function _buildBadge(mandatory, cover_dimen, cv_scale, cell_dimen)
  ...
  local badge_margin_r =
    math.max(1, math.floor(_BADGE_MARGIN_R_BASE * cv_scale))
  ...
  local badge = FrameContainer:new({
    ...
    CenterContainer:new({
      dimen = Geom:new({
        w = nb_size - 2 * SUIStyle.BADGE_BORDER_SZ,
        h = nb_size - 2 * SUIStyle.BADGE_BORDER_SZ,
      }),
      (function()
        local tw = TextWidget:new({
...
local function _assembleCoverWidget(
  item,
  content_widget,
  size,
  border,
  ...
  local cv_scale =
    math.max(0.1, math.floor((cover_h / _BASE_COVER_H) * 10) / 10)

  local folder_name_widget = _buildLabel(
    item,
    size.w - _LATERAL_PAD * 2,
    size,
    border,
    cv_scale,
    display,
  ...
  local nbitems_widget =
    _buildBadge(item.mandatory, cover_dimen, cv_scale, cell_dimen)

  local overlap = OverlapGroup:new({ dimen = cover_dimen, cover_group })
  if folder_name_widget then
    overlap[#overlap + 1] = folder_name_widget
  end
  if nbitems_widget then
    overlap[#overlap + 1] = nbitems_widget
  end
...
local function _buildQuadCover(
  item,
  img_list,
  border,
  spine_w,
...
local function _getMosaicMenuItemAndPatch()
  ...
  if not ok_mm or not MosaicMenu then
    return nil, nil
  end
...
local function _getListMenuItem()
  ...
  if not ok_lm or not ListMenu then
    return nil
  end
  local ok_up, userpatch = pcall(require, "userpatch")
  if not ok_up or not userpatch then
...
local function _installStripPatch(
  MosaicMenuItem,
  BookInfoManager,
  _STRIP_H,
  _show_title_strip,
  ...
  local ok_fm_strip, FileManager_strip =
    pcall(require, "apps/filemanager/filemanager")
  if not ok_fm_strip or not FileManager_strip then
    return
  end
  ...
  FileManager_strip.setupLayout = function(fm, ...)
    ...
    if _strip_paintTo_patched or not fm.coverbrowser then
      return
    end
    ...
    local function _mhs(fs, bold)
      local tw = TextWidget_s:new({
        text = "Ag",
        face = Font_s:getFace(SUIStyle.FACE_REGULAR, fs),
        bold = bold,
        padding = 0,
      })
    ...
    function MosaicMenuItem:paintTo(bb, x, y)
        ...
        if self.is_directory then
          ...
          if self._simpleui_strip_data == nil then
            if not self.bookinfo_found then
              ...
              local authors = info and not info.ignore_meta and info.authors
                or nil
              ...
              pcall(function()
                ...
                if
                  custom_file
                  and require("libs/libkoreader-lfs").attributes(
                      custom_file,
                      "mode"
                    )
                    == "file"
          ...
          if not self._simpleui_strip_data then
            return
          end
          ...
          if _show_title_strip and self._simpleui_strip_data.title then
            ...
            tw:free()
            if _show_author_strip then
              cur_y = cur_y + TITLE_LINE_S + GAP_S
            end
          end
      ...
      if self._simpleui_strip_bb then
        ...
        if wp_active then
          if
            not self._simpleui_strip_mask_bb
            or self._simpleui_strip_mask_bb:getWidth() ~= self.width
          then
          ...
          UI_core.paintWithAlphaMask(
            self,
            bb,
            x,
            y + self.height - _STRIP_H,
            self.width,
            _STRIP_H,
...
function M.install()
  ...
  if MosaicMenuItem._simpleui_fc_patched then
    return
  end
  ...
  if _show_title_strip or _show_author_strip then
    ...
    local function _mh(fs, bold)
      local tw = TextWidget_:new({
        text = "Ag",
        face = Font_:getFace(SUIStyle.FACE_REGULAR, fs),
        bold = bold,
        padding = 0,
      })
      local h = tw:getSize().h
    ...
    _STRIP_H = _PAD
    if _show_title_strip then
      _STRIP_H = _STRIP_H + TITLE_LINE
    end
    if _show_title_strip and _show_author_strip then
      _STRIP_H = _STRIP_H + _GAP
    end
    if _show_author_strip then
      _STRIP_H = _STRIP_H + AUTHOR_LINE
    end
  ...
  function MosaicMenuItem:update(...)
    ...
    if self._simpleui_strip_bb then
      self._simpleui_strip_bb:free()
      self._simpleui_strip_bb = nil
    end
    ...
    if not _in_strip_init and _STRIP_H > 0 and self.height then
      ...
      if
        ...
        and self.percent_finished
        and self.status ~= "complete"
        and BookInfoManager:getSetting("show_progress_in_mosaic")
      then
    ...
    if not _upvalues_searched then
      ...
      while true do
        ...
        if nm == "corner_mark_size" then
          _corner_mark_idx = ni
        end
        if nm == "collection_mark" then
          _collection_mark_idx = ni
        end
        if _corner_mark_idx and _collection_mark_idx then
          break
        end
        ni = ni + 1
      end
    ...
    if not self.is_directory and not self.file_deleted and self.filepath then
      ...
      local bi = self.menu
        and self.menu.getBookInfo
        and self.menu.getBookInfo(self.filepath)
      if bi then
        if bi.pages then
          self._fc_pages = bi.pages
        end
      ...
      self._fc_underline_color = M.getHideUnderline() and Blitbuffer.COLOR_WHITE
        or Blitbuffer.COLOR_BLACK
      ...
      if self._fc_pages_widget then
        self._fc_pages_widget:free()
      end
      if self._fc_series_widget then
        self._fc_series_widget:free()
      end
      if self._fc_new_widget then
        self._fc_new_widget:free()
      end
      ...
      if self._fc_overlay_progress then
        ...
        if has_progress then
          ...
          local prog_desc = _buildProgressBadgeDesc(
            ...
            SUIStyle.BADGE_BORDER_SZ,
            dark
          )
          if prog_desc then
            self._fc_progress_bb = prog_desc
          end
        end
      end
      ...
      if _new_mode ~= "none" then
        ...
        if is_unread then
          if _new_mode == "badge" then
            ...
            self._fc_new_widget = _buildRectBadgeWidget(
              _("New"),
              true,
              cell_min,
              dark,
    ...
    if self._foldercover_processed then
      return
    end
    if self.menu.no_refresh_covers then
      return
    end
    ...
    if HS and HS._instance and not self.menu._fc_hs_deferred then
      ...
      UIManager:nextTick(function()
        ...
        if not fm or not fm.instance or fm.instance.tearing_down then
          return
        end
    ...
    if self.entry.is_series_group then
      ...
      local sg_override_fp = folder_style ~= "quad"
        and _getCoverOverrides()[dir_path]
      ...
      if folder_style == "quad" and items then
        ...
        if #covers > 0 then
          local border, spine_w, max_img_w, max_img_h =
            _computeCellGeometry(self)
          local widget = _buildQuadCover(
            self,
            covers,
            border,
            spine_w,
    ...
    if not has_files then
      if has_subfolders and M.getSubfolderCover() and M.getRecursiveCover() then
        local cover =
          _findCoverRecursive(self.menu, dir_path, 1, 3, BookInfoManager)
        if cover then
  ...
  function MosaicMenuItem:_setFolderCover(img, display)
    ...
    local content =
      FrameContainer:new({ padding = 0, bordersize = border, image })
    _installWidget(
      self,
      _assembleCoverWidget(self, content, size, border, spine_w, display)
    )
  end
  ...
  function MosaicMenuItem:_setEmptyFolderCover(display)
    ...
    if nerd_char then
      ...
      local actual_icon_exists = lfs.attributes(actual_icon_path, "mode")
        == "file"
    ...
    local size = Geom:new({ w = img_w, h = img_h })
    local content =
      FrameContainer:new({ padding = 0, bordersize = border, bg_canvas })
    _installWidget(
      self,
      _assembleCoverWidget(self, content, size, border, spine_w, display)
    )
  ...
  function MosaicMenuItem:paintTo(bb, x, y)
    ...
    if
      (_progress_mode == "banner" or _progress_mode == "none")
      and not self.is_directory
    then
      _saved_hint = self.do_hint_opened
      self.do_hint_opened = false

      if _progress_mode == "banner" then
        _saved_icon = self.shortcut_icon
        self.shortcut_icon = nil
    ...
    if
      _progress_mode == "banner"
      and not self.is_directory
      and self.filepath
      and self.menu
      and _saved_menu_name ~= nil
      and _saved_menu_name ~= "collections"
      and ReadCollection
      and ReadCollection:isFileInCollections(self.filepath)
    then
      ...
      if cm_size and cm_widget then
        ...
        if tgt and tgt.dimen then
          ...
          if BD.mirroredUILayout() then
            ...
            ix = self.width
              - math.ceil((self.width - tgt.dimen.w) / 2)
              - cm_size
          end
          local iy = self.height
            - math.ceil((self.height - tgt.dimen.h) / 2)
            - cm_size
          local rect_size = cm_size - tgt.bordersize
          bb:paintRect(
            x + ix,
            tgt.dimen.y + iy + tgt.bordersize,
            rect_size,
            rect_size,
    ...
    if self._fc_overlay_pages and self.status ~= "complete" then
      ...
      if wg then
        ...
        if self.show_progress_bar then
          local corner_sz = _fc_coll_sz
            or math.floor(math.min(self.width, self.height) / 8)
    ...
    if self._fc_overlay_new then
      local has_progress_badge = self._fc_overlay_progress
        and self._fc_progress_bb
      if not has_progress_badge then
        ...
        elseif mode == "ribbon" then
          ...
          local eff_size =
            math.max(8, math.floor(cell_min * 0.1694 * badge_scale))
  ...
  function MosaicMenuItem:free()
    ...
    if self._simpleui_strip_mask_bb then
      self._simpleui_strip_mask_bb:free()
      self._simpleui_strip_mask_bb = nil
    end
    self._simpleui_strip_data = nil
    if orig_free then
      orig_free(self)
  ...
  if ListMenuItem and not ListMenuItem._simpleui_lm_patched then
    ...
    function ListMenuItem:update(...)
      ...
      if self.menu and self.menu.no_refresh_covers then
        return
      end
      if self._foldercover_processed then
        return
      ...
      if not entry or entry.is_file or entry.file then
        return
      end
      ...
      if entry.is_virtual_meta_leaf then
        ...
        if
          cover_specs and BookInfoManager.isCachedCoverInvalid(bi, cover_specs)
        then
          return
        end
...
function M.uninstall()
  ...
  if not MosaicMenuItem._simpleui_fc_patched then
    return
  end
  ...
  if
    MosaicMenuItem._simpleui_fc_iw_n and MosaicMenuItem._simpleui_fc_orig_iw
  then
    debug.setupvalue(
      MosaicMenuItem.update,
      MosaicMenuItem._simpleui_fc_iw_n,
  ...
  _lmc_cnt = 0
  for k in pairs(_fs_cache_a) do
    _fs_cache_a[k] = nil
  end
  for k in pairs(_fs_cache_b) do
    _fs_cache_b[k] = nil
  end
  _fs_cache_a_cnt = 0
