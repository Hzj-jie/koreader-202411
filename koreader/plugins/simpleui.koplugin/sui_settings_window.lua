...
local function buildScreens(st)
  ...
  local function buildPluginMenuScreen(ctx, make_fn)
    if type(make_fn) ~= "function" then
      return {
        SUI.ListRow({ title = _("Settings not found."), inner_w = ctx.inner_w }),
      }
    end
    ...
    return {
      ...
      SUI.ListRow({
        ...
        subtitle = (function()
          ...
          if n == 0 then
            return _("Global custom quick actions")
          end
          return string.format(
            _("Global custom quick actions  (%d/%d)"),
            n,
            Config.MAX_CUSTOM_QA
          )
        end)(),
        inner_w = iw,
        show_chevron = true,
  ...
  local function buildPages(ctx)
    ...
    for p_idx, page in ipairs(st.layout.pages) do
      local title = p_idx == 1 and _("Page 1 (Home)")
        or string.format(_("Page %d"), p_idx)
      local _p_idx = p_idx

      local del_fn = _p_idx > 1
          and function()
      ...
      local subtitle = #mod_names > 0 and table.concat(mod_names, "  ·  ")
        or _("No modules")
    ...
    return SUI.ArrangeList({
      ...
      on_change = function(new_items)
        ...
        for _, it in ipairs(new_items) do
          table.insert(new_pages, it.orig_item)
        end
    ...
    for m_idx, mod_id in ipairs(page.modules) do
      ...
      if row_counters[mod_id] then
        ...
        if entry.kind == "spacer" then
          ...
          if not mod_subtitle or mod_subtitle == "" then
            mod_subtitle = _("No actions configured")
          end
        end
      end
      table.insert(items, {
            ...
            on_tap = function()
              ...
              for p_idx, p in ipairs(st.layout.pages) do
                if p_idx ~= st.current_page then
                  local _p_idx = p_idx
                  local title = _p_idx == 1 and _("Page 1 (Home)")
              ...
              ctx.push(
                "item_picker",
                { title = _("Move to page"), items = picker_items }
              )
            end,
          },
        },
  ...
  local function buildModulePicker(ctx)
    ...
    logger.dbg(
      "simpleui: buildModulePicker: Registry.list() count =",
      #all_mods
    )
    local active_count = 0
    for k, _ in pairs(active_set) do
    ...
    for _, mod in ipairs(all_mods) do
      ...
      logger.dbg(
        ...
        is_inst,
        "in_active:",
        active_set[mod.id] ~= nil
      )
    end
    logger.dbg(
      "simpleui: isInstanciable(quick_actions_row) =",
      Registry.isInstanciable("quick_actions_row")
    )
  ...
  local function buildModuleSettings(ctx)
    ...
    if not mod or not mod.getMenuItems then
      return {}
    end
    ...
    ctx_menu.refresh = function()
      LayoutService.save(st.layout)
      ctx.repaint()
    end
    ctx_menu.show_arrange = function(params)
      ctx.push("arrange", params)
    end
  ...
  local function buildWallpaper(ctx)
    ...
    return buildPluginMenuScreen(ctx, plugin and plugin.makeWallpaperMenuItems)
  end

  -- ── 2.8. General Settings ────────────────────────────────────────────────
  local function buildGeneralSettings(ctx)
    local plugin = getPlugin()
    return buildPluginMenuScreen(ctx, plugin and plugin.makeBehaviourMenuItems)
  end
  ...
  local function buildPresets(ctx)
    ...
    if not ok or not SUIPresets then
      return {
        SUI.ListRow({
          title = _("Presets module unavailable."),
          inner_w = ctx.inner_w,
        }),
      }
    end
...
local function makeScreenTitles(st)
  return {
    ...
    page_edit = function(ctx)
      ...
      return idx == 1 and _("Page 1 (Home)") or string.format(_("Page %d"), idx)
    end,
      ...
      pages = function(ctx)
        return SUI.CenteredButtonFooter(ctx, {
          ...
          on_tap = function()
            ...
            if ctx.jump_to_last_page then
              ctx.jump_to_last_page()
            end
            ctx.repaint()
          end,
        })
