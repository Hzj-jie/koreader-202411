...
local function buildPanel(touch_menu)
  ...
  local lbl_fs = math.max(
    6,
    math.floor(
      (ok_style and SUIStyle.FS_DETAIL or 15) * (getLabelScalePct() / 100)
    )
  )
  local lbl_face =
    Font:getFace(ok_style and SUIStyle.FACE_REGULAR or "cfont", lbl_fs)
  ...
  local function makeButton(action_id)
    ...
    if is_nerd then
      ...
      icon_widget = TextWidget:new({
        text = nerd_char,
        face = Font:getFace(
          ok_style and SUIStyle.FACE_ICONS or "symbols",
          math.floor(icon_size * 0.75)
        ),
        fgcolor = Blitbuffer.COLOR_BLACK,
        padding = 0,
      })
    else
      local icon_path = ok_style
        and SUIStyle
        and entry.icon
        and SUIStyle.safeIconPath
        and SUIStyle.safeIconPath(entry.icon, nil)
      if icon_path then
    ...
    local corner_r = is_bare and 0
      or (
        (shape == "round") and math.floor(btn_size / 2)
        or math.floor(btn_size / 4)
      )
    local current_border = (
      not is_bare and (bg == "solid" or bg == "transparent")
    )
        and border_sz
      or 0
            ...
            if in_reader and not is_in_place then
              if _aid == "homescreen" then
                ...
                UIManager:scheduleIn(0, function()
                  ...
                  local plugin_new = fm_new and fm_new._simpleui_plugin
                    or plugin_live
                  plugin_new:_navigate(
                    _aid,
                    fm_new,
                    _Config().loadTabConfig(),
                    false
                  )
                end)
              end
  ...
  if showWarmth() and Device:hasNaturalLight() then
    ...
    local warmth_slider_w = inner_w
      - 2 * small_btn_w
      - max_btn_w
      - 3 * slider_gap

    -- Measure button height (reuse dummy or create one).
    local _dummy2 = Button:new({
      text = "−",
      width = small_btn_w,
      show_parent = touch_menu.show_parent,
      callback = function() end,
    ...
    local function setWarmth(warmth)
      ...
      nl.cur = powerd:toNativeWarmth(powerd:frontlightWarmth())
      nl_progress:setPosition(
        math.floor(nl.cur / nl_stride),
        nl_progress.default_position
      )
      nl_label:setText(_("Warmth") .. ": " .. tostring(nl.cur))
      UIManager:setDirty(touch_menu.show_parent, "ui")
    end
...
local function handleGesture(touch_menu, ges)
  ...
  if
    refs.fl_progress
    and refs.fl_progress.dimen
    and ges.pos:intersectWith(refs.fl_progress.dimen)
  then
...
local function patchTouchMenu()
  ...
  if not ok or not TouchMenu then
    return
  end
  if TouchMenu._sui_qs_patched then
    return
  end
  ...
  function TouchMenu:updateItems(target_page, target_item_id)
    ...
    if Device:hasBattery() then
      ...
      local sym =
        powerd:getBatterySymbol(powerd:isCharged(), powerd:isCharging(), lvl)
...
local function unpatchTouchMenu()
  ...
  if not TM or not TM._sui_qs_patched then
    return
  end
...
function QSBar.install()
      ...
      if ok_lfs and lfs then
        ...
        if lfs.attributes(icon_src, "mode") == "file" then
          ...
          pcall(function()
            ...
            local iw_init = iw._simpleui_orig_init_for_scan
              or rawget(iw, "init")
          ...
          if not injected then
            pcall(function()
              ...
              logger.info(
                "simpleui/qsbar: icon registered via IconWidget.init patch (fallback)"
              )
            end)
          end
        end
  ...
  if not ok_fm or not FMMenu then
    return
  end
  if FMMenu._sui_qs_tab_patched then
    return
  end
...
function QSBar.makeMenuItems(ctx_menu)
  local function refresh()
    ...
    if FM and FM.instance then
      _forceRebuild(FM.instance.menu)
    end

    local RUI = package.loaded["apps/reader/readerui"]
    if RUI and RUI.instance then
      _forceRebuild(RUI.instance.menu)
    end
  end
  ...
  for _, id in ipairs(QA.allIds()) do
    pool[#pool + 1] = { id = id, label = QA.getEntry(id).label }
  end
  for _, qa_id in ipairs(QA.getCustomQAList()) do
    pool[#pool + 1] = { id = qa_id, label = QA.getEntry(qa_id).label }
  end
    ...
    callback = function()
      ...
      local function on_save()
        ...
        for _, it in ipairs(sort_items) do
          new_slots[#new_slots + 1] = it.orig_item
        end
        saveSlots(new_slots)
        refresh()
      end
    ...
    items_sub[#items_sub + 1] = {
      text_func = function()
        ...
        for i, sid in ipairs(slots) do
          if sid == _id then
            return string.format("%s  (%d/%d)", _label, i, MAX_SLOTS)
          end
        end
        return _label
      end,
      checked_func = function()
        for _, sid in ipairs(getSlots()) do
          if sid == _id then
            return true
          end
        end
        ...
        if pos then
          ...
          if #slots >= MAX_SLOTS then
            ...
            UIM:show(InfoMessage:new({
              text = string.format(
                N_(
                  "Maximum of %d slot reached. Remove one first.",
                  "Maximum of %d slots reached. Remove one first.",
                  MAX_SLOTS
                ),
                MAX_SLOTS
              ),
        ...
        and function(ctx, _item)
          ...
          return SUIWindow.ListRow({
            ...
            subtitle = function()
              local slots = getSlots()
              if #slots == 0 then
                return _("No actions active yet.")
              end
              local names = {}
              for _, sid in ipairs(slots) do
            ...
            on_tap = function()
              ...
              for _, sid in ipairs(slots) do
                sort_items[#sort_items + 1] =
                  { text = labelFor(sid), orig_item = sid }
              end
              ctx.push("arrange", {
                title = _("Quick Actions"),
                items = sort_items,
                ...
                footer_action = function(ctx2)
                  ...
                  ctx2.push(
                    "item_picker",
                    { title = _("Add Action"), items = picker_items }
                  )
                end,
              })
            end,
  ...
  items[#items + 1] = {
    ...
    sub_item_table = {
      _Config().makeScaleItem({
        ...
        get = function()
          return getLabelScalePct()
        end,
        set = function(v)
          SUISettings:saveSetting(LABEL_SCALE_KEY, v)
        end,
        refresh = refresh,
      }),
      {
