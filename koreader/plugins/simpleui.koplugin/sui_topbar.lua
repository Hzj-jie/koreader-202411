...
local function _getBarBg()
  if SUISettings:isTrue("simpleui_statusbar_transparent") then
    return nil
  end
...
local function hwHasBt()
  if _hw_has_bt == nil then
    local ok, v = pcall(function()
      return Device:hasBluetoothToggle()
    end)
...
function M.SIDE_M()
  return _getCore().SIDE_M()
end
function M.TOPBAR_SIDE_M()
  return _cached("topbar_side_m", function()
    return M.SIDE_M() - 3
  end)
end
...
function M.getTopbarInfo()
  ...
  if hwHasWifi() then
    ...
    if Config.wifi_optimistic ~= nil then
      ...
      if nm then
        local ok_w, wifi_on = pcall(function()
          return nm:isWifiOn()
        end)
  ...
  if hwHasBt() then
    local ok_b, bt = pcall(function()
      return Device:isBluetoothOn()
    end)
    ...
    if pd:isFrontlightOn() then
      ...
      if type(br) == "number" then
        ...
        if type(sc_br) == "number" then
          info.brightness = sc_br > 1 and math.floor(sc_br / 255 * 100 + 0.5)
  ...
  pcall(function()
    ...
    if _topbar_disk_text and (now - (_topbar_disk_time or 0)) < 300 then
      info.disk = _topbar_disk_text
      return
    end
    local ok_util, util = pcall(require, "util")
    if not ok_util or not util or type(util.diskUsage) ~= "function" then
      return
    end
    ...
    if
      ok_df
      and usage
      and type(usage.available) == "number"
...
function M.buildTopbarWidget()
  ...
  local item_builders = {
    ...
    disk = function()
      if not info.disk then
        return nil, nil
      end
      return "\u{F0A0}", " " .. info.disk, true
    end,
    ram = function()
      if not info.ram then
        return nil, nil
      end
  ...
  local show_swipe = not center_has_items
    and SUISettings:nilOrTrue("simpleui_topbar_swipe_indicator")
  local center_w
...
local function _showTopbarSettingsWindow(plugin)
  ...
  local function buildRoot(ctx)
    if not plugin._makeTopbarMenu then
...
function M.registerTouchZones(plugin, fm_self)
  ...
  if not _topbar_enabled_cache then
    return
  end
  ...
  local topbar_zone =
    { ratio_x = 0, ratio_y = 0, ratio_w = 1, ratio_h = topbar_h / screen_h }
...
local function shouldRunTimer(plugin)
  if not shouldRefreshTopbar(plugin) then
    return false
  end
  local cfg = getTopbarConfigCached()
  if (cfg.side["clock"] or "hidden") == "hidden" then
    return false
  end
  return true
end
...
function M.refresh(plugin)
  ...
  local Device = require("device")
  if (plugin and plugin._simpleui_suspended) or Device.screen_saver_mode then
    return
  end
  ...
  for _, entry in ipairs(stack) do
    ...
    if not ok then
      logger.warn("simpleui: topbar refreshWidget failed:", tostring(err))
    end
  end
