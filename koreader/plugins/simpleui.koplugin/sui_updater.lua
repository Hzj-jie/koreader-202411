...
local function _parseRelease(body)
  ...
  if not ok_j then
    ...
    logger.warn(
      "simpleui updater: json module unavailable, using regex fallback"
    )
...
local function _unzip(zip_path, dest_dir)
  local ret =
    os.execute(string.format("unzip -o -q %q -d %q", zip_path, dest_dir))
...
local function _tmpZipPath()
  ...
  if ok and DS then
    ...
    if dir then
      candidates[#candidates + 1] = dir .. "/simpleui_update.zip"
    end
  end
...
local function _applyUpdate(download_url, new_version)
  ...
  local progress_msg =
    _toast(string.format(_("Downloading Simple UI %s…"), new_version), 120)
  ...
  local function handleInstallResult(result)
    ...
    if not result or not result.success then
      ...
      _toast(
        stage == "download" and (_("Download error: ") .. tostring(err))
...
local function _doNetworkCheck()
  ...
  _dl_url = _isValidAssetUrl(release.download_url) and release.download_url
    or nil
...
function M.scheduleAutoCheck()
  ...
  if not ok_s or not SUISettings then
    return
  end
