...
function Onboarding.show(on_finish)
  local st = {
    selected_preset = SUISettings:get("simpleui_hs_active_preset")
      or "builtin_at_a_glance",
  }
  ...
  local function buildPresetList(ctx)
    local iw = ctx.inner_w
    local rows = {}
    local builtins = SUIPresets.getBuiltinPresets
      ...
      table.insert(
        ...
        SUI.ListRow({
          ...
          on_tap = function()
            ...
            if SUIPresets.applyBuiltin then
              SUIPresets.applyBuiltin(st.selected_preset)
            end
  ...
  local function buildTipsList(ctx)
    ...
    local tips = {
      {
        ...
        desc = _(
          "Press and hold any element on the screen (like modules or bars) to quickly customize its settings."
        ),
      },
      {
        title = _("Customize even more"),
        desc = _(
          "Go to Settings > Home Screen to edit your layout whenever you want."
        ),
      },
      {
        title = _("Set your reading goal"),
        desc = _(
          "If you selected the Momentum Preset, long press the Reading Goal module to choose and set your reading goals."
        ),
      },
      {
    ...
    local fg_sub = SUIStyle.getThemeColor("text_secondary")
      or Blitbuffer.COLOR_DARK_GRAY
    local sep_color = SUIStyle.getThemeColor("separator")
