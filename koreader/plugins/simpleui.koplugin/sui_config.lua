...
if DataStorage and type(DataStorage.getDataDir) == "function" then
  ...
  if lfs_ok and lfs_m then
    local function _is_root(dir)
      return lfs_m.attributes(dir .. "/resources/icons/mdlight", "mode")
        == "directory"
    end
    if _is_root(_d) then
      _ko_root = _d .. "/"
...
if _ko_root == "" then
  ...
  if lfs_ok and lfs_m then
    ...
    for _i = 1, 8 do
      if
        lfs_m.attributes(p .. "/resources/icons/mdlight", "mode") == "directory"
      then
        _ko_root = p .. "/"
        break
      end
...
for _i, id in ipairs(M.DEFAULT_TABS) do
  if id ~= "home" then
    M.NON_HOME_DEFAULTS[#M.NON_HOME_DEFAULTS + 1] = id
  end
end
...
M.ALL_ACTIONS = {
  ...
  },
  {
    id = "bookmark_browser",
    label = _("Bookmarks"),
    icon = M.ICON.ko_bookmark,
  },
  {
    id = "wifi_toggle",
    label = _("Wi-Fi"),
    icon = M.ICON.ko_wifi_on,
  },
  {
    id = "frontlight",
    label = _("Brightness"),
    icon = M.ICON.frontlight,
  },
  {
    id = "night_mode",
    label = _("Night Mode"),
...
M.ACTION_BY_ID = {}
for _i, a in ipairs(M.ALL_ACTIONS) do
  M.ACTION_BY_ID[a.id] = a
end
...
function M.getCustomQAList()
  return _QA_lazy().getCustomQAList()
end
function M.saveCustomQAList(list)
  return _QA_lazy().saveCustomQAList(list)
end
function M.getCustomQAConfig(id)
  return _QA_lazy().getCustomQAConfig(id)
end
function M.saveCustomQAConfig(id, label, path, coll, icon, pk, pm, da)
  return _QA_lazy().saveCustomQAConfig(id, label, path, coll, icon, pk, pm, da)
end
function M.deleteCustomQA(id)
  return _QA_lazy().deleteCustomQA(id)
end
function M.purgeQACollection(coll)
  return _QA_lazy().purgeQACollection(coll)
end
function M.renameQACollection(o, n)
  return _QA_lazy().renameQACollection(o, n)
end
function M.sanitizeQASlots()
  return _QA_lazy().sanitizeQASlots()
end
function M.nextCustomQAId()
  return _QA_lazy().nextCustomQAId()
end
...
M.TOPBAR_ITEMS =
  { "clock", "wifi", "brightness", "battery", "disk", "ram", "custom_text" }
...
function M.setTopbarCustomText(s)
  if type(s) == "string" then
    ...
    while i <= #s do
      local byte = s:byte(i)
      local clen = byte >= 240 and 4
        or byte >= 224 and 3
        or byte >= 192 and 2
        or 1
      count = count + 1
      if count > TOPBAR_CUSTOM_TEXT_MAX then
        break
      end
...
function M.getTopbarConfig()
  ...
  if type(raw) == "table" then
    ...
    if type(raw.order_left) == "table" then
      for _i, v in ipairs(raw.order_left) do
        cfg.order_left[#cfg.order_left + 1] = v
      end
    end
    if type(raw.order_right) == "table" then
      for _i, v in ipairs(raw.order_right) do
        cfg.order_right[#cfg.order_right + 1] = v
      end
    end
    if type(raw.order_center) == "table" then
      for _i, v in ipairs(raw.order_center) do
        cfg.order_center[#cfg.order_center + 1] = v
      end
    end
...
function M.saveTopbarConfig(cfg)
  ...
  if tb and tb.invalidateConfigCache then
    tb.invalidateConfigCache()
  end
end
  ...
  then
    for i = 1, #cfg do
      ...
      if M.ACTION_BY_ID[id] or id:match("^custom_qa_%d+$") then
        ...
        logger.warn(
          "simpleui: loadTabConfig: ignoring unknown tab id: " .. tostring(id)
        )
      end
    end
  else
...
local function deviceHasWifi()
  if _has_wifi_toggle == nil then
    local ok, v = pcall(function()
      return getDevice():hasWifiToggle()
    end)
...
function M.wifiIcon()
  ...
  local icon_off = QA.getDefaultActionIcon("wifi_toggle_off")
    or M.ICON.ko_wifi_off

  if M.wifi_optimistic ~= nil then
    return M.wifi_optimistic and icon_on or icon_off
  end
  if not deviceHasWifi() then
    return icon_off
  end
...
function M.sanitizeLabel(s)
  ...
  if #s > M.MAX_LABEL_LEN then
    s = s:sub(1, M.MAX_LABEL_LEN)
  end
  return s
end
...
function M.nerdIconChar(icon_value)
  ...
  if not cp or cp < 0 or cp > 0x10FFFF then
    return nil
  end
  -- Encode as UTF-8.
  if cp < 0x80 then
...
local function _clamp(n)
  return math_max(SCALE_MIN, math_min(SCALE_MAX, math_floor(n)))
end
...
function M.getNavbarLabelScalePct()
  ...
  local n = tonumber(v)
  if not n then
    return NAVBAR_LABEL_SCALE_DEF
  end
  return math_max(
    NAVBAR_LABEL_SCALE_MIN,
    math_min(NAVBAR_LABEL_SCALE_MAX, math_floor(n))
  )
end

function M.setNavbarLabelScalePct(pct)
  SUISettings:set(
    NAVBAR_LABEL_SCALE_KEY,
    math_max(
      NAVBAR_LABEL_SCALE_MIN,
      math_min(NAVBAR_LABEL_SCALE_MAX, math_floor(pct))
    )
  )
end
...
function M.resetAllScales(pfx, pfx_qa)
  ...
  for _, mod in ipairs(Registry.list()) do
    if mod.id then
      ...
      SUISettings:del(
        (pfx or "simpleui_hs_") .. mod.id .. THUMB_SCALE_KEY_SUFFIX
      )
...
function M.getModuleGapPx(mod_id, pfx, mod_gap_px)
  if mod_id and pfx then
    ...
    local n = tonumber(v)
    if n then
      return math_floor(mod_gap_px * _clampGap(n) / 100)
    end
  end
  return mod_gap_px
end
...
function M.makeLabelToggleItem(mod_id, default_label, refresh, _lc)
  return {
    ...
    checked_func = function()
      return not M.isLabelHidden(mod_id)
    end,
    keep_menu_open = true,
    callback = function()
      SUISettings:set(
        _labelHideKey(mod_id),
...
local function _scaleBBToSlot(bb, target_w, target_h, align, stretch_limit)
  ...
  if src_w == target_w and src_h == target_h then
    local ok_copy, copy_bb =
      pcall(Blitbuffer.new, target_w, target_h, bb:getType())
  ...
  if stretch_limit then
    ...
    if distort <= stretch_limit then
      ...
      if ok_sc and stretched_bb then
        return stretched_bb
      end
    end
  end
  ...
  local scaled_w, scaled_h =
    math_floor(src_w * scale_factor + 0.5),
    math_floor(src_h * scale_factor + 0.5)
  local ok_sc, scaled_bb =
    pcall(_RenderImage.scaleBlitBuffer, _RenderImage, bb, scaled_w, scaled_h)
  if not (ok_sc and scaled_bb) then
    return bb
  end
  if scaled_w == target_w and scaled_h == target_h then
    return scaled_bb
  end
  local ok_slot, slot_bb =
    pcall(Blitbuffer.new, target_w, target_h, scaled_bb:getType())
  if not (ok_slot and slot_bb) then
    return scaled_bb
  end
  local src_x = align == "left" and 0
    or (
      align == "right" and scaled_w - target_w
      or math_floor((scaled_w - target_w) / 2)
    )
...
function M.isCoverMissing(filepath)
  return _bim_cover_cache[filepath .. "|__nc"] == _NO_COVER
end

local function _markNoCover(filepath)
  local probe = filepath .. "|__nc"
  if _bim_cover_cache[probe] then
    return
  end
  if _bim_cover_count >= BIM_MAX_COVERS then
    _evictOldestCover()
  end
...
function M.getCoverBB(filepath, w, h, align, stretch_limit)
  if M.isCoverMissing(filepath) then
    return nil
  end
  local key = filepath
    .. "|"
  ...
  if _lfsMode(filepath) ~= "file" then
  ...
  local function enqueueExtract()
    if M._cover_extract_pending[filepath] then
      ...
      if ex then
        if w > ex.max_cover_w then
          ex.max_cover_w = w
        end
        if h > ex.max_cover_h then
          ex.max_cover_h = h
        end
      end
      return
    ...
    if not M._cover_extract_queue then
      M._cover_extract_queue = {}
    end
...
function M.flushCoverQueue()
  ...
  for _, fp in ipairs(queue) do
    if _lfsMode(fp) == "file" then
      files[#files + 1] =
        { filepath = fp, cover_specs = M._cover_extract_specs[fp] }
    else
      M._cover_extract_pending[fp] = nil
    end
    M._cover_extract_specs[fp] = nil
...
function M.openStatsDB()
  ...
  if not _lfs_mod.attributes(db_path, "mode") then
    return nil
  end
  ...
  if not _indexes_created then
    local idx_ok = pcall(function()
      conn:exec(
        "CREATE INDEX IF NOT EXISTS idx_simpleui_book_md5 ON book(md5);"
      )
      conn:exec(
        "CREATE INDEX IF NOT EXISTS idx_simpleui_pagestat_book ON page_stat(id_book);"
      )
      conn:exec(
        "CREATE INDEX IF NOT EXISTS idx_simpleui_pagestat_time ON page_stat(start_time);"
      )
    end)
    if idx_ok then
...
function M.isFavoritesWidget(w)
  if not w or w.name ~= "collections" then
    return false
  end
  local rc = M.getReadCollection()
  ...
  for i = #stack, 1, -1 do
    ...
    if w and w.covers_fullscreen then
      ...
      if HS and HS._instance == w then
        local cur, total =
          HS._instance._current_page or 1, HS._instance._total_pages or 1
        return cur > 1, cur < total
      end
      return false, false
    end
...
function M.migrateOldCustomSlots()
  if SUISettings:get("simpleui_qa_migrated_v1") then
    return
  end
  local id_map, qa_list, qa_set = {}, M.getCustomQAList(), {}
  for _, id in ipairs(qa_list) do
    qa_set[id] = true
  end
  for slot = 1, 4 do
    local old_id, cfg =
      "custom_" .. slot, SUISettings:get("simpleui_custom_" .. slot)
    if type(cfg) == "table" and (cfg.path or cfg.collection) then
      local new_id = M.nextCustomQAId()
      M.saveCustomQAConfig(
        new_id,
        cfg.label or (_("Custom") .. " " .. slot),
        cfg.path,
        cfg.collection
      )
  ...
  for _, pfx in ipairs({ "simpleui_hs_qa_" }) do
    for slot = 1, 3 do
      local key, dqa =
        pfx .. slot .. "_items", SUISettings:get(pfx .. slot .. "_items")
      if type(dqa) == "table" then
        local changed, new_dqa = false, {}
        for _, id in ipairs(dqa) do
          if id_map[id] then
            new_dqa[#new_dqa + 1] = id_map[id]
            changed = true
...
function M.reset()
  ...
  M.cover_extraction_pending, M._cover_extract_queue, M._cover_extract_pending, M._cover_extract_specs =
    false, {}, {}, {}
  _Device, _NetworkMgr, _has_wifi_toggle, _topbar_item_labels, _SQ3, _lfs_mod, _BookInfoManager, _topbar_cfg_menu_cache, _ReadCollection =
