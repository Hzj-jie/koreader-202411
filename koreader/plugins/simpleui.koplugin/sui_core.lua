...
local function _BB()
  _Bottombar = _Bottombar or require("sui_bottombar")
  return _Bottombar
end
local function _TB()
  _Topbar = _Topbar or require("sui_topbar")
  return _Topbar
end
...
function M.resolveMenuItems(items)
  ...
  for _, item in ipairs(items) do
    ...
    if type(item.checked_func) == "function" then
      ...
      r.mandatory_func = function()
        return cf() and "\u{2713}" or ""
      end
      r.checked_func = nil
    end
...
function M.invalidateDimCache()
  ...
  if bb and bb.invalidateDimCache then
    bb.invalidateDimCache()
  end
  local tb = package.loaded["sui_topbar"]
  if tb and tb.invalidateDimCache then
    tb.invalidateDimCache()
  end
...
function M.getContentHeight()
  ...
  return Screen:getHeight()
    - _BB().TOTAL_H()
    - (topbar_on and _TB().TOTAL_TOP_H() or 0)
end
...
function M.replaceTopbar(widget, new_topbar)
  ...
  if not widget._navbar_topbar then
    return
  end
  ...
  logger.warn(
    "simpleui: replaceTopbar could not find topbar in container — skipping"
  )
end
...
function M.wrapWithNavbar(inner_widget, active_action_id, tabs, force_no_arrows)
  ...
  local wrapper_bg = (
    SUISettings:isTrue("simpleui_navbar_transparent")
    or SUISettings:isTrue("simpleui_statusbar_transparent")
    or is_bare_bar
  )
      and nil
...
function M.applyGesturePriorityHandleEvent(target)
  if not target or target._simpleui_gesture_priority_applied then
    return
  end
...
function M.unapplyGesturePriorityHandleEvent(target)
  if not target or not target._simpleui_gesture_priority_applied then
    return
  end
...
function M.getWindowStack()
  ...
  if type(UIManager._window_stack) ~= "table" then
    logger.warn(
      "simpleui: UIManager._window_stack not available — internal API changed?"
    )
    return {}
  end
  ...
  menu = Menu:new({
    ...
    onMenuSelect = function(self_menu, item)
      if item.sub_item_table or item._sui_lazy_fn then
        ...
        self_menu.item_table_stack[#self_menu.item_table_stack + 1] =
          self_menu.item_table
        self_menu:switchItemTable(
          item.text,
          M.resolveMenuItems(item.sub_item_table)
        )
      elseif item.callback then
        local _suppress = false
        local function suppress_refresh()
          _suppress = true
        end
    ...
    onCloseWidget = function()
      ...
      if not (ok and HS and HS._instance) then
        return
      end
...
local function _WC()
  _WidgetContainer = _WidgetContainer
    or require("ui/widget/container/widgetcontainer")
...
function M.progressBar(w, pct, bar_h, fg_color, bg_color)
  ...
  local bg = bg_color
    or (ok and SUIStyle.getThemeColor("progress_bg"))
    or Blitbuffer.gray(0.15)
  local fg = fg_color
    or (ok and SUIStyle.getThemeColor("progress_fg"))
  ...
  if style == "framed" then
    ...
    local fg_bar = LineWidget():new({
      dimen = Geom():new({ w = fw, h = inner_h }),
      background = fg,
    })
...
local function _validateBI(desc)
  if type(desc) ~= "table" then
    return "descriptor must be a table"
  end
  if type(desc.id) ~= "string" or desc.id == "" then
    return "descriptor.id must be a non-empty string"
  end
  if desc.widget_name == nil and type(desc.match) ~= "function" then
    return "descriptor must provide widget_name (string) or match (function)"
  end
  if desc.widget_name ~= nil and type(desc.widget_name) ~= "string" then
    return "descriptor.widget_name must be a string"
  end
  if
    desc.active_action_id ~= nil and type(desc.active_action_id) ~= "string"
  then
    return "descriptor.active_action_id must be a string when provided"
  end
  if
    desc.get_active_action ~= nil
    and type(desc.get_active_action) ~= "function"
  then
    return "descriptor.get_active_action must be a function when provided"
  end
  if
    desc.is_pageable ~= nil
    and type(desc.is_pageable) ~= "boolean"
    and type(desc.is_pageable) ~= "function"
  then
    return "descriptor.is_pageable must be boolean or nil"
  end
  if desc.on_inject ~= nil and type(desc.on_inject) ~= "function" then
    return "descriptor.on_inject must be a function when provided"
  end
  if desc.on_close ~= nil and type(desc.on_close) ~= "function" then
    return "descriptor.on_close must be a function when provided"
  end
  return nil
end
...
function M.BarInjection.register(desc)
  ...
  if err then
    logger.warn(
      "sui_core: BarInjection.register() rejected:",
      err,
      "(id=",
      tostring(desc and desc.id),
      ")"
    )
    return
  end
