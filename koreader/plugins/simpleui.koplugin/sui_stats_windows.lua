...
local function _getFinishedBooksThisYear()
  ...
  if not ok_rh or not ReadHistory or not ReadHistory.hist then
    return books
  end
  ...
  for _, entry in ipairs(ReadHistory.hist) do
    ...
    if fp and lfs.attributes(fp, "mode") == "file" then
      ...
      if summary == nil then
        local ok_open, ds = pcall(function()
          return DocSettings:open(fp)
        end)
      ...
      then
        table.insert(books, {
          ...
          filepath = fp,
          date_finished = (
            type(summary.modified) == "string" and summary.modified
          ) or nil,
          date_started = (
            type(summary.date_started) == "string" and summary.date_started
          ) or nil,
          md5 = md5_checksum,
        })
      end
...
local function _fetchBookStatsData(book)
  ...
  if
    ...
    and DocSettings:hasSidecarFile(fp)
  then
    local ok_open, ds = pcall(function()
      return DocSettings:open(fp)
    end)
  ...
  if md5 then
    local ok_q, row = pcall(function()
      return conn:rowexec(
        string.format("SELECT id FROM book WHERE md5 = '%s' LIMIT 1", md5)
      )
    end)
    if ok_q then
      id_book = row and tonumber(row) or nil
    end
  end
  if not id_book then
    pcall(function()
  ...
  pcall(function()
    title, authors, pages, last_open, highlights, notes =
  ...
  pcall(function()
    local v = G_reader_settings
      and G_reader_settings:readSetting("statistics_max_sec")
  ...
  pcall(function()
    total_days, total_time_book, total_read_pages, book_read_time =
      conn:rowexec(string.format(
        [[
                WITH ps_agg AS (
                    SELECT page,
  ...
  pcall(function()
    first_open, last_page = conn:rowexec(string.format(
      [[
  ...
  local avg_time_per_page = (book_read_pages > 0)
      and (book_read_time / book_read_pages)
    or 0
  local avg_time_per_day = (total_days > 0) and (book_read_time / total_days)
    or 0
...
local function _getStartDatesForBooks(books)
  ...
  local db_path = Config.getStatsDbPath()
  local ok_lfs, lfs = pcall(require, "libs/libkoreader-lfs")
  if not (ok_lfs and lfs.attributes(db_path, "mode")) then
    return start_dates
  end

  local ok_conn, conn = pcall(SQ3.open, db_path)
  if not (ok_conn and conn) then
    return start_dates
  end
...
local _CenterContainer, _LeftContainer, _RightContainer, _Size, _TextBoxWidget

local function _lazyLoad()
  if _CenterContainer then
    return
  end
...
local function _writeSummaryModified(filepath, date_str)
  if not filepath or not _isValidDateStr(date_str) then
    return false
  end
  local ok_ds, DocSettings = pcall(require, "docsettings")
  if not ok_ds then
...
local function _deleteSummaryField(filepath, key)
  if not filepath or not key then
    return false
  end
  local ok_ds, DocSettings = pcall(require, "docsettings")
  if not ok_ds then
...
local function _makeDateCard(
  ...
  local function makeSideTappable(
    display_str,
    title_str,
    write_fn,
    delete_fn,
  ...
  local start_widget = makeSideTappable(
    ...
    function(fp)
      _deleteSummaryField(fp, "date_started")
    end,
    original_start_str,
    on_start_saved
  )
...
function StatsWindows.showFinishedBooksDialog(initial_page)
  ...
  local face_stat_title =
    Font:getFace(SUIStyle.FACE_REGULAR, SUIStyle.FS_SUBTITLE)
  ...
  local function _buildCellGeo(inner_w)
    if _cell_geo and _cell_geo.inner_w == inner_w then
      return _cell_geo
    end
  ...
  local function buildListScreen(ctx)
    ...
    for i, book in ipairs(books) do
      ...
      rows[#rows + 1] = SUIWindow.ListRow({
        inner_w = inner_w,
        title = book.title,
        subtitle = (book.authors and book.authors ~= "") and book.authors
          or nil,
  ...
  local function buildStatsScreen(ctx)
    ...
    local pages_read_str = string.format(
      "%d (%d%%)",
      d.total_read_pages,
      math.floor(
        100 * d.total_read_pages / (d.pages > 0 and d.pages or 1) + 0.5
      )
    )
    ...
    if fp and (d.highlights > 0 or d.notes > 0) then
      ...
      if ok_bb then
        ...
        if d.highlights > 0 then
          on_tap_highlights = openBrowser
        end
        if d.notes > 0 then
          on_tap_notes = openBrowser
        end
      end
    end
    ...
    local rows_block = FrameContainer:new({
      ...
      VerticalGroup:new({
        ...
        makeIconRow(
          SUIStyle.icon("clock"),
          _("Average per day"),
          _fmtDuration(d.avg_time_per_day)
        ),
        makeRowSep(),
        makeIconRow(
          SUIStyle.icon("page"),
          _("Average per page"),
          _fmtDuration(d.avg_time_per_page)
        ),
        makeRowSep(),
        makeIconRow(
          SUIStyle.icon("highlights"),
          _("Highlights"),
          tostring(d.highlights),
          on_tap_highlights
        ),
        makeRowSep(),
        makeIconRow(
    ...
    local original_end_str = (d.last_open and _fmtDate(d.last_open))
      or "\xe2\x80\x93"

    -- Prefer sidecar dates over raw DB timestamps.
    local date_start_str = (
      type(book.date_started) == "string" and book.date_started
    ) or original_start_str
    local date_end_str = (
      type(book.date_finished) == "string" and book.date_finished
    ) or original_end_str
    ...
    local date_widget = _makeDateCard(
      ...
      function(new_date) -- on_start_saved / on_start_reset
        ...
        book_date_range[fp] =
          _fmtDateRange(new_date, book.date_finished or date_end_str)
        ctx.repaint()
      end,
      function(new_date) -- on_end_saved / on_end_reset
        book.date_finished = (new_date ~= original_end_str) and new_date or nil
        book_date_range[fp] =
          _fmtDateRange(book.date_started or start_dates[fp], new_date)
        ctx.repaint()
      end
    )
...
local function _withStmt(conn, sql, fn)
  ...
  if not ok_prep or not stmt then
    return
  end
    ...
    _withStmt(
      ...
      function(stmt)
        for row in stmt:rows() do
          if row[1] then
            range.min_year = tonumber(row[1]) or current_year
          end
          if row[2] then
            range.max_year = tonumber(row[2]) or current_year
          end
        end
      end
    )
...
local function _riGetStreaks()
  ...
  return _withStatsDb(default, function(conn)
    ...
    local function parseDateYMD(s)
      ...
      local y, m, d =
        tonumber(s:sub(1, 4)), tonumber(s:sub(6, 7)), tonumber(s:sub(9, 10))
      if y and m and d then
        return y, m, d
      end
    end
    ...
    local function isConsecDay(prev, curr)
      ...
      local expected =
        os.date("%Y-%m-%d", os.time({ year = y, month = m, day = d }) - 86400)
    ...
    local day_streaks = computeStreak(dates, isConsecDay, function(d)
    ...
    local function getTotalWeeksInYear(y)
      return tonumber(
        os.date("%V", os.time({ year = y, month = 12, day = 28 }))
      )
    end
    ...
    local function isConsecWeek(prev_ts, curr_ts)
      ...
      if cy == py and pw == cw + 1 then
        return true
      end
      if py == cy + 1 and pw == 1 and cw == getTotalWeeksInYear(cy) then
        return true
      end
      return false
    end
...
local function _riGetThisWeekStats()
  if _ri_cache["thisweek"] then
    return _ri_cache["thisweek"]
  end
  local default = { seconds = 0, pages = 0 }
...
local function _riGetThisMonthStats()
  if _ri_cache["thismonth"] then
    return _ri_cache["thismonth"]
  end
  local default = { seconds = 0, pages = 0 }
  local r = _withStatsDb(default, function(conn)
    local s = { seconds = 0, pages = 0 }
...
local function _riGetLastWeekStats()
  if _ri_cache["lastweek"] then
    return _ri_cache["lastweek"]
  end
  local default =
    { total_seconds = 0, total_pages = 0, avg_seconds = 0, avg_pages = 0 }
  local r = _withStatsDb(default, function(conn)
    local res =
      { total_seconds = 0, total_pages = 0, avg_seconds = 0, avg_pages = 0 }
    local now_ts = os.time()
...
local function _riGetAllTimeStats()
  if _ri_cache["alltime"] then
    return _ri_cache["alltime"]
  end
...
local function _riYearHeader(inner_w, year, year_range, on_prev, on_next)
  ...
  local face_chev =
    Font:getFace(SUIStyle.FACE_ICONS, math.floor(SUIStyle.FS_TITLE * 1.8))
  local CLR_BLACK = Blitbuffer.COLOR_BLACK
...
local function _riYearlyRow(
  ...
  if mode_key == "hours" then
    ...
    if yearly_stats.duration < 60 then
      left_val = "0m"
    end
  ...
  return HorizontalGroup:new({
    ...
    sep,
    makeCell(
      _riFmtCount(yearly_stats.pages),
...
local function _riMonthlyChart(
  inner_w,
  monthly_data,
  value_key,
  selected_year,
  ...
  if not monthly_data or #monthly_data == 0 then
  ...
  local row1, row2 = {}, {}
  for i = 1, 6 do
    table.insert(row1, monthly_data[i])
  end
  for i = 7, 12 do
...
local function _buildInsightsPage2(inner_w, avail_h, streaks)
  ...
  local function make3ColRow(c1, c2, c3)
    ...
    return HorizontalGroup:new({
      ...
      _CenterContainer:new({
        ...
        LineWidget:new({
          dimen = Geom:new({ w = sep_w, h = cell_h3 - Screen:scaleBySize(32) }),
          background = CLR_BORDER,
        }),
      }),
  ...
  local alltime_row = make3ColRow(
    make3Cell(
      SUIStyle.icon("clock"),
      _riFmtCount(alltime.hours),
      _("hours read")
    ),
    make3Cell(
      SUIStyle.icon("page"),
      _riFmtCount(alltime.pages),
      _("pages read")
    ),
    make3Cell(
      SUIStyle.icon("book"),
      _riFmtCount(alltime.book_count),
      _("books finished")
    )
  )
  ...
  if thisweek_stats.seconds < 60 then
    thisweek_time_str = "0m"
  end
  ...
  local cur_streak_str =
    string.format(N_("%d day", "%d days", cur_streak_val), cur_streak_val)

  local best_streak_val = streaks.days.best
  local best_streak_str =
    string.format(N_("%d day", "%d days", best_streak_val), best_streak_val)
  local best_dates = nil
  if best_streak_val > 1 and streaks.days.best_start > 0 then
    best_dates = _riFmtDate(streaks.days.best_start)
      .. " \xe2\x80\x93 "
      .. _riFmtDate(streaks.days.best_end)
  end
  ...
  local streak_row = FrameContainer:new({
    ...
    VerticalGroup:new({
      align = "left",
      makeIconRow(
        SUIStyle.icon("calendar"),
        _("Current streak"),
        cur_streak_str
      ),
      makeRowSep(),
      makeIconRow(
        SUIStyle.icon("trophy"),
        _("Best streak"),
        best_streak_str,
        best_dates
      ),
  ...
  local cur_wstreak_str =
    string.format(N_("%d week", "%d weeks", cur_wstreak_val), cur_wstreak_val)

  local best_wstreak_val = streaks.weeks.best
  local best_wstreak_str =
    string.format(N_("%d week", "%d weeks", best_wstreak_val), best_wstreak_val)
  local best_w_dates = nil
  if best_wstreak_val > 1 and streaks.weeks.best_start > 0 then
    best_w_dates = _riFmtDate(streaks.weeks.best_start)
      .. " \xe2\x80\x93 "
      .. _riFmtDate(streaks.weeks.best_end)
  end
...
local function _riGetOrFetchYear(year)
  if _ri_cache[year] then
    return _ri_cache[year]
  end
  local entry = {
...
function StatsWindows.showLoadingNotice()
  ...
  if
    ok_ss
    and SUISettings
    and not SUISettings:nilOrTrue("simpleui_stats_loading_notice")
  then
    return
  end
  local ok_im, InfoMessage = pcall(require, "ui/widget/infomessage")
  if not ok_im or not InfoMessage then
    return
  end
  UIManager:show(InfoMessage:new({
...
function StatsWindows.showReadingInsightsWindow()
  ...
  local function buildRootScreen(ctx)
    ...
    local cell_h2 = math.max(
      Screen:scaleBySize(70),
    ...
    local function makeTodayCard(icon_char, value, label)
      local face_today_val =
        Font:getFace(SUIStyle.FACE_REGULAR, math.floor(SUIStyle.FS_TITLE * 1.6))
      local face_lbl = Font:getFace(SUIStyle.FACE_REGULAR, SUIStyle.FS_DETAIL)
      ...
      local icon_w = TextWidget:new({
        ...
        face = Font:getFace(
          SUIStyle.FACE_ICONS,
          math.floor(SUIStyle.FS_TITLE * 1.6)
        ),
        fgcolor = CLR_BLACK,
      })
    ...
    local today_row = HorizontalGroup:new({
      align = "center",
    ...
    local year_header =
      _riYearHeader(inner_w, selected_year, year_range, goPrev, goNext)
    ...
    local yearly_row =
      _riYearlyRow(inner_w, yearly, _ri_mode_key, toggleMode, avail_h)

    -- ── Section 3: monthly chart ──────────────────────────────────
    local value_key = _ri_mode_key == "hours" and "hours" or "days"
    local chart =
      _riMonthlyChart(inner_w, monthly, value_key, selected_year, avail_h)
    ...
    local alltime_block, week_row, thisweek_row, month_row, streak_row, wstreak_row =
      _buildInsightsPage2(inner_w, avail_h, streaks)
  ...
  local win = SUIWindow:new({
    ...
    height = math.floor(Screen:getHeight() * 0.75),
    position = "bottom",
    navpager_mode = Config.isNavpagerEnabled and Config.isNavpagerEnabled()
      or false,
    screens = {
...
function StatsWindows.showBookStatsFromFile(filepath)
  ...
  if
    ...
    and DocSettings
    and DocSettings.hasSidecarFile
    and DocSettings:hasSidecarFile(filepath)
  then
    local ok_open, ds = pcall(function()
      return DocSettings:open(filepath)
    end)
