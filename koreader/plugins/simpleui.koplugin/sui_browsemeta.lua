...
local function _encVal(v)
  return v:gsub("/", PATH_SEP_ESC)
end
local function _decVal(v)
  return v:gsub(PATH_SEP_ESC, "/")
end
...
local function _clearCaches()
  ...
  for k in pairs(_meta_values_cache) do
    _meta_values_cache[k] = nil
  end
  for k in pairs(_matching_files_cache) do
    _matching_files_cache[k] = nil
  end
  for k in pairs(_repr_file_cache) do
    _repr_file_cache[k] = nil
  end
  for k in pairs(_author_count_cache) do
    _author_count_cache[k] = nil
  end
  for k in pairs(_calibre_index_cache) do
    _calibre_index_cache[k] = nil
  end
end
...
local function _leafPath(base_dir, dim_key, value)
  local v_enc = (value == false or value == nil) and NULL_MARKER
    or _encVal(value)
...
local function _loadCalibreIndex(dir)
  ...
  if cached ~= nil then
    return cached
  end -- false = already tried, nothing found
  ...
  if
    not ok_cm
    or not CalibreMetadata
    or type(CalibreMetadata.loadBookList) ~= "function"
  then
  ...
  for _, book in ipairs(cm.books) do
    ...
    if lpath and type(lpath) == "string" then
      ...
      if type(series_index) ~= "number" then
        series_index = nil
      end
  ...
  if next(index) then
    ...
    logger.dbg(
      "sui_browsemeta: calibre index loaded for",
      dir,
      "(",
      n,
      "entries)"
...
local function _getMatchingFiles(base_dir, filters)
  ...
  if cal_index then
    ...
    for _, row in ipairs(results) do
      ...
      if cal then
        if not row.authors then
          row.authors = cal.authors
        end
        if not row.series then
          row.series = cal.series
        end
        if not row.series_index then
          row.series_index = cal.series_index
        end
        if not row.title then
          row.title = cal.title
        end
        if not row.keywords then
          row.keywords = cal.keywords
        end
      end
    end
...
local function _getMetadataValues(base_dir, dim_key)
  ...
  table.sort(out, function(a, b)
    ...
    if not av or av == false or av == "" then
      return false
    end
    if not bv or bv == false or bv == "" then
      return true
    end
...
local function _getVirtualList(fc, path, collate)
  ...
  if not base_dir or not dim_key then
    return dirs, files
  end
  _ensureCacheBaseDir(base_dir)
...
function M.getCurrentMode(fc)
  ...
  if not path or not _isVirtual(path) then
    return "normal"
  end
...
function M.getAuthorBookCount(fc, author_name)
  if not fc or not author_name or author_name == "" then
    return 0
  end
  local base = _baseDir(fc.path)
  if not base then
...
function M.navigateToAuthorLeaf(fm, author_name, origin_file)
  if not fm or not author_name or author_name == "" then
    return
  end
...
local function _createCollectionFromVirtualFolder(vpath, fc) -- luacheck: ignore fc
  ...
  if not base_dir or not dim_key or filter_value == nil then
    return
  end
...
local function _openVirtualCoverPicker(vpath, fc)
  ...
  if #books == 0 then
    UIManager:show(
      InfoMessage:new({ text = _("No books found."), timeout = 2 })
    )
    return
  end
  ...
  buttons[#buttons + 1] = {
    {
      text = (not cur_override and "\u{2713} " or "  ")
        .. _("Auto (first book)"),
      callback = function()
  ...
  if not _patched_genit then
    ...
    FileChooser.genItemTable = function(fc, dirs, fls, path)
      ...
      for _, d in ipairs(dirs) do
        item_table[#item_table + 1] = d
      end
      for _, f in ipairs(fls) do
        item_table[#item_table + 1] = f
      end
      ...
      if not hide_up and path ~= "/" then
        table.insert(item_table, 1, {
          text = BD.mirroredUILayout() and BD.ltr("../ \u{2B06}")
            or "\u{2B06} ../",
          path = up_path,
          is_go_up = true,
        })
        ...
        dialog = ButtonDialog:new({
          ...
          buttons = (function()
            ...
            local effective_style = (ok_fc_mod and FC and FC.resolveStyle)
                and FC.resolveStyle(fc, item.path, item)
              or "single"
...
local function _removePatches()
  ...
  if not ok_fc or not FileChooser then
    return
  end
...
local function _installFMSafetyPatches()
  ...
  FM.createFolder = function(fm)
    if fm.file_chooser and _isVirtual(fm.file_chooser.path) then
      return
    end
...
local function _removeFMSafetyPatches()
  if not _patched_fm_safety then
    return
  end
  local FM = _getFileManager()
  if FM then
    if _orig_createFolder then
      FM.createFolder = _orig_createFolder
    end
    if _orig_setHome then
      FM.setHome = _orig_setHome
    end
  end
...
local function _getVirtualSubtitle(path)
  ...
  if dim_key then
    return DIMS[dim_key].label
  end
  return nil
end

local function _installTitleBarPathPatch()
  if _patched_tb_path then
    return
  end
  local FM = _getFileManager()
  if not FM then
