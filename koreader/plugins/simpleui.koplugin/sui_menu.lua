...
local function ConfirmBox()
  return require("ui/widget/confirmbox")
end
local function InputDialog()
  return require("ui/widget/inputdialog")
end
local function MultiInputDialog()
  return require("ui/widget/multiinputdialog")
end
local function PathChooser()
  return require("ui/widget/pathchooser")
end
local function SortWidget()
  return require("ui/widget/sortwidget")
end
      ...
      if lfs_ok and iw_ok and iw then
        ...
        if type(iw_init) == "function" then
          ...
          for i = 1, 64 do
            ...
            if icons_path and icons_dirs then
              break
            end
          end
          if icons_path then
        ...
        if not injected_path and not injected_dir and icon_exists then
          ...
          iw.init = function(self_iw, ...)
            if
              self_iw.icon == "simpleui_settings"
              and not self_iw.file
              and not self_iw.image
            then
          ...
          logger.info(
            "simpleui: icon registered via IconWidget.init patch (fallback)"
          )
        end
      end
    end
  ...
  SimpleUIPlugin.addToMainMenu = function(self, menu_items)
    ...
    local function modeItem(label, mode_value)
      return {
        ...
        checked_func = function()
          return Config.getNavbarMode() == mode_value
        end,
        callback = function()
    ...
    local function makePositionMenu(pos)
      ...
      local function getTabs()
        if not cached_tabs then
          cached_tabs = loadTabConfig()
        end
        return cached_tabs
      end
      ...
      local function getResolvedLabel(id)
        if not cached_labels[id] then
          ...
          elseif id == "home" then
            ...
            cached_labels[id] = (ACTION_BY_ID[id] and ACTION_BY_ID[id].label)
              or id
          end
        end
      ...
      for _i, action in ipairs(ALL_ACTIONS) do
        if actionAvailable(action.id) then
          pool[#pool + 1] = action.id
        end
      end
    ...
    local function getActionLabel(id)
      ...
      if id:match("^custom_qa_%d+$") then
        return getCustomQAConfig(id).label
      end
      if id == "home" then
        return _homeLabel()
      end
    ...
    local function makeTabsMenu(ctx_menu)
        ...
        callback = function()
          ...
          for _i, tid in ipairs(tabs) do
            sort_items[#sort_items + 1] =
              { text = getActionLabel(tid), orig_item = tid }
          end
          local function on_save()
            local new_tabs = {}
            for _i, item in ipairs(sort_items) do
              new_tabs[#new_tabs + 1] = item.orig_item
            end
      ...
      for _i, action in ipairs(ALL_ACTIONS) do
        if actionAvailable(action.id) then
          action_pool[#action_pool + 1] = action.id
        end
      end
      for _i, qa_id in ipairs(getCustomQAList()) do
      ...
      for _i, aid in ipairs(action_pool) do
        ...
        toggle_items[#toggle_items + 1] = {
          ...
          text_func = function()
            ...
            if rem <= 2 then
              return _base_label
                .. string.format(N_("  (%d left)", "  (%d left)", rem), rem)
            end
            return _base_label
          end,
          checked_func = function()
            ...
            if active_pos then
              ...
              if #tabs >= limit then
                UIManager:show(InfoMessage():new({
                  text = string.format(
                    N_(
                      "The maximum of %d tab has been reached. Remove one first.",
                      "The maximum of %d tabs has been reached. Remove one first.",
                      limit
                    ),
                    limit
                  ),
      ...
      table.sort(toggle_items, function(a, b)
        return a._base:lower() < b._base:lower()
      end)
      for _i, item in ipairs(toggle_items) do
        items[#items + 1] = item
      end
      return items
    end
    ...
    local function makePaginationBarMenu()
      ...
      return {
          ...
          sub_item_table = {
            {
              ...
              radio = true,
              checked_func = function()
                return getGeral() == "predefinido"
              end,
              callback = function()
                if getGeral() == "predefinido" then
                  return
                end
                setGeral("predefinido")
                restartPrompt(
                  _("Pagination bar set to Default.\n\nRestart now?")
                )
              end,
            },
            {
              ...
              radio = true,
              checked_func = function()
                return getGeral() == "navpager"
              end,
              help_text = _(
                "Replaces the pagination bar with Prev/Next arrows at the edges of the bottom bar.\nThe arrows dim when there is no previous or next page.\nWith navpager active, as few as 1 tab and at most 4 tabs can be configured."
              ),
              callback = function()
                if getGeral() == "navpager" then
                  return
                end
                setGeral("navpager")
            ...
            },
            {
              ...
              radio = true,
              checked_func = function()
                return getGeral() == "oculto"
              end,
              callback = function()
                if getGeral() == "oculto" then
                  return
                end
                setGeral("oculto")
          ...
          sub_item_table = {
            {
              ...
              checked_func = function()
                ...
                return not SUISettings:isTrue("simpleui_hs_pagination_hidden")
                  and (
                    SUISettings:nilOrTrue("simpleui_bar_dotpager_always")
              ...
              callback = function()
                ...
                if
                  not SUISettings:nilOrTrue("simpleui_bar_dotpager_always")
                then
                  SUISettings:saveSetting("simpleui_bar_dotpager_always", true)
                end
                plugin:_scheduleRebuild()
            ...
            },
            {
              ...
              callback = function()
                if SUISettings:isTrue("simpleui_hs_pagination_hidden") then
                  return
                end
          ...
          sub_item_table = {
            {
              ...
              checked_func = function()
                return (
                  SUISettings:readSetting("simpleui_bar_pagination_size") or "s"
                ) == "xs"
              end,
              callback = function()
        ...
        -- ── Number of pages in the title bar ──────────────────────────────
        {
          ...
          help_text = _(
            'Shows "Page X of Y" in the title bar subtitle when browsing the library, history or collections.\nNavpager enables this automatically.\nNot available when Navpager is active.'
          ),
          callback = function()
            local on =
              SUISettings:isTrue("simpleui_bar_pagination_show_subtitle")
            SUISettings:saveSetting(
              "simpleui_bar_pagination_show_subtitle",
              not on
            )
            plugin:_scheduleRebuild()
          end,
    ...
    local function makeTopbarItemsMenu()
                  ...
                  },
                  {
                    ...
                    callback = function()
                      ...
                      if text ~= "" then
                        ...
                        if not cfg.order_center then
                          cfg.order_center = {}
                        end
                        if (cfg.side[k] or "hidden") == "hidden" then
                          cfg.side[k] = "right"
                          local new_left, new_center = {}, {}
                          for _, v in ipairs(cfg.order_left) do
                            if v ~= k then
                              new_left[#new_left + 1] = v
                            end
                          end
                          cfg.order_left = new_left
                          for _, v in ipairs(cfg.order_center) do
                            if v ~= k then
                              new_center[#new_center + 1] = v
                            end
                          end
                          cfg.order_center = new_center
                          local found = false
                          for _i, v in ipairs(cfg.order_right) do
                            if v == k then
                              found = true
                              break
                            end
                          ...
                          if not found then
                            cfg.order_right[#cfg.order_right + 1] = k
                          end
        ...
        callback = function()
          ...
          for _i, key in ipairs(cfg.order_left) do
            if cfg.side[key] ~= "hidden" then
              sort_items[#sort_items + 1] =
                { text = TOPBAR_ITEM_LABEL(key), orig_item = key }
            end
          end
          sort_items[#sort_items + 1] = {
            text = _("Center"):upper(),
            orig_item = SEP_CENTER,
            is_divider = true,
          }
          ...
          local function on_save()
            ...
            for j, item in ipairs(sort_items) do
              if item.orig_item == SEP_LEFT then
                sep_left_pos = j
              end
              if item.orig_item == SEP_CENTER then
                sep_center_pos = j
              end
              if item.orig_item == SEP_RIGHT then
                sep_right_pos = j
              end
            end
            if
              not sep_left_pos
              or not sep_center_pos
              or not sep_right_pos
              or sep_left_pos > sep_center_pos
              or sep_center_pos > sep_right_pos
              or (sort_items[1] and sort_items[1].orig_item ~= SEP_LEFT)
            then
            ...
            for _i, item in ipairs(sort_items) do
              if item.orig_item == SEP_LEFT then
                current_side = "left"
              elseif item.orig_item == SEP_CENTER then
                current_side = "center"
              elseif item.orig_item == SEP_RIGHT then
                current_side = "right"
              elseif current_side == "left" then
                new_left[#new_left + 1] = item.orig_item
                cfg.side[item.orig_item] = "left"
              elseif current_side == "center" then
                new_center[#new_center + 1] = item.orig_item
                cfg.side[item.orig_item] = "center"
            ...
            for _i, key in ipairs(cfg.order_left) do
              if cfg.side[key] == "hidden" then
                new_left[#new_left + 1] = key
              end
            end
            for _i, key in ipairs(cfg.order_center) do
              if cfg.side[key] == "hidden" then
                new_center[#new_center + 1] = key
              end
            end
            for _i, key in ipairs(cfg.order_right) do
              if cfg.side[key] == "hidden" then
                new_right[#new_right + 1] = key
              end
            end
            cfg.order_left = new_left
            cfg.order_center = new_center
      ...
      local sorted_keys = {}
      for _i, k in ipairs(TOPBAR_ITEMS) do
        sorted_keys[#sorted_keys + 1] = k
      end
      table.sort(sorted_keys, function(a, b)
        return TOPBAR_ITEM_LABEL(a):lower() < TOPBAR_ITEM_LABEL(b):lower()
      end)
      ...
      for _i, key in ipairs(sorted_keys) do
        ...
        items[#items + 1] = {
          text_func = function()
            ...
            if side == "left" then
              return label .. "  \xe2\x97\x82"
            elseif side == "center" then
              return label .. "  \xe2\x97\x86"
            elseif side == "right" then
              return label .. "  \xe2\x96\xb8"
            else
              return label
            end
          end,
          ...
          checked_func = function()
            return (Config.getTopbarConfigCached().side[k] or "hidden")
              ~= "hidden"
          end,
          keep_menu_open = true,
          callback = function()
            ...
            if (cfg.side[k] or "hidden") == "hidden" then
              ...
              for _i, v in ipairs(cfg.order_left) do
                if v == k then
                  last_side = "left"
                  break
                end
              end
              for _i, v in ipairs(cfg.order_center) do
                if v == k then
                  last_side = "center"
                  break
                end
              ...
              if last_side == "left" then
                ...
                if not found then
                  cfg.order_left[#cfg.order_left + 1] = k
                end
              elseif last_side == "center" then
                ...
                if not found then
                  cfg.order_center[#cfg.order_center + 1] = k
                end
              else
                local found = false
                for _i, v in ipairs(cfg.order_right) do
      ...
      return {
        {
          ...
          checked_func = function()
            return SUISettings:nilOrTrue("simpleui_topbar_enabled")
          end,
          keep_menu_open = true,
          callback = function()
            ...
            UIManager:show(ConfirmBox():new({
              text = string.format(
                _("Status Bar will be %s after restart.\n\nRestart now?"),
        ...
        },
        {
          ...
          sub_item_table_func = function()
            return makeTopbarItemsMenu(ctx_menu)
          end,
          sui_build = ctx_menu
              and ctx_menu.is_sui
                ...
                return SUIWindow.ListRow({
                  ...
                  subtitle = function()
                    ...
                    for _, key in ipairs(cfg.order_left) do
                      if (cfg.side[key] or "hidden") ~= "hidden" then
                        names[#names + 1] = Config.TOPBAR_ITEM_LABEL(key)
                      end
                    end
                    for _, key in ipairs(cfg.order_center) do
                      if (cfg.side[key] or "hidden") == "center" then
                        names[#names + 1] = Config.TOPBAR_ITEM_LABEL(key)
                      end
                    end
                    for _, key in ipairs(cfg.order_right) do
                      if (cfg.side[key] or "hidden") == "right" then
                        names[#names + 1] = Config.TOPBAR_ITEM_LABEL(key)
                      end
                    end
                    return #names > 0 and table.concat(names, "  ·  ")
                      or _("No items selected.")
                  end,
                  inner_w = ctx.inner_w,
                  show_chevron = true,
                  on_tap = function()
                          ...
                          if (cfg2.side[k] or "hidden") == "hidden" then
                            ...
                            picker_items[#picker_items + 1] = {
                              ...
                              on_tap = function(picker_ctx)
                                cfg2.side[_k] = "right"
                                local new_left, new_center = {}, {}
                                for _, v in ipairs(cfg2.order_left) do
                                  if v ~= _k then
                                    new_left[#new_left + 1] = v
                                  end
                                end
                                cfg2.order_left = new_left
                                for _, v in ipairs(cfg2.order_center) do
                                  if v ~= _k then
                                    new_center[#new_center + 1] = v
                                  end
                                end
                                ...
                                local found = false
                                for _, v in ipairs(cfg2.order_right) do
                                  if v == _k then
                                    found = true
                                    break
                                  end
                                ...
                                if not found then
                                  cfg2.order_right[#cfg2.order_right + 1] = _k
                                end
        ...
        },
        {
          ...
          value_func = function()
            return Config.getTopbarSizePct() .. "%"
          end,
          keep_menu_open = true,
          callback = function()
        ...
        },
        {
          ...
          keep_menu_open = true,
          checked_func = function()
            return SUISettings:nilOrTrue("simpleui_topbar_swipe_indicator")
          end,
          callback = function()
            SUISettings:saveSetting(
              "simpleui_topbar_swipe_indicator",
        ...
        },
        {
          ...
          keep_menu_open = true,
          checked_func = function()
            return Config.getWifiHideWhenOff()
          end,
          callback = function()
      ...
      return {
        {
          ...
          checked_func = function()
            return SUISettings:nilOrTrue("simpleui_bar_enabled")
          end,
          keep_menu_open = true,
          callback = function()
            ...
            UIManager:show(ConfirmBox():new({
              text = string.format(
                _("Navigation Bar will be %s after restart.\n\nRestart now?"),
                on and _("disabled") or _("enabled")
              ),
              ok_text = _("Restart"),
              cancel_text = _("Later"),
          ...
          sui_build = ctx_menu
              ...
              and function(ctx, _item)
                ...
                return SUIWindow.ListRow({
                  ...
                  inner_w = ctx.inner_w,
                  item_count = function()
                    return #loadTabConfig()
                  end,
                  max_items = function()
                    return Config.effectiveMaxTabs()
                  end,
                  show_chevron = true,
                  on_tap = function()
                    local tabs = loadTabConfig()
                        ...
                        if #sort_items > min_tabs then
                          item.on_delete = function()
                            ...
                            for j, v in ipairs(sort_items) do
                              if v.orig_item == _tid then
                                table.remove(sort_items, j)
                                break
                              end
                            end
                          ...
                          if not is_sel then
                            ...
                            picker_items[#picker_items + 1] = {
                              ...
                              on_tap = function(picker_ctx)
                                ...
                                local limit = Config.effectiveMaxTabs()
                                if #cur >= limit then
                                  local InfoMessage = ctx_menu
                                      and ctx_menu.InfoMessage
                                    or require("ui/widget/infomessage")
                                  local uim = ctx_menu and ctx_menu.UIManager
                                    or require("ui/uimanager")
                                  local N_ = ctx_menu and ctx_menu.N_
                                    or require("sui_i18n").ngettext
                                  uim:show(InfoMessage:new({
                                    text = string.format(
                                      N_(
                                        "The maximum of %d tab has been reached. Remove one first.",
          ...
          sub_item_table = {
            {
              ...
              radio = true,
              checked_func = function()
                return require("sui_bottombar").getBarStyle() == "default"
              end,
              keep_menu_open = true,
              callback = function()
            ...
            },
            {
              ...
              radio = true,
              checked_func = function()
                return require("sui_bottombar").getBarStyle() == "framed"
              end,
              keep_menu_open = true,
              callback = function()
            ...
            },
            {
              ...
              radio = true,
              checked_func = function()
                return require("sui_bottombar").getBarStyle() == "bare"
              end,
              keep_menu_open = true,
              callback = function()
        ...
        Config.makeScaleItem({
          ...
          get = function()
            return Config.getIconScalePct()
          end,
          set = function(pct)
            Config.setIconScalePct(pct)
          end,
          refresh = function()
            UI.invalidateDimCache()
        ...
        Config.makeScaleItem({
          ...
          get = function()
            return Config.getNavbarLabelScalePct()
          end,
          set = function(pct)
            Config.setNavbarLabelScalePct(pct)
          end,
          refresh = function()
            UI.invalidateDimCache()
            plugin:_rebuildAllNavbars()
        ...
        Config.makeScaleItem({
          ...
          get = function()
            return Config.getBottomMarginPct()
          end,
          set = function(pct)
    ...
    local function makeTitleBarItemsForCtx(ctx)
      ...
      for _i, item in ipairs(Titlebar.ITEMS) do
        if item.ctx == ctx then
          ...
          items[#items + 1] = {
            text_func = function()
              local state = Titlebar.isItemVisible(item_id) and _("On")
                or _("Off")
              return item_label() .. " — " .. state
            end,
            checked_func = function()
              return Titlebar.isItemVisible(item_id)
            end,
            enabled_func = function()
            ...
            keep_menu_open = true,
            callback = function()
              Titlebar.setItemVisible(
                item_id,
                not Titlebar.isItemVisible(item_id)
              )
    ...
    local function makeTitleBarArrangeMenu(ctx_menu, ctx, cfg_getter, cfg_saver)
      ...
      local function on_save()
        ...
        for _i, id in ipairs(cfg.order_left) do
          if not Titlebar.isItemVisible(id) then
    ...
    local function makeTitleBarSUIBuild(
      ctx_menu,
      title_str,
      tb_ctx,
      cfg_getter,
                        ...
                        if not Titlebar.isItemVisible(item.id) then
                          ...
                          picker_items[#picker_items + 1] = {
                            ...
                            on_tap = function(picker_ctx)
                              ...
                              cfg.order_left = new_left
                              local found = false
                              for _, v in ipairs(cfg.order_right) do
                                if v == _id then
                                  found = true
                                  break
                                end
                              ...
                              if not found then
                                cfg.order_right[#cfg.order_right + 1] = _id
                              end
                              cfg_saver(cfg)
                              _reapplyAllTitlebars()
                              picker_ctx.pop()
                    ...
                    for _i, item in ipairs(Titlebar.ITEMS) do
                      if item.ctx == tb_ctx and item.no_side then
                        ...
                        result_items[#result_items + 1] = {
                          text_func = function()
                            local state = Titlebar.isItemVisible(item_id)
                                and _("On")
                              or _("Off")
                            return item_label() .. " — " .. state
                          end,
                        ...
                        return SUIWindow.ArrangeList({
                          ...
                          on_change = function(items_to_save)
                            ...
                            for _i, sitem in ipairs(items_to_save) do
                              if sitem.orig_item == SEP_LEFT then
                                current_side = "left"
                              elseif sitem.orig_item == SEP_RIGHT then
                                current_side = "right"
                              elseif current_side == "left" then
                                new_left[#new_left + 1] = sitem.orig_item
                                cfg2.side[sitem.orig_item] = "left"
                              elseif current_side == "right" then
                                new_right[#new_right + 1] = sitem.orig_item
                                cfg2.side[sitem.orig_item] = "right"
                              end
                            end
                            for _i, key in ipairs(cfg2.order_left) do
                              if not Titlebar.isItemVisible(key) then
                                new_left[#new_left + 1] = key
                              end
                            end
                            for _i, key in ipairs(cfg2.order_right) do
                              if not Titlebar.isItemVisible(key) then
                                new_right[#new_right + 1] = key
                              end
                            end
    ...
    local function makeTitleBarFMMenu(ctx_menu)
      ...
      items[#items + 1] = {
        ...
        keep_menu_open = true,
        callback = function()
          makeTitleBarArrangeMenu(
            ctx_menu,
            "fm",
            Titlebar.getFMConfig,
            Titlebar.saveFMConfig
          )
        end,
      }
      return items
      ...
      items[#items + 1] = {
        ...
        callback = function()
          makeTitleBarArrangeMenu(
            ctx_menu,
            "sub",
            Titlebar.getSubConfig,
            Titlebar.saveSubConfig
          )
        end,
      }
      return items
    ...
    local function makeTitleBarMenu(ctx_menu)
      local function sizeItem(label, key)
        return {
          ...
          keep_menu_open = true,
          checked_func = function()
            return require("sui_titlebar").getSizeKey() == key
          end,
          callback = function()
      ...
      return {
        {
          ...
          checked_func = function()
            return require("sui_titlebar").isEnabled()
          end,
          separator = true,
          callback = function()
            local Titlebar = require("sui_titlebar")
        ...
        },
        {
          ...
          sub_item_table_func = function()
            return makeTitleBarFMMenu(ctx_menu)
          end,
          sui_build = makeTitleBarSUIBuild(
            ctx_menu,
            _("Library Buttons"),
            "fm",
            require("sui_titlebar").getFMConfig,
            require("sui_titlebar").saveFMConfig
          ),
        },
        {
          ...
          sub_item_table_func = function()
            return makeTitleBarSubMenu(ctx_menu)
          end,
          sui_build = makeTitleBarSUIBuild(
            ctx_menu,
            _("Sub-page Buttons"),
            "sub",
            require("sui_titlebar").getSubConfig,
            require("sui_titlebar").saveSubConfig
          ),
        },
        {
          ...
          sui_build = function(ctx)
            return require("sui_window").SectionLabel({
              text = _("Appearance"):upper(),
              inner_w = ctx.inner_w,
            })
          end,
    ...
    self._goalTapCallback = function()
      ...
      if ok_rg and RG then
        RG.showAnnualGoalDialog(function()
          refreshHomescreen()
        end)
      end
    end
    ...
    local function getQAPool()
      ...
      for _i, qa_id in ipairs(getCustomQAList()) do
        local _qid = qa_id
        available[#available + 1] =
          { id = _qid, label = getCustomQAConfig(_qid).label }
      end
      return available
    end
    ...
    local function makeQAMenu(ctx, slot_n)
      ...
      local function getItems()
        return SUISettings:readSetting(items_key) or {}
      end
      local function isSelected(id)
        for _i, v in ipairs(getItems()) do
          if v == id then
            return true
          end
        end
      ...
      local items_sub = {}
      local sorted_pool = {}
      for _i, a in ipairs(getQAPool()) do
        sorted_pool[#sorted_pool + 1] = a
      end
      table.sort(sorted_pool, function(a, b)
      ...
      items_sub[#items_sub + 1] = {
        ...
        callback = function()
          ...
          local pool_labels = {}
          for _i, a in ipairs(getQAPool()) do
            pool_labels[a.id] = a.label
          end
          local sort_items = {}
          for _i, id in ipairs(qa_ids) do
            sort_items[#sort_items + 1] =
              { text = pool_labels[id] or id, orig_item = id }
          end
          UIManager:show(SortWidget():new({
      ...
      return {
        {
          text = _("Hide Label"),
          checked_func = function()
            return not SUISettings:nilOrTrue(labels_key)
          end,
          keep_menu_open = true,
          separator = true,
          callback = function()
            SUISettings:saveSetting(
              labels_key,
              not SUISettings:nilOrTrue(labels_key)
            )
            ctx.refresh()
          end,
        },
              ...
              and function(ctx2, _item)
                ...
                return SUIWindow.ListRow({
                  ...
                  subtitle = function()
                    ...
                    local pool_labels = {}
                    for _, a in ipairs(getQAPool()) do
                      pool_labels[a.id] = a.label
                    end
                    local names = {}
                  ...
                  on_tap = function()
                    ...
                    ctx2.push("arrange", {
                      ...
                      footer_action = function(ctx3)
                        ...
                        local sorted_pool2 = {}
                        for _, a in ipairs(getQAPool()) do
                          sorted_pool2[#sorted_pool2 + 1] = a
                        end
                        table.sort(sorted_pool2, function(a, b)
                          return a.label:lower() < b.label:lower()
                        end)
                            ...
                            picker_items[#picker_items + 1] = {
                              ...
                              on_tap = function(picker_ctx)
                                ...
                                if #cur >= MAX_QA_ITEMS then
                                  local InfoMessage = ctx.InfoMessage
                                    or require("ui/widget/infomessage")
                                  local uim = ctx.UIManager
                                    or require("ui/uimanager")
                                  local N_ = ctx.N_
                                    or require("sui_i18n").ngettext
                                  uim:show(InfoMessage:new({
    ...
    local function makeModulesMenu(ctx)
      ...
      local function loadOrder()
        ...
        local seen = {}
        local result = {}
        for _loop_, v in ipairs(saved) do
          seen[v] = true
          result[#result + 1] = v
        end
        for _loop_, v in ipairs(default) do
          if not seen[v] then
            result[#result + 1] = v
          end
        end
      ...
      local function makeToggleItem(mod)
        ...
        return {
          ...
          checked_func = function()
            return Registry.isEnabled(_mod, ctx.pfx)
          end,
          keep_menu_open = true,
          callback = function()
        ...
        for _loop_, mod in ipairs(Registry.list()) do
          if type(mod.getMenuItems) == "function" then
            ...
            local text_fn = function()
              ...
              return count_lbl and (_(_mod.name) .. "  " .. count_lbl) -- FIX: Force translation
            ...
            if _mod.id:match("_row_") then
              qa_items[#qa_items + 1] = {
                text_func = text_fn,
                sub_item_table_func = function()
                  return _mod.getMenuItems(ctx_menu)
                end,
              }
            else
              items[#items + 1] = {
      ...
      return {
        {
          ...
          callback = function()
            ...
            HS.styleSetWallpaperEnabled(
              not SUISettings:isTrue("simpleui_style_wallpaper_enabled")
            )
          ...
          sub_item_table_func = function()
            ...
            for _, wp in ipairs(wps) do
              ...
              items[#items + 1] = {
                ...
                radio = true,
                checked_func = function()
                  return HS.styleGetWallpaper() == _wp.path
                end,
                keep_menu_open = true,
                callback = function()
            ...
            if #items == 0 then
              items[#items + 1] =
                { text = _("No wallpapers found."), enabled = false }
            end
            items[#items + 1] = {
              text = _("Place images in:"),
              enabled = false,
              separator = true,
            }
            items[#items + 1] =
              { text = HS.styleGetWallpapersDir(), enabled = false }
            return items
          end,
        },
        ...
        -- Transparent navigation bar (active only when wallpaper is enabled and selected)
        {
          ...
          checked_func = function()
            if Bottombar.getBarStyle() == "bare" then
              return true
            end
            local HS = require("sui_homescreen")
        ...
        },
        {
          ...
          checked_func = function()
            return require("sui_homescreen").styleGetWallpaperStretch()
          end,
          keep_menu_open = true,
          callback = function()
            local HS = require("sui_homescreen")
        ...
        },
        {
          ...
          checked_func = function()
            return require("sui_homescreen").styleGetWallpaperAutoRotate()
          end,
          keep_menu_open = true,
          callback = function()
            local HS = require("sui_homescreen")
        ...
        },
        {
          ...
          checked_func = function()
            return require("sui_homescreen").styleGetWallpaperInvertNight()
          end,
          keep_menu_open = true,
          callback = function()
            local HS = require("sui_homescreen")
            HS.styleSetWallpaperInvertNight(
              not HS.styleGetWallpaperInvertNight()
            )
            _applyFullLayoutRefresh()
          end,
        },
      ...
      return {
        {
          ...
          checked_func = function()
            return G_reader_settings:readSetting("start_with")
              == "homescreen_simpleui"
          end,
          keep_menu_open = true,
          callback = function()
            local is_hs = G_reader_settings:readSetting("start_with")
              == "homescreen_simpleui"
            G_reader_settings:saveSetting(
              "start_with",
              is_hs and "filemanager" or "homescreen_simpleui"
            )
          end,
        },
        {
          ...
          sub_item_table = {
            ...
            },
            {
              ...
              value_func = function()
                return Config.getModuleScalePct() .. "%"
              end,
              keep_menu_open = true,
              callback = function()
                local SpinWidget = require("ui/widget/spinwidget")
                UIManager:show(SpinWidget:new({
                  ...
                  callback = function(spin)
                    ...
                    local HS = package.loaded["sui_homescreen"]
                    if HS and HS.invalidateLabelCache then
                      HS.invalidateLabelCache()
                    end
                    _applyFullLayoutRefresh()
              ...
              callback = function()
                if Config.isScaleLinked() then
                  ...
                  UIManager_:show(InfoMessage:new({
                    text = _(
                      'Disable "Lock Scale" first to set a custom label scale.'
                    ),
                    timeout = 3,
                  }))
                  return
            ...
            },
            {
              ...
              callback = function()
                ...
                UIManager:show(ConfirmBox:new({
                  text = _(
                    "Reset all scales to default (100%)? This cannot be undone."
                  ),
                  ok_text = _("Reset"),
                  ok_callback = function()
        ...
        },
        {
          ...
          help_text = _(
            "When closing a book from the Home Screen, go back to the folder where the book is located instead of the Library home folder."
          ),
          checked_func = function()
        ...
        },
        {
          ...
          help_text = _(
            'Show a brief "Closing book…" notice when closing a book, preventing accidental double-taps while e-ink refreshes.'
          ),
          sub_item_table = {
            {
              text = _("Always"),
              ...
              checked_func = function()
                local mode =
                  SUISettings:readSetting("simpleui_hs_closing_notice_mode")
                if not mode then
              ...
              callback = function()
                SUISettings:saveSetting(
                  "simpleui_hs_closing_notice_mode",
                  "always"
                )
              end,
            },
            {
              ...
              keep_menu_open = true,
              checked_func = function()
                return SUISettings:readSetting(
                  "simpleui_hs_closing_notice_mode"
                ) == "gesture_only"
              end,
              callback = function()
            ...
            },
            {
              ...
              checked_func = function()
                local mode =
                  SUISettings:readSetting("simpleui_hs_closing_notice_mode")
                if not mode then
                  return not SUISettings:nilOrTrue("simpleui_hs_closing_notice")
                end
                return mode == "never"
              end,
              callback = function()
        ...
        },
        {
          ...
          help_text = _(
            'Show a brief "Loading statistics\xe2\x80\xa6" notice when opening a statistics window, preventing accidental double-taps while e-ink refreshes.'
          ),
          checked_func = function()
        ...
        },
        {
          ...
          help_text = _(
            "Warn when modules on a page exceed the available screen height."
          ),
          checked_func = function()
        ...
        },
        {
          text = _("Settings on Long Tap"),
          help_text = _(
            "When enabled, long-pressing a module on the home screen opens its settings menu.\nDisable this to prevent settings from appearing on long tap."
          ),
          checked_func = function()
        ...
        },
        {
          ...
          help_text = _(
            "When a finished book is deleted from the device, keep it counted in the Books Read statistics on the Home Screen.\n\nBooks changed back to Reading or Abandoned are automatically removed from this list."
          ),
          checked_func = function()
            return SUISettings:nilOrTrue(
              "simpleui_preserve_deleted_books_in_stats"
            )
          end,
          keep_menu_open = true,
          callback = function()
            local on =
              SUISettings:nilOrTrue("simpleui_preserve_deleted_books_in_stats")
            SUISettings:saveSetting(
              "simpleui_preserve_deleted_books_in_stats",
              not on
            )
      ...
      return {
        {
          text = _("Modules"),
          sub_item_table_func = function()
            return makeModulesMenu(HOMESCREEN_CTX)
          end,
        },
        {
          text = _("Wallpaper"),
        ...
        },
        {
          text = _("Behaviour"),
          sub_item_table_func = function()
            return makeBehaviourMenuItems(HOMESCREEN_CTX)
          end,
        },
      }
    end
      ...
      return {
        {
          text = _("Status Bar"),
          sub_item_table_func = function()
            return makeTopbarMenu(ctx_menu)
          end,
        },
        {
          text = _("Navigation Bar"),
          sub_item_table_func = function()
            return makeNavbarMenu(ctx_menu)
          end,
        },
        {
          text = _("Title Bar"),
          sub_item_table_func = function()
            return makeTitleBarMenu(ctx_menu)
          end,
        },
        {
          text = _("Pagination Bar"),
          sub_item_table_func = function()
            return makePaginationBarMenu(ctx_menu)
          end,
        },
        {
        ...
        -- ── Cover Settings ───────────────────────────────────
        {
          ...
          sub_item_table = {
              ...
              sub_item_table = {
                {
                  ...
                  radio = true,
                  checked_func = function()
                    return FC.getFolderStyle() == "single"
                  end,
                  keep_menu_open = true,
                  callback = function()
              ...
              sub_item_table = {
                -- ── Badges ────────────────────────────────────────
                {
                  ...
                  sub_item_table = {
                    ...
                    {
                      text_func = function()
                        return _("Number of Books in Folder")
                      end,
                      value_func = function()
                        if FC.getBadgeHidden() then
                          return _("Hidden")
                        end
                        return FC.getBadgePosition() == "top" and _("Top")
                          or _("Bottom")
                      end,
                      sub_item_table = {
                        {
                          ...
                          sub_item_table = {
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return not FC.getBadgeHidden()
                                  and FC.getBadgePosition() == "top"
                              end,
                              keep_menu_open = true,
                              callback = function()
                            ...
                            },
                            {
                              ...
                              checked_func = function()
                                return not FC.getBadgeHidden()
                                  and FC.getBadgePosition() == "bottom"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeHidden(false)
                        ...
                        },
                        {
                          ...
                          value_func = function()
                            return FC.getBadgeColorFolder() == "dark"
                                and _("Dark")
                              or _("Light")
                          end,
                          enabled_func = function()
                          ...
                          sub_item_table = {
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getBadgeColorFolder() == "light"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeColorFolder("light")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                    ...
                    {
                      text_func = function()
                        return _("Number of Pages")
                      end,
                      value_func = function()
                        return FC.getOverlayPages() and _("Visible")
                          or _("Hidden")
                      end,
                      sub_item_table = {
                        {
                          ...
                          sub_item_table = {
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getOverlayPages()
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setOverlayPages(true)
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return not FC.getOverlayPages()
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setOverlayPages(false)
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                        ...
                        },
                        {
                          ...
                          value_func = function()
                            return FC.getBadgeColorPages() == "dark"
                                and _("Dark")
                              or _("Light")
                          end,
                          enabled_func = function()
                          ...
                          sub_item_table = {
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getBadgeColorPages() == "light"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeColorPages("light")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                    ...
                    {
                      text_func = function()
                        return _("Series Index")
                      end,
                      value_func = function()
                        return FC.getOverlaySeries() and _("Visible")
                          or _("Hidden")
                      end,
                      sub_item_table = {
                        {
                          ...
                          sub_item_table = {
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getOverlaySeries()
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setOverlaySeries(true)
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return not FC.getOverlaySeries()
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setOverlaySeries(false)
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                        ...
                        },
                        {
                          ...
                          value_func = function()
                            return FC.getBadgeColorSeries() == "dark"
                                and _("Dark")
                              or _("Light")
                          end,
                          enabled_func = function()
                          ...
                          sub_item_table = {
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getBadgeColorSeries() == "light"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeColorSeries("light")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                    ...
                    -- ── Progress ──────────────────────────────────────────
                    {
                      ...
                      sub_item_table = {
                          ...
                          sub_item_table = {
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getProgressMode() == "banner"
                              end,
                              keep_menu_open = true,
                              callback = function()
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getProgressMode() == "native"
                              end,
                              keep_menu_open = true,
                              callback = function()
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getProgressMode() == "none"
                              end,
                              keep_menu_open = true,
                              callback = function()
                        ...
                        },
                        {
                          ...
                          value_func = function()
                            return FC.getBadgeColorProgress() == "dark"
                                and _("Dark")
                              or _("Light")
                          end,
                          enabled_func = function()
                          ...
                          sub_item_table = {
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getBadgeColorProgress() == "light"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeColorProgress("light")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                      ...
                      sub_item_table = {
                        {
                          ...
                          sub_item_table = {
                            ...
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getNewMode() == "ribbon"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setNewMode("ribbon")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                        ...
                        },
                        {
                          ...
                          value_func = function()
                            return FC.getBadgeColorNew() == "dark" and _("Dark")
                              or _("Light")
                          end,
                          enabled_func = function()
                            return FC.getNewMode() ~= "none"
                          ...
                          sub_item_table = {
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getBadgeColorNew() == "dark"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeColorNew("dark")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                            {
                              ...
                              radio = true,
                              checked_func = function()
                                return FC.getBadgeColorNew() == "light"
                              end,
                              keep_menu_open = true,
                              callback = function()
                                FC.setBadgeColorNew("light")
                                FC.invalidateCache()
                                _refreshFC()
                              end,
                            },
                ...
                {
                  text_func = function()
                    return _("Finished Books")
                  end,
                  value_func = function()
                    if not FC.getFadeFinished() then
                      return _("Off")
                    end
                ...
                -- ── Folder Name ───────────────────────────────────────────────────
                {
                  text = _("Folder Name"),
                  enabled_func = function()
                    return not FC.getShowTitleStrip()
                  end,
                  sub_item_table = {
                    {
                      text_func = function()
                      ...
                      value_func = function()
                        if FC.getLabelMode() == "hidden" then
                          return _("Hidden")
                        end
                        return FC.getLabelStyle() == "alpha"
                            and _("Transparent")
                          or _("Solid")
                      end,
                      sub_item_table = {
                        {
                          ...
                          radio = true,
                          checked_func = function()
                            return FC.getLabelMode() ~= "hidden"
                              and FC.getLabelStyle() == "solid"
                          end,
                          keep_menu_open = true,
                          callback = function()
                      ...
                      sub_item_table = {
                        {
                          ...
                          radio = true,
                          checked_func = function()
                            return FC.getLabelPosition() == "top"
                          end,
                          keep_menu_open = true,
                          callback = function()
                        ...
                        },
                        {
                          ...
                          radio = true,
                          checked_func = function()
                            return FC.getLabelPosition() == "center"
                          end,
                          keep_menu_open = true,
                          callback = function()
                        ...
                        },
                        {
                          ...
                          radio = true,
                          checked_func = function()
                            return FC.getLabelPosition() == "bottom"
                          end,
                          keep_menu_open = true,
                          callback = function()
                  ...
                  sub_item_table = {
                    {
                      ...
                      checked_func = function()
                        return not FC.getShowTitleStrip()
                          and not FC.getShowAuthorStrip()
                      end,
                      keep_menu_open = true,
                      callback = function()
                        if
                          FC.getShowTitleStrip() or FC.getShowAuthorStrip()
                        then
                          FC.setShowTitleStrip(false)
                          FC.setShowAuthorStrip(false)
                          local CB = ctx_menu and ctx_menu.ConfirmBox
                            or ConfirmBox()
                          local UIM = ctx_menu and ctx_menu.UIManager
                            or UIManager
                          UIM:show(CB:new({
            ...
            -- ── Uniformize Covers (2:3) ────────────────────────────────
            {
              ...
              checked_func = function()
                return FC.getCoverMode() == "2_3"
              end,
              keep_menu_open = true,
              callback = function()
                FC.setCoverMode(
                  FC.getCoverMode() == "2_3" and "default" or "2_3"
                )
                _refreshFC()
              end,
            },
      ...
      return {
          ...
          sub_item_table = {
            -- ── System Icons ──────────────────────────────────────────
            {
              text_func = function()
                ...
                if not ok_ss or not SUIStyle then
                  return _("System Icons")
                end
                local has_custom = false
                for _, s in ipairs(SUIStyle.SLOTS) do
                  if
                    s.group ~= "sui_qa_defaults"
                    and SUIStyle.getIcon(s.id) ~= nil
                  then
                    has_custom = true
                    break
                  end
              ...
              sui_build = ctx_menu
                  ...
                  and function(ctx, _item)
                    local SUIWindow = require("sui_window")
                    return SUIWindow.ListRow({
                      title = type(_item.text_func) == "function"
                          and _item.text_func()
                        or _item.text,
                      inner_w = ctx.inner_w,
                      show_chevron = true,
                      on_tap = function()
                        ...
                        if SUIStyle.sui_build_system_icons then
                          SUIStyle.sui_build_system_icons(plugin, ctx_menu, ctx)
                        end
                      end,
                    })
                  end
            ...
            -- ── Quick Actions Icons ────────────────────────────────────
            {
              text_func = function()
                ...
                if ok_qa and QA2 then
                  ...
                  if
                    not has_custom
                    and QA2.getDefaultActionIcon("wifi_toggle_off") ~= nil
                  then
                    has_custom = true
                  end
                end
                ...
                return _("Quick Actions Icons")
                  .. (has_custom and "  \u{270E}" or "")
              end,
              sub_item_table_func = function()
                local ok_qa, QA2 = pcall(require, "sui_quickactions")
                if not ok_qa or not QA2 then
                  return {}
                end
                  ...
                  and function(ctx, _item)
                    ...
                    return SUIWindow.ListRow({
                      ...
                      on_tap = function()
                        ...
                        if QA2.sui_build_qa_icons then
                          QA2.sui_build_qa_icons(plugin, ctx_menu, ctx)
                        end
                      end,
                    })
                  end
            ...
            -- ── Icon Packs ────────────────────────────────────────────
            {
              ...
              sub_item_table_func = function()
                ...
                local function _reapplyAllPacks()
                  ...
                  if ok_ss2 and SS2 then
                    ...
                    if ok_ui2 then
                      for _, entry in ipairs(UIManager2._window_stack or {}) do
                        ...
                        if
                          w
                          and (w.name == "collections" or w.name == "coll_list")
                        then
                  ...
                  if ok_qa and QA_pack and QA_pack.invalidateCustomQACache then
                    QA_pack.invalidateCustomQACache()
                  end
                  local ok_fc, FC = pcall(require, "sui_foldercovers")
                  if ok_fc and FC and FC.invalidateCache then
                    FC.invalidateCache()
                  end
                  local FM = package.loaded["apps/filemanager/filemanager"]
                    ...
                    if dir and lfs.attributes(dir, "mode") == "directory" then
                      for fname in lfs.dir(dir) do
                        if
                          fname ~= "."
                          and fname ~= ".."
                          and fname:lower():match("%.zip$")
                        then
                          zips[#zips + 1] =
                            { name = fname, path = dir .. "/" .. fname }
                        end
                      end
                      table.sort(zips, function(a, b)
            ...
            -- Icon Presets, 4 levels deep).
            {
              ...
              sub_item_table_func = function()
                ...
                local function _reapplyAll()
                  ...
                  -- Invalidate Quick Actions & Folder Covers caches
                  if QA2 and QA2.invalidateCustomQACache then
                    QA2.invalidateCustomQACache()
                  end
                  local ok_fc, FC = pcall(require, "sui_foldercovers")
                  if ok_fc and FC and FC.invalidateCache then
                    FC.invalidateCache()
                ...
                for _k, name in ipairs(names) do
                  ...
                  select_items[#select_items + 1] = {
                    ...
                    radio = true,
                    checked_func = function()
                      return SUISettings:get("simpleui_icon_active_preset")
                        == _name
                    end,
                    callback = function()
                      if not IP then
                    ...
                    sub_item_table_func = function()
                      ...
                      for _k, name in ipairs(IP and IP.listNames() or {}) do
                        ...
                        sub[#sub + 1] = {
                          ...
                          sub_item_table = {
                            {
                              text = _("Update with current settings"),
                              callback = function()
                                UIManager:show(ConfirmBox():new({
                                  text = string.format(
                                    _(
                                      'Overwrite preset "%s" with the current icon settings?'
                                    ),
                                    _name
                                  ),
                                  ok_text = _("Overwrite"),
                                  ...
                                  buttons = {
                                    {
                                      ...
                                      },
                                      {
                                        ...
                                        callback = function()
                                          local new_name =
                                            dialog2:getInputText()
                                          new_name = new_name
                                              and new_name:match("^%s*(.-)%s*$")
                                            or ""
                                          if
                                            new_name == ""
                                            or new_name == _name
        ...
        -- ── UI Font ───────────────────────────────────────────────────
        {
          ...
          sub_item_table_func = function()
            ...
            if not ok_fi or type(items) ~= "table" then
              return {}
            end
            return items
          end,
          ...
          sub_item_table = {
            {
              ...
              checked_func = function()
                return (
                  SUISettings:get("simpleui_style_progress_bar_type") or "flat"
                ) == "flat"
              end,
              keep_menu_open = true,
              callback = function()
    ...
    local function makeAboutMenuItems(ctx_menu)
      ...
      if not ok or type(Meta) ~= "table" or Meta.name ~= "simpleui" then
        ...
        Meta = (
          rok
          and type(rmeta) == "table"
          and rmeta.name == "simpleui"
          and rmeta
        ) or {}
      end
      return {
        ...
        },
        {
          ...
          separator = true,
          callback = function()
            local ConfirmBoxWidget = ctx_menu and ctx_menu.ConfirmBox
              or ConfirmBox()
            local UIM = ctx_menu and ctx_menu.UIManager or UIManager
            UIM:show(ConfirmBoxWidget:new({
              ...
              ok_callback = function()
                ...
                if
                  G_reader_settings:readSetting("start_with")
                  == "homescreen_simpleui"
                then
    ...
    menu_items.simpleui = {
      ...
      sub_item_table = {
        -- ── Enable / Disable toggle ───────────────────────────────────────
        {
          ...
          checked_func = function()
            return SUISettings:nilOrTrue("simpleui_enabled")
          end,
          callback = function()
            ...
            if
              on
              and G_reader_settings:readSetting("start_with")
                == "homescreen_simpleui"
            then
              G_reader_settings:saveSetting("start_with", "filemanager")
            end
            ...
            UIManager:show(ConfirmBox():new({
              text = string.format(
                _("Simple UI will be %s after restart.\n\nRestart now?"),
                on and _("disabled") or _("enabled")
              ),
              ok_text = _("Restart"),
              cancel_text = _("Later"),
        ...
        -- Book-browser settings: folder covers, series grouping, browse by meta.
        {
          text = _("Library"),
          sub_item_table_func = function()
            return plugin.makeLibraryMenuItems()
          end,
        },
        -- ── Appearance ───────────────────────────────────────────────────
        ...
        -- (4 levels deep) and Style > UI Font.
        {
          text = _("Style"),
          sub_item_table_func = function()
            return plugin.makeStyleMenuItems(nil)
          end,
        },
        -- ── Quick Actions ────────────────────────────────────────────────
        -- Global custom quick actions (not homescreen-specific).
        {
          text_func = function()
            ...
            if rem <= 0 then
              return string.format(
                _("Quick Actions  (%d/%d — at limit)"),
                n,
                MAX_CUSTOM_QA
              )
            end
            return string.format(
              _("Quick Actions  (%d/%d — %d left)"),
              n,
              MAX_CUSTOM_QA,
              rem
            )
        ...
        -- -----------------------------------------------------------------
        {
          ...
          separator = true,
          sub_item_table_func = function()
            return plugin.makeAboutMenuItems(nil)
          end,
        },
      },
    }
    ...
    do
      ...
      local banner = (ok_u and Updater) and Updater.build_update_banner_item()
        or nil
      if banner then
