...
local settings = G_reader_settings:readTableRef("vocabulary_builder", { enabled = false, with_context = true })
...
function MenuDialog:setupPluginMenu()
  ...
  local reverse_button = {
    text = settings.reverse and gettext("Reverse order") or gettext("Reverse order and show only reviewable"),
    callback = function()
      self:onExit()
...
function MenuDialog:setupBookMenu(sort_item, onSuccess)
  ...
  local change_title_button = {
    ...
    callback = function()
      ...
      self:showParent():showParent():showChangeBookTitleDialog(sort_item, onSuccess)
    end,
  }
    ...
    HorizontalGroup:new({
      ...
      HorizontalGroup:new({
        ...
        VerticalGroup:new({
          ...
          point_widget,
          VerticalSpan:new({
            height = self.height - point_widget_height - self.point_v_spacer.width,
          }),
        }),
        VerticalGroup:new({
          dimen = Geom:new({
          ...
          LeftContainer:new({
            dimen = Geom:new({
              w = text_max_width,
              h = math.floor(self.height - word_height - self.v_spacer.width * 2.2),
            }),
            HorizontalGroup:new({
              subtitle_prefix,
              TextWidget:new({
                ...
                max_width = math.ceil(
                  math.max(5, text_max_width - subtitle_prefix:getSize().w - Size.padding.fullscreen)
                ),
                fgcolor = subtitle_color,
              }),
            }),
...
function VocabItemWidget:showMore()
  local dialogue = WordInfoDialog:new({
    ...
    dates = gettext("Added on") .. " " .. os.date("%Y-%m-%d", self.item.create_time) .. " | " .. gettext(
      "Review scheduled at"
...
function VocabItemWidget:onTap(_, ges)
  if self.has_review_buttons then
    ...
    if BD.mirroredUILayout() then
      if
        ges.pos.x > self.more_button:getSize().x
        and ges.pos.x < self.more_button:getSize().x + self.more_button:getSize().w * 2
      then
              ...
              callback = function()
                ...
                if ok then
                  ...
                  table.insert(sort_items, #sort_items, {
                    ...
                    hold_callback = function(sort_item, onSuccess)
                      self:showParent():showChangeBookTitleDialog(sort_item, onSuccess)
                    end,
                  })
...
function VocabularyBuilderWidget:init()
  ...
  self:mergeSize(self.width or Screen:getWidth(), self.height or Screen:getHeight())
  ...
  self.review_button_width =
    math.min(math.max(self.review_button_width, temp_button:getSize().w), Screen:getWidth() / 4)
  temp_button:free()
...
function VocabularyBuilderWidget:refreshFooter()
  ...
  table.insert(
    self.page_info,
    has_sync and self.footer_sync or HorizontalSpan:new({ width = self.footer_left_corner_width })
  )
    ...
    buttons = {
      {
        ...
        },
        {
          ...
          callback = function()
            local text_info =
              gettext([[You can use two wildcards when searching: the percent sign (%) and the underscore (_).
...
function VocabularyBuilderWidget:setupItemHeight()
  ...
  local content_height = self:getSize().h - self.title_bar:getHeight() - self.footer_height - Size.padding.large
  ...
  for _ = 1, DB:selectCount(self) do
    table.insert(vocab_items, {
      callback = function(item)
        ...
        UIManager:broadcastEvent(Event:new("LookupWord", item.word, true, nil, nil, nil))
      end,
    })
  end
  return vocab_items
  ...
  table.insert(buttons, 1, {
    {
      ...
      callback = function()
        ...
        UIManager:broadcastEvent(Event:new("WordLookedUp", dict_popup.lookupword, book_title, true)) -- is_manual: true
