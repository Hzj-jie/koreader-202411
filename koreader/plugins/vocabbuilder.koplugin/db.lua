...
function VocabularyBuilder:_select_items(items, start_idx, reload_time, search_text)
  ...
  if search_text then
    sql = string.format(
      "SELECT * FROM vocabulary INNER JOIN title ON title_id = title.id WHERE word LIKE '%s' LIMIT 32 OFFSET %d",
      search_text,
      start_idx - 1
    )
  elseif not reload_time then
    sql = string.format(
      "SELECT * FROM vocabulary INNER JOIN title ON title_id = title.id AND filter = true ORDER BY due_time limit %d OFFSET %d;",
      32,
      start_idx - 1
    )
  else
    sql = string.format(
      [[SELECT * FROM vocabulary INNER JOIN title
                ON title_id = title.id AND filter = true
                WHERE due_time <= ]]
        .. reload_time
        .. " ORDER BY due_time desc limit %d OFFSET %d;",
      32,
      start_idx - 1
    )
  end
...
function VocabularyBuilder:select_items(vocab_widget, start_idx, end_idx)
  ...
  if not start_cursor then
    return
  end
  self:_select_items(
    items,
    start_cursor,
    vocab_widget:check_reverse() and vocab_widget.reload_time,
...
function VocabularyBuilder:insertOrUpdate(entry)
  ...
  stmt = conn:prepare(
    [[INSERT INTO vocabulary (word, title_id, create_time, due_time, review_time, prev_context, next_context, highlight)
...
function VocabularyBuilder:updateBookIdOfWord(word, id)
  if not word or type(id) ~= "number" then
    return
  end
  local conn = SQ3.open(db_location)
