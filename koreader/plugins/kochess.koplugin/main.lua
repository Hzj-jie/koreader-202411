...
function Kochess:initializeEngine()
  ...
  if not self.engine then
    ...
    UIManager:show(
      infoMessage:new({ text = "Error", message = "Motor no arranca." })
    )
    return
  end
        ...
        if line:match("^info ") then
          ...
          if mp == 1 then
            ...
            if mate then
              ...
              if self.eval_turn == Chess.BLACK then
                mv = -mv
              end
              self.last_mate = mv
              self.last_cp = nil
...
function Kochess:initializeGameLogic()
  ...
  self.timer = Timer:new(
    { [Chess.WHITE] = 1800, [Chess.BLACK] = 1800 },
...
function Kochess:initializeBoard()
  self.board = ChessBoard:new({
    ...
    height = math.floor(0.7 * self.full_height),
    moveCallback = function(move)
      self:onMoveExecuted(move)
    end,
    onPromotionNeeded = function(f, t, c)
      self:openPromotionDialog(f, t, c)
    end,
  })
end
...
function Kochess:buildUILayout()
  ...
  local log_h = math.max(
    100,
    self.full_height - title_bar:getSize().h - board_h - status_bar:getSize().h
  )
  local toolbar_width =
    math.floor(math.min(log_h / 4 + 16, self.full_width / 3))
  ...
  self.pgn_log =
    self:createPgnLogWidget(gettext("Welcome!"), pgn_w, log_h - eval_h)
  ...
  local toolbar = VerticalGroup:new({
    ...
    self:createToolbarButton("chevron.left", toolbar_width - 8, 40, function()
      self:handleUndoMove(false)
    end),
    self:createToolbarButton("chevron.right", toolbar_width - 8, 40, function()
      self:handleRedoMove(false)
    end),
    self:createToolbarButton("bookmark", toolbar_width - 8, 40, function()
  ...
  local main_vgroup = VerticalGroup:new({
    ...
    FrameContainer:new({
      background = BACKGROUND_COLOR,
      padding = 0,
      HorizontalGroup:new({ height = log_h, toolbar, pgn_with_eval }),
    }),
    status_bar,
  })
...
function Kochess:updatePgnLogInitialText()
  local text = gettext("Kochess Ready.\nWhite to play.")
  if self.engine and self.engine.state.uciok then
    text = text .. "\nEngine: " .. (self.engine.state.id_name or "Stockfish")
  end
...
local function formatEval(self)
  ...
  elseif abs < 0.50 then
    tag = (v > 0) and "(slight advantage for White)"
      or "(slight advantage for Black)"
  elseif abs < 1.00 then
    tag = (v > 0) and "(small advantage for White)"
      or "(small advantage for Black)"
  elseif abs < 2.00 then
    tag = (v > 0) and "(clear advantage for White)"
      or "(clear advantage for Black)"
  elseif abs < 4.00 then
    tag = (v > 0) and "(winning advantage for White)"
      or "(winning advantage for Black)"
  else
    tag = (v > 0) and "(decisive advantage for White)"
      or "(decisive advantage for Black)"
  end
...
function Kochess:updateEvalLine()
  Logger.info(
    "KOCHESS: EvalLine -> %s (cp=%s mate=%s)",
...
function Kochess:createPgnLogWidget(txt, w, h)
...
function Kochess:onMoveExecuted(move)
  ...
  Logger.info(
    "KOCHESS: Opening detected: " .. (opening and opening.name or "none")
  )
  ...
  if san:find("#", 1, true) then
    ...
    local winner_color = (self.game.turn() == Chess.WHITE) and Chess.BLACK
      or Chess.WHITE
...
function Kochess:launchNextMove()
  ...
  if
    self.engine
    and self.engine.state.uciok
    and not self.game.is_human(self.game.turn())
  then
    self:launchUCI()
  end
end
...
function Kochess:uciMove(str)
  local m = self.game.move({
    from = str:sub(1, 2),
    to = str:sub(3, 4),
    promotion = (#str == 5 and str:sub(5, 5) or nil),
  })
  if m then
    self.board:handleGameMove(m)
    self:onMoveExecuted(m)
  end
end
...
function Kochess:updateTimerDisplay()
  local ind = self.running
      and ((self.game.turn() == Chess.WHITE and " < ") or " > ")
    or " || "
  self.status_bar:setTitle(
    self.timer:formatTime(self.timer:getRemainingTime(Chess.WHITE))
      .. ind
      .. self.timer:formatTime(self.timer:getRemainingTime(Chess.BLACK))
  )
  UIManager:setDirty(self.status_bar, "ui")
end

function Kochess:updatePlayerDisplay()
  local function lbl(c)
    return self.game.is_human(c) and "Human" or "Engine"
  end
...
function Kochess:showMateDialog(winner_color)
  local winner = (winner_color == Chess.WHITE) and gettext("White")
    or gettext("Black")

  UIManager:show(ConfirmBox:new({
    text = string.format(gettext("Checkmate!\n%s wins."), winner),
  ...
  UIManager:show(PathChooser:new({
    ...
    onConfirm = function(path)
      ...
      if not fh then
        UIManager:show(infoMessage:new({
          text = gettext("Error"),
          message = gettext("Could not open file:\n") .. path,
        }))
        return
      end
...
function Kochess:handleSaveFile(dialog, filename_input, current_dir)
  ...
  if not fh then
    UIManager:show(infoMessage:new({
      text = gettext("Error"),
      message = gettext("Could not save file:\n") .. tostring(err),
    }))
    return
  end
...
function Kochess:openSaveDialog()
  ...
  dialog = InputDialog:new({
    title = gettext("Save current game as"),
    buttons = {
      {
        {
          text = gettext("Cancel"),
  ...
  local browse_button = ButtonWidget:new({
    ...
    callback = function()
      UIManager:show(PathChooser:new({
        path = current_dir,
        title = gettext("Select Save Folder"),
        select_file = false,
        show_files = true,
        parent = dialog,
...
function Kochess:openPromotionDialog(f, t, c)
  ...
  local dialog =
    InputDialog:new({ title = gettext("Promote to"), buttons = {} })
  local btns = {}
