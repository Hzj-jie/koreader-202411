...
function Kochess:init()
    ...
    Dispatcher:registerAction("kochess", {
        category = "none", event = "KochessStart", title = gettext("Chess Game"), general = true,
    })
...
function Kochess:addToMainMenu(menu_items)
    menu_items.kochess = {
        text = gettext("Chess Game"), sorting_hint = "games", callback = function() self:startGame() end, keep_menu_open = false,
    }
end
...
function Kochess:buildUILayout()
    ...
    self.pgn_log = self:createPgnLogWidget(gettext("Welcome!"), pgn_w, log_h - eval_h)
...
function Kochess:showMateDialog(winner_color)
    local winner = (winner_color == Chess.WHITE) and gettext("White") or gettext("Black")
...
function Kochess:createTitleBar()
    return TitleBarWidget:new{ fullscreen=true, title=gettext("Kochess"), left_icon="home", left_icon_tap_callback=function() self:resetGame() end, close_callback=function() self.timer:stop(); if self.engine then self.engine:stop() end; UIManager:close(self) end }
end
    ...
    UIManager:show(
        PathChooser:new{
            ...
            onConfirm = function(path)
                ...
                if not fh then
                    UIManager:show(infoMessage:new{
                        text = gettext("Error"), message = gettext("Could not open file:\n") .. path,
                    })
                    return
                end
...
function Kochess:handleSaveFile(dialog, filename_input, current_dir)
    ...
    if not fh then
        UIManager:show(infoMessage:new{
            text = gettext("Error"), message = gettext("Could not save file:\n") .. tostring(err),
        })
        return
    end
    ...
    UIManager:show(infoMessage:new{
        text = gettext("Saved"), message = gettext("Game saved to:\n") .. fullpath
    })
end
    ...
    local content = FrameContainer:new{
        ...
        VerticalGroup:new{
            ...
            HorizontalGroup:new{
                ...
                TextWidget:new{ text = gettext("Folder") .. ":", face = Font:getFace("cfont", 22) },
                dir_label,
            ...
            HorizontalGroup:new{
                spacing = Size.padding.large,
                TextWidget:new{ text = gettext("Filename") .. ":", face = Font:getFace("cfont", 22) },
                filename_input,
            },
            CenterContainer:new{
                dimen = Geometry:new{
...
function Kochess:openPromotionDialog(f,t,c)
    ...
    local dialog = InputDialog:new{ title=gettext("Promote to"), buttons={} }
    local btns = {}
