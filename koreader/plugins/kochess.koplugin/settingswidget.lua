...
function SettingsWidget:new(opts)
  ...
  assert(
    opts.onApply and type(opts.onApply) == "function",
...
function SettingsWidget:initializeState()
  ...
  local currentSkill = (self.parent and tonumber(self.parent.current_skill))
    or nil
  ...
  if not currentSkill then
    ...
    Logger.info(
      "Engine option Skill Level (default): "
        .. (skillOpt and tostring(skillOpt.value) or "nil")
    )
...
function SettingsWidget:buildPlayerTypeGroup()
    ...
    return {
      {
        {
          text = gettext("Human"),
          checked = self.changes.human_choice[color],
          color = color,
        },
      },
      {
        {
          text = gettext("Robot"),
          checked = not self.changes.human_choice[color],
          color = color,
        },
      },
    }
  ...
  local function onSelect(entry)
    self.changes.human_choice[entry.color] = (entry.text == gettext("Human"))
    self:markDirty()
  end

  -- White
  local wtxt = TextWidget:new({
    text = gettext("White") .. ":",
  ...
  self.playerSettingsGroup = HorizontalGroup:new({
    ...
    TextWidget:new({
      text = gettext("Player Type") .. ":",
      face = Font:getFace("cfont", 22),
    }),
    VerticalGroup:new({
      spacing = Size.padding.small,
...
function SettingsWidget:buildSkillGroup()
  ...
  local function onClick(delta)
    self.changes.skill_level = math.max(
      ...
      math.min(
        self.max_skill,
        (tonumber(self.changes.skill_level) or 5) + delta
      )
    )
    updateDisplay()
    self:markDirty()
  ...
  local ctrl =
    HorizontalGroup:new({ spacing = Size.padding.small, decBtn, tv, incBtn })
  self.skillSettingsGroup = HorizontalGroup:new({
    width = self.dialog.element_width,
    TextWidget:new({
      text = gettext("Engine Skill") .. ":",
      face = Font:getFace("cfont", 22),
    }),
    ctrl,
  })
...
function SettingsWidget:buildEloGroup()
  ...
  self.eloSettingsGroup = HorizontalGroup:new({
    width = self.dialog.element_width,
    TextWidget:new({
      text = gettext("Engine ELO Strength") .. ":",
      face = Font:getFace("cfont", 22),
    }),
    ctrl,
  })
...
function SettingsWidget:buildTimeGroups()
  ...
  local function openSubDialog(color, btn)
    ...
    timeDlg = InputDialog:new({
      title = gettext(color .. " Time Settings"),
      description = gettext("Time (min + sec):"),
      allow_newline = false,
      input = inputFmt,
      input_type = "number",
      save_callback = function(txt)
        ...
        if not nb or not ni then
          UIManager:showMessage(
            ...
            gettext("Enter 'minutes + seconds' (e.g. '5 + 0').")
          )
          return
        end
        nb =
          math.max(self.min_base_min, math.min(self.max_base_min, tonumber(nb)))
        ni =
          math.max(self.min_incr_sec, math.min(self.max_incr_sec, tonumber(ni)))
  ...
  local wbtn
  local wtxt = TextWidget:new({
    text = gettext("White Time: "),
    face = Font:getFace("cfont", 22),
  })
  wbtn = ButtonWidget:new({
    text = fmt(
      self.changes.time_control[Chess.WHITE].base_minutes,
      self.changes.time_control[Chess.WHITE].incr_seconds
    ),
    callback = function()
      openSubDialog(Chess.WHITE, wbtn)
    end,
    face = Font:getFace("cfont", 20),
    padding = Size.padding.small,
    radius = Size.radius.button,
    parent = self.dialog,
    width = self.dialog.element_width / 2
      - wtxt:getSize().w
  ...
  local bbtn
  local btxt = TextWidget:new({
    text = gettext("Black Time: "),
    face = Font:getFace("cfont", 22),
  })
  bbtn = ButtonWidget:new({
    text = fmt(
      self.changes.time_control[Chess.BLACK].base_minutes,
      self.changes.time_control[Chess.BLACK].incr_seconds
    ),
    callback = function()
      openSubDialog(Chess.BLACK, bbtn)
    end,
    face = Font:getFace("cfont", 20),
    padding = Size.padding.small,
    radius = Size.radius.button,
    ...
    VerticalGroup:new({
      ...
      self.engine.state.uciok
          and CenterContainer:new({
            dimen = Geometry:new({
              w = D.width,
              h = self.playerSettingsGroup:getSize().h,
            }),
      ...
      self.engine.state.uciok
          and CenterContainer:new({
            dimen = Geometry:new({
              w = D.width,
              h = self.skillSettingsGroup:getSize().h,
            }),
