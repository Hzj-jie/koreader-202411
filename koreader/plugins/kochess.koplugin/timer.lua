...
function Timer:start()
  if not self.running then
    ...
    if self.callback then
      Utils.pollingLoop(TIMER_TIMEOUT, function()
...
function Timer:stop()
  if self.running then
    ...
    self.time[self.currentPlayer] = math.max(
      0,
      self.time[self.currentPlayer]
        - elapsed
...
function Timer:switchPlayer()
  ...
  self.currentPlayer = (self.currentPlayer == Chess.WHITE) and Chess.BLACK
    or Chess.WHITE
