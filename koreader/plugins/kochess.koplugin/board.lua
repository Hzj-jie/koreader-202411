...
function Board:init()
  ...
  logger.dbg(
    string.format(
      "Initializing board: %dx%d, button size: %d",
      self.width,
      self.height,
      self.button_size
    )
...
function Board:applySquareColors()
  for rank = 0, BOARD_SIZE - 1 do
    for file = 0, BOARD_SIZE - 1 do
      ...
      local color = ((file + rank) % 2 == 1) and Blitbuffer.COLOR_LIGHT_GRAY
...
function Board:handleClick(file, rank)
  ...
  if
    self.game
    and self.game.is_human
    and (not self.game.is_human(self.game.turn()))
  then
    logger.dbg(
      "Chess: ignoring input, engine turn ("
        .. tostring(self.game.turn())
        .. ")"
    )
  ...
  local is_my_piece = clicked_piece
    and (clicked_piece.color == self.game.turn())
  ...
  if self.selected then
    ...
    elseif is_my_piece then
      ...
      logger.dbg(
        "Chess switch selection: " .. self.selected .. " -> " .. square
      )
      self:unmarkSelected(self.selected)
...
function Board:handleMove(from, to)
  ...
  if piece and piece.type == Chess.PAWN then
    ...
    if
      (piece.color == Chess.WHITE and to_rank_num == 8)
      or (piece.color == Chess.BLACK and to_rank_num == 1)
    then
...
function Board:handleMoveFlags(move, to)
  ...
  elseif move.flags == Chess.FLAGS.KSIDE_CASTLE then
    ...
    self:updateSquare(
      Board.idToPosition(Board.toId(rook_from_file_id, rank_index))
    )
    self:updateSquare(
      Board.idToPosition(Board.toId(rook_to_file_id, rank_index))
    )
...
function Board.positionToColor(position)
  if type(position) == "string" and #position == 2 then
    ...
    then
      local file_idx = string.byte(file_char) - string.byte("a")
      local rank_idx = tonumber(rank_char) - 1
      return (file_idx + rank_idx) % 2 == 1 and Blitbuffer.COLOR_LIGHT_GRAY
        or Blitbuffer.COLOR_DARK_GRAY
    end
  end
  return nil
...
function Board:allMarginSizes()
  ...
  return Geom:new({
    w = (self.margin + self.bordersize) * 2
      + self._padding_right
      + self._padding_left,
    h = (self.margin + self.bordersize) * 2
      + self._padding_top
