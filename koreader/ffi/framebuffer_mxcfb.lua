...
local function mxc_update(
  ...
  fb.debug(
    string.format(
      "mxc_update: %ux%u region @ (%u, %u) with marker %u (WFM: %u & UPD: %u)",
      w,
      h,
      x,
      y,
      marker,
      ioc_data.waveform_mode,
      ioc_data.update_mode
    )
  )
...
function framebuffer:init()
  ...
  elseif self.device:isKobo() then
    ...
    if
      self.mech_wait_update_complete == kobo_mxc_wait_for_update_complete
      or self.mech_wait_update_complete == kobo_mtk_wait_for_update_complete
    then
      self.marker_data = ffi.new("uint32_t[1]")
    elseif
      self.mech_wait_update_complete == kobo_mk7_mxc_wait_for_update_complete
