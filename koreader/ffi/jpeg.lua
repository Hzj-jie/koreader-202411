...
function Jpeg.encodeToFile(filename, source_ptr, w, stride, h, quality, color_type, subsample)
  ...
  then
    local fhandle = C.open(
      filename,
      bit.bor(C.O_WRONLY, C.O_CREAT, C.O_TRUNC, C.O_CLOEXEC),
      ffi.cast("int", bit.bor(C.S_IRUSR, C.S_IWUSR, C.S_IRGRP, C.S_IROTH))
    )
    if fhandle >= 0 then
...
function Jpeg.convertToGray(source_ptr, stride, h)
  for y = 0, h - 1 do
    ...
    for _ = 0, tonumber(stride), 3 do
      local r = 0.299 * source_ptr[offs]
      offs = offs + 1
      local g = 0.587 * source_ptr[offs]
      offs = offs + 1
      local b = 0.114 * source_ptr[offs]
      offs = offs + 1
