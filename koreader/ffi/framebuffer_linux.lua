...
function framebuffer:fbinfoOverride(finfo, vinfo) end
...
function framebuffer:reinit()
  ...
  self.bb = BB.new(
    vinfo.xres,
    vinfo.yres,
    BB["TYPE_BB" .. bpp] or BB["TYPE_BBRGB" .. bpp],
    self.data,
    finfo.line_length,
    stride_pixels
  )
...
function framebuffer:afterPaint()
  if
    self._forced_rotation
    and self.forced_rotation
    and self.forced_rotation.every_paint
    and self.forced_rotation.restore
  then
    self:setHWRotation(self.native_rotation_mode)
  end
