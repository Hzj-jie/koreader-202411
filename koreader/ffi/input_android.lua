...
local function keyEventHandler(key_event)
  ...
  if code == C.AKEYCODE_VOLUME_UP or code == C.AKEYCODE_VOLUME_DOWN then
    if android.getVolumeKeysIgnored() then
      return 0 -- event not consumed
    end
  elseif
    code == C.AKEYCODE_MEDIA_PLAY_PAUSE
  ...
  then
    return 0 -- event not consumed
  elseif code == C.AKEYCODE_MUTE or code == C.AKEYCODE_VOLUME_MUTE then
    if android.getVolumeKeysIgnored() then
      return 0
    else
      return 1
