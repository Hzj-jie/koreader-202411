...
function fb:init()
  ...
  if self.screen_size.w > self.screen_size.h and self.is_always_portrait then
    ...
    self.debug(
      "FB: This prevents the use of blitting optimizations. This should instead be fixed on the device's side on startup."
    )
    self.bb:rotate(-90)
    self.blitbuffer_rotation_mode = self.bb:getRotation()
...
function fb:setViewport(viewport)
  if
    viewport.x < 0
    or viewport.x > self.screen_size.w
    or viewport.y < 0
    or viewport.y > self.screen_size.h
    or viewport.w < 0
    or viewport.x + viewport.w > self.screen_size.w
    or viewport.h < 0
    or viewport.y + viewport.h > self.screen_size.h
  then
...
-- To be overriden.
function fb:setSwipeAnimations(enabled) end

-- To be overriden.
function fb:setSwipeDirection(direction) end
