  ...
  for joystick_counter = 0, num_joysticks - 1 do
    if SDL.SDL_IsGameController(joystick_counter) ~= 0 then
      ...
      if S.controller ~= nil then
        ...
        io.write(
          "SDL: could not open gamecontroller ",
          joystick_counter,
          ": ",
          ffi.string(SDL.SDL_GameControllerNameForIndex(joystick_counter)),
          "\n"
...
local function handleWindowEvent(event_window)
  ...
  elseif
    event_window.event == SDL.SDL_WINDOWEVENT_RESIZED
    or event_window.event == SDL.SDL_WINDOWEVENT_SIZE_CHANGED
    or event_window.event == SDL.SDL_WINDOWEVENT_MOVED
  then
...
local function handleJoyAxisMotionEvent(event)
  ...
  if (value > -neutral_max_val) and (value < neutral_max_val) then
    return
  end
  ...
  if since_last_ev <= min_time_since_last_ev then
    return
  end
...
function S.waitForEvent(sec, usec)
  ...
  elseif event.type == SDL.SDL_WINDOWEVENT then
    ...
    --- Controllers ---
  elseif
    event.type == SDL.SDL_CONTROLLERDEVICEADDED
    or event.type == SDL.SDL_CONTROLLERDEVICEREMOVED
    or event.type == SDL.SDL_CONTROLLERDEVICEREMAPPED
  then
    openGameController()
    --- Sticks & triggers ---
...
function S.setWindowIcon(icon)
  if not icon then
    error("setWindowIcon: no icon path given")
  end
  ...
  local surface = SDL.SDL_CreateRGBSurfaceWithFormatFrom(
    icon_bb.data,
    icon_bb:getWidth(),
    icon_bb:getHeight(),
    icon_bit_depth,
...
function S.gameControllerRumble(left_intensity, right_intensity, duration)
  if S.controller == nil or not SDL_Linked_Version_AtLeast(2, 0, 9) then
    return
  end
