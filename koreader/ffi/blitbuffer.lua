...
local threshold_map_o8x8 = ffi.new("uint8_t[64]", {
  [0] = 1,
  49,
  13,
  61,
...
local function dither_o8x8(x, y, v)
  ...
  local q = (
    l + (t >= threshold_map_o8x8[band(x, 7) + (8 * band(y, 7))] and 1 or 0)
  ) * 17
...
function Color4L_mt.__index:blend(color, coverage)
  ...
  local value = div4080(
    band(self.a, 0x0F) * 0x11 * bxor(alpha, 0xFF) + color:getR() * alpha
  )
...
function Color4U_mt.__index:blend(color, coverage)
  ...
  -- simplified: we expect a 8bit grayscale "color" as parameter
  local value =
    div255((orig + rshift(orig, 4)) * bxor(alpha, 0xFF) + color:getR() * alpha)
...
function Color4L_mt.__index:pmulblend(color)
  ...
  -- simplified: we expect a 8bit grayscale "color" as parameter
  local value =
    div4080(band(self.a, 0x0F) * 0x11 * bxor(alpha, 0xFF) + color:getR() * 0xFF)
  self:set(Color4L(value))
end
function Color4U_mt.__index:pmulblend(color)
  ...
  -- simplified: we expect a 8bit grayscale "color" as parameter
  local value =
    div255((orig + rshift(orig, 4)) * bxor(alpha, 0xFF) + color:getR() * 0xFF)
  self:set(Color4U(value))
end
...
-- to Color4L:
function Color4L_mt.__index:getColor4L()
  return Color4L(band(0x0F, self.a))
end
function Color4U_mt.__index:getColor4L()
  return Color4L(rshift(self.a, 4))
end
function Color8_mt.__index:getColor4L()
  return Color4L(rshift(self.a, 4))
end
function Color8A_mt.__index:getColor4L()
  return Color4L(rshift(self.a, 4))
end
--[[
...
-- to Color4U:
function Color4L_mt.__index:getColor4U()
  return Color4U(lshift(self.a, 4))
end
function Color4U_mt.__index:getColor4U()
  return Color4U(band(0xF0, self.a))
end
function Color8_mt.__index:getColor4U()
  return Color4U(band(0xF0, self.a))
end
function Color8A_mt.__index:getColor4U()
  return Color4U(band(0xF0, self.a))
end
...
function ColorRGB24_mt.__index:getColor4U()
  return Color4U(
    band(0xF0, rshift(4898 * self.r + 9618 * self.g + 1869 * self.b, 14))
  )
end
...
function ColorRGB24_mt.__index:getColor8()
  return Color8(
    rshift(4898 * self:getR() + 9618 * self:getG() + 1869 * self:getB(), 14)
  )
end
...
function ColorRGB16_mt.__index:getColorRGB16()
  return ColorRGB16(self.v)
end
function ColorRGB24_mt.__index:getColorRGB16()
  return ColorRGB16(
    lshift(band(self.r, 0xF8), 8)
      + lshift(band(self.g, 0xFC), 3)
      + rshift(self.b, 3)
  )
end
...
function Color8_mt.__index:getColorRGB24()
  return ColorRGB24(self.a, self.a, self.a)
end
...
function ColorRGB16_mt.__index:getColorRGB24()
  ...
  local b = band(self.v, 0x001F)
  return ColorRGB24(
    lshift(r, 3) + rshift(r, 2),
    lshift(g, 2) + rshift(g, 4),
    lshift(b, 3) + rshift(b, 2)
  )
end
...
-- modifications:
-- inversion:
function Color4L_mt.__index:invert()
  return Color4L(bxor(self.a, 0x0F))
end
function Color4U_mt.__index:invert()
  return Color4U(bxor(self.a, 0xF0))
end
...
function BB_mt.__index:setRotation(rotation_mode)
  self.config = bor(
    band(self.config, bxor(MASK_ROTATED, 0xFF)),
    lshift(rotation_mode, SHIFT_ROTATED)
  )
end
...
function BB_mt.__index:setInverse(inverse)
  self.config = bor(
    band(self.config, bxor(MASK_INVERSE, 0xFF)),
    lshift(inverse, SHIFT_INVERSE)
  )
end
...
function BB_mt.__index:setAllocated(allocated)
  self.config = bor(
    band(self.config, bxor(MASK_ALLOCATED, 0xFF)),
    lshift(allocated, SHIFT_ALLOCATED)
  )
  if allocated == 1 then
    ffi.gc(self, BB.gc)
  else
...
-- Bits per pixel
function BB4_mt.__index:getBpp()
  return 4
end
...
function BB4_mt.__index:getBytesPerPixel()
  return 0.5
end
function BB8_mt.__index:getBytesPerPixel()
  return 1
end
function BB8A_mt.__index:getBytesPerPixel()
  return 2
end
function BBRGB16_mt.__index:getBytesPerPixel()
  return 2
end
function BBRGB24_mt.__index:getBytesPerPixel()
  return 3
end
function BBRGB32_mt.__index:getBytesPerPixel()
  return 4
end
function BB_mt.__index:isRGB()
  local bb_type = self:getType()
  if
    bb_type == TYPE_BBRGB16
    or bb_type == TYPE_BBRGB24
    or bb_type == TYPE_BBRGB32
  then
    return true
  end
  return false
...
function BB4_mt.__index.getMyColor(color)
  return color:getColor4L()
end
...
BBRGB24_mt.__index.setPixelColorizeRGB32 =
  BBRGB16_mt.__index.setPixelColorizeRGB32
BBRGB32_mt.__index.setPixelColorizeRGB32 =
  BBRGB16_mt.__index.setPixelColorizeRGB32
-- Invert
...
function BB_mt.__index:blitDefault(
  dest,
  dest_x,
  dest_y,
  offs_x,
...
function BB8_mt.__index:blitTo8(
  dest,
  dest_x,
  dest_y,
  offs_x,
  ...
  if
    ...
    or self:getRotation() ~= 0
    or dest:getRotation() ~= 0
    or (self:getInverse() ~= dest:getInverse())
  then
    return self:blitDefault(
      dest,
      dest_x,
      dest_y,
      offs_x,
...
function BBRGB32_mt.__index:blitToRGB32(
  dest,
  dest_x,
  dest_y,
  offs_x,
  ...
  then
    ...
    for y = dest_y, dest_y + height - 1 do
      -- BBRGB32 is 4 bytes per pixel
      local srcp = ffi.cast(uint8pt_rodata, self.data)
        + self.stride * o_y
        + lshift(offs_x, 2)
      local dstp = ffi.cast(uint8pt, dest.data)
        + dest.stride * y
        + lshift(dest_x, 2)
...
function BB_mt.__index:blitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  width, dest_x, offs_x = BB.checkBounds(
    width,
    dest_x or 0,
    offs_x or 0,
    self:getWidth(),
  ...
  height, dest_y, offs_y = BB.checkBounds(
    height,
    dest_y or 0,
    offs_y or 0,
    self:getHeight(),
...
function BB_mt.__index:addblitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
...
-- straight alpha
function BB_mt.__index:alphablitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_alpha_blit_from(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
...
function BB_mt.__index:ditheralphablitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_dither_alpha_blit_from(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
    ...
    self:blitFrom(
      ...
      width,
      height,
      self.setPixelDitherBlend
    )
  end
end
-- premultiplied alpha
function BB_mt.__index:pmulalphablitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_pmulalpha_blit_from(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
    ...
    self:blitFrom(
      ...
      width,
      height,
      self.setPixelPmulBlend
    )
  end
end
-- premultiplied alpha w/ dithering (dithering only if target is BB8)
function BB_mt.__index:ditherpmulalphablitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_dither_pmulalpha_blit_from(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
    ...
    self:blitFrom(
      ...
      width,
      height,
      self.setPixelDitherPmulBlend
    )
  end
end

-- simple blitting w/ dithering (dithering only if target is BB8)
function BB_mt.__index:ditherblitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
...
-- invert blitting
function BB_mt.__index:invertblitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_invert_blit_from(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
    ...
    self:blitFrom(
      ...
      width,
      height,
      self.setPixelInverted
    )
  end
end

-- colorize area using source blitbuffer as a alpha-map
function BB_mt.__index:colorblitFrom(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_color_blit_from(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
    ...
    self:blitFrom(
      ...
      width,
      height,
      self.setPixelColorize,
      c
    )
  end
end

function BB_mt.__index:colorblitFromRGB32(
  source,
  dest_x,
  dest_y,
  offs_x,
  ...
  if self:canUseCbbTogether(source) then
    ...
    cblitbuffer.BB_color_blit_from_RGB32(
      ffi.cast(P_BlitBuffer, self),
      ffi.cast(P_BlitBuffer_ROData, source),
      dest_x,
      dest_y,
...
function BB_mt.__index:fill(value)
  if self:canUseCbb() then
    cblitbuffer.BB_fill(ffi.cast(P_BlitBuffer, self), value:getColor8().a)
  else
...
function BB_mt.__index:invertRect(x, y, w, h)
  ...
  if self:canUseCbb() then
    ...
    if x == 0 and w == self.w then
      ...
      if bbtype == TYPE_BBRGB32 then
        local p =
          ffi.cast(uint32pt, ffi.cast(uint8pt, self.data) + self.stride * y)
      ...
      elseif bbtype == TYPE_BBRGB16 then
        local p =
          ffi.cast(uint16pt, ffi.cast(uint8pt, self.data) + self.stride * y)
        for i = 1, self.pixel_stride * h do
          p[0] = bxor(p[0], 0xFFFF)
          p = p + 1
        end
      ...
      if bbtype == TYPE_BBRGB32 then
        for j = y, y + h - 1 do
          local p = ffi.cast(
            uint32pt,
            ffi.cast(uint8pt, self.data) + self.stride * j
          ) + x
          for i = 0, w - 1 do
            p[0] = bxor(p[0], 0x00FFFFFF)
            p = p + 1
    ...
    if setter == self.setPixel then
      ...
      if x == 0 and w == self.w then
        ...
        if bbtype == TYPE_BBRGB32 then
          ...
          local p = ffi.cast(
            P_ColorRGB32,
            ffi.cast(uint8pt, self.data) + self.stride * y
          )
          for i = 1, self.pixel_stride * h do
            p[0] = src
            p = p + 1
        ...
        elseif bbtype == TYPE_BB8A then
          local src = v:getColor8A()
          local p =
            ffi.cast(P_Color8A, ffi.cast(uint8pt, self.data) + self.stride * y)
          for i = 1, self.pixel_stride * h do
            p[0] = src
            p = p + 1
          end
...
function BB_mt.__index:paintRectRGB32(x, y, w, h, color, setter)
  ...
  local c = color and color:getColorRGB32()
    or ColorRGB32(0x80, 0x80, 0x80, 0xFF)
  if self:canUseCbb() and setter == self.setPixel then
    cblitbuffer.BB_fill_rect_RGB32(ffi.cast(P_BlitBuffer, self), x, y, w, h, c)
  else
    -- We can only do fast filling when there's no complex processing involved (i.e., simple setPixel only)
    local bbtype = self:getType()
...
function BB_mt.__index:paintRoundedCorner(
  off_x,
  off_y,
  w,
  h,
  ...
  if self:canUseCbb() then
    cblitbuffer.BB_paint_rounded_corner(
      ffi.cast(P_BlitBuffer, self),
      off_x,
      off_y,
      w,
...
@c:  color for loading bar
--]]
function BB_mt.__index:progressBar(
  x,
  y,
  w,
  h,
...
function BB_mt.__index:lightenRect(x, y, w, h, by)
  ...
  if self:canUseCbb() then
    ...
    cblitbuffer.BB_blend_rect(ffi.cast(P_BlitBuffer, self), x, y, w, h, color)
  else
...
function BB_mt.__index:multiplyRectRGB(x, y, w, h, color)
  ...
  if self:canUseCbb() then
    ...
    cblitbuffer.BB_blend_RGB_multiply_rect(
      ffi.cast(P_BlitBuffer, self),
      x,
      y,
      w,
...
function BB_mt.__index:blendRectRGB32(x, y, w, h, color)
  ...
  if self:canUseCbb() then
    ...
    cblitbuffer.BB_blend_RGB32_over_rect(
      ffi.cast(P_BlitBuffer, self),
      x,
      y,
      w,
...
function BB_mt.__index:copy()
  ...
  local copy =
    mytype(self.w, self.pixel_stride, self.h, self.stride, buffer, self.config)
  copy:setAllocated(1)
  return copy
end
...
function BBRGB24_mt.__index:writePNG(filename, bgr)
  ...
  if bgr then
    return self:writePNGFromBGR(filename)
  end
...
for name, func in pairs(BB_mt.__index) do
  if not BB4_mt.__index[name] then
    BB4_mt.__index[name] = func
  end
...
function BB.new(width, height, buffertype, dataptr, stride, pixel_stride)
  ...
  local bits_per_pixel = assert(
    BB.TYPE_TO_BPP[tonumber(buffertype)],
    "unknown buffer type " .. tostring(buffertype)
  )
  ...
  if buffertype == TYPE_BB4 then
    bb = BlitBuffer4(width, pixel_stride, height, stride, nil, 0)
  elseif buffertype == TYPE_BB8 then
    bb = BlitBuffer8(width, pixel_stride, height, stride, nil, 0)
  elseif buffertype == TYPE_BB8A then
    bb = BlitBuffer8A(width, pixel_stride, height, stride, nil, 0)
  elseif buffertype == TYPE_BBRGB16 then
    bb = BlitBufferRGB16(width, pixel_stride, height, stride, nil, 0)
  elseif buffertype == TYPE_BBRGB24 then
    bb = BlitBufferRGB24(width, pixel_stride, height, stride, nil, 0)
  elseif buffertype == TYPE_BBRGB32 then
    bb = BlitBufferRGB32(width, pixel_stride, height, stride, nil, 0)
  else
