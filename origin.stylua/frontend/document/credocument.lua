...
function CreDocument:init()
  ...
  self._view_mode = G_defaults:readSetting("DCREREADER_VIEW_MODE") == "scroll" and self.SCROLL_VIEW_MODE
  ...
  if text_range then
    ...
    if text_range.pos0 and text_range.pos1 then
      ...
      for i = 1, #word_boxes do
        ...
        word_boxes[i] = { x = v.x0, y = v.y0, w = v.x1 - v.x0, h = v.y1 - v.y0 }
      end
...
function CreDocument:setupCallCache()
    ...
    self.getCallCacheStatistics = function()
      ...
      for _, k in ipairs(hit_keys) do
        ...
        table.insert(
          res,
          string.format(
            "    %s: %d/%d hits (%d%%) %.3fs%s saved, %.3fs used (%d%% saved)",
            k,
            hits,
            total,
            pct_hit,
      ...
      for _, k in ipairs(hit_keys) do
        ...
        table.insert(
          ...
          string.format(
            ...
            k,
            hits,
            1000 * hits_duration / hits,
            1000 * missed_duration / misses,
            misses
          )
        )
      end
      ...
      table.insert(
        res,
        string.format(
          "  cpu time used: %.3fs, saved: %.3fs (%d%% saved)",
          total_duration,
  ...
  for name, func in pairs(CreDocument) do
    if type(func) == "function" then
      ...
      if name == "setBatteryState" then
        no_wrap = true -- except this one
      elseif name == "setPageInfoOverride" then
        no_wrap = true -- and this one
      elseif name == "setViewMode" then
        no_wrap = true -- and this one
      elseif name:sub(1, 3) == "set" then
        add_reset = true
      elseif name:sub(1, 6) == "toggle" then
        add_reset = true
      elseif name:sub(1, 6) == "update" then
        add_reset = true
      elseif name:sub(1, 6) == "enable" then
        add_reset = true
      elseif name == "zoomFont" then
        add_reset = true -- not used by koreader
      elseif name == "resetCallCache" then
        add_reset = true
      elseif name == "cacheFlows" then
        add_reset = true
      ...
      elseif name == "clearSelection" then
        add_buffer_trash = true
      elseif name == "highlightXPointer" then
        add_buffer_trash = true
      elseif name == "getWordFromPosition" then
        add_buffer_trash = true
      elseif name == "getTextFromPositions" then
        add_buffer_trash = true
      elseif name == "getTextFromXPointers" then
        add_buffer_trash = true
      elseif name == "findText" then
        add_buffer_trash = true
      elseif name == "resetBufferCache" then
        add_buffer_trash = true

        -- These may change page/pos
      elseif name == "gotoPage" then
        set_tag = "page"
        set_arg = 2
        set_arg2 = 3
      elseif name == "gotoPos" then
        set_tag = "pos"
        set_arg = 2
      elseif name == "drawCurrentViewByPos" then
        set_tag = "pos"
        set_arg = 6
      ...
      elseif name == "getCurrentPage" then
        no_wrap = true
      elseif name == "getCurrentPos" then
        no_wrap = true
      elseif name == "getVisiblePageCount" then
        no_wrap = true
      elseif name == "getVisiblePageNumberCount" then
        no_wrap = true
      elseif name == "getCoverPageImage" then
        no_wrap = true
      elseif name == "getDocumentFileContent" then
        no_wrap = true
      elseif name == "getHTMLFromXPointer" then
        no_wrap = true
      elseif name == "getHTMLFromXPointers" then
        no_wrap = true
      elseif name == "getImageFromPosition" then
        no_wrap = true
      elseif name == "getTextFromXPointer" then
        no_wrap = true
      elseif name == "getPageOffsetX" then
        no_wrap = true
      elseif name == "getNextVisibleWordStart" then
        no_wrap = true
      elseif name == "getNextVisibleWordEnd" then
        no_wrap = true
      elseif name == "getPrevVisibleWordStart" then
        no_wrap = true
      elseif name == "getPrevVisibleWordEnd" then
        no_wrap = true
      elseif name == "getPrevVisibleChar" then
        no_wrap = true
      elseif name == "getNextVisibleChar" then
        no_wrap = true
      elseif name == "getCacheFilePath" then
        no_wrap = true
      elseif name == "getStatistics" then
        no_wrap = true
      elseif name == "getNormalizedXPointer" then
        no_wrap = true
      elseif name == "getNextPage" then
        no_wrap = true
      elseif name == "getPrevPage" then
        no_wrap = true
      elseif name == "getPageFlow" then
        no_wrap = true
      elseif name == "getPageNumberInFlow" then
        no_wrap = true
      elseif name == "getTotalPagesLeft" then
        no_wrap = true
      elseif name == "getDocumentRenderingHash" then
        no_wrap = true
      elseif name == "getPartialRerenderingsCount" then
        no_wrap = true

        -- Some get* have different results by page/pos
      elseif name == "getLinkFromPosition" then
        cache_by_tag = true
      elseif name == "getPageLinks" then
        cache_by_tag = true
      elseif name == "getScreenBoxesFromPositions" then
        cache_by_tag = true
      elseif name == "getScreenPositionFromXPointer" then
        cache_by_tag = true
      elseif name == "getXPointer" then
        cache_by_tag = true
      elseif name == "isXPointerInCurrentPage" then
        cache_by_tag = true
      elseif name == "getPageMapCurrentPageLabel" then
        cache_by_tag = true
      elseif name == "getPageMapVisiblePageLabels" then
        cache_by_tag = true

        -- Assume all remaining get* can have their results
        -- cached globally by function arguments
      elseif name:sub(1, 3) == "get" then
        cache_global = true
      ...
      if add_reset then
        self[name] = function(...)
          ...
          if do_log then
            logger.dbg("callCache:", name, "resetting cache")
          end
          self._callCacheReset()
          return func(...)
        end
      elseif add_buffer_trash then
        self[name] = function(...)
          if do_log then
            logger.dbg("callCache:", name, "resetting buffer")
          end
      ...
      elseif set_tag then
        self[name] = function(...)
          if do_log then
            logger.dbg("callCache:", name, "setting tag")
          end
      ...
      elseif cache_by_tag then
        ...
        self[name] = function(...)
          ...
          if results then
            if do_log then
              logger.dbg("callCache:", name, "cache hit:", cache_key)
            end
            ...
            if do_log then
              logger.dbg("callCache:", name, "cache miss:", cache_key)
            end
            results = { func(...) }
  ...
  self.drawCurrentView = function(this, target, x, y, rect, pos)
    ...
    if do_draw then
      if do_log then
        logger.dbg("callCache: ########## drawCurrentView: full draw")
      end
      ...
      if do_log then
        logger.dbg("callCache: ---------- drawCurrentView: light draw")
      end
