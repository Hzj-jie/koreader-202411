...
local function is_wiki_page(link_url)
  ...
  if wiki_lang and wiki_page then
    ...
    if not epub_fullpath then
      ...
      if not dir then
        dir = require("apps/filemanager/filemanagerutil").getDefaultDir()
      end
      if dir then
    ...
    sub_item_table = {
      {
        ...
        callback = function()
          G_reader_settings:saveSetting("tap_to_follow_links", not isTapToFollowLinksOn())
        end,
      ...
      },
      {
        ...
        callback = function()
          G_reader_settings:saveSetting("tap_ignore_external_links", not isTapIgnoreExternalLinksEnabled())
        end,
        help_text = _([[
...
You can still follow them from the dictionary window or the selection menu after holding on them.]]),
      ...
      },
      {
        ...
        callback = function()
          G_reader_settings:saveSetting("swipe_to_go_back", not isSwipeToGoBackEnabled())
        end,
        help_text = _(
      ...
      },
      {
        ...
        callback = function()
          G_reader_settings:saveSetting("swipe_to_follow_nearest_link", not isSwipeToFollowNearestLinkEnabled())
        end,
        help_text = _(
      ...
      },
      {
        ...
        callback = function()
          G_reader_settings:saveSetting("swipe_ignore_external_links", not isSwipeIgnoreExternalLinksEnabled())
        end,
        help_text = _([[
...
You can still follow external links from the dictionary window or the selection menu after holding on them.]]),
      ...
      },
      {
        ...
        callback = function()
          G_reader_settings:saveSetting("swipe_to_jump_to_latest_bookmark", not isSwipeToJumpToLatestBookmarkEnabled())
        end,
        help_text = _(
          [[
...
If any of the other Swipe to follow link options is enabled, this will work only when the current page contains no link.]]
  ...
  if self.ui.rolling then
    ...
    table.insert(menu_items.follow_links.sub_item_table, 2, {
      ...
      callback = function()
        G_reader_settings:saveSetting("larger_tap_area_to_follow_links", not isLargerTapAreaToFollowLinksEnabled())
      end,
      help_text = _(
    ...
    table.insert(menu_items.follow_links.sub_item_table, 4, {
      ...
      callback = function()
        G_reader_settings:saveSetting("footnote_link_in_popup", not isFootnoteLinkInPopupEnabled())
      end,
      help_text = _([[
...
From the footnote popup, you can jump to the footnote location in the book by swiping to the left.]]),
    ...
    table.insert(footnote_popup_settings_items, {
      ...
      callback = function()
        G_reader_settings:saveSetting("link_prefer_footnote", not isPreferFootnoteEnabled())
      end,
