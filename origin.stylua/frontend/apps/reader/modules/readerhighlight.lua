  ...
  table.insert(hl_sub_item_table, {
    text_func = function()
      return T(
        ...
        G_reader_settings:readSetting("highlight_lighten_factor", 0.2)
      )
    end,
    enabled_func = function()
...
If you wish your highlights to be saved in the document, just move it to a writable directory first.]]
  ...
  for i, v in ipairs(long_press_action) do
    table.insert(menu_items.long_press.sub_item_table, {
      text = v[1],
      checked_func = function()
        return G_reader_settings:readSetting("default_highlight_action", "ask")
          == v[2]
      end,
      radio = true,
      callback = function()
  ...
  table.insert(menu_items.long_press.sub_item_table, {
    text_func = function()
      local position =
        G_reader_settings:readSetting("highlight_dialog_position", "center")
  ...
  return {
    {
      ...
      hold_callback = function()
        ...
        local curr_val =
          G_reader_settings:getSettingForExt("panel_zoom_enabled", ext)
        G_reader_settings:saveSettingForExt(
          "panel_zoom_enabled",
          not curr_val,
          ext
        )
  ...
  if not clear then
    ...
    if self.is_word_selection then
      ...
      if
        ...
        and G_reader_settings:readSetting("default_highlight_action", "ask")
          == "ask"
      then
      ...
      if
        G_reader_settings:readSetting("default_highlight_action", "ask")
        == "ask"
      then
        handle_long_hold = false
      end
...
function ReaderHighlight:onHoldRelease()
  ...
  local default_highlight_action =
    G_reader_settings:readSetting("default_highlight_action", "ask")
...
function ReaderHighlight:onReadSettings(config)
  ...
  if self.ui.paging then
    ...
    local ext = util.getFileNameSuffix(self.ui.document.file)
    G_reader_settings:initializeExtSettings(
      "panel_zoom_enabled",
      { cbz = true, cbt = true }
    )
    G_reader_settings:initializeExtSettings(
    ...
    if config:has("panel_zoom_enabled") then
      ...
      self.panel_zoom_enabled = G_reader_settings:getSettingForExt(
        "panel_zoom_enabled",
        ext
      ) or false
    end
