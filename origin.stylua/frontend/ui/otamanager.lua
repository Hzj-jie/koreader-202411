...
function OTAManager:checkUpdate()
  ...
  local local_ok, local_version = pcall(function()
    ...
    if rev then
      return Version:getNormalizedVersion(rev)
    end
  end)
...
function OTAManager:fetchAndProcessUpdate()
  ...
  elseif ota_version == -1 then
    UIManager:show(InfoMessage:new({
      text = T(
        _("Device no longer supported.\n\nPlease check %1"),
        "https://github.com/koreader/koreader/wiki/deprecated-devices"
      ),
    }))
  ...
  for _, server in ipairs(self.ota_servers) do
    local server_item = {
      ...
      checked_func = function()
        return self:getOTAServer() == server
      end,
      callback = function()
...
function OTAManager:genChannelList()
  ...
  for _, channel in ipairs(self.ota_channels) do
    local channel_item = {
      ...
      checked_func = function()
        return self:getOTAChannel() == channel
      end,
      callback = function()
        self:setOTAChannel(channel)
      end,
    }
