...
function IME:uniqueMap(code)
  ...
  if self.W and code:find(self.W) then
    ...
    local idx = binarysearch(self.sorted_codes, code, function(v)
      return v
    end)
    if idx == #self.sorted_codes then
      return true
    elseif not idx then
      idx = binarysearch(self.sorted_codes, code, function(v)
        return string.sub(v or "", 1, #code)
      end)
...
function IME:searchStartWith(code)
  if self.exact_match then
    return
  end
  local result = binarysearch(self.sorted_codes, code, function(v)
    return string.sub(v or "", 1, #code)
  end)
  if result then
    ...
    if key then
      ...
      if new_candi and #new_candi > 0 then
        ...
        if self.auto_separate_callback() then
          ...
          table.insert(
            _stack,
            { code = key, index = 1, char = new_candi[1] or "", candi = new_candi }
          )
        end
        self:refreshHintChars(inputbox)
      end
