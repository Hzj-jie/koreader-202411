...
function NetworkMgr:sysfsInterfaceOperational()
  ...
  local out
  local file =
    io.open("/sys/class/net/" .. self.interface .. "/operstate", "re")
...
function NetworkMgr:enableWifi(wifi_cb, interactive)
  ...
  elseif status == EBUSY then
    ...
    logger.warn(
      "NetworkMgr:enableWifi: A previous connection attempt is still ongoing!"
    )
...
function NetworkMgr:promptWifi(complete_callback, long_press, interactive)
  local text =
    _("Wi-Fi is enabled, but you're currently not connected to a network.")
  ...
  if self.pending_connectivity_check then
    text = text
      .. "\n"
      .. _(
        "KOReader is currently waiting for connectivity. This may take up to 45s, so you may just want to try again later."
      )
  end
...
function NetworkMgr:turnOnWifiAndWaitForConnection(callback)
  ...
  elseif status == EBUSY then
    logger.warn(
      "NetworkMgr:turnOnWifiAndWaitForConnection: A previous connection attempt is still ongoing!"
    )
...
function NetworkMgr:afterWifiAction(callback)
  ...
  local wifi_disable_action =
    G_reader_settings:readSetting("wifi_disable_action")
...
function NetworkMgr:goOnlineToRun(callback)
  ...
  if G_reader_settings:readSetting("wifi_enable_action") ~= "turn_on" then
    logger.warn(
      "NetworkMgr:goOnlineToRun: Cannot run callback because device is offline and wifi_enable_action is not turn_on"
    )
    return false
  end
  ...
  while not self.is_connected do
    if iter == 0 then
      ...
      info =
        InfoMessage:new({ text = _("Waiting for network connectivity…") })
      UIManager:show(info)
      UIManager:forceRePaint()
    end
  ...
  if success then
    ...
    logger.info(
      "Successfully connected to Wi-Fi (after",
      iter * 0.25,
      "seconds)!"
    )
    self.wifi_was_on = true
    ...
    -- We're not connected :(
    logger.info(
      "Failed to connect to Wi-Fi after",
      iter * 0.25,
      "seconds, giving up!"
    )
    self:_abortWifiConnection()
...
function NetworkMgr:getInfoMenuTable()
  return {
    ...
    keep_menu_open = true,
    enabled_func = function()
      return self:isNetworkInfoAvailable()
    end,
    callback = function()
...
function NetworkMgr:getAfterWifiActionMenuTable()
  local wifi_disable_action_setting = G_reader_settings:readSetting(
    "wifi_disable_action"
  ) or "prompt"
  ...
  return {
    text_func = function()
      return T(
        _("Action when done with Wi-Fi: %1"),
...
function NetworkMgr:getDismissScanMenuTable()
  return {
    ...
    enabled_func = function()
      return Device:hasWifiManager() or Device:isEmulator()
    end,
    callback = function()
...
function NetworkMgr:getMenuTable(common_settings)
  ...
  if Device:hasWifiToggle() then
    ...
    common_settings.network_after_wifi_action =
      self:getAfterWifiActionMenuTable()
  end
end
...
function NetworkMgr:reconnectOrShowNetworkMenu(complete_callback, interactive)
  ...
  if success then
    ...
    UIManager:show(InfoMessage:new({
      ...
      text = T(
        _(
          Device:isKindle() and "Connecting to network %1…"
            or "Connected to network %1"
        ),
    ...
    logger.dbg(
      "NetworkMgr: Failed to connect:",
      err_msg,
      "; last attempt on ssid:",
      ssid and util.fixUtf8(ssid, "�") or "<none>"
    )
  end
