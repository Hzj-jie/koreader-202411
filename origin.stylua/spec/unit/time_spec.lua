describe("Time module", function()
  ...
  it("should calculate durations", function()
    ...
    time.now = now
    assert.is.equal(
      time.to_s(time.s(4) + time.us(500000)),
      time.to_s(time.since(time1))
    )
    assert.is.equal(
      time.to_ms(time.s(4) + time.us(500000)),
      time.to_ms(time.since(time1))
    )
    assert.is.equal(
      time.to_us(time.s(4) + time.us(500000)),
