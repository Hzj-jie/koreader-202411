...
*   **Issue Reference in Commit Messages**: If there is a known bug or issue, always add "issue #<bug-id>" or "fix #<bug-id>" in the commit message, depending on whether the issue is related or fixed:
    *   Use `fix #<issue-id>` if the change fully resolves the issue.
    *   Use `issue #<issue-id>` if the change is related to, or a part of, the work for the issue.
...
*   **VCS Push Policy**: Never push commits automatically to remote repositories. Keep changes local, and let the user perform the push, or only push when explicitly requested by the user.
