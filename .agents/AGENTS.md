...
*   **Tracking Removed Files**: When deleting files from the repository that were previously present under `koreader/`, you must update [remove-removed-files.sh](file:///usr/local/google/home/zijiehe/git/koreader-202411/remove-removed-files.sh) to include their paths so they can be cleaned up on target devices.
