describe("OPDSBrowser module", function()
  ...
  describe("Browser Instance, Catalog CRUD & Dialogs", function()
    it(
      "should create OPDSBrowser menu instance and perform CRUD on servers",
      function()
        ...
        browser:editCatalogFromInput(
          { "Edited Catalog", "https://edited.opds/feed", "user2", "pass2" },
          item_to_edit
        )
        ...
        assert.are.not_equal(
          "Edited Catalog",
          after_del_tbl[#after_del_tbl].text
        )
      end
    )
  ...
  describe("Feed Fetching, Parsing & Caching", function()
    ...
    it(
      "should handle fetchFeed with various HTTP status codes and headers_only",
      function()
        local browser = OPDSBrowser:new({ title = "OPDS Catalogs" })

        -- Mock http.request for 200 OK
        local mock_xml =
          '<?xml version="1.0" encoding="utf-8"?><feed xmlns="http://www.w3.org/2005/Atom"><title>Test Feed</title></feed>'
    ...
    it("should parse feed and utilize catalog cache", function()
      local browser = OPDSBrowser:new({ title = "OPDS Catalogs" })
      local mock_feed_xml =
        '<?xml version="1.0"?><feed xmlns="http://www.w3.org/2005/Atom"><title>Catalog</title></feed>'

      local fetch_stub = stub(
        browser,
        "fetchFeed",
        function(self_b, url, headers_only)
    ...
    it("should extract search templates from OpenSearch description", function()
            ...
            Url = {
              {
                ...
                template = "https://test.opds/search?q={searchTerms}",
              },
            },
          },
        }
      ...
      local template =
        browser:getSearchTemplate("https://test.opds/opensearch.xml")
  ...
  describe("Item Table Generation & Actions", function()
    ...
    it(
      "should generate item table from catalog with links, search, borrow, stream, and downloads",
      function()
        local browser = OPDSBrowser:new({ title = "OPDS Catalogs" })
        ...
        local template_stub = stub(browser, "getSearchTemplate", function()
          return "https://test.opds/search?q=%s"
        end)
        local items = browser:genItemTableFromCatalog(
          mock_catalog,
          "https://test.opds/catalog"
        )
    ...
    it(
      "should handle downloadFile with network responses and overwrite confirmations",
      function()
        local browser = OPDSBrowser:new({ title = "OPDS Catalogs" })
        local downloaded_path = nil
        browser.file_downloaded_callback = function(path)
    ...
    it(
      "should abort downloadFile when download directory is read-only",
      function()
        local browser = OPDSBrowser:new({ title = "OPDS Catalogs" })
        local downloaded_path = nil
        browser.file_downloaded_callback = function(path)
        ...
        G_reader_settings:save("download_dir", "/ro_dir")

        local DownloadMgr = require("ui/downloadmgr")
        local check_stub = stub(DownloadMgr, "checkDownloadDir").returns(false)
        local http_stub = stub(http, "request")

        browser:downloadFile("test_ro.epub", "https://test.opds/test_ro.epub")

        assert.stub(http_stub).was_not_called()
    ...
    it(
      "should handle searchCatalog, onMenuSelect, onMenuHold, onReturn, onHoldReturn, and onNextPage",
      function()
        local browser = OPDSBrowser:new({ title = "OPDS Catalogs" })
        browser.paths = {}
        browser.item_table =
          { hrefs = { next = "https://test.opds/feed?page=2" } }
        ...
        UIManager.show = function(self_uim, widget)
          if
            widget.buttons
            and widget.buttons[1]
            and widget.buttons[1][2]
            and widget.buttons[1][2].callback
          then
          ...
          elseif
            widget.buttons
            and widget.buttons[1]
            and widget.buttons[1][1]
            and widget.buttons[1][1].callback
          then
        ...
        browser:onMenuSelect({
          acquisitions = { { type = "application/epub+zip", href = "/a.epub" } },
          title = "Book",
        })
        ...
        browser:onMenuSelect({
          searchable = true,
          url = "https://test.opds/search?q=%s",
        })

        -- onMenuSelect for catalog
        browser:onMenuSelect({
          text = "Subcatalog",
          url = "https://test.opds/sub",
        })
        ...
        table.insert(
          browser.paths,
          { url = "https://test.opds/1", title = "Level 1" }
        )
        table.insert(
          browser.paths,
          { url = "https://test.opds/2", title = "Level 2" }
        )
