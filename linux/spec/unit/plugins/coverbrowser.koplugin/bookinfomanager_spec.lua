describe("Coverbrowser BookInfoManager module", function()
  ...
  describe("Initialization & Properties", function()
    it(
      "should expose BookInfoManager class table and correct properties",
      function()
  ...
  describe("Database Lifecycle & Utilities", function()
    it(
      "should create database, connect, get size and book count, and delete DB",
      function()
  ...
  describe("Settings Management", function()
    it(
      "should load, save, get, and toggle settings in DB config table",
      function()
        BookInfoManager:createDB()
        BookInfoManager:openDbConnection()
  ...
  describe("Book Info Records Operations", function()
    it(
      "should return pseudo-bookinfo for directories or non-provider files",
      function()
        ...
        local noprov_info =
          BookInfoManager:getBookInfo("/tmp/test_nonexistent.xyz123", false)
    ...
    it("should set properties, query, and delete book info records", function()
      ...
      -- Update property
      BookInfoManager:setBookInfoProperties(
        dummy_file,
        { title = "Updated Title", ignore_meta = false }
      )
  ...
  describe("Cover scaling and Subprocess helpers", function()
    it("should calculate cached cover size and check validity", function()
      ...
      local invalid = BookInfoManager.isCachedCoverInvalid(
        fake_info,
        { max_cover_w = 200, max_cover_h = 300 }
      )
      assert.truthy(invalid == nil or type(invalid) == "boolean")
    end)

    it(
      "should store and retrieve compressed cover data with get_cover = true",
      function()
        BookInfoManager:createDB()
        BookInfoManager:openDbConnection()
        ...
        local comp_data, comp_size =
          zstd.zstd_compress(sample_bb.data, cover_size)
    ...
    it(
      "should handle subprocess tracking and extraction helpers safely",
      function()
        local lfs = require("libs/libkoreader-lfs")
    ...
    it(
      "should handle extractBookInfo directly with documents and handle error states",
      function()
        BookInfoManager:createDB()
        BookInfoManager:openDbConnection()
        ...
        local ok = BookInfoManager:extractBookInfo(
          sample_file,
          { max_cover_w = 100, max_cover_h = 150 }
        )
        ...
        BookInfoManager:setBookInfoProperties(
          sample_file,
          { in_progress = BookInfoManager.max_extract_tries + 1 }
        )
        ...
        if uf then
          uf:write("corrupt header")
          uf:close()
        end
        local DocumentRegistry = require("document/documentregistry")
        local open_stub = stub(DocumentRegistry, "openDocument", function()
          return nil
        end)
    ...
    it(
      "should handle cover and metadata ignore, delete, and clean operations",
      function()
        BookInfoManager:createDB()
        BookInfoManager:openDbConnection()
