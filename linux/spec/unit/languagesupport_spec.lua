...
describe("LanguageSupport", function()
  ...
  describe("plugin registration", function()
    it("should register a valid plugin successfully", function()
      ...
      local plugin = {
        name = "test_plugin",
        supportsLanguage = function(self, lang)
          return lang == "ja"
        end,
      }
  ...
  describe("improveWordSelection", function()
    ...
    it("should return early if no document is attached", function()
      ...
      local plugin = {
        name = "test_plugin",
        supportsLanguage = function()
          return true
        end,
        onWordSelection = function()
          return { 2, 6 }
        end,
      }
    ...
    it(
      "should call matching language plugin and return improved selection",
      function()
        local ls =
          LanguageSupport:new({ ui = mock_ui, document = mock_document })
        ...
        assert
          .spy(mock_document.getTextFromXPointers).was
          .called_with(mock_document, 2, 6, true)
        assert
          .spy(mock_document.getScreenBoxesFromPositions).was
          .called_with(mock_document, 2, 6, true)
      end
    )

    it(
      "should fall back to other plugins if the preferred plugin does not exist or returns nil",
      function()
        local ls =
          LanguageSupport:new({ ui = mock_ui, document = mock_document })
        ...
        local ja_plugin = {
          ...
          onWordSelection = spy.new(function()
            return nil
          end), -- fails to improve
        }
  ...
  describe("extraDictionaryFormCandidates", function()
    ...
    it(
      "should call WordLookup on preferred language plugin and return candidates",
      function()
        local ls = LanguageSupport:new({ ui = mock_ui })
        mock_ui.doc_props = { language = "ja" }
    ...
    it(
      "should fall back to other plugins if preferred plugin returns nil",
      function()
        local ls = LanguageSupport:new({ ui = mock_ui })
        mock_ui.doc_props = { language = "ja" }
  ...
  describe("addToMainMenu", function()
    ...
    it(
      "should add language_support item with submenus sorted alphabetically",
      function()
        local ls = LanguageSupport:new({ ui = mock_ui })
        ...
        assert.are.equal(
          ...
          menu_items.language_support.text
        )
