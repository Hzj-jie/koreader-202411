describe("util module", function()
  ...
  describe("lfs.dir() contract", function()
    it("returns (iter, dir_obj) on existing directory", function()
      local lfs = require("libs/libkoreader-lfs")
      local iter, dir_obj, err = lfs.dir(".")
      assert.is_not_nil(iter)
      assert.is_equal("function", type(iter))
      assert.is_not_nil(dir_obj)
      assert.is_nil(err)
    end)

    it("returns (nil, nil, err) on non-existent directory without throwing", function()
      local lfs = require("libs/libkoreader-lfs")
      local iter, dir_obj, err = lfs.dir("/nonexistent/directory/path/here/12345")
      assert.is_nil(iter)
      assert.is_nil(dir_obj)
      assert.is_not_nil(err)
