describe("PluginLoader module", function()
  ...
  setup(function()
    ...
    lfs.dir = function(path)
      if path == "plugins" then
        local list =
          { "checkers.koplugin", "mock2.koplugin", "zsync.koplugin", ".", ".." }
        local i = 0
        return function()
          i = i + 1
          return list[i]
    ...
    lfs.attributes = function(path, request)
      ...
      if
        string.find(path_str, "checkers.koplugin")
        or string.find(path_str, "mock2.koplugin")
        or string.find(path_str, "zsync.koplugin")
        or string.find(path_str, "extra1.koplugin")
      then
        if request == "mode" or not request then
          if
            string.match(path_str, "%.koplugin$")
            or path_str == "/extra/plugins"
          then
            return "directory"
          else
            return "file"
  ...
  describe("default-disable via list", function()
    it(
      "should dynamically disable plugin by default when present in default disabled list on first start",
      function()
        local enabled, disabled = PluginLoader:loadPlugins()
        assert.are.equal(1, #enabled)
        assert.are.equal(1, #disabled)
  ...
  describe("Settings Storage and Defaults", function()
    ...
    it(
      "should respect DEFAULT_DISABLED_PLUGINS and store nil if value matches default",
      function()
