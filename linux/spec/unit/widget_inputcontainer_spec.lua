describe("InputContainer widget", function()
  ...
  it("should register touch zones", function()
    ...
    assert.is.same(
      ...
      ic._ordered_touch_zones[1].handler
    )
    ...
    assert.is.same(
      math.floor(screen_height * 0.1),
      ic._ordered_touch_zones[2].gs_range.range.y
    )
    assert.is.same(
      screen_width / 2,
      ic._ordered_touch_zones[2].gs_range.range.w
    )
  ...
  it("should support indirect overrides for touch zones", function()
    ...
    assert.is.same(
      ...
      ic._ordered_touch_zones[8].def.id
    )
    assert.is.same(
      "readerhighlight_hold_pan",
      ic._ordered_touch_zones[9].def.id
    )
  end)
  ...
  it(
    "should consume unhandled user input events if it is modal and in the window stack",
    function()
  ...
  it(
    "should not consume unhandled user input events if it is modal but not in the window stack",
    function()
      local Event = require("ui/event")
      local ic = InputContainer:new({ modal = true })
  ...
  it("should update dimensions with mergeSize and mergePosition", function()
    local Geom = require("ui/geometry")
    local ic = InputContainer:new({
      dimen = Geom:new({ x = 0, y = 0, w = 300, h = 400 }),
    })
  ...
  it("should return dirtyRegion for input container", function()
    local Geom = require("ui/geometry")
    local ic = InputContainer:new({
      dimen = Geom:new({ x = 0, y = 0, w = 300, h = 400 }),
    })
