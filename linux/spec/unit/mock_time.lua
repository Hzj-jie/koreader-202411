...
function MockTime:install()
  ...
  if self.original_tv_boottime_or_realtime_coarse == nil then
    self.original_tv_boottime_or_realtime_coarse =
      time.boottime_or_realtime_coarse
    assert(self.original_tv_boottime_or_realtime_coarse ~= nil)
  end
  if self.original_tv_now == nil then
    self.original_tv_now = time.monotonic
  ...
  if self.original_tv_boottime_or_realtime_coarse_time == nil then
    self.original_tv_boottime_or_realtime_coarse_time =
      time.boottime_or_realtime_coarse
  ...
  self.monotonic_time = tonumber(timespec_time.tv_sec) * 1e6
    + math.floor(tonumber(timespec_time.tv_nsec) / 1000)
  ...
  time.boottime_or_realtime_coarse = function()
    logger.dbg(
      "MockTime:Time.boottime: ",
      self.boottime_or_realtime_coarse_time
    )
...
function MockTime:increase_boottime_or_realtime_coarse(value)
  ...
  self.boottime_or_realtime_coarse =
    math.floor(self.boottime_or_realtime_coarse + value)
...
function MockTime:increase(value)
  ...
  self.boottime_or_realtime_coarse_time =
    math.floor(self.boottime_or_realtime_coarse_time + value_time)
  logger.dbg(
    "MockTime:increase (boottime) ",
    self.boottime_or_realtime_coarse_time
  )
