describe("sysfs_light", function()
  ...
  describe("guards and input assertions", function()
    it(
      "should assert if setBrightness is called with out of range value",
      function()
    ...
    it(
      "should assert if setWarmth is called with out of range value",
      function()
        local light = SysfsLight:new({})
        assert.has_error(function()
          light:setWarmth(-1)
    ...
    it(
      "should assert if setNaturalBrightness is called with out of range values",
      function()
        local light = SysfsLight:new({})
        assert.has_error(function()
          light:setNaturalBrightness(-1, 50)
        end)
        assert.has_error(function()
          light:setNaturalBrightness(50, 101)
        end)
      end
    )
  end)
  ...
  describe("setNaturalBrightness with frontlight_mixer", function()
    ...
    it(
      "should write to sysfs directly if frontlight_ioctl is absent",
      function()
        local light = SysfsLight:new({
          frontlight_mixer = "/sys/mixer",
          frontlight_white = "/sys/white",
    ...
    it(
      "should set warmth without inverting if nl_inverted is false/nil",
      function()
        local light = SysfsLight:new({
          frontlight_mixer = "/sys/mixer",
          nl_inverted = false,
    ...
    it(
      "should set warmth with inverting (nl_max - warmth) if nl_inverted is true",
      function()
        local light = SysfsLight:new({
          frontlight_mixer = "/sys/mixer",
          nl_max = 100,
  ...
  describe(
    "setNaturalBrightness without frontlight_mixer (RGB LEDs)",
    function()
      it(
        "should compute and set white, green, red lights with bl_power logic",
        function()
          local light = SysfsLight:new({
            frontlight_white = "/sys/white",
          ...
          assert
            .spy(mock_ffi_util.writeToSysfs).was
            .called_with(31, "/sys/white/bl_power")
          assert
            .spy(mock_ffi_util.writeToSysfs).was
            .called_with(151, "/sys/white/brightness")

          assert
            .spy(mock_ffi_util.writeToSysfs).was
            .called_with(31, "/sys/green/bl_power")
          assert
            .spy(mock_ffi_util.writeToSysfs).was
            .called_with(104, "/sys/green/brightness")

          assert
            .spy(mock_ffi_util.writeToSysfs).was
            .called_with(31, "/sys/red/bl_power")
          assert
            .spy(mock_ffi_util.writeToSysfs).was
            .called_with(169, "/sys/red/brightness")
        end
      )
      ...
      it("should turn off bl_power if values are <= 0", function()
        ...
        assert
          .spy(mock_ffi_util.writeToSysfs).was
          .called_with(0, "/sys/white/bl_power")
        assert
          .spy(mock_ffi_util.writeToSysfs).was
          .called_with(0, "/sys/white/brightness")
      end)
    end
  )
end)
