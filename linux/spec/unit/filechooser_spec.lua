describe("FileChooser module", function()
  ...
  it(
    "should test all collate algorithms and mandatory text functions",
    function()
      ...
      assert.is_string(
        fc.collates.access.mandatory_func({ attr = { last_read = 86400 } })
      )
      assert.is_string(
        fc.collates.access.mandatory_func({ attr = { last_read = 86450 } })
      )
      ...
      assert.is_true(
        sort_date(
          { attr = { modification = 3000 } },
          { attr = { modification = 1000 } }
        )
      )
      assert.is_string(
        fc.collates.date.mandatory_func({ attr = { modification = 1000 } })
      )

      -- 5. size
      local sort_size = fc.collates.size.init_sort_func()
      assert.is_true(
        sort_size({ attr = { size = 100 } }, { attr = { size = 200 } })
      )

      -- 6. type
      local type_item1 = { text = "doc.pdf", path = "/tmp/doc.pdf", attr = {} }
      local type_item2 =
        { text = "doc.epub", path = "/tmp/doc.epub", attr = {} }
      ...
      assert.is_true(
        sort_type(
          { text = "a.pdf", suffix = "pdf" },
          { text = "b.pdf", suffix = "pdf" }
        )
      )

      -- 7. percent_unopened_first & percent_unopened_last
      local sort_unopened_first =
        fc.collates.percent_unopened_first.init_sort_func()
      assert.is_true(
        sort_unopened_first(
          { opened = false, text = "a" },
          { opened = true, text = "b" }
      ...
      assert.is_true(
        sort_unopened_first(
          { opened = true, percent_finished = 0.2 },
          { opened = true, percent_finished = 0.8 }
        )
      )
      assert.is_true(
        sort_unopened_first(
      ...
      local sort_unopened_last =
        fc.collates.percent_unopened_last.init_sort_func()
      assert.is_true(
        sort_unopened_last(
          { opened = true, text = "a" },
          { opened = false, text = "b" }
      ...
      assert.are_equal(
        "–",
        fc.collates.percent_unopened_first.mandatory_func({ opened = false })
      )
      assert.are_equal(
        "50 %",
        fc.collates.percent_unopened_first.mandatory_func({
          opened = true,
      ...
      assert.is_true(
        sort_pnat(
          { sort_percent = 0.8, text = "a" },
          { sort_percent = 0.5, text = "b" }
        )
      )
      assert.is_false(
        sort_pnat(
          { sort_percent = 1, text = "a" },
          { sort_percent = 0.5, text = "b" }
        )
      )
      assert.is_true(
        sort_pnat(
          { sort_percent = 0.5, text = "a" },
          { sort_percent = 1, text = "b" }
        )
      )
      assert.is_true(
        sort_pnat(
  ...
  it("should test show_dir and show_file filtering rules", function()
    ...
    fc.file_filter = function(filename)
      return filename:match("%.epub$") ~= nil
    end
  ...
  it("should test showFileInBold options", function()
    ...
    G_named_settings.show_file_in_bold = function()
      return "new"
    end
  ...
  it("should get collate, sorting functions and clear sorting cache", function()
    ...
    local static_sort =
      FileChooser.getSortingFunction(FileChooser, fc.collates.strcoll, false)
  ...
  it(
    "should generate item tables with parent navigation and collection / shortcut stars",
    function()
      local fc = FileChooser:new({
        dimen = Screen:getSize(),
        path = "/tmp/testdir",
      ...
      local files = {
        {
          text = "book.epub",
          path = "/tmp/testdir/book.epub",
          is_file = true,
          attr = { mode = "file", size = 1024 },
        },
      }
  ...
  it(
    "should handle navigation: changeToPath, goHome, onFolderUp, and toggleShowFilesMode",
    function()
      local fc = FileChooser:new({
        dimen = Screen:getSize(),
        path = "/tmp",
  ...
  it(
    "should handle menu selection, hold, and selectAllFilesInFolder",
    function()
      local closed = false
      ...
      assert.is_true(
        fc:onMenuSelect({ is_file = false, path = "/tmp", is_go_up = true })
      )
  ...
  it("should handle getNextFile in folder", function()
    ...
    require("document/documentregistry").hasProvider = function()
      return true
    end
  ...
  it("should test _pathUnwritable with require_writable option", function()
    ...
    assert.is_false(fc:_pathUnwritable())
    assert.is_false(fc:_pathUnwritable("/other"))

    fc.require_writable = true
    assert.is_true(fc:_pathUnwritable())
    assert.is_true(fc:_pathUnwritable("/other"))
    ...
    fc.path = "/default/path"
    fc:_pathUnwritable()
    assert.are_equal("/default/path", checked_path)
    fc:_pathUnwritable("/custom/path")
    assert.are_equal("/custom/path", checked_path)
