describe("AutoTurn plugin tests", function()
  ...
  local function assertPageTurnCalled(distance)
    ...
    assert.is_true(
      #page_turns > 0,
      "Expected onGotoViewRel to be called, but it wasn't"
    )
    if distance then
      local found = false
      ...
      assert.is_true(
        found,
        "Expected onGotoViewRel with distance "
          .. tostring(distance)
          .. " to be called"
      )
    end
  ...
  local function assertPageTurnNotCalled()
    ...
    assert.are.equal(
      0,
      #page_turns,
      "Expected onGotoViewRel NOT to be called, but it was"
    )
  end
  ...
  local function saveAutoTurnSettings(settings)
    ...
    local autoturn_settings =
      LuaSettings:open(DataStorage:getSettingsDir() .. "/autoturn.lua")
  ...
  it("should load settings and register to menu on init", function()
    ...
    assert
      .spy(mock_menu.registerToMainMenu)
      .was_called_with(mock_menu, match.ref(widget))
  end)
  ...
  it(
    "should not trigger page turn if topmost widget is not ReaderUI",
    function()
  ...
  it(
    "should postpone page turn when a user action occurs mid-timeout",
    function()
  ...
  it(
    "should show DateTimeWidget, set timeout on apply, and reload settings",
    function()
  ...
  it(
    "should disable autoturn and update settings when DateTimeWidget cancel is triggered",
    function()
  ...
  it(
    "should show SpinWidget and update distance when callback is triggered",
    function()
