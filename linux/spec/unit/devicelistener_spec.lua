describe("devicelistener", function()
  ...
  before_each(function()
    ...
    mock_powerd = {
      fl_max = 100,
      fl_warmth_max = 10,
      frontlightIntensity = spy.new(function()
        return 50
      end),
      frontlightWarmth = spy.new(function()
        return 5
      end),
      turnOffFrontlight = spy.new(function() end),
      setIntensity = spy.new(function() end),
      updateResumeFrontlightState = spy.new(function() end),
      fromNativeWarmth = spy.new(function(self, w)
        return w * 10
      end),
      toNativeWarmth = spy.new(function(self, w)
        return math.floor(w / 10)
      end),
      setWarmth = spy.new(function() end),
      isFrontlightOn = spy.new(function()
        return true
      end),
      toggleFrontlight = spy.new(function(self, cb)
        if cb then
          cb()
        end
        return true
    ...
    mock_device = {
      screen = mock_screen,
      hasFrontlight = spy.new(function()
        return true
      end),
      hasNaturalLight = spy.new(function()
        return true
      end),
      hasGSensor = spy.new(function()
        return true
      end),
      isAlwaysFullscreen = spy.new(function()
        return false
      end),
    ...
    mock_reader_settings = {
      ...
      makeTrue = spy.new(function(self, key)
        reader_settings_data[key] = true
      end),
      makeFalse = spy.new(function(self, key)
        reader_settings_data[key] = false
      end),
      delete = spy.new(function(self, key)
        reader_settings_data[key] = nil
      end),
    ...
    mock_uimanager = {
      ...
      scheduleIn = spy.new(function(self, _delay, cb)
        if cb then
          cb()
        end
      end),
  ...
  describe("onToggleNightMode", function()
    it(
      "should save setting and toggle night mode in screen and UIManager",
      function()
        local listener = DeviceListener:new({ ui = {} })
        listener:onToggleNightMode(true)

        assert
          .spy(G_reader_settings.save).was
          .called_with(G_reader_settings, "night_mode", true, false)
    ...
    it(
      "should NOT reset document call cache if provider is not crengine",
      function()
        local mock_document = {
          provider = "mupdf",
          resetCallCache = spy.new(function() end),
  ...
  describe("changeFlIntensity", function()
    it("should return false if device has no frontlight", function()
      mock_device.hasFrontlight = spy.new(function()
        return false
      end)
      local listener = DeviceListener:new({})
  ...
  describe("onSetFlIntensity", function()
    it(
      "should return true and do nothing if new intensity is same as current",
      function()
        local listener = DeviceListener:new({})
    ...
    it("should turn off frontlight if new intensity <= 0", function()
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Frontlight disabled.")
    end)
    ...
    it("should set intensity if new intensity > 0", function()
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Frontlight intensity set to 50.")
    end)
  end)

  describe("changeFlWarmth", function()
    it("should return false if device has no natural light", function()
      mock_device.hasNaturalLight = spy.new(function()
        return false
      end)
      local listener = DeviceListener:new({})
    ...
    it(
      "should change warmth and call onSetFlWarmth with mapped native value",
      function()
        local listener = DeviceListener:new({})
  ...
  describe("onSetFlWarmth", function()
    ...
    it("should clamp warmth and notify", function()
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Warmth set to 0.")
  ...
  describe("onToggleFrontlight", function()
    it("should toggle frontlight and notify when on", function()
      mock_powerd.isFrontlightOn = spy.new(function()
        return true
      end)
      local listener = DeviceListener:new({})
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Frontlight disabled.", "test")
      assert.spy(mock_powerd.updateResumeFrontlightState).was.called(1)
    end)
    ...
    it("should toggle frontlight and notify when off", function()
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Frontlight enabled.", "test")
      assert.spy(mock_powerd.updateResumeFrontlightState).was.called(1)
    end)

    it("should notify unchanged if powerd toggle returns false", function()
      mock_powerd.toggleFrontlight = spy.new(function()
        return false
      end)
      local listener = DeviceListener:new({})
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Frontlight unchanged.", "test")
    end)
  end)
  ...
  describe("GSensor toggles", function()
    it(
      "onToggleGSensor should flip setting and toggle sensor (currently off)",
      function()
        ...
        assert
          .spy(mock_notification.notify).was
          .called_with(mock_notification, "Accelerometer rotation events on.")
      end
    )

    it(
      "onToggleGSensor should flip setting and toggle sensor (currently on)",
      function()
        ...
        assert
          .spy(mock_device.toggleGSensor).was
          .called_with(mock_device, false)
        assert
          .spy(mock_notification.notify).was
          .called_with(mock_notification, "Accelerometer rotation events off.")
      end
    )
    ...
    it("onTempGSensorOn should do nothing if sensor is already on", function()
      ...
      assert
        .spy(mock_notification.notify).was
        .called_with(mock_notification, "Accelerometer rotation events already on.")
    end)

    it(
      "onTempGSensorOn should temporarily turn sensor on if it was off",
      function()
        G_reader_settings:makeTrue("input_ignore_gsensor")
        local listener = DeviceListener:new({})
        ...
        assert
          .spy(mock_notification.notify).was
          .called_with(mock_notification, "Accelerometer rotation events on for 5 seconds.")
    ...
    it(
      "onLockGSensor should flip lock setting and call Device:lockGSensor",
      function()
        ...
        assert
          .spy(mock_notification.notify).was
          .called_with(mock_notification, "Orientation locked.")
        ...
        assert
          .spy(mock_notification.notify).was
          .called_with(mock_notification, "Orientation unlocked.")
      end
    )
  end)
  ...
  describe("Screen Rotation Controls", function()
    it(
      "onIterateRotation should rotate CW (step 1) when ccw is false",
      function()
        mock_screen.getRotationMode = function()
          return 0
        end
        local listener = DeviceListener:new({})
        ...
        assert
          ...
          .called_with(mock_uimanager, { name = "SetRotationMode", arg = 1 })
      end
    )

    it(
      "onIterateRotation should rotate CCW (step -1) when ccw is true",
      function()
        mock_screen.getRotationMode = function()
          return 0
        end
        ...
        assert
          .spy(mock_uimanager.broadcastEvent).was
          .called_with(mock_uimanager, { name = "SetRotationMode", arg = 3 })
      end
    )
    ...
    it("onSwapRotation should swap landscape (3) to portrait (2)", function()
      ...
      assert
        .spy(mock_uimanager.broadcastEvent).was
        .called_with(mock_uimanager, { name = "SetRotationMode", arg = 2 })
    end)
  end)

  describe("Button Mapping, Refresh, and Utilities", function()
    it(
      "onSetRefreshRate should set full_refresh_count and call updateRefreshRate",
      function()
        local listener = DeviceListener:new({})
        listener:onSetRefreshRate(5)

        assert
          .spy(mock_named_settings.set.full_refresh_count).was
          .called_with(5)
    ...
    describe("onSwapPageTurnButtons", function()
      it(
        "side = left: should disable global inversion and flip left-side page-turn buttons inversion",
        function()
          ...
          assert
            .spy(mock_notification.notify).was
            .called_with(mock_notification, "Left-side page-turn buttons inverted.")
        end
      )

      it(
        "side = right: should disable global inversion and flip right-side page-turn buttons inversion",
        function()
          G_reader_settings:makeTrue("input_invert_page_turn_keys")
          ...
          assert.spy(mock_device.invertButtons).was.called(1)
          assert.is_true(
            G_reader_settings:isTrue("input_invert_right_page_turn_keys")
          )
          assert.spy(mock_device.invertButtonsRight).was.called(1)
          assert
            .spy(mock_notification.notify).was
            .called_with(mock_notification, "Right-side page-turn buttons inverted.")
        end
      )

      it(
        "side = nil: if both left and right are inverted, should clear them all and notify no longer inverted",
        function()
          ...
          assert
            .spy(mock_notification.notify).was
            .called_with(mock_notification, "Page-turn buttons no longer inverted.")
        end
      )

      it(
        "side = nil: if only left was inverted, should clear left and toggle global inversion",
        function()
          G_reader_settings:makeTrue("input_invert_left_page_turn_keys")
          G_reader_settings:makeFalse("input_invert_right_page_turn_keys")
          ...
          assert
            .spy(mock_notification.notify).was
            .called_with(mock_notification, "Page-turn buttons inverted.")
        end
      )
    end)
    ...
    describe("onToggleKeyRepeat", function()
      it(
        "toggle = true: should make input_no_key_repeat false and call toggleKeyRepeat(true)",
        function()
          ...
          assert
            .spy(mock_device.toggleKeyRepeat).was
            .called_with(mock_device, true)
        end
      )

      it(
        "toggle = false: should make input_no_key_repeat true and call toggleKeyRepeat(false)",
        function()
      ...
      it(
        "toggle = nil: should flip input_no_key_repeat and call toggleKeyRepeat",
        function()
          G_reader_settings:makeFalse("input_no_key_repeat")
          local listener = DeviceListener:new({})
    ...
    it("onRequestUSBMS should call start on MassStorage", function()
      ...
      assert
        .spy(mock_mass_storage.start).was
        .called_with(mock_mass_storage, false)
    end)
    ...
    describe("UIManager and exit/restart callbacks", function()
      ...
      it(
        "onExitKOReader should delegate to ui.menu:exitOrRestart with callback",
        function()
      ...
      it(
        "onRestart should restart KOReader via callback to exitOrRestart",
        function()
          listener:onRestart()
      ...
      it(
        "onFullRefresh should broadcast UpdateFooter and schedule refresh",
        function()
          listener:onFullRefresh()

          assert
            .spy(mock_uimanager.broadcastEvent).was
            .called_with(mock_uimanager, "UpdateFooter")
          assert
            .spy(mock_uimanager.scheduleRefresh).was
            .called_with(mock_uimanager, "full")
        end
      )

      it(
        "onResume should restore Gestures handling in InputContainer by setting setIgnoreTouchInput to false",
        function()
          listener:onResume()
          assert
            .spy(mock_uimanager.setIgnoreTouchInput).was
            .called_with(mock_uimanager, false)
        end
      )
    end)
