describe("WakeupMgr", function()
  ...
  setup(function()
    ...
    WakeupMgr.validateWakeupAlarmByProximity = spy.new(function()
      return true
    end)
  ...
  describe("dodgy_rtc support", function()
    ...
    before_each(function()
      ...
      dodgy_mgr.validateWakeupAlarmByProximity = spy.new(function()
        return true
      end)
    ...
    it("should chain alarms when delay is > 0xFFFF", function()
      ...
      assert.is_true(
        math.abs(dodgy_mgr._task_queue[1].epoch - epoch_18930) <= 1
      )
      assert.is_true(
        math.abs(dodgy_mgr._task_queue[2].epoch - epoch_84465) <= 1
      )
      assert.is_true(
        math.abs(dodgy_mgr._task_queue[3].epoch - epoch_150000) <= 1
      )

      assert.are.equal(
        dodgy_mgr.DummyTaskCallback,
        dodgy_mgr._task_queue[1].callback
      )
      assert.are.equal(
        dodgy_mgr.DummyTaskCallback,
        dodgy_mgr._task_queue[2].callback
      )
      assert.are.equal(mock_callback, dodgy_mgr._task_queue[3].callback)
    end)

    it(
      "should execute dummy alarms and progress the chain without executing real callback",
      function()
        dodgy_mgr:addTask(delay, mock_callback)
    ...
    it(
      "should remove the entire chain of dummy alarms when final task is removed by callback",
      function()
        dodgy_mgr:addTask(delay, mock_callback)
        assert.are.equal(3, #dodgy_mgr._task_queue)
    ...
    it(
      "should remove the entire chain of dummy alarms when final task is removed by epoch",
      function()
        dodgy_mgr:addTask(delay, mock_callback)
