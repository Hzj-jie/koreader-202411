describe("AutoWarmth plugin tests", function()
    ...
    it("should set initial dawn warmth at midnight", function()
        ...
        assert.stub(Device.powerd.setWarmth).was_called_with(match.is_table(), 60, match._)
    end)
    ...
    describe("Dispatcher actions", function()
        ...
        it("should register actions with the Dispatcher", function()
            ...
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "show_ephemeris", {
                category = "none",
                event = "ShowEphemeris",
                title = "Show ephemeris",
                general = true,
            })
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "auto_warmth_off", {
                category = "none",
                event = "AutoWarmthOff",
                title = "Auto warmth off",
                screen = true,
            })
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "auto_warmth_activate_sun", {
                category = "none",
            ...
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "auto_warmth_activate_schedule", {
                category = "none",
                event = "AutoWarmthMode",
                arg = 2,
            ...
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "auto_warmth_activate_closer_midnight", {
                category = "none",
                event = "AutoWarmthMode",
                arg = 4,
            ...
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "auto_warmth_activate_closer_noon", {
                category = "none",
                event = "AutoWarmthMode",
                arg = 3,
            ...
            assert.stub(Dispatcher.registerAction).was_called_with(Dispatcher, "auto_warmth_cycle_trough", {
                category = "none",
                event = "AutoWarmthMode",
        ...
        it("should toggle AutoWarmth off via onAutoWarmthOff", function()
            ...
            assert.stub(Notification.notify).was_called_with(match.is_table(), "Auto warmth turned off")
        end)
        ...
        it("should select modes via onAutoWarmthMode with arguments", function()
            ...
            assert.stub(Notification.notify).was_called_with(match.is_table(), "Auto warmth use schedule")

            -- Mode 3: Closer Noon
            AutoWarmth:onAutoWarmthMode(3)
            assert.are.equal(3, AutoWarmth.activate)
            assert.stub(Notification.notify).was_called_with(match.is_table(), "Auto warmth use whatever is closer to noon")

            -- Mode 4: Closer Midnight
            AutoWarmth:onAutoWarmthMode(4)
            assert.are.equal(4, AutoWarmth.activate)
            assert.stub(Notification.notify).was_called_with(match.is_table(), "Auto warmth use whatever is closer to midnight")

            -- Mode 1: Sun position
            AutoWarmth:onAutoWarmthMode(1)
            assert.are.equal(1, AutoWarmth.activate)
            assert.stub(Notification.notify).was_called_with(match.is_table(), "Auto warmth use sun position")
        end)

        it("should cycle through modes sequentially via onAutoWarmthMode without arguments", function()
    ...
    describe("Solar Position Warmth Transitions", function()
        ...
        it("should transition warmth dynamically after civil dawn towards sunrise", function()
            ...
            UIManager:handleInput()
            assert.stub(Device.powerd.setWarmth).was_called_with(match.is_table(), 48, match._)
            ...
            UIManager:handleInput()
            assert.stub(Device.powerd.setWarmth).was_called_with(match.is_table(), 20, match._)
        end)

        it("should maintain daytime warmth until sunset, then transition to evening warmth", function()
            ...
            UIManager:handleInput()
            assert.stub(Device.powerd.setWarmth).was_called_with(match.is_table(), 38, match._)
        ...
        it("should recalculate schedule on the next day after midnight update", function()
    ...
    describe("Fixed Schedule Warmth Transitions", function()
        ...
        it("should transition warmth dynamically based on the schedule", function()
            ...
            UIManager:handleInput()
            assert.stub(Device.powerd.setWarmth).was_called_with(match.is_table(), 40, match._)
        ...
        it("should maintain daytime warmth until sunset schedule, then transition", function()
    ...
    describe("Frontlight Toggling - Zero Offset", function()
        ...
        it("should toggle frontlight at sunrise and sunset with zero offset", function()
    ...
    describe("Frontlight Toggling - 600s Offset", function()
        ...
        it("should toggle frontlight at sunrise and sunset with 600s offset", function()
            -- At startup (midnight), frontlight was turned ON by toggleFrontlight.
            -- Let's clear turnOnFrontlight stub to inspect subsequent actions.
            Device.powerd.turnOnFrontlight:clear()
    ...
    describe("Suspend and Resume Lifecycle", function()
        ...
        it("should not apply warmth transitions while suspended, and catch up instantly on resume", function()
            ...
            assert.stub(Device.powerd.setWarmth).was_called_with(match.is_table(), 20, true)
            Device.powerd.setWarmth:clear()
        ...
        it("should catch up correctly on resume when resumed on a different day", function()
            -- Clear the stub
            Device.powerd.setWarmth:clear()
