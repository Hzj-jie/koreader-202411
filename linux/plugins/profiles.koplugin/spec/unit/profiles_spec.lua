describe("Profiles plugin", function()
  ...
  before_each(function()
    test_profiles_file = DataStorage:getSettingsDir() .. "/test_profiles_" .. os.time() .. ".lua"
  ...
  describe("Initialization & Loading", function()
    it("should initialize Profiles plugin class and register to main menu", function()
    ...
    it("should load profiles and assign profile names if missing", function()
      ...
      assert.are.equal("Night Mode", profiles_instance.data["Night Mode"].settings.name)
    end)
  ...
  describe("Dispatcher Registration", function()
    it("should register profiles with settings.registered = true when action registration is requested", function()
      ...
      assert.stub(register_stub).was.called_with(
        Dispatcher,
        "profile_exec_RegProfile",
        match.is_table()
      )
  ...
  describe("Profile Creation from Book Settings", function()
    it("should extract profile from current book settings in non-rolling mode", function()
      mock_ui.rolling = nil
      mock_ui.document = {
        configurable = {
      ...
      local profile = profiles_instance:getProfileFromCurrentBookSettings("BookProfile")
      assert.are.equal("BookProfile", profile.settings.name)
      assert.are.equal(20, profile.kopt_font_size)
    end)

    it("should extract profile from current book settings in rolling mode", function()
      ...
      local profile = profiles_instance:getProfileFromCurrentBookSettings("RollingProfile")
  ...
  describe("Profile Management Operations", function()
    ...
    it("should update autoexec entries when profile name changes or is removed", function()
    ...
    it("should update profile actions when action names are renamed or removed", function()
      profiles_instance.data = {
        ["Profile1"] = {
      ...
      assert.are.equal("action_new", profiles_instance.data.Profile1.settings.order[1])
  ...
  describe("Profile Execution", function()
    it("should execute profile via Dispatcher", function()
      ...
      assert.stub(exec_stub).was.called_with(
        Dispatcher,
        profiles_instance.data["TestProfile"],
        { qm_show = false }
      )
      exec_stub:revert()
    end)

    it("should execute autoexec profile on next tick when ask is not set", function()
      profiles_instance.data = {
    ...
    it("should show ConfirmBox when auto_exec_ask is true", function()
      profiles_instance.data = {
        ["AskProfile"] = { settings = { name = "AskProfile", auto_exec_ask = true } },
      }
  ...
  describe("Event Triggers", function()
    ...
    it("should handle executeAutoExecEvent for Start and Resume events", function()
    ...
    it("should handle onPathChanged with matching has and has_not patterns", function()
      profiles_instance.autoexec = {
        PathChanged = {
    ...
    it("should handle onReaderReady and onCloseDocument when not reloading", function()
      profiles_instance.autoexec = {
        ReaderReady = { P1 = true },
      ...
      assert.stub(exec_event_stub).was.called_with(
        profiles_instance,
        "ReaderReady"
      )
  ...
  describe("Menu Generators", function()
    ...
    it("should generate autoexec menu item and handle callback", function()
      profiles_instance.autoexec = {}
      local item = profiles_instance:genAutoExecMenuItem("On Start", "Start", "P1")
