...
describe("Single Square Move Generation #movegen", function()
  ...
  forEach(positions, function(position)
    ...
    it(position.fen .. " " .. position.square, function()
      local moves =
        chess.moves({ square = position.square, verbose = position.verbose })
...
describe("Algebraic Notation #an", function()
  ...
  forEach(positions, function(position)
    local chess = Chess(position.fen)
    it(
      "should generate Standard Algebraic Notation - " .. position.fen,
      function()
    ...
    it("position should pass - " .. tostring(position.should_pass), function()
      ...
      for _, square in ipairs(chess.SQUARES) do
        ...
        if pp == nil then
          ...
          if
            not (piece and piece.type == pp.type and piece.color == pp.color)
          then
            passed = false
            break
          end
  ...
  forEach(tests, function(t, i)
    forEach(newline_chars, function(newline, j)
      it(i .. string.char(96 + j), function()
        ...
        local result = chess.load_pgn(
          table.concat(t.pgn, newline),
          { sloppy = sloppy, newline_char = newline }
        )
...
describe("Load Comments #ldcmts", function()
  local tests = {
    {
  ...
  forEach(positions, function(position)
    it(
      string.format(
        "%s (valid: %s)",
        position.fen,
        tostring(position.error_number == 0)
      ),
      function()
...
describe("Regression Tests #reg", function()
  ...
  it(
    "Github Issue #85 (white) - SetUp and FEN should be accepted in load_pgn",
    function()
      local chess = Chess()
      local pgn = {
        '[SetUp "1"]',
  ...
  it(
    "Github Issue #85 (black) - SetUp and FEN should be accepted in load_pgn",
    function()
      local chess = Chess()
      local pgn = {
        '[SetUp "1"]',
  ...
  it(
    "Github Issue #129 load_pgn() should not clear headers if PGN contains SetUp and FEN tags",
    function()
      local pgn = {
        '[Event "Test Olympiad"]',
        '[Site "Earth"]',
  ...
  it(
    "Github Issue #282 - playing a move on an empty board throws an error",
    function()
  ...
  it(
    "Github Issue #279 - load_pgn duplicate last move if it has a comment",
    function()
      local history = {
        "e4",
        "e5",
